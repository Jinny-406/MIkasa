...
def _try_dispatch_background_run(
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("mikasa_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional script run each tick; stdout is injected into the agent's prompt as context (with no_agent=True the script IS the job). Relative paths resolve under {display_mikasa_home()}/scripts/; .sh/.bash via bash, else Python. On update, '' clears."
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("mikasa_INTERACTIVE")
        or env_var_enabled("mikasa_GATEWAY_SESSION")
        or env_var_enabled("mikasa_EXEC_ASK")
    )
