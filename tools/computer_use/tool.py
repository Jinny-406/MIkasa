...
def _new_backend(permission_mode: str) -> ComputerUseBackend:
    backend_name = os.environ.get("mikasa_COMPUTER_USE_BACKEND", "cua").lower()
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$MIKASA_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from mikasa_constants import get_mikasa_dir  # lazy so tests can patch get_mikasa_dir
    cache_dir = get_mikasa_dir(subdir, legacy)
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$MIKASA_HOME/cache/vision/``) and merge
