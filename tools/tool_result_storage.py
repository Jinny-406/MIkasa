...
is ALWAYS host-side ``$MIKASA_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $MIKASA_HOME/cache/spillover as a Path (not created)."""
    from mikasa_constants import get_mikasa_home
    return get_mikasa_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $MIKASA_HOME/cache/spillover; returns path str or None."""
    try:
...
HEREDOC_MARKER = "mikasa_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
