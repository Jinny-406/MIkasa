...
def blobs_dir() -> Path:
    return get_mikasa_home() / ".curator_backups" / "blobs"
...
def fill_snapshot_from_curator_backup(
    ...
    for rel, data in extra.items():
        ...
        if not _is_within(skills, dest) or not _is_within(get_mikasa_home(), dest):
            continue
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """Every entry path must be under MIKASA_HOME — a hand-edited ledger must not
