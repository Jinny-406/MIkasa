...
from mikasa_constants import get_bundled_skills_dir, get_mikasa_home, get_optional_skills_dir
...
# retarget MIKASA_HOME after import, and frozen constants would resolve (and for
...
# once under the launch MIKASA_HOME and later scope requests to a different profile via
# set_mikasa_home_override(). See #65828.
_mikasa_home_AT_IMPORT = MIKASA_HOME
...
def _mikasa_home() -> Path:
    return _live(MIKASA_HOME, _mikasa_home_AT_IMPORT, get_mikasa_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _mikasa_home() / "skills")
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    essential_only = (_mikasa_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing MIKASA_HOME / malicious manifest entry).
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_mikasa_home() / NO_BUNDLED_SKILLS_MARKER).exists()
