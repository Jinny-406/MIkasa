...
class HermesMCPOAuthProvider(HermesProviderMixin, *_SDK_BASES):
    ...
    async def async_auth_flow(self, request):  # type: ignore[override]
        try:  # pre-flow hook: reload from disk if it changed (non-fatal on error)
            await get_manager().invalidate_if_disk_changed(self._hermes_server_name, MIKASA_HOME=self._mikasa_home)
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._mikasa_home = key[0]
            return entry.provider

    @staticmethod
    def _key(server_name: str, MIKASA_HOME: str | Path | None = None) -> tuple[str, str]:
        from mikasa_constants import get_mikasa_home
        home = Path(MIKASA_HOME) if MIKASA_HOME is not None else get_mikasa_home()
    ...
    def remove(self, server_name: str, *, MIKASA_HOME: str | Path | None = None) -> _ProviderEntry | None:
        """Evict the provider from cache AND delete tokens from disk (``hermes mcp remove`` / forced re-auth)."""
        entry = self.evict(server_name, MIKASA_HOME=MIKASA_HOME)
        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, MIKASA_HOME=MIKASA_HOME)
        logger.info("MCP OAuth '%s': evicted from cache and removed from disk", server_name)
        return entry

    def restore_entry(self, server_name: str, entry: _ProviderEntry | None, *, MIKASA_HOME: str | Path | None = None) -> None:
        """Restore a provider entry removed for a failed reauthorization."""
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, MIKASA_HOME), entry)

    def evict(self, server_name: str, *, MIKASA_HOME: str | Path | None = None) -> _ProviderEntry | None:
        """Drop only the in-process provider, preserving persisted OAuth state."""
        with self._entries_lock:
            return self._entries.pop(self._key(server_name, MIKASA_HOME), None)

    async def invalidate_if_disk_changed(self, server_name: str, *, MIKASA_HOME: str | Path | None = None) -> bool:
        """Force the SDK provider to reload when the tokens file mtime changed (e.g. a cron refresh); True if so."""
        from tools.mcp_oauth import _get_token_dir, _safe_filename
        entry = self._entries.get(self._key(server_name, MIKASA_HOME))
        if entry is None or entry.provider is None:
            return False
        async with entry.lock:
            try:
                mtime_ns = (_get_token_dir(MIKASA_HOME) / f"{_safe_filename(server_name)}.json").stat().st_mtime_ns
            except OSError:
                return False
