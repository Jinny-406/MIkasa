...
def _real_profile_unsupported_reason(browser) -> Optional[str]:
    ...
    """
    from mikasa_cli.browser_connect import UNSUPPORTED_CHANNEL
    if browser is None:
...
def _real_profile_snapshot_error(err: str) -> str:
    ...
    from mikasa_cli.browser_connect import _PROFILE_LOCKED_PREFIX
...
def _real_profile_cdp() -> tuple:
    ...
    if not _cloud._use_real_profile():
        ...
        try:
            from mikasa_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    from mikasa_cli.browser_connect import (chromium_executable, detect_default_chromium,
