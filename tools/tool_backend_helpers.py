...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``mikasa_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``mikasa_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
...
# (mikasa_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
