...
from mikasa_constants import secure_parent_dir
...
def _get_token_dir(MIKASA_HOME: str | Path | None = None) -> Path:
    """``MIKASA_HOME/mcp-tokens/`` — per-profile token directory."""
    from mikasa_constants import get_mikasa_home

    return Path(MIKASA_HOME if MIKASA_HOME is not None else get_mikasa_home()) / "mcp-tokens"
...
class HermesTokenStorage:
    """Persist OAuth state as ``MIKASA_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, MIKASA_HOME: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._mikasa_home = Path(MIKASA_HOME) if MIKASA_HOME is not None else None

    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._mikasa_home) / f"{self._server_name}{suffix}"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._mikasa_home)
...
def remove_oauth_tokens(server_name: str, *, MIKASA_HOME: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    HermesTokenStorage(server_name, MIKASA_HOME=MIKASA_HOME).remove()
