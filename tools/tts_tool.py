...
def get_env_value(name, default=None):
    ...
    try:
        from mikasa_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _get_default_output_dir() -> str:
    from mikasa_constants import get_mikasa_dir
    return str(get_mikasa_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    MIKASA_HOME and later scope requests to a different profile via
    ``mikasa_constants.set_mikasa_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped MIKASA_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("mikasa_cli.config not available, using default TTS config")
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("mikasa_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_mikasa_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'mikasa_xai_user_agent': ('tools.xai_http', 'mikasa_xai_user_agent'),
