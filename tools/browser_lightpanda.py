...
def _home_candidates() -> list:
    ...
    try:
        ...
        candidates.append(Path(get_mikasa_home()) / "bin" / "lightpanda")
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    fallbacks), then installer/agent-browser locations, then ``$MIKASA_HOME/bin``. No Windows build."""
    if os.name == "nt":
...
def _state_dir() -> Path:
    from mikasa_constants import get_mikasa_home
    path = Path(get_mikasa_home()) / "cache" / "browser-use" / "lightpanda"
...
def _cdp_ready(url: str, timeout: float = 0.2) -> bool:
    try:
        from mikasa_cli.browser_connect import is_browser_debug_ready
