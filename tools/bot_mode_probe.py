...
def _default_home() -> str:
    """Ambient MIKASA_HOME (env, else ~/.hermes) as a string."""
    return os.getenv("mikasa_HOME") or os.path.expanduser("~/.hermes")
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    the AGENT'S OWN resolved home (session-db derived), not ambient MIKASA_HOME — build threads
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        token = set_mikasa_home_override(str(resolved))
        try:
