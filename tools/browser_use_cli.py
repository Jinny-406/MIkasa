...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_mikasa_home()) / "cache" / ".browser_use_default_notice"
...
def _managed_bin_dir() -> str:
    """$MIKASA_HOME/bin — where install.sh puts uv/uvx and install_cli() links browser-use."""
    return str(Path(get_mikasa_home()) / "bin")


def _find_cli() -> Optional[List[str]]:
    """Locate the browser-use CLI, or None when it can't be run. MANAGED-FIRST: Hermes' own ``$MIKASA_HOME/bin``
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    the binary into ``$MIKASA_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()`` resolves it for every profile.
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_mikasa_home()) / "cache" / "browser-use" / "workspace" / safe
