...
# get_temp_dir() defaults to MIKASA_HOME/cache/terminal (real storage, not tmpfs), so
...
def _default_terminal_temp_dir() -> "Path | None":
    """Return MIKASA_HOME/cache/terminal, or None if unresolvable."""
    try:
        from mikasa_constants import get_mikasa_home
        return get_mikasa_home() / "cache" / "terminal"
    except Exception:
        return None
...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local MIKASA_HOME override, then the subprocess HOME contract."""
    from mikasa_constants import apply_subprocess_home_env, get_mikasa_home_override
    try:
        if value := get_mikasa_home_override():
            env["mikasa_HOME"] = value
    except Exception:
        pass
...
mikasa_subprocess_env = hermes_subprocess_env  # backward compat alias
...
def build_subprocess_env(
    ...
    bridges MIKASA_HOME + HOME and ``extra`` is applied last so caller overrides win."""
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("mikasa_GIT_BASH_PATH")
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "mikasa_GIT_BASH_PATH=%s fails to start; using %s instead", custom, candidate)
            return candidate
    if candidates:
        probe_details = "\n".join(
...
def _managed_runtime_path_entries() -> list[str]:
    """Existing Hermes-managed runtime dirs: ``$MIKASA_HOME/node`` (+``/bin``) and
    ``$MIKASA_HOME/bin`` (managed ``uv``). Per call, not cached: home is
    profile-scoped and a managed tree can appear mid-process."""
    try:
        from mikasa_constants import get_mikasa_home, iter_hermes_node_dirs
        return [str(d) for d in (*iter_hermes_node_dirs(), get_mikasa_home() / "bin") if d.is_dir()]
    except Exception:
        return []
...
# configured MIKASA_HOME spelling (possibly a junction to another drive).
...
_hermes_repo_root_aliases: tuple[Path, ...] = _build_hermes_repo_root_aliases(
    _hermes_repo_root, Path(__file__).absolute().parents[2], get_process_mikasa_home())
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        (Termux has no /tmp), ``MIKASA_HOME/cache/terminal`` (real storage: tmpfs /tmp
        ...
        so always the MIKASA_HOME cache dir with forward slashes (bash- and Python-valid)."""
        if _IS_WINDOWS:
            cache_dir = (_default_terminal_temp_dir()
                         or Path(tempfile.gettempdir()) / "mikasa_terminal")
