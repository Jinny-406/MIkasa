...
_DEBUG_INTERRUPT = bool(os.getenv("mikasa_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    overlays/SIF cache). ``TERMINAL_SANDBOX_DIR`` overrides ``{MIKASA_HOME}/sandboxes``."""
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    p = Path(custom) if custom else get_mikasa_home() / "sandboxes"
...
class BaseEnvironment(ABC):
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"mikasa_STDIN_{uuid.uuid4().hex[:12]}"
