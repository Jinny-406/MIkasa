...
def _build_hermes_repo_root_aliases(
    ...
    ``gateway_windows._preserve_mikasa_home_path`` (physical path under the resolved
    MIKASA_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary MIKASA_HOME descendants as Hermes-owned. A repo-level junction
    ...
    # and the repo lives beside the profiles dir (as get_default_mikasa_root() does).
