...
MIKASA_HOME = get_mikasa_home()  # all skills live in ~/.hermes/skills/ (seeded from bundled)
...
def _skills_dir() -> Path:
    ...
    it, else live profile-scoped MIKASA_HOME (long-lived runtimes may import before profile set)."""
    configured = Path(SKILLS_DIR)
    return configured if configured != _SKILLS_DIR_AT_IMPORT else get_mikasa_home() / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from mikasa_HOME/.env."""
    env_path = get_mikasa_home() / ".env"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("mikasa_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from gateway.session_context import get_session_env
                resolved_platform = get_session_env("mikasa_SESSION_PLATFORM") or ""
