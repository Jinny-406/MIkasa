...
async def send_sticker(args) -> dict:
    ...
    target = (args.get("chat_id", "") or "").strip() or _session_env("mikasa_SESSION_CHAT_ID")
    if not target:
...
async def send_dm(args) -> dict:
    ...
    group_code = args.get("group_code", "")
    if not group_code and (chat_id := _session_env("mikasa_SESSION_CHAT_ID")).startswith("group:"):
...
def _check_yuanbao():
    ...
    return _session_env("mikasa_SESSION_PLATFORM") == "yuanbao" or _get_active_adapter() is not None
