...
def _node_fallback(command: str) -> str:
    ...
    MIKASA_HOME = os.path.expanduser(os.getenv("mikasa_HOME", os.path.join(home, ".hermes")))
    ...
    candidates = (os.path.join(MIKASA_HOME, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``mikasa_constants._candidate_node_command_names``). ``windows``
