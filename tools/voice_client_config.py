...
def _deepinfra_model(section: Dict[str, Any], kind: str) -> Optional[str]:
    ...
    from mikasa_cli.models import deepinfra_model_ids
...
def _resolve_stt_client_config() -> Dict[str, Any]:
    ...
    if provider == "deepinfra":
        ...
        if not api_key:
            return _relay("no credentials")
        from mikasa_cli.models import deepinfra_base_url
...
def resolve_client_voice_config() -> Dict[str, Any]:
    ...
    Callers scope the profile via ``mikasa_constants.set_mikasa_home_override``
