...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.hermes/\.env|\~/\.hermes/\.env',
     "mikasa_env_access", "critical", "exfiltration", "directly references Hermes secrets file"),
    ...
    (_prose_modify_re(_HERMES_CONFIG_FILES),
     "mikasa_config_mod", "high", "persistence", "modification language aimed at Hermes configuration files (verify intent)"),
    (_shell_write_re(_HERMES_CONFIG_FILES),
     "mikasa_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Hermes configuration files"),
    (r'\.hermes/config\.yaml|\.hermes/SOUL\.md',
     "mikasa_config_ref", "low", "persistence", "references Hermes configuration files (informational; only modification intent is scored)"),
