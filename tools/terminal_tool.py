...
# display_mikasa_home imported lazily at call site (stale-module safety during hermes update)
...
def _current_session_key() -> str:
    ...
    return get_session_env("mikasa_SESSION_KEY", "")
...
def _current_session_profile() -> str:
    ...
    return get_session_env("mikasa_SESSION_PROFILE", "")
...
def _ensure_terminal_env_bridged() -> None:
    ...
    with _quiet("terminal config → env fallback bridge failed"):
        from mikasa_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    from mikasa_cli.config import _is_ssh_remote_tilde_cwd
