...
def _global_allow_private_urls() -> bool:
    ...
    (``get_mikasa_home_override()`` set) bypass the process-global cache — a multiplex gateway serves
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("mikasa_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def _install_ssrf_guard_on_client(client: Any, *, is_async: bool = False) -> None:
    ...
    import contextvars
    var_name = "mikasa_ssrf_async_origin_schemes" if is_async else "mikasa_ssrf_origin_schemes"
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("mikasa_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("mikasa_ssrf_origin_schemes")
