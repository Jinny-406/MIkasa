...
# The broad "mikasa_" prefix is deliberately NOT safe — it leaked config vars
...
# while still inheriting MIKASA_HOME.
_HERMES_CHILD_ALLOWED = frozenset({
    "mikasa_HOME", "mikasa_PROFILE", "mikasa_CONFIG", "mikasa_ENV", "mikasa_DELEGATED_CHILD_CONTEXT",
})
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["mikasa_RPC_SOCKET"] = rpc_endpoint
    child_env["mikasa_RPC_TOKEN"] = rpc_token
    ...
    _tz_name = os.getenv("mikasa_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("mikasa_TIMEZONE", None)
    ...
    if _uses_hermes_python_environment(child_python):
        ...
        # Surface once per interpreter so "import mikasa_constants fails" is diagnosable.
