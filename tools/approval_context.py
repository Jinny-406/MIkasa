...
# os.environ["mikasa_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_hermes_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("mikasa_interactive", None)
...
def _is_interactive_cli() -> bool:
    ...
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("mikasa_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("mikasa_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    return _session_env("mikasa_SESSION_PLATFORM")


def _is_cron_approval_context() -> bool:
    """True when the current approval decision is running inside cron."""
    return is_truthy_value(_session_env("mikasa_CRON_SESSION"))
...
def _is_single_query_approval_context() -> bool:
    ...
    return is_truthy_value(_session_env("mikasa_SINGLE_QUERY_SESSION"))
...
def _is_gateway_approval_context() -> bool:
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("mikasa_GATEWAY_SESSION") or bool(_get_session_platform())
