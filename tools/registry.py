...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_mikasa_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
_BROWSER_IDENTITY_KEYS = (
    "mikasa_SESSION_ID",
    "mikasa_BROWSER_CONTROL_PRINCIPAL",
    "mikasa_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved MIKASA_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("mikasa_plugins."):
