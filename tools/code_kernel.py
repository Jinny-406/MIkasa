...
_SENTINEL = os.environ["mikasa_KERNEL_SENTINEL"]
_CAPTURE_LIMIT = {capture_limit}
_SPILL_DIR = os.environ.get("mikasa_KERNEL_SPILL_DIR", "")
_SPILL_CAP = {spill_cap}
_PARENT_PROCESS_HANDLE = os.environ.pop("mikasa_KERNEL_PARENT_PROCESS_HANDLE", "")
_PARENT_DEATH_FD = os.environ.pop("mikasa_KERNEL_PARENT_DEATH_FD", "")
...
def _resolve_owner(task_id: str) -> str:
    ...
    try:
        ...
        if is_delegated_child_context():
            ...
            child_id = get_session_env("mikasa_SESSION_ID", "") or (task_id or "")
...
def _bind_rpc_socket(kernel: SessionKernel) -> str:
    ...
    if _IS_WINDOWS:
        ...
        rpc_endpoint = kernel.sock_path = os.path.join(sock_tmpdir, f"mikasa_rpc_{uuid.uuid4().hex}.sock")
...
def _parent_process_handle(child_env: Dict[str, str]):
    ...
    try:
        ...
        if handle:
            child_env["mikasa_KERNEL_PARENT_PROCESS_HANDLE"] = str(int(handle))
            startupinfo = subprocess.STARTUPINFO()
            startupinfo.lpAttributeList = {"handle_list": [int(handle)]}
    except (AttributeError, ImportError, OSError, TypeError, ValueError):
        if handle and close is not None:
            close(handle)
        child_env.pop("mikasa_KERNEL_PARENT_PROCESS_HANDLE", None)
        handle = close = startupinfo = None
...
def _spawn(kernel: SessionKernel, *, child_python: str, child_cwd: str,
    ...
    kernel.tmpdir = tempfile.mkdtemp(prefix="mikasa_kernel_")
    ...
    for name, src in (("mikasa_tools.py", generate_hermes_tools_module(list(sandbox_tools))),
                      ("mikasa_kernel_runner.py", KERNEL_RUNNER_SOURCE)):
    ...
    if not _IS_WINDOWS:
        ...
        child_env["mikasa_KERNEL_PARENT_DEATH_FD"] = str(death_r)
        pass_fds = (death_r,)
    try:
        kernel.proc = subprocess.Popen(
            [child_python, os.path.join(kernel.tmpdir, "mikasa_kernel_runner.py")],
