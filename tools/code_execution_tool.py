...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_mikasa_dir("cache/exec", "exec_spill")
...
_FAILURE_HINT_RULES = (
    (r"cannot import name '(\w+)' from 'mikasa_tools'", _missing_hermes_tools_import_hint),
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["mikasa_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("mikasa_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    _attempts = 2 if os.environ.get("mikasa_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
_RPC_DIR = os.environ.get("mikasa_RPC_DIR") or os.path.join(tempfile.gettempdir(), "mikasa_rpc")
_seq = 0
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    try:
        ...
        rpc_thread.start()
        env_prefix = (f"mikasa_RPC_DIR={quoted_rpc_dir} HERMES_RPC_TOKEN={shlex.quote(rpc_token)} "
                      "PYTHONDONTWRITEBYTECODE=1")
        tz = os.getenv("mikasa_TIMEZONE", "").strip()
        if tz:
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        ...
        f"Available via `from mikasa_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "parameters": {
            ...
            "properties": {
                "code": {"type": "string", "description": (
                    ...
                    f"`from mikasa_tools import {import_str}` "
