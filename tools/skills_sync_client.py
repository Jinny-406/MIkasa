...
def resolve_sync_base_url() -> Optional[str]:
    """mikasa_SYNC_BASE_URL -> ``sync.base_url`` -> production plane, without trailing slash
    (``/v1/sync/`` is appended by the client). None only if the default is blanked out."""
    env = os.getenv("mikasa_SYNC_BASE_URL")
...
def sync_feature_enabled() -> bool:
    ...
    return _sync_config_bool("mikasa_SYNC_ENABLED", "enabled", default=False)
...
def sync_org_auto_propose() -> bool:
    ...
    return _sync_config_bool("mikasa_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False)
...
def sync_default_opt_in() -> bool:
    ...
    return _sync_config_bool("mikasa_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)
...
def stable_device_id() -> str:
    ...
    from mikasa_SYNC_DEVICE_NAME (first use only, for Hermes Cloud) or a friendly default, then persisted."""
    with suppress(OSError):
        val = _device_id_path().read_text(encoding="utf-8").strip()
        if val:
            return val
    val = (os.environ.get("mikasa_SYNC_DEVICE_NAME") or "").strip() or _default_device_label()
    try:
