...
from mikasa_constants import get_mikasa_dir, get_mikasa_home
...
def _contained_host_path(rel: str, MIKASA_HOME: Path, abs_msg: str, traversal_msg: str) -> Optional[Path]:
    """Resolve *rel* under MIKASA_HOME, refusing absolute paths and escapes."""
    if os.path.isabs(rel):
        logger.warning(abs_msg, rel)
        return None
    host_path = MIKASA_HOME / rel
    from tools.path_security import validate_within_dir  # resolves symlinks and ``..`` before checking

    if containment_error := validate_within_dir(host_path, MIKASA_HOME):
...
def register_credential_file(relative_path: str, container_base: str = "/root/.hermes") -> bool:
    """Register a MIKASA_HOME-relative credential file for mounting; True if it exists and was registered.

    Rejects absolute paths and traversal out of MIKASA_HOME. Containment alone is not
    enough: MIKASA_HOME holds the MASTER stores (``.env``, ``auth.json``, ``mcp-tokens/``),
    ...
    resolved = _contained_host_path(
        relative_path, get_mikasa_home(),
        "credential_files: rejected absolute path %r (must be relative to MIKASA_HOME)",
    ...
    if not resolved.is_file():
        ...
        return False
    # Master credential stores are never mountable, even though they sit inside MIKASA_HOME and therefore
...
def _skill_dir_roots(container_base: str) -> Iterator[Tuple[Path, str]]:
    ...
    skills_dir = get_mikasa_home() / "skills"
...
# (new_subpath, old_name) pairs matching mikasa_constants.get_mikasa_dir().
...
def _cache_dir_roots(container_base: str, *, create_missing: bool) -> Iterator[Tuple[Path, str]]:
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_mikasa_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # get_mikasa_dir already picked new-vs-legacy, so this can't shadow a legacy dir.
            try:
                ...
                # bind-mounted dir costs nothing; a missing mount costs the feature. get_mikasa_dir()
...
def get_cache_directory_mounts(container_base: str = "/root/.hermes") -> List[Dict[str, str]]:
    """Bind-mount entries for each cache directory (host layout via ``get_mikasa_dir``)."""
