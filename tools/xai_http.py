...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_mikasa_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`mikasa_cli.config.get_env_value` so tests can patch ``tools.xai_http.get_env_value``.
    """
    try:
        from mikasa_cli.config import get_env_value as _hermes_get_env_value
    except ImportError:
...
def hermes_xai_user_agent() -> str:
    """Return a stable Mikasa-specific User-Agent for xAI HTTP calls."""
    try:
        from mikasa_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Mikasa-Agent/{__version__}"


mikasa_xai_user_agent = hermes_xai_user_agent  # backward compat alias
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    ...
    try:
        from mikasa_constants import get_mikasa_home
        marker_dir = get_mikasa_home() / "state"
...
def _xai_base_url_override() -> str:
    ...
    return str(get_env_value("mikasa_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    via ``mikasa_cli.config.get_env_value`` so keys stored in ``~/.hermes/.env`` (the standard Hermes
