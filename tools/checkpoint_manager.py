...
CHECKPOINT_BASE = get_mikasa_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("mikasa_CHECKPOINT_TIMEOUT", 30)))
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir() if base.exists() else ():
        ...
        try:
            if (child / "mikasa_WORKDIR").exists():
                workdir = (child / "mikasa_WORKDIR").read_text(encoding="utf-8").strip()
