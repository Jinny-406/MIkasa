...
def _mirror_sent_message(platform_name, chat_id, mirror_text, thread_id):
    ...
    try:
        ...
        return bool(mirror_to_session(
            ...
            source_label=get_session_env("mikasa_SESSION_PLATFORM", "cli"),
...
def _maybe_skip_cron_duplicate_send(platform_name: str, chat_id: str, thread_id: str | None):
    ...
    from gateway.session_context import get_session_env
    auto_platform = get_session_env("mikasa_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    auto_chat_id = get_session_env("mikasa_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not (auto_platform and auto_chat_id and auto_platform == platform_name and auto_chat_id == str(chat_id)
            and (get_session_env("mikasa_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None) == thread_id):
        return None
