...
GitHub releases to $MIKASA_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_mikasa_home()), ".tirith-install-failed")
...
def _hermes_bin_dir() -> str:
    """$MIKASA_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_mikasa_home()), "bin"), exist_ok=True)
    return d
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MIKASA_HOME/bin/tirith -> ``(installed_path,
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $MIKASA_HOME/bin."""
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $MIKASA_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
