...
# Sensitive write targets, matched via ~ / $HOME / $MIKASA_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once MIKASA_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HERMES_ENV_PATH = (
    r'(?:~\/\.hermes/|(?:\$home|\$\{home\})/\.hermes/|(?:\$MIKASA_HOME|\$\{MIKASA_HOME\})/)' r'\.env\b'
)
...
_HERMES_CONFIG_PATH = (
    r'(?:~\/\.hermes/|(?:\$home|\$\{home\})/\.hermes/|(?:\$MIKASA_HOME|\$\{MIKASA_HOME\})/)' r'config\.yaml\b'
)
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/MIKASA_HOME set
    ...
    command = _rewrite_resolved_mikasa_home(command)
...
def _rewrite_resolved_mikasa_home(command: str) -> str:
    """Resolved MIKASA_HOME (and its realpath) -> ``~/.hermes/`` so the _HERMES_CONFIG_PATH /
    ...
    try:
        ...
        home = get_mikasa_home().expanduser()
