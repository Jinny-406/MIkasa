...
from mikasa_cli.goals import judge_goal
...
def _visible(*, to_env_worker: bool) -> bool:
    ...
    if _is_delegated_child_context():
        return False
    if os.environ.get("mikasa_KANBAN_TASK") and _is_dispatcher_owned_worker():
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if _is_delegated_child_context() or not _is_dispatcher_owned_worker():
        return None
    return os.environ.get("mikasa_KANBAN_TASK") or None
...
def _own_task_env(task_id: str, var: str) -> Optional[str]:
    ...
    return os.environ.get(var) if os.environ.get("mikasa_KANBAN_TASK") == task_id else None


def _worker_run_id(task_id: str) -> Optional[int]:
    """This worker's dispatcher run id when it is scoped to task_id."""
    raw = _own_task_env(task_id, "mikasa_KANBAN_RUN_ID")
    try:
...
def _stamp_worker_session_metadata(task_id: str, metadata: Optional[dict]) -> Optional[dict]:
    ...
    session_id = _own_task_env(task_id, "mikasa_SESSION_ID")
...
def _enforce_worker_task_ownership(tid: str) -> None:
    ...
    """
    env_tid = os.environ.get("mikasa_KANBAN_TASK")
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    tid = os.environ.get("mikasa_KANBAN_TASK")
    now = time.monotonic()
    ...
    try:
        ...
        with _board(None, quiet_close=True) as (kb, conn):
            ops = ((kb.heartbeat_claim, {"claimer": os.environ.get("mikasa_KANBAN_CLAIM_LOCK")}),
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    own = (os.environ.get("mikasa_PROFILE") or "").strip()
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        kb.heartbeat_claim(conn, tid, claimer=os.environ.get("mikasa_KANBAN_CLAIM_LOCK"))
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = os.environ.get("mikasa_PROFILE") or "worker"
    with _board(args.get("board")) as (kb, conn):
...
def _handle_create(args: dict, **kw) -> str:
    ...
    session_id = (args.get("session_id") or _current_origin_session_id()
                  or os.environ.get("mikasa_SESSION_ID"))
    ...
    with _board(args.get("board")) as (kb, conn):
        if project_id is None and workspace_kind is None and workspace_path is None:
            self_tid = os.environ.get("mikasa_KANBAN_TASK")
        ...
        new_tid = kb.create_task(
            ...
            parents=tuple(parents), tenant=args.get("tenant") or os.environ.get("mikasa_TENANT"),
            ...
            created_by=os.environ.get("mikasa_PROFILE") or "worker", session_id=session_id)
...
def _resolve_notify_target() -> Optional[dict[str, Any]]:
    ...
    platform, chat_id = env("mikasa_SESSION_PLATFORM", ""), env("mikasa_SESSION_CHAT_ID", "")
    if not platform or not chat_id:
        session_key = env("mikasa_SESSION_KEY", "") or os.environ.get("mikasa_SESSION_KEY", "")
        if not session_key:
            return None
        platform, chat_id = "tui", session_key
    chat_type = env("mikasa_SESSION_CHAT_TYPE", "") or None
    thread_id = env("mikasa_SESSION_THREAD_ID", "") or None
    message_id = env("mikasa_SESSION_MESSAGE_ID", "") or ""
    notifier_profile = env("mikasa_SESSION_PROFILE", "") or os.environ.get("mikasa_PROFILE")
    ...
    return dict(
        ...
        user_id=env("mikasa_SESSION_USER_ID", "") or None,
        user_id_alt=env("mikasa_SESSION_USER_ID_ALT", "") or None,
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    try:
        ...
        from mikasa_cli import kanban_db_notify as _kbn
