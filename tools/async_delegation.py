...
def _db_path():
    return get_mikasa_home() / "state.db"
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from mikasa_state_repair import apply_durability_barriers
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {k: v for k in _ROUTING_KEYS if (v := get_session_env(f"mikasa_SESSION_{k.upper()}", ""))}
...
def _current_origin_session_id() -> str:
    ...
    try:
        from gateway.session_context import get_session_env
        is_api = get_session_env("mikasa_SESSION_PLATFORM", "") == "api_server"
        return (get_session_env("mikasa_SESSION_CHAT_ID", "") or "") if is_api else ""
    except Exception:
        return ""
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    try:
        # Propagate the dispatching profile so the detached child resolves get_mikasa_home() correctly.
