...
from mikasa_cli.config import cfg_get, load_config, read_raw_config
...
def _config_cdp_url() -> str:
    ...
    try:
        from mikasa_cli.config import read_raw_config  # late-bound: tests patch the source module
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    with _sessions_lock:
        ...
        if identity is None:
            identity = {"user_id": f"mikasa_{uuid.uuid4().hex[:10]}", "session_key": f"task_{task_id[:16]}"}
...
def _save_screenshot(content: bytes) -> str:
    """Write PNG bytes under ``$MIKASA_HOME/browser_screenshots`` and return the path."""
    from mikasa_constants import get_mikasa_home
    screenshots_dir = get_mikasa_home() / "browser_screenshots"
