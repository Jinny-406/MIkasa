...
_STATE_DB_GUARD_BYPASS_ENV = "mikasa_STATE_DB_GUARD_BYPASS"


def _real_platform_state_root() -> Optional[Path]:
    """The REAL platform-default Hermes root. Avoids ``Path.home()`` /
    ``mikasa_constants`` (tests monkeypatch Path.home to a tempdir); ``expanduser``
...
#: Exported by the hermetic conftest alongside the MIKASA_HOME redirect. Unlike
...
# : Env marker exported by the hermetic test conftest at the same moment it : redirects ``MIKASA_HOME`` to
...
# received the patched ``MIKASA_HOME`` also received the marker, : and a child that resolves a production DB
# while carrying it is, by : definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "mikasa_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    rebuilt env loses PYTEST_* and the MIKASA_HOME redirect together, ancestry
    ...
    loses at the same moment it loses the ``MIKASA_HOME`` redirect: that child aims at the production DB
