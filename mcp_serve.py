...
def _mikasa_home() -> Path:
    try:
        ...
        return Path(os.environ.get("mikasa_HOME", Path.home() / ".hermes"))


def _get_sessions_dir() -> Path:
    return _mikasa_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_mikasa_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_mikasa_home() / "channel_directory.json")
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    ...
    mcp = MCPServer("hermes", instructions=(
        "Mikasa Agent messaging bridge. Use these tools to interact with "
