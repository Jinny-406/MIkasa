"""Centralized logging setup for Mikasa Agent.
...
from mikasa_constants import get_config_path, get_mikasa_home, mkdir_under_mikasa_home
...
def _install_session_record_factory() -> None:
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.MIKASA_HOME = str(get_mikasa_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.MIKASA_HOME = ""  # type: ignore[attr-defined]
        return record
...
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "mikasa_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("mikasa_cli", "cli"),
    "cron": ("cron",),
    "gui": ("mikasa_cli.web_server", "mikasa_cli.pty_bridge", "tui_gateway", "uvicorn"),
}


def setup_logging(
    *,
    MIKASA_HOME: Optional[Path] = None,
    ...
    home = MIKASA_HOME or get_mikasa_home()
    log_dir = mkdir_under_mikasa_home(home / "logs")
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from mikasa_cli.config import is_managed
...
def _new_file_handler(
    ...
    mkdir_under_mikasa_home(path.parent)
...
class _ProfileRoutingFileHandler(logging.Handler):
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "mikasa_home", "")
        try:
...
def _read_logging_config():
    ...
    try:
        ...
        # mikasa_cli.main's early parse (one config.yaml parse per process);
        # fall back to a direct parse for bare hermes_logging consumers.
        try:
            from mikasa_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
