...
def _settings() -> dict:
    ...
    issuer = setting("mikasa_DASHBOARD_OIDC_ISSUER", "issuer")
    client_id = setting("mikasa_DASHBOARD_OIDC_CLIENT_ID", "client_id")
    if not issuer or not client_id:
        raise SkipRegistration(
            ...
            "mikasa_DASHBOARD_OIDC_CLIENT_ID) or under "
    ...
    return {
        ...
        "scopes": setting("mikasa_DASHBOARD_OIDC_SCOPES", "scopes") or _DEFAULT_SCOPES,
        # Credential: canonical home is the env var / ~/.hermes/.env. Empty ⇒ public client.
        "client_secret": setting("mikasa_DASHBOARD_OIDC_CLIENT_SECRET", "client_secret")}
