...
from mikasa_cli.dashboard_auth import LoginStart, ProviderError, Session
...
def _settings() -> dict:
    ...
    client_id = resolve_env_or_cfg("mikasa_DASHBOARD_OAUTH_CLIENT_ID", section.get("client_id", ""))
    portal_url = resolve_env_or_cfg("mikasa_DASHBOARD_PORTAL_URL", section.get("portal_url", "")) or _DEFAULT_PORTAL_URL
    if not client_id:
        raise SkipRegistration(
            "mikasa_DASHBOARD_OAUTH_CLIENT_ID is not set (and dashboard.oauth.client_id "
            "in config.yaml is empty). The Nous Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Mikasa Agent instance — set it to "
    ...
    if not client_id.startswith("agent:"):
        raise SkipRegistration(
            f"mikasa_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match the contract "
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('mikasa_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('mikasa_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('mikasa_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('mikasa_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
