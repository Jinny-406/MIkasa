...
from mikasa_cli.dashboard_auth import DashboardAuthProvider, Session, TokenPrincipal
...
def _settings() -> dict:
    ...
    secret = os.environ.get("mikasa_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        raise SkipRegistration(
            "mikasa_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent >=256-bit secret "
    ...
    if reason is not None:
        raise SkipRegistration(
            f"mikasa_DASHBOARD_DRAIN_SECRET rejected — {reason}. The drain endpoint stays disabled (fail-closed).",
            level="warning")
...
def register(ctx) -> None:
    ...
    try:
        from mikasa_cli.dashboard_auth.token_auth import register_token_route
