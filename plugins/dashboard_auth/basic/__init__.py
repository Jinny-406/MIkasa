...
from mikasa_cli.dashboard_auth import DashboardAuthProvider, InvalidCredentialsError, RefreshExpiredError, Session
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    raw = resolve_env_or_cfg("mikasa_DASHBOARD_BASIC_AUTH_SECRET", cfg_section.get("secret"))
    if not raw:
        logger.info(
            ...
            "mikasa_DASHBOARD_BASIC_AUTH_SECRET) for stable sessions.")
...
def _settings() -> dict:
    ...
    username = setting("mikasa_DASHBOARD_BASIC_AUTH_USERNAME", "username")
    password_hash = setting("mikasa_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", "password_hash")
    plaintext = setting("mikasa_DASHBOARD_BASIC_AUTH_PASSWORD", "password")
    ttl_raw = setting("mikasa_DASHBOARD_BASIC_AUTH_TTL_SECONDS", "session_ttl_seconds")
    if not username:
    ...
    plaintext_from_env = os.environ.get("mikasa_DASHBOARD_BASIC_AUTH_PASSWORD", "").strip()
...
_PLUGIN_COMPAT_LAZY = {
    'LoginStart': ('mikasa_cli.dashboard_auth', 'LoginStart'),
}
