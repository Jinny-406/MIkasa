...
``$MIKASA_HOME/plugins/<name>/``, project ``./.hermes/plugins/<name>/`` (opt-in via
...
ENTRY_POINTS_GROUP = "mikasa_agent.memory_providers"
...
def _get_project_plugins_dir() -> Optional[Path]:
    ...
    try:
        from mikasa_cli.plugins import _env_enabled

        if not _env_enabled("mikasa_ENABLE_PROJECT_PLUGINS"):
            return None
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from mikasa_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from mikasa_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
