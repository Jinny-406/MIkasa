...
from mikasa_constants import get_mikasa_home  # noqa: F401 — patched by tests
...
def _prompt_api_key(label: str, env_var: str, MIKASA_HOME: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    env_path = Path(MIKASA_HOME) / ".env"
...
def _persist_provider_config(MIKASA_HOME: str, config: dict, provider_config: dict, env_writes: dict[str, str], label: str, key_line: str, server: str | None = None) -> None:
    ...
    Mem0MemoryProvider().save_config(provider_config, MIKASA_HOME)
    if env_writes:
        _write_env(Path(MIKASA_HOME) / ".env", env_writes)
    if server:
...
def _setup_platform(MIKASA_HOME: str, config: dict, flags: dict[str, str]) -> None:
    """Platform mode setup — prompts for API key (secret -> .env), user/agent ids and rerank (-> mem0.json)."""
    from . import _read_mem0_json
    provider_config = _read_mem0_json(Path(MIKASA_HOME) / "mem0.json")
    print("\n  Configuring mem0:\n")
    ...
    _persist_provider_config(MIKASA_HOME, config, provider_config, env_writes, "mem0", "API keys saved to .env")
...
def _setup_selfhosted(MIKASA_HOME: str, config: dict, flags: dict[str, str]) -> None:
    ...
    _persist_provider_config(MIKASA_HOME, config, provider_config, env_writes, "mem0 (self-hosted)", "API key saved to .env", server=host)
...
def _finish_oss(MIKASA_HOME: str, config: dict, oss_config: dict, env_writes: dict[str, str], user_id: str, agent_id: str, pgvector_config: dict | None = None) -> None:
    """Shared OSS tail: write secrets + mem0.json, install deps, activate, check, summarize."""
    from . import _read_mem0_json
    if env_writes:
        _write_env(Path(MIKASA_HOME) / ".env", env_writes)
    config_path = Path(MIKASA_HOME) / "mem0.json"  # merge-write, plain text (platform path uses save_config's 0600 atomic write)
...
def _setup_oss(MIKASA_HOME: str, config: dict, flags: dict[str, str]) -> None:
    """OSS mode — non-interactive when --mode was given, otherwise curses pickers."""
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(MIKASA_HOME, config)
        return
    ...
    if flags.get("dry_run"):
        ...
        print("  [dry-run] No files written.\n")
        return
    _finish_oss(MIKASA_HOME, config, oss_config, env_writes, flags.get("user_id") or os.getenv("USER", "hermes-user"), "hermes")
...
def _configure_model_provider(kind: str, registry: dict, MIKASA_HOME: str, env_writes: dict[str, str], llm: tuple[str, dict] | None = None) -> tuple[str, dict, str, str | None]:
    ...
    if pdef["needs_key"]:
        if llm is None or pid != llm[0]:
            if key := _prompt_api_key(pdef["label"] if llm is None else f"{pdef['label']} embedder", pdef["env_var"], MIKASA_HOME):
...
def _setup_oss_interactive(MIKASA_HOME: str, config: dict) -> None:
    env_writes: dict[str, str] = {}
    llm_id, llm_def, llm_model, llm_url = _configure_model_provider("LLM", LLM_PROVIDERS, MIKASA_HOME, env_writes)
    embedder_id, _, embedder_model, embedder_url = _configure_model_provider("Embedder", EMBEDDER_PROVIDERS, MIKASA_HOME, env_writes, llm=(llm_id, llm_def))
    ...
    _finish_oss(MIKASA_HOME, config, oss_config, env_writes, user_id, agent_id, pgvector_config)
...
def _install_provider_deps(llm_id: str, embedder_id: str, vector_id: str) -> None:
    ...
    for dep in sorted(deps):
        ...
        try:
            # Environment-aware install: sealed hosted venvs redirect to the durable data-volume target instead of /opt/mikasa.
...
def post_setup(MIKASA_HOME: str, config: dict) -> None:
    ...
    handler(MIKASA_HOME, config, flags)
