...
self-hosted Mem0 server (MEM0_HOST, HTTP), or OSS Memory. Secrets live in $MIKASA_HOME/.env
(MEM0_API_KEY, MEM0_HOST); settings in $MIKASA_HOME/mem0.json via `hermes memory setup`:
...
def _load_config() -> dict:
    """Env vars provide defaults; $MIKASA_HOME/mem0.json overrides individual keys.
    ...
    file_cfg = _read_mem0_json(get_mikasa_home() / "mem0.json")
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, MIKASA_HOME):
        """Merge-write config to $MIKASA_HOME/mem0.json."""
        from utils import atomic_json_write
        config_path = Path(MIKASA_HOME) / "mem0.json"
