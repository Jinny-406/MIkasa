...
Config in $MIKASA_HOME/config.yaml under plugins.hermes-memory-store: db_path ($MIKASA_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from mikasa_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, MIKASA_HOME):
        ...
        config_path = Path(MIKASA_HOME) / "config.yaml"
        try:
            import yaml
            from mikasa_cli.config import read_user_config_raw  # raw read: merged defaults must not be persisted
    ...
    def get_config_schema(self):
        ...
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_mikasa_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _mikasa_home = str(get_mikasa_home())
        db_path = self._config.get("db_path", _mikasa_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $MIKASA_HOME so paths resolve to the active profile
            db_path = db_path.replace("$MIKASA_HOME", _mikasa_home).replace("${MIKASA_HOME}", _mikasa_home)
