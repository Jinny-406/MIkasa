...
_SYNC_TRACE_ENV = "mikasa_OPENVIKING_SYNC_TRACE"
...
def _mikasa_home_path() -> Path:
    try:
        ...
        env_home = os.environ.get("mikasa_HOME")
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    log_path = _mikasa_home_path() / _OPENVIKING_SERVER_LOG_RELATIVE_PATH
    try:
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._session_id, self._turn_count, self._mikasa_home = "", 0, ""
    ...
    def post_setup(self, MIKASA_HOME: str, config: dict) -> None:
        ...
        _setup.run_setup(MIKASA_HOME, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._mikasa_home = str(kwargs.get("mikasa_home") or "").strip() or str(_mikasa_home_path())
    ...
    def _state_path(self, kind: str, name: str) -> Optional[Path]:
        """Marker/lock file under MIKASA_HOME: ``pending`` -> pending_sessions/<sid>.json,
        ``lock`` -> runs/<run_id>.lock; an empty run id maps to the legacy recovery lock."""
        name = str(name or "").strip()
        if not self._mikasa_home or (not name and kind != "lock"):
            return None
        if kind == "pending":
            return Path(self._mikasa_home) / _PENDING_SESSIONS_RELATIVE_DIR / f"{quote(name, safe='')}.json"
        return Path(self._mikasa_home) / _RUN_LOCKS_RELATIVE_DIR / (f"{quote(name, safe='')}.lock" if name else _LEGACY_RECOVERY_LOCK_FILENAME)
    ...
    def _pending_sessions(self) -> List[tuple[str, str]]:
        ...
        directory = Path(self._mikasa_home) / _PENDING_SESSIONS_RELATIVE_DIR if self._mikasa_home else None
