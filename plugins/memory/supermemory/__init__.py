...
def _load_supermemory_config(MIKASA_HOME: Optional[str] = None) -> dict:
    """Defaults overlaid with $MIKASA_HOME/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if MIKASA_HOME is not None:
        config.update({k: v for k, v in _read_json_dict(Path(MIKASA_HOME) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, MIKASA_HOME: str) -> None:
    from utils import atomic_json_write
    config_path = Path(MIKASA_HOME) / "supermemory.json"
...
def _probe_supermemory_connection(api_key: str, MIKASA_HOME: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(MIKASA_HOME)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._mikasa_home = ""
    ...
    def save_config(self, values, MIKASA_HOME):
        ...
        _save_supermemory_config(sanitized, MIKASA_HOME)

    def get_status_config(self, provider_config: dict) -> dict:
        from mikasa_constants import get_mikasa_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_mikasa_home())))}

    def post_setup(self, MIKASA_HOME: str, config: dict) -> None:
        from mikasa_cli.config import save_config
        from mikasa_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, MIKASA_HOME=MIKASA_HOME)
        ...
        status = _probe_supermemory_connection(api_key, MIKASA_HOME)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from mikasa_constants import get_mikasa_home
        self._mikasa_home = kwargs.get("mikasa_home") or str(get_mikasa_home())
        self._session_id, self._turn_count, self._session_turns = session_id, 0, []
        config = _load_supermemory_config(self._mikasa_home)
