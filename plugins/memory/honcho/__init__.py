...
Config chain: $MIKASA_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
"""
...
class HonchoMemoryProvider(DialecticMixin, MemoryProvider):
    ...
    def save_config(self, values, MIKASA_HOME):
        """Merge ``values`` into $MIKASA_HOME/honcho.json (Honcho SDK native format)."""
        from pathlib import Path
        from utils import atomic_json_write
        from plugins.memory.honcho.client import _read_config
        config_path = Path(MIKASA_HOME) / "honcho.json"
        try:
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        if cfg.session_strategy == "per-session":
            ...
            try:
                ...
                self._manager.migrate_memory_files(self._session_key, str(get_mikasa_home() / "memories"))
