...
Config file resolution: $MIKASA_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
def resolve_active_host() -> str:
    ...
    explicit = os.environ.get("mikasa_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    try:
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $MIKASA_HOME/honcho.json -> default profile's honcho.json
    ...
    local_path = get_mikasa_home() / "honcho.json"
    if local_path.exists():
        return local_path
    default_path = _get_default_mikasa_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_mikasa_home() later — those resolvers read a ContextVar that
    ...
    def from_env(cls, workspace_id: str = "hermes", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), MIKASA_HOME=get_mikasa_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=path, MIKASA_HOME=get_mikasa_home(),
        )
...
def spawn_context_thread(target, *, name: str, daemon: bool = True, args: tuple = ()) -> "_threading.Thread":
    ...
    (set_mikasa_home_override) and a plain Thread starts EMPTY, so ambient resolution on it
