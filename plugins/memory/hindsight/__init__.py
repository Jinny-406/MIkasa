...
Config: $MIKASA_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Mikasa Agent and the User"
...
def _context_thread(target, name: str) -> threading.Thread:
    ...
    closed without the profile's secret scope + MIKASA_HOME override. (The shared
...
def _load_config() -> dict:
    """$MIKASA_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    (legacy, shared), else environment variables."""
    for path in (get_mikasa_home() / "hindsight" / "config.json", Path.home() / ".hindsight" / "config.json"):
        if path.exists():
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, MIKASA_HOME):
        """Merge *values* into $MIKASA_HOME/hindsight/config.json."""
        from utils import atomic_json_write
        config_path = Path(MIKASA_HOME) / "hindsight" / "config.json"
    ...
    def post_setup(self, MIKASA_HOME: str, config: dict) -> None:
        ...
        run_setup(self, MIKASA_HOME, config)
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Mikasa Agent and the User"},
    ...
    def _daemon_start_worker(self) -> None:
        import traceback
        log_path = get_mikasa_home() / "logs" / "hindsight-embed.log"
