...
def _select(title: str, items: list, values: list, current) -> str | None:
    ...
    from mikasa_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
...
def run_setup(provider, MIKASA_HOME: str, config: dict) -> None:
    ...
    env_writes: dict = {}
    hermes_env = Path(MIKASA_HOME) / ".env"
    ...
    save_config(config)
    provider.save_config(provider_config, MIKASA_HOME)
    if env_writes:
    ...
    if mode == "local_embedded":
        ...
        with contextlib.suppress(Exception):
            materialized_config = json.loads(
                (Path(MIKASA_HOME) / "hindsight" / "config.json").read_text(encoding="utf-8")
            )
        llm_api_key = (
