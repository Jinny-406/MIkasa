...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'mikasa_meet_sink'}", str(pcm_path)]
...
def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("mikasa_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("mikasa_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("mikasa_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("mikasa_MEET_AUTH_STATE", "").strip(),
        guest_name=env("mikasa_MEET_GUEST_NAME", "Mikasa Agent"),
        duration_s=_parse_duration(env("mikasa_MEET_DURATION", "")),
        realtime=env("mikasa_MEET_MODE", "transcribe").strip().lower() == "realtime",
        ...
        realtime_api_key=env("mikasa_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("mikasa_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("mikasa_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("mikasa_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("mikasa_MEET_LOBBY_TIMEOUT", "300")))
