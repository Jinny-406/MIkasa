...
One active meeting at a time, recorded in ``$MIKASA_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_mikasa_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Mikasa Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "mikasa_MEET_URL": url, "mikasa_MEET_OUT_DIR": str(out),
           "mikasa_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "mikasa_MEET_HEADED"),
        (auth_state, "mikasa_MEET_AUTH_STATE"),
        (duration, "mikasa_MEET_DURATION"),
        (mode, "mikasa_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "mikasa_MEET_REALTIME_MODEL"),
        (realtime_voice, "mikasa_MEET_REALTIME_VOICE"),
        (realtime_instructions, "mikasa_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("mikasa_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["mikasa_MEET_REALTIME_KEY"] = realtime_api_key
