...
def _runtime_record_path() -> Path:
    from mikasa_constants import get_mikasa_home  # honors profile overrides
    return get_mikasa_home() / "runtime" / _RUNTIME_RECORD_NAME
...
def _reinstall_sidecar_deps() -> None:
    ...
    if not npm:
        ...
        return
    from mikasa_cli._subprocess_compat import windows_hide_flags  # no console flash on Windows
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _start_sidecar(self) -> None:
        ...
        from mikasa_cli._subprocess_compat import windows_hide_flags  # hide child console on Windows
