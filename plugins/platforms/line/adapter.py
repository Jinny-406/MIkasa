...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, MIKASA_HOME).
        ...
        try:
            ...
            MIKASA_HOME = Path(get_mikasa_home()).resolve()
        except Exception:
            MIKASA_HOME = Path.home().joinpath(".hermes").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), MIKASA_HOME)):
...
def interactive_setup() -> None:
    ...
    try:
        from mikasa_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("mikasa_cli.config not available; set LINE_* vars manually in ~/.hermes/.env")
        return
