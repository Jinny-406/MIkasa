...
from mikasa_constants import (find_node_executable, get_mikasa_dir, with_hermes_node_path)
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path = Path(extra.get("session_path", get_mikasa_dir("platforms/whatsapp/session", "whatsapp/session")))
    ...
    def _bridge_env(self) -> dict:
        ...
        for _key, _v in [("WHATSAPP_MODE", _wenv("WHATSAPP_MODE", "self-chat"))] + [(k, _wenv(k)) for k in _BRIDGE_PASSTHROUGH_ENV]:
            if _v:
                bridge_env[_key] = _v
        # Without these the bridge hardcodes ~/.hermes/{image,audio,document}_cache (wrong under MIKASA_HOME/profiles/cache layout).
...
def interactive_setup() -> None:
    ...
    from mikasa_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success
