...
def build_agent_card(*, name: str, url: str, description: str, skills: Optional[list[dict]] = None,
    ...
    card: dict[str, Any] = {
        ...
        "version": "1.0.0",
        "provider": {"organization": os.getenv("A2A_PROVIDER_ORG", "Mikasa Agent"), "url": os.getenv("A2A_PROVIDER_URL", "") or url},
...
def _conv_path(context_id: str) -> Path:
    ...
    return _mikasa_home() / "a2a_conversations" / f"{safe}.jsonl"
...
def list_conversations() -> list[str]:
    ...
    return sorted(p.stem for p in (_mikasa_home() / "a2a_conversations").glob("*.jsonl"))
