...
legacy single-user mode). Files under ``${MIKASA_HOME}``: ``google_chat_user_tokens/
...
from mikasa_constants import display_mikasa_home, get_mikasa_home
...
def _token_rel(email: Optional[str]) -> str:
    """mikasa_HOME-relative token file: per-user under the tokens dir, else the legacy path."""
...
def _user_tokens_dir() -> Path:
    return get_mikasa_home() / "google_chat_user_tokens"


def _token_path(email: Optional[str] = None) -> Path:
    """Per-user token path for ``email``, or the legacy single-user path."""
    return get_mikasa_home() / _token_rel(email)


def _client_secret_path() -> Path:
    return get_mikasa_home() / "google_chat_user_client_secret.json"


def _pending_auth_path(email: Optional[str] = None) -> Path:
    if email:
        return get_mikasa_home() / "google_chat_user_oauth_pending" / f"{_sanitize_email(email)}.json"
    return get_mikasa_home() / "google_chat_user_oauth_pending.json"
...
def install_deps() -> bool:
    ...
    try:
        from mikasa_cli.tools_config import _pip_install
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into MIKASA_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    print(f"Profile path: {display_mikasa_home()}/{_token_rel(email)}")
