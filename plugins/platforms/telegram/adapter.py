...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("mikasa_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "mikasa_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", 0.3, min_value=0.08, max_value=2.0)
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "mikasa_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 1.0, min_value=self._text_batch_delay_seconds, max_value=4.0)
    ...
    def _prune_stale_dm_topic_binding(self, chat_id: Any, thread_id: Any, *, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        if db is None or not hasattr(db, "delete_telegram_topic_binding"):
            return
        try:
            profile_name = (metadata or {}).get("mikasa_profile") or getattr(self, "_hermes_profile_name", None) or "default"
    ...
    async def _register_command_menu(self) -> None:
        ...
        from mikasa_cli.commands_platforms import telegram_menu_commands, telegram_menu_max_commands
        if not self._bot:
            return
    ...
    async def _build_ptb_requests(self) -> tuple:
        ...
        request_kwargs = {
            "connection_pool_size": env_int("mikasa_TELEGRAM_HTTP_POOL_SIZE", 512),
            "pool_timeout": env_float("mikasa_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
            "connect_timeout": env_float("mikasa_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
            "read_timeout": env_float("mikasa_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
            "write_timeout": env_float("mikasa_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
        ...
        disable_fallback = os.getenv("mikasa_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"}
        fallback_ips = [] if disable_fallback else self._fallback_ips()
        if not fallback_ips and not disable_fallback:
            discovery_timeout = self._env_float_clamped("mikasa_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT", 5.0, min_value=0.0)
    ...
    async def _initialize_app_with_retries(self, builder) -> None:
        ...
        _max_connect = 8
        _init_timeout = env_float("mikasa_TELEGRAM_INIT_TIMEOUT", 30.0)  # per attempt
            ...
            try:
                if asyncio.get_running_loop().time() >= _total_deadline:
                    raise OSError(
                        ...
                        f"mikasa_TELEGRAM_INIT_TIMEOUT to a lower value.")
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        try:
            from mikasa_cli.models_catalog_static import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(self, query, data: str, chat_id: str) -> None:
        ...
        if data.startswith("mp:"):  # provider selected: show model buttons (page 0)
            ...
            group_id = data[4:]
            try:
                from mikasa_cli.models_catalog_static import PROVIDER_GROUPS
    ...
    async def _handle_update_prompt_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        ...
        try:
            from mikasa_constants import get_mikasa_home
            response_path = get_mikasa_home() / ".update_response"
    ...
    def _reload_dm_topics_from_config(self) -> None:
        ...
        try:
            from mikasa_cli.config import load_config_readonly  # canonical loader: managed overlay + ${VAR}
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("mikasa_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    from mikasa_cli import setup as _setup_mod
