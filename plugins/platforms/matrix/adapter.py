...
from mikasa_constants import get_mikasa_dir as _get_mikasa_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def _resolve_store_dir(self) -> Path:
        ...
        self._store_dir = _get_mikasa_dir("platforms/matrix/store", "matrix/store")
        return self._store_dir

    @property
    def _crypto_db_path(self) -> Path:
        return (self._store_dir or _get_mikasa_dir("platforms/matrix/store", "matrix/store")) / "crypto.db"
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("mikasa_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("mikasa_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    identifier=self._user_id, password=self._password, device_name="Mikasa Agent",
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        words, or "Mikasa Agent" would become "Agent"."""
        if not body:
            return ""
        if self._user_id:
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        txn_id = f"mikasa_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
