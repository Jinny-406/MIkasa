...
# long after profile/MIKASA_HOME overrides have been applied, so freezing is safe.
RULES_FILE = get_mikasa_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_mikasa_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("mikasa_cli.env_loader", fromlist=["load_mikasa_dotenv"]).load_mikasa_dotenv()
    except Exception:
        pass
