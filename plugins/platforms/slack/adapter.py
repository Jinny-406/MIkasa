...
_MODEL_PICKER_PROVIDER_ACTION = "mikasa_model_provider"
_MODEL_PICKER_MODEL_ACTION = "mikasa_model_model"
_MODEL_PICKER_BACK_ACTION = "mikasa_model_back"
_MODEL_PICKER_CANCEL_ACTION = "mikasa_model_cancel"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    if not text.startswith("!"):
        return text
    try:
        from mikasa_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
    ...
    def _register_bolt_handlers(self) -> None:
        ...
        from mikasa_cli.commands_platforms import slack_native_slashes
        ...
        self._app.action("mikasa_feedback")(self._handle_feedback_action)
        ...
        self._app.action("mikasa_clarify_other")(self._handle_clarify_action)
    ...
    async def send_exec_approval(
        ...
        def _build() -> Tuple[str, list]:
            ...
            actions = [
                self._button("Allow Once", "mikasa_approve_once", session_key, style="primary")]
            if not smart_denied and allow_session:
                actions.append(self._button("Allow Session", "mikasa_approve_session", session_key))
                if allow_permanent:
                    actions.append(
                        self._button("Always Allow", "mikasa_approve_always", session_key))
            actions.append(self._button("Deny", "mikasa_deny", session_key, style="danger"))
            blocks = [
                {
                    "type": "section",
        ...
        def _build() -> Tuple[str, list]:
            ...
            blocks = [
                {"type": "section", "text": {"type": "mrkdwn", "text": f"*{_title}*\n\n{body}"}},
                {
                    "type": "actions",
                    "elements": [
                        self._button("Approve Once", "mikasa_confirm_once", value, style="primary"),
                        self._button("Always Approve", "mikasa_confirm_always", value),
                        self._button("Cancel", "mikasa_confirm_cancel", value, style="danger")]}]
    ...
    async def send_model_picker(
        ...
        try:
            ...
            try:
                from mikasa_cli.providers import get_label
    ...
    async def send_clarify(
        ...
        def _build() -> Tuple[str, list]:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append(
                    self._button(
                        label[:75], f"mikasa_clarify_choice_{idx}",
                        f"{clarify_id}|{idx}", emoji=True))
            elements.append(
                self._button("✏️ Other…", "mikasa_clarify_other", f"{clarify_id}|other", emoji=True)
            )
    ...
    _APPROVAL_CHOICES: ClassVar[Dict[str, str]] = {
        "mikasa_approve_once": "once", "mikasa_approve_session": "session",
        "mikasa_approve_always": "always", "mikasa_deny": "deny"}
    ...
    _CONFIRM_CHOICES: ClassVar[Dict[str, str]] = {
        "mikasa_confirm_once": "once", "mikasa_confirm_always": "always",
        "mikasa_confirm_cancel": "cancel"}
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "mikasa_clarify_other" or token == "other":
    ...
    def _slash_command_text(command: dict) -> str:
        ...
        from mikasa_cli.commands_platforms import slack_subcommand_map
...
def _load_slack_bot_tokens(raw_token: str, *, quiet: bool) -> List[str]:
    ...
    try:
        ...
        tokens_file = get_mikasa_home() / "slack_tokens.json"
...
def _write_slack_manifest_and_instruct() -> None:
    """Write the manifest under MIKASA_HOME and print paste instructions; non-fatal."""
    from mikasa_cli.cli_output import print_info, print_success, print_warning
    try:
        from mikasa_cli.slack_cli import _build_full_manifest
        from mikasa_constants import get_mikasa_home
        manifest = _build_full_manifest(
            bot_name="Hermes", bot_description="Your Hermes agent on Slack")
        target = _Path(get_mikasa_home()) / "slack-manifest.json"
...
def interactive_setup() -> None:
    ...
    from mikasa_cli.config import get_env_value, remove_env_value, save_env_value
    from mikasa_cli.cli_output import (
