...
_NATIVE_SLASH_COMMANDS: tuple = (
    ...
    ("voice", "Toggle voice reply mode",
     ...
     "/voice {mode}", None),
    ("update", "Update Mikasa Agent to the latest version", (), "/update", "Update initiated~"),
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("mikasa_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("mikasa_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            env_key="mikasa_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold", 2,
            env_key="mikasa_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_mikasa_home())
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("mikasa_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        from mikasa_constants import get_mikasa_home
        directory = get_mikasa_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "mikasa_voice", f"ack_{_uuid.uuid4().hex[:12]}.mp3",
        )
    ...
    def _register_slash_commands(self) -> None:
        ...
        def _auto_register(name: str, description: str, args_hint: str) -> None:
            ...
            try:
                ...
                pass
        try:
            from mikasa_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
            try:
        ...
        try:
            from mikasa_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        from mikasa_cli.commands_platforms import discord_skill_commands_by_category
...
_YAML_WEBSOCKET_LIVENESS_KEYS = (
    ("websocket_liveness_interval_seconds", "liveness_interval_seconds", "mikasa_DISCORD_LIVENESS_INTERVAL_SECONDS"),
    ("websocket_liveness_failure_threshold", "liveness_failure_threshold", "mikasa_DISCORD_LIVENESS_FAILURE_THRESHOLD"),
