...
_LANGFUSE_KEY_PREFIXES: Dict[str, str] = {
    "mikasa_LANGFUSE_PUBLIC_KEY": "pk-lf-",
    "mikasa_LANGFUSE_SECRET_KEY": "sk-lf-",
}
...
def _debug(message: str) -> None:
    if _env("mikasa_LANGFUSE_DEBUG").lower() in {"1", "true", "yes", "on"}:
...
def _capture_mode() -> str:
    ...
    value = _env("mikasa_LANGFUSE_CAPTURE").lower()
...
def _build_client() -> Optional[Langfuse]:
    ...
    public_key, secret_key = (_env(f"mikasa_LANGFUSE_{n}") or _env(f"LANGFUSE_{n}") for n in ("PUBLIC_KEY", "SECRET_KEY"))
    ...
    placeholder_issues = [issue for issue in (
        _validate_langfuse_key("mikasa_LANGFUSE_PUBLIC_KEY", public_key),
        _validate_langfuse_key("mikasa_LANGFUSE_SECRET_KEY", secret_key),
    ) if issue]
    ...
    for key, name, default in (("base_url", "BASE_URL", "https://cloud.langfuse.com"), ("environment", "ENV", ""),
                               ("release", "RELEASE", "")):
        value = _env(f"mikasa_LANGFUSE_{name}") or _env(f"LANGFUSE_{name}") or default
        if value:
            kwargs[key] = value
    sample_rate = _env("mikasa_LANGFUSE_SAMPLE_RATE")
    if sample_rate:
        try:
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("mikasa_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
