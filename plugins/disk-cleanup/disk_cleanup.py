...
after 14 days; empty dirs under MIKASA_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly MIKASA_HOME and /tmp/hermes-*; never ~/.hermes/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$MIKASA_HOME/disk-cleanup/<name>`` — deliberately outside ``$MIKASA_HOME/logs/``."""
    return get_mikasa_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under MIKASA_HOME or ``/tmp/hermes-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_mikasa_home())
        return True
    parts = path.parts
...
# Top-level MIKASA_HOME dirs whose empty subdirs are never swept (last row: user project trees).
...
# Top-level MIKASA_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
@functools.lru_cache(maxsize=1)  # built lazily so MIKASA_HOME resolves once
def _protected_cron_paths() -> frozenset:
    ...
    return frozenset(str(x) for parent in ("cron", "cronjobs") for base in (get_mikasa_home() / parent,)
                     for x in (base, base / "output", base / "jobs.json", base / ".tick.lock"))


# Paths under $MIKASA_HOME that must NEVER be deleted by quick(), regardless of what the stored category
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MIKASA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    for item, p, age in _live_items(load_tracked(), datetime.now(timezone.utc), log_stale=True):
        ...
        if err is None:
            ...
            errors.append(err)
            new_tracked.append(item)
    empty_removed = _sweep_empty_dirs(get_mikasa_home())
...
def _sweep_empty_dirs(MIKASA_HOME: Path) -> int:
    """Remove empty dirs under MIKASA_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under MIKASA_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(MIKASA_HOME, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under MIKASA_HOME (/tmp/hermes-*) — name rules only
        rel = path.resolve().relative_to(get_mikasa_home())
