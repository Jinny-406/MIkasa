...
from mikasa_cli import kanban_db
...
from mikasa_cli.kanban_db import KANBAN_ATTACHMENT_MAX_BYTES, _collision_free_path, _safe_attachment_name
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    try:
        from mikasa_cli import web_server_chat as _ws
    except Exception:
        return True
...
def _run_aux(board: Optional[str], module: str, fn: str, task_id: str, author: Optional[str]) -> Any:
    ...
    def _run():
        return getattr(importlib.import_module(f"mikasa_cli.{module}"), fn)(task_id, author=(author or None))
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        # dashboard backend may run under a different MIKASA_HOME than the board's profile.
        if task and task.status == "ready" and task.assignee:
            try:
                from mikasa_cli.kanban import _check_dispatcher_presence
                from mikasa_constants import get_mikasa_home
                running, message = _check_dispatcher_presence(MIKASA_HOME=get_mikasa_home())
...
def model_options():
    ...
    try:
        from mikasa_cli.inventory import build_models_payload, load_picker_context
...
async def export_board_endpoint(slug: str, body: ExportBoardBody):
    ...
    from mikasa_cli import kanban_transfer
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    with _errors_to_500("failed to update profile"):
        ...
        if canon == "default":
            from mikasa_constants import get_mikasa_home  # type: ignore
            profile_dir = Path(get_mikasa_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    with _errors_to_500("describer crashed"):
        from mikasa_cli import profile_describer
