...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("mikasa_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("mikasa_SESSION_SOURCE", "")
...
_mikasa_home = get_mikasa_home()  # read by agent_init via _ra()._mikasa_home
_loaded_env_paths = load_mikasa_dotenv(MIKASA_HOME=_mikasa_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[mikasa-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from mikasa_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        return cfg if cfg is not None else env_float("mikasa_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("mikasa_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("mikasa_API_CALL_STALE_TIMEOUT") is not None)
