"""Shared constants for Mikasa Agent.
...
_UNSET = object()
_MIKASA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_MIKASA_HOME_OVERRIDE", default=_UNSET)

# Backwards compatibility alias
_HERMES_HOME_OVERRIDE = _MIKASA_HOME_OVERRIDE
...
INDICATOR_STYLES: tuple[str, ...] = ("ascii", "emoji", "kaomoji", "unicode", "pulse")
DEFAULT_INDICATOR_STYLE: str = "kaomoji"


def set_mikasa_home_override(path: str | Path | None) -> Token:
    """Set a context-local Mikasa home override and return its reset token.

    Deliberately does not mutate ``os.environ`` (shared by every thread in the process).
    """
    value: str | object = _UNSET if path is None else str(path)
    return _MIKASA_HOME_OVERRIDE.set(value)


# Backwards compatibility alias
set_hermes_home_override = set_mikasa_home_override


def reset_mikasa_home_override(token: Token) -> None:
    """Restore the previous context-local Mikasa home override."""
    _MIKASA_HOME_OVERRIDE.reset(token)


# Backwards compatibility alias
reset_hermes_home_override = reset_mikasa_home_override


def get_mikasa_home_override() -> str | None:
    """Return the active context-local Mikasa home override, if any."""
    override = _MIKASA_HOME_OVERRIDE.get()
    return str(override) if override is not _UNSET and override else None


# Backwards compatibility alias
get_hermes_home_override = get_mikasa_home_override


def _get_env_or_fallback(mikasa_key: str, hermes_key: str) -> str:
    """Get MIKASA_* env var, falling back to HERMES_* for backwards compatibility."""
    return os.environ.get(mikasa_key, "") or os.environ.get(hermes_key, "")


def _get_platform_default_mikasa_home() -> Path:
    """Return the platform-native default Mikasa home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "mikasa"
    return Path.home() / ".mikasa"


# Backwards compatibility alias
_get_platform_default_hermes_home = _get_platform_default_mikasa_home


def _warn_profile_fallback_once() -> None:
    """Warn once when MIKASA_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
    global _profile_fallback_warned
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_mikasa_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[MIKASA_HOME fallback] MIKASA_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass MIKASA_HOME explicitly "
            f"(see issue #18594)."
        )
...
def get_mikasa_home() -> Path:
    """Mikasa home: context-local override → ``MIKASA_HOME`` env var → platform default."""
    override = get_mikasa_home_override()
    if override:
        return Path(override)
    if not _get_env_or_fallback("MIKASA_HOME", "HERMES_HOME").strip():
        _warn_profile_fallback_once()
    return get_process_mikasa_home()


# Backwards compatibility alias
get_hermes_home = get_mikasa_home
...
def mikasa_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Mikasa home/profile dir.
    ...
    """
    candidate = Path(path) if path is not None else get_mikasa_home()
    raw = str(candidate)
...
# Backwards compatibility alias
hermes_home_key = mikasa_home_key
MIKASA_HOME_key = mikasa_home_key


def reset_mikasa_home_key_cache() -> None:
    """Forget every remembered home key (for tests that move a home dir on disk)."""
    _HOME_KEY_CACHE.clear()


# Backwards compatibility alias
reset_hermes_home_key_cache = reset_mikasa_home_key_cache


def get_process_mikasa_home() -> Path:
    """Mikasa home of the running process, ignoring task overrides.
    ...
    """
    val = _get_env_or_fallback("MIKASA_HOME", "HERMES_HOME").strip()
    return Path(val) if val else _get_platform_default_mikasa_home()


# Backwards compatibility alias
get_process_hermes_home = get_process_mikasa_home


# get_default_mikasa_root() memo keyed on (native home, MIKASA_HOME) so it stays
# fresh when a test or plugin mutates MIKASA_HOME; saves ~80us/call at 31+ sites.
_default_mikasa_root_memo: "tuple[str, str, Path] | None" = None


def get_default_mikasa_root() -> Path:
    """Root Mikasa dir for profile-level ops: ``<root>`` when ``MIKASA_HOME=<root>/profiles/<name>``."""
    global _default_mikasa_root_memo
    native_home = _get_platform_default_mikasa_home()
    env_home = _get_env_or_fallback("MIKASA_HOME", "HERMES_HOME")
    memo = _default_mikasa_root_memo
    ...
    if env_home:
        ...
        try:
            env_path.resolve().relative_to(native_home.resolve())  # under ~/.mikasa (normal or profile mode)
        except ValueError:  # Docker/custom root: <root>/profiles/<name> -> <root>, else MIKASA_HOME itself
            result = env_path.parent.parent if env_path.parent.name == "profiles" else env_path
    _default_mikasa_root_memo = (str(native_home), env_home, result)
    return result


# Backwards compatibility alias
get_default_hermes_root = get_default_mikasa_root
...
# Files marking a real Mikasa home; arbitrary dirs with a ``profiles`` segment lack them.
_MIKASA_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_mikasa_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<mikasa-home>/profiles``.

    Accepts the classic ``~/.mikasa`` layout, a root carrying Mikasa-home marker files, a
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _MIKASA_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_mikasa_root().resolve(strict=False)
    except OSError:
        return False


# Backwards compatibility alias
_is_hermes_profiles_root = _is_mikasa_profiles_root


def named_profile_home(path: str | Path) -> Path | None:
    """Return ``<root>/profiles/<name>`` when *path* is that home or under it.

    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Mikasa home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_mikasa_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".mikasa":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def mkdir_under_mikasa_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_mikasa_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("MIKASA_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers."""
    return _packaged_dir("MIKASA_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers."""
    return _packaged_dir("MIKASA_BUNDLED_SKILLS", default, "skills")


def get_mikasa_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    """
    node_dir = (home or get_mikasa_home()) / "node"
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Mikasa-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_mikasa_node_path())
    return result is not None and result.returncode == 0


_MIKASA_NODE_TARGET_MAJOR = int(_get_env_or_fallback("MIKASA_NODE_TARGET_MAJOR", "HERMES_NODE_TARGET_MAJOR") or "22")
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/mikasa_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks mikasa-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def hermes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Mikasa-managed node/npm/npx shim exists on disk."""
...
# Backwards compatibility alias
mikasa_managed_node_tree_present = hermes_managed_node_tree_present
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Mikasa-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `mikasa update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_MIKASA_NODE_TARGET_MAJOR}.x/"
    index_bytes = _fetch_url(index_url, 60)
    if index_bytes is None:
        return None
    pattern = rf"node-v{_MIKASA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%MIKASA_HOME%\\node`` on Windows.
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MIKASA_HOME": str(get_mikasa_home()), **extra_env},
...
def bootstrap_hermes_managed_node() -> str | None:
    """Install a Mikasa-managed Node tree under ``$MIKASA_HOME/node`` and return its npm path.

    Mikasa never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    ...
    if sys.platform == "win32":
        ...
        # MIKASA_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, MIKASA_NODE_SKIP_LINKS="1")
    if not ok:
        return None
...
# Backwards compatibility alias
bootstrap_mikasa_managed_node = bootstrap_hermes_managed_node


def heal_hermes_managed_node() -> bool:
    """Redownload Mikasa-managed Node when the tree exists but is broken; at most once per process.
...
# Backwards compatibility alias
heal_mikasa_managed_node = heal_hermes_managed_node
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    for candidate in _iter_managed_node_candidates(_candidate_node_command_names("node"), home):
        ...
        if "-" in version:
            return True
        return major < _MIKASA_NODE_TARGET_MAJOR
    return False


def find_hermes_node_executable(command: str) -> str | None:
    """Mikasa-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
...
# Backwards compatibility alias
find_mikasa_node_executable = find_hermes_node_executable
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Mikasa-managed Node directories prepended to PATH."""
...
# Backwards compatibility alias
with_mikasa_node_path = with_hermes_node_path
...
def display_mikasa_home() -> str:
    """User-facing ``~/`` display string for MIKASA_HOME (``~/.mikasa/profiles/coder``)."""
    home = get_mikasa_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\mikasa/skills/
...
# Backwards compatibility alias
display_hermes_home = display_mikasa_home


def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved MIKASA_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks mikasa-user traversal in Docker (UID 10000).
    # A credential file here means MIKASA_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "mikasa-agent install directory (%s). Credential files are "
            "normally stored under the mikasa home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{MIKASA_HOME}/home`` when the profile-home directory exists."""
    mikasa_home = get_mikasa_home_override() or (env or {}).get("MIKASA_HOME") or _get_env_or_fallback("MIKASA_HOME", "HERMES_HOME")
    if not mikasa_home:
        return None
    profile_home = os.path.join(mikasa_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "MIKASA_REAL_HOME"), _env_get(env, "HERMES_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    ...
    already running with ``HOME={MIKASA_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{MIKASA_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Mikasa's subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["MIKASA_REAL_HOME"] = real_home
        env["HERMES_REAL_HOME"] = real_home  # Backwards compatibility
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under MIKASA_HOME."""
    return get_mikasa_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under MIKASA_HOME."""
    return get_mikasa_home() / "skills"


def get_env_path() -> Path:
    """Return the path to the ``.env`` file under MIKASA_HOME."""
    return get_mikasa_home() / ".env"
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Mikasa (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("mikasa_"))
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    return [
        ...
        "    mikasa update",
        "If that also fails, reinstall: https://mikasa-agent.nousresearch.com",
    ]
