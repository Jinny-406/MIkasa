...
Capabilities:
- Persist telephony provider credentials to the Hermes .env file ($MIKASA_HOME/.env)
...
def _mikasa_home() -> Path:
    return Path(os.environ.get("mikasa_HOME", "~/.hermes")).expanduser()


def _env_path() -> Path:
    return _mikasa_home() / ".env"


def _config_path() -> Path:
    return _mikasa_home() / "config.yaml"


def _state_path() -> Path:
    return _mikasa_home() / "telephony_state.json"
