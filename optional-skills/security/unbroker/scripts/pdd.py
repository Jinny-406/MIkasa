...
Run it through the `terminal` tool (it can read PII files under MIKASA_HOME);
...
def cmd_setup(args) -> None:
    if getattr(args, "auto", False):
        ...
        # asking anyone. Read creds from $MIKASA_HOME/.env too (the terminal shell doesn't export
...
def cmd_doctor(args) -> None:
    ...
    cfg = config_mod.load_config()
    caps = config_mod.detect_capabilities(config_mod.dotenv_env())  # see creds in $MIKASA_HOME/.env too
    ...
    if engaged:
        L.append(f"        key: {crypto.identity_path()} (0600) - guards casual/backup/commit "
                 "exposure, NOT a full-MIKASA_HOME read")
    ...
    if not crypto.is_engaged():
        L.append("  Storage: dossiers are PLAINTEXT JSON (0600 under MIKASA_HOME). "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--profile",
                   help="user-data-dir (default: $MIKASA_HOME/chrome-debug, a dedicated debug profile)")
