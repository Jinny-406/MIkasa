...
All per-subject data lives under PDD_DATA_DIR (default: $MIKASA_HOME/unbroker),
...
def MIKASA_HOME() -> Path:
    return Path(os.environ.get("mikasa_HOME") or (Path.home() / ".hermes"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else MIKASA_HOME() / "unbroker"
