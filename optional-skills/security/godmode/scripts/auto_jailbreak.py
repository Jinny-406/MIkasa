...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("mikasa_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("mikasa_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
MIKASA_HOME = Path(os.getenv("mikasa_HOME", Path.home() / ".hermes"))
CONFIG_PATH = MIKASA_HOME / "config.yaml"
PREFILL_PATH = MIKASA_HOME / "prefill.json"
