"""SQLite journal-mode and PRAGMA policy for state.db (split from mikasa_state).
...
from mikasa_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
def _wal_reset_repair_hint() -> str:
    ...
    """
    try:
        from mikasa_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from mikasa_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
