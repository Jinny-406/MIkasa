...
ENV_STARTUP_WATCHDOG = "mikasa_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "mikasa_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (mikasa_cli.main / hermes_cli.gateway /
...
def _process_mikasa_home() -> Path:
    """mikasa_HOME for diagnostic files — stdlib-only replica of the mikasa_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<MIKASA_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_mikasa_home()
