...
def detect_service_manager() -> ServiceManagerKind:
    ...
    from mikasa_cli.gateway import is_macos, is_windows, supports_systemd_services
...
class _HostServiceManager:
    ...
    def _backend_module(self):
        import importlib
        import mikasa_cli
        importlib.import_module(f"mikasa_cli.{self._backend}")
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's MIKASA_HOME blindly: derive the shared profile root and map the suffix to the root
    ...
    root = MIKASA_HOME.parent.parent if MIKASA_HOME.parent.name == "profiles" else MIKASA_HOME
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources MIKASA_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to hermes.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root MIKASA_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``MIKASA_HOME``, two profiles that
        ...
        # mikasa_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export HERMES_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its MIKASA_HOME. Without it
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            f': "${{MIKASA_HOME:=/opt/data}}"\n'
            f'log_dir="$MIKASA_HOME/logs/gateways/{prof}"\n'
