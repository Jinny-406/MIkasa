...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from mikasa_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.MIKASA_HOME, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_mikasa_home_override(flow.MIKASA_HOME)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.MIKASA_HOME)))
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, MIKASA_HOME=flow.MIKASA_HOME)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, MIKASA_HOME=flow.MIKASA_HOME)
                    raise
        finally:
