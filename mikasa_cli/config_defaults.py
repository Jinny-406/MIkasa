"""Default configuration data for Mikasa Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from mikasa_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "providers": {
        "google": {
            "api_key": "AIzaSyB5dV_033WSjZiuVQXFSsGidhvhqjBGCzE",
    ...
    "session": {
        ...
        # $MIKASA_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # GODMODE: Uncensored response system (inspired by G0DM0D3). When enabled, detects model
        # refusals and retries with jailbreak templates. Coaching removes disclaimers from responses.
        "godmode": {
            "enabled": True,           # Master switch for the GODMODE system
            "coaching": True,          # PLINY coaching loop to improve refused responses
            "max_retries": 2,          # Max GODMODE template retries per refusal
            "ultraplinian": False,     # Multi-model racing (requires multiple provider configs)
        },
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else MIKASA_HOME/cache/terminal (auto-pruned
        ...
        # MIKASA_HOME/home; "real" = force real HOME; "profile" = force MIKASA_HOME/home when it
    ...
    "display": {
        ...
        # `path`/`paths` = MIKASA_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_mikasa_home()/pets/; empty → first installed
    ...
    "moa": {
        ...
        # <MIKASA_HOME>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    "skills": {
        ...
        # MIKASA_HOME, scanned alongside the local dir.
        "create_dir": "",
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "⚕ *Mikasa Agent*" header; "" disables; \n allowed.
    },
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $MIKASA_HOME/plugins/ <name>/),
    ...
    "updates": {
        ...
        # config.yaml, .env, auth.json, profile DBs) into <MIKASA_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``hermes backup`` zip in
        # <MIKASA_HOME>/backups/ (``hermes import`` restores; slow on large homes; ``--backup``
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <MIKASA_HOME>/lsp/bin/ on
...
OPTIONAL_ENV_VARS = {
    ...
    "mikasa_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "mikasa_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "mikasa_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
