...
def _render_browse_page(c: Console, deduped, page_items, page: int, total_pages: int,
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from Jinny[/]")
    c.print()
...
_INSTALL_PANELS = {
    "official": (
        "[bold bright_cyan]This is an official optional skill maintained by Jinny.[/]\n\n"
        "It ships with mikasa-agent but is not activated by default.\n"
...
def _confirm_install(c: Console, bundle, category: str) -> bool:
    ...
    files_at = f"Files will be at: [cyan]{display_mikasa_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]"
...
def do_list(source_filter: str = "all", enabled_only: bool = False,
    ...
    profile's config — ``-p`` swaps MIKASA_HOME at process start, so no profile flag here."""
    from tools.skills_hub import HubLockFile, ensure_hub_dirs
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            _print_error(c, "GitHub authentication required.\n"
                            f"Set GITHUB_TOKEN in {display_mikasa_home()}/.env "
...
# tests that patch("mikasa_cli.skills_hub.do_install") still intercept.
_CLI_ACTIONS = {
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from mikasa_cli/main.py."""
