...
from mikasa_cli._subprocess_compat import noninteractive_git_env
from mikasa_cli.cli_output import line_input
from mikasa_cli.config import cfg_get
from mikasa_cli.plugin_capabilities import _child_dict
...
def _plugins_dir() -> Path:
    ...
    plugins = get_mikasa_home() / "plugins"
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if manifest_file is None:
        if not _has_portable_manifest(plugin_dir):
            return {}
        try:
            from mikasa_cli.agent_plugins import read_agent_plugin_manifest
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_mikasa_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_mikasa_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_mikasa_home()}/.env later)[/dim]")
...
def _install_metadata_path() -> Path:
    return get_mikasa_home() / "plugins" / ".install-metadata.json"
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    from mikasa_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
...
def cmd_update(name: str) -> None:
    ...
    if declared_caps:
        from mikasa_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``mikasa setup`` / ``mikasa plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from mikasa_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from mikasa_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from mikasa_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from mikasa_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
...
def _discover_all_plugins() -> list:
    ...
    from mikasa_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from mikasa_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from mikasa_cli.curses_ui import _addnstr, flush_stdin
...
def _get_plugin_toolset_key(name: str) -> Optional[str]:
    ...
    def _from_manifest_on_disk() -> Optional[str]:
        from mikasa_cli.plugins import get_bundled_plugins_dir
        return next((
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from mikasa_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from mikasa_cli.plugin_index import SECURITY_FOOTER, load_index, search_index
    console = _console()
...
def _action_pack(args):
    from mikasa_cli.plugin_packs import pack_command
    pack_command(args)
...
def cmd_compat(args: Any | None = None) -> None:
    ...
    import sys
    from pathlib import Path
    from mikasa_cli.plugin_compat import (
