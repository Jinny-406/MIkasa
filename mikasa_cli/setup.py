"""Interactive setup wizard for Mikasa Agent (config lives in ~/.hermes/).
...
from mikasa_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_mikasa_home is
# imported lazily at call sites (stale-module safety during hermes update).
from mikasa_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_mikasa_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_mikasa_home,
)
from mikasa_cli.colors import Colors, color
from mikasa_cli.cli_output import print_error, print_info, print_success, print_warning
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    _info("The interactive wizard cannot be used here.", None,
          ...
          "Run 'mikasa setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("mikasa_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from mikasa_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from mikasa_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    return os.environ.get("mikasa_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from mikasa_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'mikasa setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from mikasa_cli.main import select_provider_and_model
    try:
    ...
    # keep safe defaults (configure via `mikasa auth add` / `mikasa setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    remove_env_value("mikasa_MAX_ITERATIONS")
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          ...
          "  Run `mikasa setup agent` later to customize.")
...
def setup_tools(config: dict, first_install: bool = False):
    """`mikasa setup tools` == `mikasa tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from mikasa_cli.tools_config import tools_command
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from mikasa_cli.observability.shared_metrics import SharedMetricsStore
        from mikasa_cli.observability.shared_metrics_sender import reconcile_send_consent
        from mikasa_cli.sqlite_util import write_txn
...
from mikasa_cli.setup_tts import setup_tts  # noqa: E402
from mikasa_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from mikasa_cli.setup_platforms import setup_gateway  # noqa: E402
from mikasa_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from mikasa_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from mikasa_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``mikasa setup <section>``: run one SETUP_SECTIONS entry under the banner."""
...
def _run_full_setup(config: dict, MIKASA_HOME, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {MIKASA_HOME}", f"Install dir:  {PROJECT_ROOT}", None,
    ...
    def _gateway_step() -> None:
        ...
        from mikasa_cli.gateway import ensure_gateway_service
...
def _run_setup_wizard_impl(args):
    ...
    ``mikasa setup <section>`` from SETUP_SECTIONS."""
    ...
    from mikasa_cli.auth import get_active_provider
    is_existing = bool(get_env_value("OPENROUTER_API_KEY") or get_env_value("OPENAI_BASE_URL")
                       or get_active_provider() is not None)
    _print_banner("│             ⚕ Mikasa Agent Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your Mikasa Agent installation.       │",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, MIKASA_HOME))])
            return
        ...
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'mikasa setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(MIKASA_HOME)  # before configuration begins
        if migration_ran:
            config = load_config()
        ...
        if runner is not None:
            from mikasa_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, MIKASA_HOME, is_existing))])
            return
    _run_full_setup(config, MIKASA_HOME, is_existing=is_existing, migration_ran=migration_ran)
    ...
    _print_setup_summary(config, MIKASA_HOME)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('mikasa_cli.nous_subscription', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('mikasa_constants', 'get_optional_skills_dir'),
