...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (prev.startswith("Run 'mikasa doctor'") and last.startswith("Run 'mikasa setup'")):
        return text.rstrip()
    lines = lines[:-2]
    _drop_trailing_blank(lines)
...
def _split_line(line: str) -> list[str]:
    ...
    # See #78293.
    # See #83934.
    from mikasa_cli._subprocess_compat import split_command_line
    try:
...
def _clean_summary(text: str | None) -> str:
    ...
    return "" if summary.startswith("Run `mikasa ") else summary
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from mikasa_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from mikasa_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        parser = _ArgumentParser(prog="mikasa", add_help=False)
        subparsers = parser.add_subparsers(dest="_console_command")
        module = importlib.import_module(self.module)
        entry = getattr(module, self.builder)
        main_handler = (
            (lambda: getattr(importlib.import_module("mikasa_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"mikasa_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"mikasa_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "portal": (_CliSurface("adder", "mikasa_cli.portal_cli", "add_parser"), "info, tools"),
    "project": (
        _CliSurface("builder", "mikasa_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "mikasa_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "mikasa_cli.send_cmd", "register_send_subparser")
...
_BLOCKED_PAIRS = {
    ("config", "edit"): "`config edit` opens an editor and is not available in Mikasa Console.",
    ("mcp", "serve"): "`mcp serve` starts a server and is not available in Mikasa Console.",
    ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Mikasa Console.",
    ("skills", "config"): "`skills config` is interactive and is not available in Mikasa Console.",
    ("skills", "publish"): "`skills publish` is not available in Mikasa Console.",
    ("portal", "login"): "`portal login` is interactive and is not available in Mikasa Console.",
    ("portal", "open"): "`portal open` opens a browser and is not available in Mikasa Console.",
    ("kanban", "tail"): "`kanban tail` streams output and is not available in Mikasa Console.",
    ("kanban", "watch"): "`kanban watch` streams output and is not available in Mikasa Console.",
    ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Mikasa Console.",
    ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Mikasa Console.",
    ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Mikasa Console.",
    ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Mikasa Console.",
    ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Mikasa Console.",
    ("kanban", "gc"): "`kanban gc` is not available in Mikasa Console.",
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in Mikasa Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in Mikasa Console.",
}

class HermesConsoleEngine:

    """Curated line-command executor for Mikasa Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Mikasa Console does not run shell syntax. Use one supported "
                    "Mikasa command at a time.")
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["Mikasa Console", "", "Supported commands:"]
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Mikasa Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Mikasa Console."
        if first in _BLOCKED_TOP:
            return f"`mikasa {first}` is not available in Mikasa Console."
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in Mikasa Console.")
...
_version = _simple_command(
    "version", "mikasa_cli._startup_fast", "print_fast_version_info", check_updates=True)


def _status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "status")
    from mikasa_cli.status import show_status
...
_doctor = _simple_command(
    "doctor", "mikasa_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "mikasa_cli.config", "show_config")
_cron_status = _simple_command("cron status", "mikasa_cli.cron", "cron_status")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from mikasa_cli.logs import list_logs, tail_log
    if ns.log_name == "list":
...
def _config_path(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from mikasa_cli.config import get_config_path
...
def _config_set(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from mikasa_cli.config import set_config_value
...
def _config_migrate(_engine: HermesConsoleEngine, args: list[str]) -> None:
    ...
    from mikasa_cli.config import migrate_config
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from mikasa_state import SessionExportTooLargeError, resolved_max_export_messages
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> None:
    ...
    from mikasa_state import DEFAULT_DB_PATH
    from mikasa_state_repair import _db_opens_cleanly, repair_state_db_schema
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from mikasa_cli.cron import cron_list
...
_BUILTIN_COMMANDS = (
    (("status",), "status", "Show Mikasa component status.", _status, ""),
    (("version",), "version", "Show Mikasa version information.", _version, ""),
    (("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, ""),
    (("logs",), "logs [name] [-n N]", "Show recent Mikasa logs.", _logs, ""),
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update Mikasa configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    ...
    say("Mikasa Console. Type `help` for commands, `exit` to quit.")
    while True:
        say("mikasa> ", end="", flush=True)
        line = stdin.readline()
        if line == "":
            say("")
            return 0
