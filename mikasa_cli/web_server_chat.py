...
from mikasa_cli.pty_session import PtySessionRegistry
...
try:
    if sys.platform.startswith("win"):
        from mikasa_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    else:
        from mikasa_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    """
    from mikasa_cli.web_server import _is_accepted_host, app
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    """
    from mikasa_cli.web_server import _SESSION_TOKEN, app
    auth_required = bool(getattr(app.state, "auth_required", False))
    if auth_required:
        # Lazy import — keeps this function importable in test harnesses
        # that don't bring in the dashboard_auth layer.
        from mikasa_cli.dashboard_auth.audit import AuditEvent, audit_log
        from mikasa_cli.dashboard_auth.ws_tickets import (
...
def _resolve_chat_argv(
    ...
    ``profile`` scopes the ENTIRE chat by pointing ``MIKASA_HOME`` at the profile
    dir, the same propagation ``hermes -p <name>`` performs.
    """
    from mikasa_cli.web_server_profiles import _config_profile_scope, _resolve_profile_dir
    from mikasa_cli.web_server_sessions import _open_session_db_for_profile, _session_latest_descendant
    from mikasa_cli.main import PROJECT_ROOT
    from mikasa_cli.main_tui_launch import _apply_tui_python_env, _make_tui_argv
    ...
    # scope overrides MIKASA_HOME before config is bridged into the env.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["mikasa_HOME"] = str(profile_dir)
    try:
    ...
    env.setdefault("mikasa_TUI_DISABLE_MOUSE", "1")
    env.setdefault("mikasa_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["mikasa_TUI_RESUME"] = resume

    if sidecar_url:
        env["mikasa_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # which inherits the profile MIKASA_HOME set above.
    if profile_dir is None and (gateway_ws_url := _build_gateway_ws_url()):
        env["mikasa_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    from mikasa_cli.web_server import app
    explicit = os.environ.get("mikasa_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _server_internal_ws_url(path: str, **extra_qs) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from mikasa_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def _resolve_chat_argv_async(
    ...
    from mikasa_cli.web_server import _get_chat_argv_lock, app
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    from mikasa_cli.web_server import _get_pty_active_session_files
