...
def _debug(msg: str) -> None:
    """mikasa_VOICE_DEBUG=1 breadcrumb on stderr (the TUI gateway shows it as a gateway.stderr
...
def _speak_whole_file(text: str) -> None:
    ...
    os.makedirs(os.path.join(tempfile.gettempdir(), "mikasa_voice"), exist_ok=True)
    mp3_path = os.path.join(tempfile.gettempdir(), "mikasa_voice", f"tts_{time.strftime('%Y%m%d_%H%M%S')}.mp3")
