"""Configuration management for Mikasa Agent: config.yaml / .env loading, saving,
...
from mikasa_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from mikasa_cli.secret_prompt import masked_secret_prompt
# Re-export from mikasa_constants — canonical definition lives there.
from mikasa_constants import get_mikasa_home, get_process_mikasa_home  # noqa: F401
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "mikasa_HOME", "mikasa_PROFILE", "mikasa_CONFIG", "mikasa_ENV",
    "mikasa_CONFIG_PATH", "mikasa_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "mikasa_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "mikasa_COPILOT_ACP_COMMAND", "mikasa_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "mikasa_YOLO_MODE", "mikasa_ACCEPT_HOOKS", "mikasa_REDACT_SECRETS",
    "mikasa_INTERACTIVE", "mikasa_EXEC_ASK", "mikasa_GATEWAY_SESSION",
    "mikasa_CRON_SESSION", "mikasa_SINGLE_QUERY_SESSION",
    "mikasa_SESSION_KEY", "mikasa_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Hermes runtime location and security policy (MIKASA_HOME, HERMES_YOLO_MODE, ...) "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "mikasa_LANGFUSE_ENV", "mikasa_LANGFUSE_RELEASE", "mikasa_LANGFUSE_SAMPLE_RATE",
    "mikasa_LANGFUSE_MAX_CHARS", "mikasa_LANGFUSE_CAPTURE", "mikasa_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "mikasa_ACP_AUTH_METHOD", "mikasa_ACP_AUTO_APPROVE", "mikasa_COPILOT_ACP_COMMAND",
    "mikasa_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
def get_managed_system() -> Optional[str]:
    ...
    MIKASA_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("mikasa_MANAGED", "").strip().lower() or None
    managed_marker = get_mikasa_home() / ".managed"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    ``$MIKASA_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because MIKASA_HOME is shared data: a container and a host
    ...
    ``/opt/mikasa`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of hermes_cli/), NOT of $MIKASA_HOME,
    ...
    method = _install_method_stamp(get_mikasa_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``mikasa_constants.is_container``."""
    try:
...
Notes:
  ...
  • Your config and session history live under ``$MIKASA_HOME`` (``/opt/data``
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from mikasa_HOME/.container-mode.
    ...
    if os.environ.get("mikasa_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_mikasa_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "mikasa_bin": info.get("mikasa_bin", "/data/current-package/bin/hermes")}


# ---- Config paths / MIKASA_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    if ignore_user_config or os.environ.get("mikasa_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    The entrypoint chowns MIKASA_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    ...
    return _env_int("mikasa_UID"), _env_int("mikasa_GID")
...
def _chown_to_hermes_uid(path) -> None:
    ...
    :func:`ensure_mikasa_home` on Docker deployments. See #34107.
    """
...
def _secure_dir(path):
    ...
    MIKASA_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse MIKASA_HOME to
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("mikasa_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
...
def _is_container() -> bool:
    ...
    if (os.environ.get("mikasa_CONTAINER") or os.environ.get("mikasa_SKIP_CHMOD")
...
_mikasa_home_ENSURED: set = set()
_mikasa_home_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_mikasa_home():
    ...
    from mikasa_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _mikasa_home_ENSURED and home.is_dir():
        return
    if is_managed():
        ...
        try:
            if not home.is_dir():
                raise RuntimeError(f"mikasa_HOME {home} does not exist.")
        ...
        _secure_dir(home)
        for subdir in _mikasa_home_SUBDIRS:
            d = home / subdir
            d.mkdir(parents=True, exist_ok=True)
            _secure_dir(d)
        _ensure_default_soul_md(home)

    _mikasa_home_ENSURED.add(key)


# ---- Config loading/saving ----

from mikasa_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from mikasa_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use mikasa_cli.config.<name>)
...
# Back-compat re-exports — :mod:`mikasa_cli.personality` owns personality/overlay semantics.
from mikasa_cli.personality import (  # noqa: E402,F401
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'mikasa setup' to create a valid config")]
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        from mikasa_constants import display_mikasa_home

        hint_path = display_mikasa_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from mikasa_cli.config_migrations import (
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from mikasa_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from mikasa_cli.toolset_validation import validate_platform_toolsets
        from mikasa_cli.toolset_scope import toolset_allowed_for_platform
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_mikasa_dotenv()``, in-process
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_mikasa_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_mikasa_dotenv() pins unexpanded literals
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    return f"mikasa_CUSTOM_{slug}_API_KEY" if slug else "mikasa_CUSTOM_API_KEY"
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from mikasa_cli.credential_lifecycle import save_provider_env_credential
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    try:
        env_ghost = load_env().get("mikasa_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from mikasa_cli.personality import active_personality_name
...
def show_config():
    ...
    from mikasa_cli.auth import get_anthropic_key
    ...
    print(color("  mikasa setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    return provider, configured_model or _model_assignment_text(env.get("mikasa_MODEL", ""))
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_mikasa_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from mikasa_cli.credential_lifecycle import remove_provider_env_credential
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/mikasa`` inside the published image. It is a property of
    the running interpreter, NOT of ``$MIKASA_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$MIKASA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/mikasa`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('mikasa_cli.route_identity', 'normalize_route_base_url'),
}
