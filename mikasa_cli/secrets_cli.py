...
# parse-time from ``mikasa_cli.main``, so the backend import stays lazy (nothing touches ``bw``
...
from mikasa_cli._secrets_common import (
...
from mikasa_cli.config import get_env_path, load_config, save_config, save_env_value
...
def __getattr__(name: str):
    ...
    so ``import mikasa_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
# ── Argparse wiring — called from mikasa_cli.main ──
