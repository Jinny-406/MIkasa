"""Messaging-platform setup wizards (Telegram, BlueBubbles, webhooks) and the ``mikasa setup
...
logger = logging.getLogger("mikasa_cli.setup")
...
def _profile_name_from_mikasa_home(MIKASA_HOME) -> str | None:
    """Return the active profile name when MIKASA_HOME is a profile dir."""
    return MIKASA_HOME.name if MIKASA_HOME.parent.name == "profiles" else None


def _setup_telegram_auto_result():
    """Attempt automatic Telegram bot creation via managed QR onboarding."""
    from mikasa_cli.setup import get_mikasa_home
    try:
        from mikasa_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    profile_name: str | None = None
    with contextlib.suppress(Exception):
        profile_name = _profile_name_from_mikasa_home(Path(get_mikasa_home()))
...
def _declines_reconfigure(env_var: str, label: str, question: str) -> bool:
    ...
    from mikasa_cli.setup import get_env_value, print_info, prompt_yes_no
...
def _save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from mikasa_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from mikasa_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from mikasa_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from mikasa_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from mikasa_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from mikasa_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _obtain_telegram_token():
    ...
    from mikasa_cli.setup import _info, print_error, prompt
...
def _setup_telegram():
    ...
    from mikasa_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
...
def _setup_bluebubbles():
    ...
    from mikasa_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
...
def _setup_webhooks():
    ...
    from mikasa_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
...
def _warn_missing_home_channels() -> None:
    ...
    from mikasa_cli.setup import get_env_value, _info, print_warning
    missing_home = [
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from mikasa_cli.setup import print_error, prompt_yes_no
    from mikasa_cli.gateway import (
...
def setup_gateway(config: dict):
    ...
    from mikasa_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from mikasa_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if not selected:
        print_info("No platforms selected. Run 'mikasa setup gateway' later to configure.")
    ...
    from mikasa_cli.gateway import _is_service_running, supports_systemd_services, ensure_gateway_service
