...
if TYPE_CHECKING:
    from mikasa_cli.kanban_db import Task
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    ...
    assignee)."""
    try:
        from mikasa_cli.profiles import profile_exists
    except Exception:
        return None
...
def _module_hermes_argv() -> list[str]:
    """Interpreter-bound Hermes CLI invocation (``mikasa_cli.main`` is the
...
def _resolve_worker_cli_toolsets(MIKASA_HOME: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_mikasa_home_override(MIKASA_HOME)
        try:
            cfg = load_config()
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for MIKASA_HOME=%r (%s)",
            MIKASA_HOME,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, MIKASA_HOME: Optional[str]) -> list[str]:
    ...
    cmd = [
        *_resolve_hermes_argv(),
        ...
        "--cli",
        # Workers run under a profile-scoped MIKASA_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(MIKASA_HOME)
    if worker_toolsets:
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    from mikasa_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject MIKASA_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_mikasa_home() falls back to the DEFAULT
    # profile root because `hermes -p` applies its override before
    # mikasa_constants is imported.
    try:
        env["mikasa_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # HERMES_PROFILE (set below) instead.
        pass
    if task.tenant:
        env["mikasa_TENANT"] = task.tenant
    env["mikasa_KANBAN_TASK"] = task.id
    env["mikasa_KANBAN_WORKSPACE"] = workspace
    ...
    env["mikasa_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["mikasa_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["mikasa_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["mikasa_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["mikasa_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["mikasa_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `hermes -p` rewrites MIKASA_HOME (symlink / Docker layouts).
    env["mikasa_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["mikasa_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["mikasa_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["mikasa_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads HERMES_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["mikasa_PROFILE"] = profile_arg
    # `--cli` is the highest-precedence TUI override; dropping HERMES_TUI covers
    # older hermes builds on PATH that predate the flag's precedence.
    env.pop("mikasa_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("mikasa_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`mikasa` executable not found on PATH. "
            "Install Mikasa Agent or activate its venv before running the kanban dispatcher."
        )
...
from mikasa_cli import kanban_db as _kb  # noqa: E402
from mikasa_cli import kanban_db_connect as _kbc  # noqa: E402
from mikasa_cli import kanban_db_workspace as _kbw  # noqa: E402
