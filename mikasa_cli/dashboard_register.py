...
def _print_post_register_hint(
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    from mikasa_cli.config import get_env_path
...
def cmd_dashboard_register(args) -> None:
    ...
    from mikasa_cli.auth import AuthError, resolve_nous_access_token
    from mikasa_cli.config import is_managed, save_env_value
    ...
    try:
        ...
        if isinstance(exc, AuthError) and getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nous Portal.\n"
                  "  Run `mikasa setup` (or `mikasa auth add nous`) first, then retry.")
        else:
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("mikasa_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("mikasa_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    try:  # client_id is load-bearing: fatal on failure
        save_env_value("mikasa_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
    ...
    existing_portal = _env_value("mikasa_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("mikasa_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("mikasa_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("mikasa_DASHBOARD_PUBLIC_URL", public_url))
