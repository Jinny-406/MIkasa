...
def _pip_install_vercel(package):
    """uv when Hermes has one ($MIKASA_HOME/bin is never on PATH, so which() misses it and
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `mikasa setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    # Image and resource limits use defaults; tune via `mikasa setup terminal`.
