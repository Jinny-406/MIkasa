...
def real_profile_copy_dir(browser: str) -> str:
    ...
    return str(get_mikasa_home() / "browser-profile" / browser)
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    ``MIKASA_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
        from mikasa_cli.config import _secure_dir, _secure_file
        if not contents:
            _secure_dir(path)
            return
...
def cleanup_real_profile_snapshots() -> None:
    ...
    root = str(get_mikasa_home() / "browser-profile")
...
def chrome_debug_data_dir() -> str:
    return str(get_mikasa_home() / "chrome-debug")
