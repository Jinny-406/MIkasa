...
from mikasa_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import mikasa_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $MIKASA_HOME/node is not on PATH, so
...
def _has_hermes_agent_browser() -> bool:
    from mikasa_constants import get_mikasa_home  # late import: tests patch mikasa_constants.get_mikasa_home
    ...
    # install.sh installs into $MIKASA_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-HermesHome", str(get_mikasa_home())]
    else:
