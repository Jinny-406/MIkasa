...
def _can_import_hermes_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``mikasa_cli.main`` unaided.
    ...
    ok = _run_quiet(
        [key, "-I", "-c", "import mikasa_cli.main"],
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    ...
    Prefer the real ``hermes`` launcher; fall back to ``<python> -m mikasa_cli.main desktop``.
    """
    from mikasa_cli.relaunch import resolve_hermes_bin
    ...
    if not _can_import_hermes_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import mikasa_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "mikasa_cli.main", "desktop"]
    if bin_path:
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m mikasa_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_hermes_bin_for_desktop_entry(
    ...
    if resolve_fn is None:
        from mikasa_cli.relaunch import resolve_hermes_bin as resolve_fn
