...
from mikasa_cli.providers import custom_provider_aliases, custom_provider_slug
from utils import base_url_hostname

logger = logging.getLogger("mikasa_cli.runtime_provider")
...
def _rp():
    ...
    import mikasa_cli.runtime_provider as origin
    return origin
...
def canonical_custom_identity(*, base_url: Optional[str] = None, config_provider: Optional[str] = None,
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("mikasa_INFERENCE_PROVIDER", "").strip()
...
def _resolve_llamacpp_runtime(requested_provider: str, explicit_api_key: Optional[str]) -> Dict[str, Any]:
    ...
    rp = _rp()
    try:
        from mikasa_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from mikasa_cli.models import normalize_opencode_base_url, opencode_model_api_mode
