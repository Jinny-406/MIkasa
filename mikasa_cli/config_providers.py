...
``mikasa_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``mikasa_cli.config``
...
logger = logging.getLogger("mikasa_cli.config")
...
def get_custom_provider_model_capability(
    ...
    from mikasa_cli.config import get_compatible_custom_providers, load_config_readonly
