...
def _warm_gateway_module() -> None:
    ...
    for mod in (
        "mikasa_cli.gateway",
        "mikasa_cli.auth",  # provider auth state → copilot_auth → subprocess
        "mikasa_cli.copilot_auth",
        "mikasa_cli.runtime_provider",
        "mikasa_cli.skin_engine",  # resolve_skin() config + skin engine init
        "mikasa_cli.inventory",  # provider catalogs + models.dev cache
        "mikasa_cli.model_switch",
    ):
        try:
            __import__(mod)
        except Exception:
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    """
    target = os.environ.get("mikasa_DESKTOP_READY_FILE")
    if not target:
        return
...
def _start_parent_death_watchdog() -> None:
    ...
    """
    raw_pid = os.environ.get("mikasa_PARENT_PID")
    # Empty inherited values mean "absent", not "marker present but blank".
    start_marker = os.environ.get("mikasa_PARENT_START_MARKER") or None
    nonce = os.environ.get("mikasa_PARENT_NONCE") or None
    ...
    try:
        poll = max(0.5, float(os.environ.get("mikasa_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
