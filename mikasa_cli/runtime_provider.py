...
from mikasa_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
...
from mikasa_cli import config as _config_mod
from mikasa_cli import models as _models  # attribute access keeps ``hermes_cli.models.<name>`` patches effective
from mikasa_constants import OPENROUTER_BASE_URL
from mikasa_cli.providers import determine_api_mode, is_official_openai_host, nous_api_mode
from utils import base_url_host_matches, base_url_hostname, env_int


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``mikasa_cli.config.load_config`` patched
...
def _nous_min_key_ttl() -> int:
    return max(60, env_int("mikasa_NOUS_MIN_KEY_TTL_SECONDS", 1800))


def _resolve_nous_creds() -> Dict[str, Any]:
    return resolve_nous_runtime_credentials(timeout_seconds=float(_getenv("mikasa_NOUS_TIMEOUT_SECONDS", "15")))
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    return _getenv("mikasa_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from mikasa_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from mikasa_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        ...
                        "config.yaml if they aren't embedded in the credentials. Run `mikasa setup` to install Vertex support.")
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('mikasa_cli.providers', 'custom_provider_aliases'),
