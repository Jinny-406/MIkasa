...
``hermes_cli.models`` at call time so ``patch("mikasa_cli.models.<name>")`` mocks keep intercepting.
"""
...
from mikasa_cli.urllib_security import url_origin

# Log-record parity with the origin module.
logger = logging.getLogger("mikasa_cli.models")
...
def _get_ollama_base_url() -> str:
    ...
    from mikasa_cli.models import _get_model_config_dict
...
def probe_ollama_local_models(
    ...
    from mikasa_cli.models import _HERMES_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from mikasa_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    from mikasa_cli.models import _HERMES_USER_AGENT
...
def _lmstudio_fetch_raw_models(
    ...
    from mikasa_cli.models import _urlopen_model_catalog_request
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_mikasa_home() / "ollama_cloud_models_cache.json"
...
def _load_ollama_cloud_cache(*, ignore_ttl: bool = False) -> Optional[dict]:
    ...
    from mikasa_cli.models import _read_json_cache
...
def _save_ollama_cloud_cache(models: list[str]) -> None:
    ...
    from mikasa_cli.models import _write_json_cache
