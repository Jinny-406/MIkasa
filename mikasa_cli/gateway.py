...
import asyncio
import contextlib
from mikasa_cli.cli_output import line_input
import json
import logging
import os
import shlex
...
from mikasa_cli import setup_platforms
...
# display_mikasa_home is imported lazily: mikasa_constants may be a cached pre-update version.
...
from mikasa_cli.colors import Colors, color
...
def _scan_gateway_pids(
    ...
    current_home = str(get_mikasa_home().resolve())
    # Forward slashes on both sides of the MIKASA_HOME= match (mirrors gateway.status).
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"mikasa_home={current_home_lc}" in command_lc
            )

        # Default profile: accept unless argv advertises another profile. MIKASA_HOME may come via
        # env (invisible to wmic/CIM), so only a non-matching explicit MIKASA_HOME= disqualifies.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        return not ("mikasa_home=" in command_lc and f"mikasa_home={current_home_lc}" not in command_lc)
...
def _windows_process_listing() -> str | None:
    ...
    from mikasa_cli._subprocess_compat import bounded_probe_run
...
def find_profile_gateway_processes(exclude_pids: set | None = None, *, strict: bool = False) -> list[ProfileGatewayProcess]:
    ...
    try:
        ...
        from mikasa_cli.profiles import list_profiles
    except Exception:
        if strict:
            raise
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "mikasa_cli.main"]
    if profile != "default":
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from mikasa_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (MIKASA_HOME,
    ...
    if sys.platform == "win32":
        try:
            from mikasa_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        import sys
        import time
        from mikasa_cli._subprocess_compat import (
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_mikasa_home()) / "logs"
        ...
        # MIKASA_HOME) the windowless base interpreter needs to import mikasa_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _mikasa_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """``MIKASA_HOME`` from the on-disk unit file — what refresh/compare already read, and reliable under ``sudo``."""
    ...
    for line in text.splitlines():
        ...
        if body.startswith("Environment="):
            ...
            if body.startswith("mikasa_HOME="):
                return body.split("=", 1)[1].strip().strip('"') or None
    return None


def _sync_mikasa_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``MIKASA_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_mikasa_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    # On-disk unit first; ``systemctl show`` for units that only exist in the manager.
    unit_home = (_mikasa_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        env_line = _systemctl_show(("Environment",), system=True).get("Environment", "")
        unit_home = _parse_kv_pairs(env_line.split()).get("mikasa_HOME", "").strip()
    if unit_home and os.environ.get("mikasa_HOME", "").strip() != unit_home:
        os.environ["mikasa_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {sudo}mikasa gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}mikasa gateway restart{scope_flag}")
...
def _s6_gateway_snapshot(gateway_pids: tuple[int, ...]) -> GatewayRuntimeSnapshot | None:
    ...
    from mikasa_cli.service_manager import detect_service_manager, get_service_manager
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from mikasa_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print(pids_line)
        print("  This is usually a manual foreground/tmux/nohup run, so `mikasa gateway`")
        print("  can refuse to start another copy until this process stops.")


def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``mikasa gateway status``."""
    try:
        from mikasa_cli.profiles import get_active_profile_name
...
def _gateway_list() -> None:
    ...
    try:
        from mikasa_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from mikasa_cli.gateway_windows import get_task_name  # profile-aware task name
            _task_name = get_task_name()
        except Exception:
            _task_name = "mikasa_Gateway"
...
from mikasa_constants import is_container, is_termux, is_wsl
...
def _gw_windows():
    ...
    from mikasa_cli import gateway_windows
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("mikasa_GATEWAY_DETACHED"))
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from mikasa_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
SERVICE_DESCRIPTION = "Mikasa Agent Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    """Service-name suffix for MIKASA_HOME: "" for the default root, the profile name for
    ``<root>/profiles/<name>``, else a short hash of the path."""
    import hashlib
    from mikasa_constants import get_default_mikasa_root
    home = get_mikasa_home().resolve()
    default = get_default_mikasa_root().resolve()
    if home == default:
        return ""
    # Fallback: short hash for arbitrary MIKASA_HOME paths
    return _profile_name_from_home(home, default) or hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(MIKASA_HOME: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *MIKASA_HOME*/*default_root* let a
    sudo/root process generate a unit for another user (the defaults would refer to root)."""
    from mikasa_constants import get_default_mikasa_root
    home = Path(MIKASA_HOME or str(get_mikasa_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_mikasa_root().resolve()
    if home == default:
        return ""
...
def get_service_name() -> str:
    """Systemd service name: ``hermes-gateway`` for default MIKASA_HOME, ``hermes-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "mikasa_cli.main gateway",
    "mikasa_cli/main.py gateway",
    "gateway/run.py",
    " hermes gateway ",
    "/hermes gateway ",
)
...
def print_legacy_unit_warning() -> None:
    ...
    if not legacy:
        return
    print_warning("Legacy Mikasa gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Mikasa gateway units found.")
        return 0, []

    print()
    print("Legacy Mikasa gateway unit(s) found:")
    for name, path, is_system in legacy:
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``mikasa setup``/``import``).
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed MIKASA_HOME never
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            from mikasa_constants import venv_python_path
        except ImportError:
            # Update-boundary: a gateway restarted mid-update can hold a stale mikasa_constants
            # without this symbol; see _reload_mikasa_constants() in hermes_cli/managed_uv.py.
            from mikasa_cli.managed_uv import _reload_mikasa_constants
            venv_python_path = _reload_mikasa_constants().venv_python_path
...
def _mikasa_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current MIKASA_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.hermes[/profiles/x]`` → ``/home/alice/.hermes[/profiles/x]``; custom paths kept as-is."""
    current_hermes_raw = os.environ.get("mikasa_HOME", "").strip()
    current_hermes = Path(current_hermes_raw).expanduser() if current_hermes_raw else get_mikasa_home()
    # Keep paths lexical: resolving a non-existent path can bake a different MIKASA_HOME into the unit.
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    MIKASA_HOME = get_mikasa_home()
    extras = (project_root / "node_modules" / ".bin", MIKASA_HOME / "node" / "bin", MIKASA_HOME / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (MIKASA_HOME, else PROJECT_ROOT). cwd is
...
def _systemd_watchdog_seconds(MIKASA_HOME: str | Path | None = None) -> int:
    """Resolve the managed-overlay-aware watchdog setting for a service home."""
    override_token = reset_home_override = None
    if MIKASA_HOME is not None:
        from mikasa_constants import (reset_mikasa_home_override, set_mikasa_home_override)
        override_token = set_mikasa_home_override(MIKASA_HOME)
        reset_home_override = reset_mikasa_home_override
    try:
...
def _append_node_dir_for_service(path_entries: list[str], hermes_root: Path | None = None) -> None:
    ...
    from mikasa_constants import (hermes_managed_node_tree_present, iter_hermes_node_dirs)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        MIKASA_HOME = _mikasa_home_for_target_user(home_dir)
        # Profile arg relative to the TARGET user's ~/.hermes when MIKASA_HOME lives under it.
        target_root = Path(home_dir) / ".hermes"
        try:
            Path(MIKASA_HOME).resolve().relative_to(target_root.resolve())
            profile_arg = _profile_arg(MIKASA_HOME, default_root=target_root)
        except ValueError:
            profile_arg = _profile_arg(MIKASA_HOME)
        # Remap paths under the calling user's home (/root/) to the target user's so the service can read them.
        python_path = _remap_path_for_user(python_path, home_dir)
        working_dir = str(MIKASA_HOME) if MIKASA_HOME else _remap_path_for_user(working_dir, home_dir)
        ...
        _append_node_dir_for_service(_target_node_entries, Path(MIKASA_HOME) if MIKASA_HOME else None)
        ...
        MIKASA_HOME = str(get_mikasa_home().resolve())
        profile_arg = _profile_arg(MIKASA_HOME)
        user_home = Path.home()
        identity_lines = env_lines = ""
        wanted_by = "default.target"

    watchdog_seconds = _systemd_watchdog_seconds(MIKASA_HOME)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m mikasa_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="mikasa_HOME={MIKASA_HOME}"
Environment="mikasa_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def systemd_unit_is_current(system: bool = False) -> bool:
    # MIKASA_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    ...
    _sync_mikasa_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir MIKASA_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    test/E2E harness generated it; installing it leaves the gateway "running" but deaf to every platform."""
    import re
    import tempfile
    candidates = re.findall(r'mikasa_HOME=([^"\n]+)', definition)
    candidates += re.findall(r"<key>MIKASA_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp MIKASA_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: MIKASA_HOME resolves to a temporary directory ({temp_home}).")
    print(
        "  This usually means a test/E2E environment exported MIKASA_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # systemd_unit_is_current is the MIKASA_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    ...
    # pytest-tmp MIKASA_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/hermes_test"', "/hermes_test/")):
        return False

    # Structural variant: refuse ANY temp-dir MIKASA_HOME (manual E2E homes lack the pytest markers).
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo hermes gateway uninstall --system")
    print_info("         mikasa gateway install")
    print_info("         mikasa gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("mikasa_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("mikasa_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "mikasa_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin MIKASA_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_mikasa_home_from_systemd_unit(system=system)
...
def systemd_start(system: bool = False):
    ...
    # MIKASA_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_restart(system: bool = False):
    ...
    # MIKASA_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_mikasa_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_mikasa_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m mikasa_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "mikasa_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    return [get_python_path(), "-m", "mikasa_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    """
    from mikasa_cli._subprocess_compat import windows_detach_popen_kwargs
    log_dir = get_mikasa_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    from mikasa_constants import display_mikasa_home as _dhh
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from mikasa_cli.resource_limits import configured_nofile_soft_limit
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    MIKASA_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: mikasa gateway start")
    ...
    if deep:
        log_file = get_mikasa_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/mikasa" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        from mikasa_constants import get_default_mikasa_root
        default_root = get_default_mikasa_root()
    except Exception:
        return False
    ...
    try:
        ...
        cfg = {}
        if cfg_path.exists():
            from mikasa_cli.config import read_user_config_raw
        ...
        from mikasa_cli.profiles import normalize_profile_name
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other MIKASA_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's MIKASA_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (MIKASA_HOME=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("mikasa_home", "<unknown>"),
                )
        except Exception:
            pass
        return
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("mikasa_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Mikasa gateway as root inside the official Docker image.")
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/mikasa/docker/entrypoint.sh before the Hermes command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$MIKASA_HOME and break later non-root dashboard/gateway runs."
    )
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from mikasa_startup_watchdog import (
        ...
        try:
            from mikasa_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("mikasa_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from mikasa_constants import get_mikasa_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
            line = {
...
def _respawn_storm_backoff() -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["mikasa_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["mikasa_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from mikasa_startup_watchdog import kick_startup_watchdog
...
def _telegram_auto_setup(token_var: str) -> tuple[bool, object]:
    ...
    if prompt("  Choice [1/2]", default="1").strip() != "1":
        return False, None
    try:
        from mikasa_cli.telegram_managed_bot import (
...
def _setup_standard_platform(platform: dict):
    ...
    from mikasa_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _running_under_s6() -> bool:
    from mikasa_cli.service_manager import detect_service_manager
...
def _setup_weixin():
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_mikasa_home())))
    except KeyboardInterrupt:
        print()
...
def _builtin_setup_fn(key: str):
    ...
    from mikasa_cli import setup as _s
    return {
...
def _dispatch_via_service_manager_if_s6(action: str, profile: str | None = None) -> bool:
    ...
    from mikasa_cli.service_manager import (
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("mikasa_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # HERMES_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("mikasa_S6_SUPERVISED_CHILD"):
        return False
