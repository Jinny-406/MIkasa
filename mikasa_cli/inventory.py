...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from mikasa_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _external_process_signed_in(slug: str) -> bool:
    ...
    try:
        from mikasa_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
def _provider_is_keyless(slug: str) -> bool:
    ...
    try:
        ...
        overlay = MIKASA_OVERLAYS.get(slug)
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from mikasa_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from mikasa_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from mikasa_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (mikasa_home_key(), endpoint_scope)
