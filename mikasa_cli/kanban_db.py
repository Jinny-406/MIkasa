...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    from mikasa_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("mikasa_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    try:
        from mikasa_cli.lifecycle import invoke_hook
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    try:
        from mikasa_cli.lifecycle import has_hook
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    return _env_int("mikasa_KANBAN_CLAIM_TTL_SECONDS", DEFAULT_CLAIM_TTL_SECONDS, minimum=1)
...
def _resolve_crash_grace_seconds() -> int:
    ...
    return _env_int("mikasa_KANBAN_CRASH_GRACE_SECONDS", DEFAULT_CRASH_GRACE_SECONDS)


def _resolve_rate_limit_cooldown_seconds() -> int:
    """``HERMES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` (0 = next tick, for tests) else default."""
    return _env_int("mikasa_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", DEFAULT_RATE_LIMIT_COOLDOWN_SECONDS)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "mikasa_kanban_current_board_override", default=None,
)
...
def kanban_home() -> Path:
    """``HERMES_KANBAN_HOME`` else ``get_default_mikasa_root()``. Shared across
    profiles BY DESIGN: resolving through the active profile's MIKASA_HOME would
    fork the board per profile and break the dispatcher/worker handoff."""
    override = os.environ.get("mikasa_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from mikasa_constants import get_default_mikasa_root
    return get_default_mikasa_root()
...
def get_current_board() -> str:
    ...
    for candidate in (
        ...
        os.environ.get("mikasa_KANBAN_BOARD", "").strip(),
    ):
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    return _board_path("mikasa_KANBAN_DB", board, ("kanban.db",), "kanban.db")
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    return _board_path("mikasa_KANBAN_WORKSPACES_ROOT", board, ("kanban", "workspaces"), "workspaces")
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    return _board_path("mikasa_KANBAN_ATTACHMENTS_ROOT", board, ("kanban", "attachments"), "attachments")
...
def _resolve_project_link(
    ...
    from mikasa_cli import projects_db as _pdb
...
from mikasa_cli.kanban_db_connect import (  # noqa: E402
    _INITIALIZED_PATHS,
    init_db,
    write_txn,
)
from mikasa_cli.kanban_db_workspace import (  # noqa: E402
    _cleanup_workspace,
    _is_managed_scratch_path,
    _managed_scratch_path_info,
    _scratch_workspace,
)
from mikasa_cli.kanban_db_dispatch import (  # noqa: E402
...
_PLUGIN_COMPAT_LAZY = {
    'DEFAULT_BUSY_TIMEOUT_MS': ('mikasa_cli.kanban_db_connect', 'DEFAULT_BUSY_TIMEOUT_MS'),
    'DEFAULT_LOG_BACKUP_COUNT': ('mikasa_cli.kanban_db_dispatch', 'DEFAULT_LOG_BACKUP_COUNT'),
    'DEFAULT_LOG_ROTATE_BYTES': ('mikasa_cli.kanban_db_dispatch', 'DEFAULT_LOG_ROTATE_BYTES'),
    'DERIVED_MAX_IN_PROGRESS_CEILING': ('mikasa_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_CEILING'),
    'DERIVED_MAX_IN_PROGRESS_FLOOR': ('mikasa_cli.kanban_db_dispatch', 'DERIVED_MAX_IN_PROGRESS_FLOOR'),
    'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS': ('mikasa_cli.kanban_db_dispatch', 'KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS'),
    'KanbanDbCorruptError': ('mikasa_cli.kanban_db_connect', 'KanbanDbCorruptError'),
    'MEMORY_GUARD_MB_PER_WORKER': ('mikasa_cli.kanban_db_dispatch', 'MEMORY_GUARD_MB_PER_WORKER'),
    'RepairResult': ('mikasa_cli.kanban_db_connect', 'RepairResult'),
    'add_notify_sub': ('mikasa_cli.kanban_db_notify', 'add_notify_sub'),
    'advance_notify_cursor': ('mikasa_cli.kanban_db_notify', 'advance_notify_cursor'),
    'check_respawn_guard': ('mikasa_cli.kanban_db_dispatch', 'check_respawn_guard'),
    'claim_unseen_events_for_sub': ('mikasa_cli.kanban_db_notify', 'claim_unseen_events_for_sub'),
    'configured_max_in_progress': ('mikasa_cli.kanban_db_dispatch', 'configured_max_in_progress'),
    'connect': ('mikasa_cli.kanban_db_connect', 'connect'),
    'connect_closing': ('mikasa_cli.kanban_db_connect', 'connect_closing'),
    'count_notify_subs': ('mikasa_cli.kanban_db_notify', 'count_notify_subs'),
    'count_running_tasks': ('mikasa_cli.kanban_db_dispatch', 'count_running_tasks'),
    'count_running_tasks_other_boards': ('mikasa_cli.kanban_db_dispatch', 'count_running_tasks_other_boards'),
    'derive_default_max_in_progress': ('mikasa_cli.kanban_db_dispatch', 'derive_default_max_in_progress'),
    'detect_crashed_workers': ('mikasa_cli.kanban_db_dispatch', 'detect_crashed_workers'),
    'detect_stale_running': ('mikasa_cli.kanban_db_dispatch', 'detect_stale_running'),
    'dispatch_once': ('mikasa_cli.kanban_db_dispatch', 'dispatch_once'),
    'enforce_max_runtime': ('mikasa_cli.kanban_db_dispatch', 'enforce_max_runtime'),
    'has_spawnable_ready': ('mikasa_cli.kanban_db_dispatch', 'has_spawnable_ready'),
    'has_spawnable_review': ('mikasa_cli.kanban_db_dispatch', 'has_spawnable_review'),
    'heartbeat_worker': ('mikasa_cli.kanban_db_dispatch', 'heartbeat_worker'),
    'list_notify_subs': ('mikasa_cli.kanban_db_notify', 'list_notify_subs'),
    'purge_stale_done_notify_subs': ('mikasa_cli.kanban_db_notify', 'purge_stale_done_notify_subs'),
    'reap_worker_zombies': ('mikasa_cli.kanban_db_dispatch', 'reap_worker_zombies'),
    'reconcile_orphaned_running': ('mikasa_cli.kanban_db_dispatch', 'reconcile_orphaned_running'),
    'remove_notify_sub': ('mikasa_cli.kanban_db_notify', 'remove_notify_sub'),
    'repair_db': ('mikasa_cli.kanban_db_connect', 'repair_db'),
    'resolve_max_in_progress': ('mikasa_cli.kanban_db_dispatch', 'resolve_max_in_progress'),
    'resolve_workspace': ('mikasa_cli.kanban_db_workspace', 'resolve_workspace'),
    'review_dispatch_enabled': ('mikasa_cli.kanban_db_dispatch', 'review_dispatch_enabled'),
    'rewind_notify_cursor': ('mikasa_cli.kanban_db_notify', 'rewind_notify_cursor'),
    'run_daemon': ('mikasa_cli.kanban_db_dispatch', 'run_daemon'),
    'set_branch_name': ('mikasa_cli.kanban_db_workspace', 'set_branch_name'),
    'set_workspace_path': ('mikasa_cli.kanban_db_workspace', 'set_workspace_path'),
    'unseen_events_for_sub': ('mikasa_cli.kanban_db_notify', 'unseen_events_for_sub'),
    'worker_log_rotation_config': ('mikasa_cli.kanban_db_dispatch', 'worker_log_rotation_config'),
}
