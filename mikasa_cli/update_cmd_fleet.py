...
from mikasa_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("mikasa_cli.update_cmd")

# Under MIKASA_HOME (not next to the venv): records the fleet-restart obligation
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    ...
    path = get_mikasa_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """mikasa_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from mikasa_cli.update_cmd import get_mikasa_home
    return get_mikasa_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    from mikasa_cli.update_cmd import _capture_head_sha, _m
    try:
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from mikasa_cli.update_cmd import _current_checkout_sha
    try:
        from mikasa_cli.update_receipt import read_latest_receipt
...
def _apply_pending_fleet_restart_catchup() -> None:
    ...
    """
    from mikasa_cli.update_cmd import _run_pending_fleet_restart
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from mikasa_cli.gateway import is_macos
    if not failed_units:
        return
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    ...
    """
    from mikasa_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from mikasa_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from mikasa_cli.update_cmd import (
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    try:
        ...
        from mikasa_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from mikasa_cli.gateway import _get_restart_exit_wait_budget
...
def _print_legacy_units_warning() -> None:
    ...
    from mikasa_cli.gateway import (has_legacy_hermes_units, _find_legacy_hermes_units, supports_systemd_services)
...
def _collect_fleet_snapshot(restart, rows_expected: bool) -> list:
    ...
    """
    from mikasa_cli.update_receipt import collect_fleet_versions
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from mikasa_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from mikasa_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import mikasa_cli.update_receipt as _ur
