...
# Install-scoped breadcrumbs live next to the venv (not under $MIKASA_HOME)
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from mikasa_cli import _install_repair as _ir
...
_UPDATE_REEXEC_ENV = "mikasa_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    cmd = [str(python_exe), "-m", "mikasa_cli.main", *sys.argv[1:]]
...
def _verify_console_scripts_installed(
    ...
    _report_still_missing(
        _missing(), "Workaround: python -m mikasa_cli.main <command>",
