...
from mikasa_constants import get_mikasa_home, get_optional_mcps_dir
from mikasa_cli._subprocess_compat import noninteractive_git_env
from mikasa_cli.colors import Colors, color
from mikasa_cli.config import load_config, save_config, get_env_value, save_env_value
from mikasa_cli.cli_output import prompt as _prompt_input
...
def _install_root() -> Path:
    ...
    root = get_mikasa_home() / "mcp-installs"
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from mikasa_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from mikasa_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    from mikasa_cli.mcp_config import _save_mcp_server
