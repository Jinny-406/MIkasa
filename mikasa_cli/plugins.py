...
from mikasa_constants import get_mikasa_home, MIKASA_HOME_key
from registration_lifecycle import replacement_coordinator
from utils import env_var_enabled
from mikasa_cli.config import load_config_readonly
from mikasa_cli.middleware import VALID_MIDDLEWARE
from mikasa_cli.plugin_capabilities import plugin_capability_granted
from mikasa_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from mikasa_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from mikasa_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from mikasa_cli.plugins_loader import (
...
from mikasa_cli.plugins_dispatch import (  # noqa: F401 — re-exported
...
from mikasa_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from mikasa_cli.plugins_state import (
...
def get_bundled_plugins_dir() -> Path:
    ...
    env_override = os.getenv("mikasa_BUNDLED_PLUGINS")
    if env_override:
...
_PLUGINS_DEBUG = env_var_enabled("mikasa_PLUGINS_DEBUG")
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    logger.debug("mikasa_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled")
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        segments = self._segments(key)
        from mikasa_cli import config as config_mod
    ...
    def platform_actions(self):
        ...
        from mikasa_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        ...
        derived from ``MIKASA_HOME`` — not ``_cli_ref``, which is None outside the interactive CLI —
    ...
    def register_approval_transport(self, name: str, present_fn: Callable) -> None:
        ...
        from mikasa_cli.approval_transport import RegisteredApprovalTransport
        ...
        entry = RegisteredApprovalTransport(
            ...
            profile_home=str(get_mikasa_home().resolve()),
        )
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        from mikasa_cli.dashboard_auth import DashboardAuthProvider
        from mikasa_cli.dashboard_auth.registry import register_global_provider, unregister_global_provider
    ...
    def register_auxiliary_task(
        ...
        from mikasa_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "agent.secret_sources.registry",
     ...
     "Register a :class:`agent.secret_sources.base.SecretSource`, run by ``load_mikasa_dotenv()`` "
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        self.scope_key = scope_key or MIKASA_HOME_key()
        ...
        # is keyed per (MIKASA_HOME, plugin_id) and every inverse is identity-conditional — one
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("mikasa_SAFE_MODE"):
                logger.info("mikasa_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_mikasa_dotenv() ran at import, before plugin secret sources existed: re-pull.
                # Plugin secret sources register during discover; the initial load_mikasa_dotenv() already
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from mikasa_cli.env_loader import load_mikasa_dotenv, reset_secret_source_cache
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh MIKASA_HOME/.plugin-compat-report.json from this discovery pass (hermes_cli.plugin_compat).
        ...
        """
        try:
            from mikasa_cli.plugin_compat import compat_report
    ...
    def register_approval_transport(self, name: str, present_fn: Callable, *, plugin_id: str) -> None:
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            name=clean, present=present_fn, plugin_id=plugin_id, profile_home=str(get_mikasa_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None or registered.profile_home != str(get_mikasa_home().resolve()):
            return None
        return registered
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        if _env_enabled("mikasa_SAFE_MODE"):
            return False
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                from mikasa_cli.agent_plugins import _discover_mcp
                if _discover_mcp(Path(manifest.path), get_mikasa_home() / "plugin-data"
...
# ``set_mikasa_home_override()``; a single slot would leak one profile's plugin/context-engine state
...
def _plugin_home_key() -> Path:
    ...
    try:
        return get_mikasa_home().expanduser().resolve()
    except Exception:
        return get_mikasa_home().expanduser()
...
def get_plugin_manager() -> PluginManager:
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=MIKASA_HOME_key(current_home))
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from mikasa_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def _plugin_toolset_keys_cache_path() -> Path:
    return get_mikasa_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from mikasa_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('mikasa_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('mikasa_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('mikasa_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('mikasa_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('mikasa_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('mikasa_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('mikasa_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('mikasa_cli.plugins_dispatch', 'format_system_prompt_section'),
    'reset_mikasa_home_override': ('mikasa_constants', 'reset_mikasa_home_override'),
    'set_mikasa_home_override': ('mikasa_constants', 'set_mikasa_home_override'),
}
