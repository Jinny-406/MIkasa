...
from mikasa_cli.auth import AuthError, resolve_provider
from mikasa_cli.colors import Colors, color
from mikasa_cli.config import get_env_path, get_env_value, get_mikasa_home, load_config
from mikasa_cli.models import provider_label
from mikasa_cli.runtime_provider import resolve_requested_provider
from mikasa_cli.vercel_auth import describe_vercel_auth
from mikasa_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from mikasa_constants import OPENROUTER_MODELS_URL
...
def _render_gateway(ctx):
    ...
    try:
        from mikasa_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
...
def _render_cron(ctx):
    ...
    jobs_file = get_mikasa_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from mikasa_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_mikasa_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from mikasa_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'mikasa doctor' for detailed diagnostics", "  Run 'mikasa setup' to configure"),
            Colors.DIM)
    print()
...
def show_status(args):
    """Show status of all Mikasa Agent components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('mikasa_cli.nous_account', 'get_nous_portal_account_info'),
