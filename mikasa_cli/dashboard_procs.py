...
Helpers defined in ``mikasa_cli.main_dashboard`` / ``mikasa_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("hermes", "mikasa_cli.main", "mikasa_cli/main.py"))
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Hermes
    ...
    with contextlib.suppress(Exception):
        ...
        from mikasa_cli.process_identity import ledger_entries
...
def _mikasa_home_for_pid(pid: int) -> str | None:
    """Best-effort ``MIKASA_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("mikasa_HOME"):
            return home
    try:
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"mikasa_HOME="):
...
def _profile_key_for_respawn(argv: list[str], MIKASA_HOME: str | None = None) -> str:
    """Stable owner key: ``MIKASA_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if MIKASA_HOME:
        parts = _resolved_home(MIKASA_HOME).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(MIKASA_HOME)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, MIKASA_HOME)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``MIKASA_HOME`` (the argv-only respawn would come back on this install's home
    ...
    for _pid, argv, MIKASA_HOME in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and MIKASA_HOME and _normalized_home_for_compare(MIKASA_HOME) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, MIKASA_HOME)
...
def _exclude_pids_from_env() -> set[int]:
    ...
    out: set[int] = set()
    for part in os.environ.get("mikasa_DESKTOP_CHILD_PID", "").split(","):
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    ...
    """
    from mikasa_cli import main_dashboard as _dash
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid] and (cmdline := _dash._dashboard_cmdline_for_pid(pid)):
                # Manual process: exact argv + MIKASA_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot MIKASA_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _mikasa_home_for_pid(pid)
...
def _detect_concurrent_hermes_instances(
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from mikasa_cli.main_install_repair import _hermes_exe_shims, _is_windows
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    """
    cmd = command.lower()
    if "serve" not in cmd or ("hermes" not in cmd and "mikasa_cli" not in cmd):
        return False
...
def _mikasa_home_dir() -> Path:
    """Resolved Hermes home (MIKASA_HOME override or ~/.hermes)."""
    override = os.environ.get("mikasa_HOME", "").strip()
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
        ...
        return False
    # Suffix-only check of logPath so a relocated MIKASA_HOME can't reject a legitimate backend.
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{MIKASA_HOME}/desktop-ssh/<ownershipId>/backend.lock.json`` records
    (best-effort: a bad record contributes no PID; never raises)."""
    import json
    root = base_dir if base_dir is not None else _mikasa_home_dir() / _REMOTE_LOCK_SUBDIR
    owned: set[int] = set()
    try:
