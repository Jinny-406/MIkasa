...
_UPDATE_CRITICAL_MODULES = "mikasa_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from mikasa_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from mikasa_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    ...
    probe = (
        ...
        # only our own packages count. Roots come from mikasa_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('mikasa_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        from mikasa_cli import tools_config
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from mikasa_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    from mikasa_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from mikasa_constants import is_wsl
    ...
    shared_hermes_root = get_default_mikasa_root()
...
# canonical case: ``mikasa_cli.main`` used to import it at startup while resolving external secret sources;
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "mikasa_cli.main", "desktop", "--build-only"]
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    """
    from mikasa_cli.update_cmd import _path_uid
    try:
