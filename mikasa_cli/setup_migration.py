...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    from mikasa_cli.setup import get_env_value
    try:
...
def _skip_configured_section(config: dict, section_key: str, label: str) -> bool:
    ...
    from mikasa_cli.setup import print_success, prompt_yes_no
...
def _print_migration_preview(report: dict):
    ...
    config semantics)."""
    from mikasa_cli.setup import color, Colors, print_info
...
def _run_migrator(mod, openclaw_dir: Path, MIKASA_HOME: Path, selected, *, execute: bool, overwrite: bool):
    """Run a Migrator with the fixed first-time-setup options and return its report."""
    return mod.Migrator(
        source_root=openclaw_dir.resolve(), target_root=MIKASA_HOME.resolve(), execute=execute,
...
def _migration_step(label: str, log_label: str, fn):
    ...
    from mikasa_cli.setup import print_warning
    try:
        return fn()
...
def _offer_openclaw_migration(MIKASA_HOME: Path) -> bool:
    ...
    def _preview():
        ...
        return selected, _run_migrator(mod, openclaw_dir, MIKASA_HOME, selected, execute=False, overwrite=True)
    ...
    report = _migration_step("Migration failed", "OpenClaw migration error", lambda: _run_migrator(
        mod, openclaw_dir, MIKASA_HOME, selected, execute=True, overwrite=False))
