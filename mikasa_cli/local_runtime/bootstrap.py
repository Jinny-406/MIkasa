...
from mikasa_cli.local_runtime.gguf import SPLIT_PART_RE, model_id_from_stem
...
def _detect_gpu_vendor() -> str | None:
    ...
    from mikasa_cli.local_runtime.hardware import _nvidia_smi_path
...
def models_dir() -> Path:
    ...
    return get_default_mikasa_root() / "models"
...
def _presets_stale() -> bool:
    ...
    with suppress(Exception):
        from mikasa_cli.local_runtime.presets import read_preset_decisions
...
def _stop_state_server(state: dict) -> None:
    ...
    server."""
    from mikasa_cli.local_runtime.endpoint import _pid_alive
...
def _generate_presets(mdir: Path, preset_path: Path) -> Path | None:
    ...
    """
    from mikasa_cli.local_runtime.hardware import probe_budget
    from mikasa_cli.local_runtime.presets import generate_presets
...
_PLUGIN_COMPAT_LAZY = {
    'get_mikasa_home': ('mikasa_constants', 'get_mikasa_home'),
}
