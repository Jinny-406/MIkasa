...
def source_built_gui_artifacts(MIKASA_HOME: Path) -> "list[Path]":
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    from mikasa_cli.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(MIKASA_HOME: Path) -> bool:
    """True when a usable Python agent install exists under MIKASA_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((MIKASA_HOME / "hermes-agent" / sub).is_dir() for sub in ("mikasa_cli", "venv", ".venv"))


def gui_is_installed(MIKASA_HOME: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(MIKASA_HOME), *packaged_gui_app_paths(), desktop_userdata_dir()))


def gui_install_summary(MIKASA_HOME: "Path | None" = None) -> dict:
    ...
    return {"mikasa_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(MIKASA_HOME: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from mikasa_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
