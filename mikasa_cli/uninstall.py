...
def _strip_hermes_path_lines(content: str) -> str:
    """Drop the ``# Mikasa Agent`` marker (+ its PATH line) and any hermes PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Mikasa Agent' in line or '# hermes-agent' in line:
            skip_next = True
            continue
...
def remove_wrapper_script():
    ...
    def _unlink_ours(wrapper: Path) -> bool:
        ...
        return _unlink_if('mikasa_cli' in content or 'hermes-agent' in content, wrapper)
...
def remove_node_symlinks(MIKASA_HOME: Path) -> list:
    ...
    node_dir = (MIKASA_HOME / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from mikasa_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from mikasa_cli.gateway import _systemctl_cmd, get_service_name, get_systemd_unit_path
...
def _remove_launchd_gateway() -> bool:
    ...
    from mikasa_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from mikasa_cli import gateway_windows as gw
...
# MIKASA_HOME / HERMES_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
...
def _hermes_path_markers(MIKASA_HOME: Path, *, include_managed_bin: bool = False) -> list[str]:
    ...
    root = str(MIKASA_HOME).rstrip("\\/")
    subs = ("hermes-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(MIKASA_HOME: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Hermes-owned entries from User-scope PATH in the registry (see ``_hermes_path_markers``)."""
    markers = tuple(m.lower() for m in _hermes_path_markers(MIKASA_HOME, include_managed_bin=include_managed_bin))
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete MIKASA_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("mikasa_HOME", "mikasa_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(MIKASA_HOME: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``MIKASA_HOME`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (MIKASA_HOME / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    ...
    try:
        ...
        from mikasa_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from mikasa_constants import get_default_mikasa_root
        bin_dir = get_default_mikasa_root() / "bin"
    except Exception as e:
...
def _is_default_mikasa_home(MIKASA_HOME: Path) -> bool:
    """Return True when ``MIKASA_HOME`` points at the default (non-profile) root."""
    try:
        from mikasa_constants import get_default_mikasa_root
        return MIKASA_HOME.resolve() == get_default_mikasa_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    from the current MIKASA_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m mikasa_cli.main` (the `hermes` wrapper may be half-gone).
    hermes_invocation = [sys.executable, "-m", "mikasa_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's MIKASA_HOME directory.
...
def run_gui_uninstall(args):
    ...
    from mikasa_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    MIKASA_HOME = get_mikasa_home()
    summary = gui_install_summary(MIKASA_HOME)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {MIKASA_HOME}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Mikasa agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(MIKASA_HOME):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Mikasa agent at {MIKASA_HOME / 'hermes-agent'}")
        print(f"  • Your config, sessions, and secrets under {MIKASA_HOME}")
        print()
    ...
    print()
    uninstall_gui(MIKASA_HOME)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Mikasa agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, MIKASA_HOME=MIKASA_HOME, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie MIKASA_HOMEs and systemd units behind.
    is_default_profile = _is_default_mikasa_home(MIKASA_HOME)
    ...
    print()
    _print_box("│            ⚕ Mikasa Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {MIKASA_HOME / 'config.yaml'}")
    print(f"  Secrets: {MIKASA_HOME / '.env'}")
    print(f"  Data:    {MIKASA_HOME / 'cron/'}, {MIKASA_HOME / 'sessions/'}, {MIKASA_HOME / 'logs/'}")
    print()
    ...
    # services, alias wrappers, MIKASA_HOME dirs) — otherwise they leave zombie services behind.
    ...
    _perform_uninstall(
        project_root=project_root, MIKASA_HOME=MIKASA_HOME, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, MIKASA_HOME: Path, full_uninstall: bool) -> None:
    ...
    if not full_uninstall:
        print(f"  • Keep Hermes config/data: {MIKASA_HOME}")
    else:
        print(f"  • Hermes config/data: {MIKASA_HOME}")
        profiles = _discover_named_profiles() if _is_default_mikasa_home(MIKASA_HOME) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$MIKASA_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: MIKASA_HOME is
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_mikasa_home(MIKASA_HOME)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(MIKASA_HOME))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Hermes-owned PATH entries in User environment"),
        (windows, "Removing MIKASA_HOME / HERMES_GIT_BASH_PATH User env vars...",
        ...
        (True, "Removing Hermes-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(MIKASA_HOME), "Removed {}", "No Hermes-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE MIKASA_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from mikasa_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(MIKASA_HOME):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(MIKASA_HOME), "Removed {}",
    ...
    if full_uninstall:
        ...
        _rmtree_step(MIKASA_HOME)
    else:
        log_info(f"Keeping configuration and data in {MIKASA_HOME}")
    ...
    print()
    print("Thank you for using Mikasa Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    ...
    ``mikasa_constants`` + ``hermes_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
