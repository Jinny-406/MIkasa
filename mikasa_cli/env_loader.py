...
# Once-per-process guards: load_mikasa_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# MIKASA_HOME paths already pulled external secrets for: load_mikasa_dotenv() runs at import time from
...
def get_secret_source_values(MIKASA_HOME: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``MIKASA_HOME``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(MIKASA_HOME).resolve()), {}))


def hydrate_profile_secret_sources(MIKASA_HOME: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(MIKASA_HOME))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_mikasa_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
        ...
        local_env["mikasa_HOME"] = str(home)
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `mikasa setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from mikasa_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_mikasa_dotenv(
    *,
    MIKASA_HOME: str | os.PathLike | None = None,
    ...
    home_path = Path(MIKASA_HOME or os.getenv("mikasa_HOME", Path.home() / ".hermes"))
    ...
    if is_multiplex_active() and get_mikasa_home_override() is not None:
        ...
        if load_external_secrets:
            from mikasa_cli import _early_recovery
    ...
    # `mikasa setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_mikasa_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_mikasa_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    # load_mikasa_dotenv() calls per startup don't re-fetch / re-print — error retries within one process
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    if home_path == _process_mikasa_home():
        try:
...
def _process_mikasa_home() -> Path:
    """The MIKASA_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_mikasa_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``mikasa_constants.get_process_mikasa_home()`` is the override-immune
    resolver built for exactly this; delegate to it.
    """
    try:
        from mikasa_constants import get_process_mikasa_home
