...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "mikasa_version", "install_method", "os_family"})
...
def client_resource(
    ...
    return {
        ...
        "mikasa_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("mikasa_version")
    return (
