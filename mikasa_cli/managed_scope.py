...
DISTINCT from ``mikasa_cli.config.is_managed()`` / ``HERMES_MANAGED`` (a coarse package-manager
...
def get_managed_dir() -> Optional[Path]:
    ...
    """
    override = os.environ.get("mikasa_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
    elif _under_pytest():
        return None
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    try:
        ...
        from mikasa_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
