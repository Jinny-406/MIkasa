...
01-hermes-setup (the stage2 hook) has chowned the volume and seeded $MIKASA_HOME, but
...
def reconcile_profile_gateways(
    *, MIKASA_HOME: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$MIKASA_HOME``): ``hermes_cli.gateway`` maps an empty profile suffix to it,
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        MIKASA_HOME, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(MIKASA_HOME)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(MIKASA_HOME)
        _register_service(scandir, "default", start=default_should_start)
    actions.append(_slot_action("default", MIKASA_HOME, default_prior_state, default_should_start))

    profiles_root = MIKASA_HOME / "profiles"
    ...
    if not dry_run:
        _write_reconcile_log(MIKASA_HOME, actions)
    return actions


def _maybe_migrate_legacy_gateway_run_state(
    MIKASA_HOME: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
    ...
    state_file = MIKASA_HOME / "gateway_state.json"
    if state_file.exists():
        return None
    if os.environ.get("mikasa_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _write_reconcile_log(MIKASA_HOME: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $MIKASA_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted MIKASA_HOME both
    ...
    MIKASA_HOME = Path(os.environ.get("mikasa_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(MIKASA_HOME=MIKASA_HOME, scandir=scandir)
    for a in actions:
