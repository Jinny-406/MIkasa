...
looked up on ``hermes_cli.models`` at call time so ``patch("mikasa_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from mikasa_cli.models_reasoning_caps import _seed_reasoning_caps
...
def nous_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    ...
    try:
        from mikasa_cli.nous_account import nous_policy_present
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from mikasa_cli.models import _pricing_profile_key
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from mikasa_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from mikasa_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
