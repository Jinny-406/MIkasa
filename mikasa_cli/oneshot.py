...
def run_oneshot(
    ...
    env_model_early = os.getenv("mikasa_INFERENCE_MODEL", "").strip()
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    env_model = os.getenv("mikasa_INFERENCE_MODEL", "").strip()
    ...
    try:
        from mikasa_cli import model_switch as _ms
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("mikasa_INFERENCE_PROVIDER", "").strip().lower() or "auto"
...
def _run_agent(
    ...
    from mikasa_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
