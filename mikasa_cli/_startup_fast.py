...
mikasa_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def active_profile_may_override_home(hermes_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect MIKASA_HOME?"""
...
def _resolved_home() -> str:
    return os.environ.get("mikasa_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("mikasa_DEV") == "1" or is_container_startup_environment():
        return False
    MIKASA_HOME = os.environ.get("mikasa_HOME", "").strip()
    if MIKASA_HOME:
        if os.path.exists(os.path.join(MIKASA_HOME, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(MIKASA_HOME)))
        return parent_name != "profiles" and active_profile_may_override_home(MIKASA_HOME)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        ...
        from mikasa_cli import __release_date__, __version__

        print(f"Mikasa Agent v{__version__} ({__release_date__})")
    ...
    try:
        from mikasa_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    if is_termux and os.environ.get("mikasa_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
