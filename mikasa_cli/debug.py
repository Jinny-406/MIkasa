"""``hermes debug`` debug tools for Mikasa Agent."""
...
def _pending_file() -> Path:
    return get_mikasa_home() / "pastes" / "pending.json"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from mikasa_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_mikasa_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    ...
    from mikasa_cli.dump import run_dump
    capture = io.StringIO()
