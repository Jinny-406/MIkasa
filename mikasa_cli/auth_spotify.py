...
def _spotify_client_id(explicit: Optional[str] = None, state: Optional[Dict[str, Any]] = None) -> str:
    client_id = _spotify_setting(
        state, "client_id", ("mikasa_SPOTIFY_CLIENT_ID", "SPOTIFY_CLIENT_ID"), "", explicit=explicit,
    )
    if client_id:
        return client_id
    raise _spotify_err(
...
def _spotify_redirect_uri(explicit: Optional[str] = None, state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "redirect_uri", ("mikasa_SPOTIFY_REDIRECT_URI", "SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "api_base_url", ("mikasa_SPOTIFY_API_BASE_URL",), DEFAULT_SPOTIFY_API_BASE_URL, strip_slash=True,
    )


def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    return _spotify_setting(
        state, "accounts_base_url", ("mikasa_SPOTIFY_ACCOUNTS_BASE_URL",), DEFAULT_SPOTIFY_ACCOUNTS_BASE_URL,
        strip_slash=True,
    )
...
def resolve_spotify_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from mikasa_cli.auth import _auth_store_lock, _is_expiring, _load_auth_store, _load_provider_state, _quarantine_flat_oauth_state, _refresh_spotify_oauth_state, _save_auth_store, _store_provider_state
...
def get_spotify_auth_status() -> Dict[str, Any]:
    from mikasa_cli.auth import _is_expiring, get_provider_auth_state
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    from mikasa_cli.auth import _is_remote_session
    ...
    save_env_value("mikasa_SPOTIFY_CLIENT_ID", raw)
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("mikasa_SPOTIFY_REDIRECT_URI", redirect_uri_hint)
...
def login_spotify_command(args) -> None:
    from mikasa_cli.auth import _auth_store_lock, _can_open_graphical_browser, _is_remote_session, _load_auth_store, _print_loopback_ssh_hint, _save_auth_store, _store_provider_state, get_provider_auth_state
    existing_state = get_provider_auth_state("spotify") or {}

    # No client_id anywhere -> wizard instead of "mikasa_SPOTIFY_CLIENT_ID is required".
    try:
