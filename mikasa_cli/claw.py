...
from mikasa_cli.config import get_mikasa_home, get_config_path, load_config, save_config
from mikasa_constants import get_optional_skills_dir
from mikasa_cli.setup import (Colors, color, print_header, print_info, print_success, print_error,
                              prompt_yes_no)
...
_OPENCLAW_SCRIPT_INSTALLED = get_mikasa_home() / "skills" / _SCRIPT_REL
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Warn if a Mikasa gateway has connected platforms (token conflicts, e.g. Telegram 409)."""
    ...
    if connected and _warn_running(
        auto_yes, "Mikasa gateway is running with active connections: " + ", ".join(connected), [],
        ("Migrating bot tokens while the gateway is active will cause "
         "conflicts (Telegram, Discord, and Slack only allow one active "
         "session per token).",
         "Recommendation: stop the gateway first with 'mikasa gateway stop'."),
...
def _cmd_migrate(args):
    ...
    opts.MIKASA_HOME = get_mikasa_home()
    print()
    print_header("Migration Settings")
    _info(f"Source:      {opts.source_dir}", f"Target:      {opts.MIKASA_HOME}",
...
def _load_migrator(script_path: Path, opts: SimpleNamespace) -> Optional[Callable[[bool], dict]]:
    ...
    return lambda execute: mod.Migrator(
        source_root=opts.source_dir.resolve(), target_root=opts.MIKASA_HOME.resolve(),
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    if not opts.no_backup:
        try:
            from mikasa_cli.backup import create_pre_migration_backup
            from mikasa_cli.sizefmt import format_bytes as _format_size
            backup_archive = create_pre_migration_backup(MIKASA_HOME=opts.MIKASA_HOME)
            if backup_archive:
                print()
