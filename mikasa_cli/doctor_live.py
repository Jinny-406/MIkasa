...
from mikasa_cli.doctor import _section, check_info
from mikasa_cli.doctor_report import check_fail, check_ok, check_warn
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from mikasa_cli.doctor import mikasa_HOME, PROJECT_ROOT
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (MIKASA_HOME / "node" / "bin", MIKASA_HOME / "node", MIKASA_HOME / "node_modules" / ".bin"):
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    from mikasa_cli.mcp_config import _probe_single_server
