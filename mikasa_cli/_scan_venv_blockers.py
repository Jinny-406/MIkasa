...
def _updater_owned_backend_entry(pid: int, cmdline: str) -> dict | None:
    ...
    """
    try:
        from mikasa_cli.update_cmd import _mikasa_holder_subcommand  # noqa: PLC0415

        purpose = _mikasa_holder_subcommand(cmdline)
    except Exception:
        return None
    if purpose not in _UPDATER_STOPPABLE_PURPOSES:
        return None
    try:
        from mikasa_cli.process_identity import ledger_entries, spawner_is_dead  # noqa: PLC0415
