...
# to the checkout (NOT in MIKASA_HOME): __pycache__ is per-checkout state shared
# by every profile.
...
def _record_bytecode_fingerprint() -> None:
    ...
    from mikasa_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    """
    from mikasa_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    ...
    return _web_project_root(web_dir) / "mikasa_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Path of the web UI build stamp under $MIKASA_HOME."""
    from mikasa_constants import get_mikasa_home
    return get_mikasa_home() / "web-ui-build-stamp.json"
