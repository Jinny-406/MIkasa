...
in :func:`run_migrations`. Every write goes through ``mikasa_cli.config._persist_migration`` so a
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_mikasa_home()}/config.yaml and run `mikasa setup` to "
        f"regenerate, or manually set _config_version: {SUPPORT_FLOOR_VERSION} "
        "after reviewing the changelog.")


def _cfg():
    """Return the live ``mikasa_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
    from mikasa_cli import config
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$MIKASA_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
        user_plugins_dir = _c.get_mikasa_home() / "plugins"
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        curator_dir = _c.get_mikasa_home() / "logs" / "curator"
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from mikasa_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "mikasa_NEMO_RELAY_PLUGINS_TOML.")
