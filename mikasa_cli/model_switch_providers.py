...
from mikasa_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("mikasa_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from mikasa_cli.model_switch import _extra_headers_from_config
...
def _fetch_picker_live_models(
    ...
    from mikasa_cli.models import _get_ollama_native_headers, cached_fetch_api_models
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from mikasa_cli.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from mikasa_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    ...
    from mikasa_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from mikasa_cli.models import _AGGREGATOR_PROVIDERS
    from mikasa_cli.providers import ALIASES
...
def _auth_store_has_provider(*keys: str) -> bool:
    ...
    try:
        from mikasa_cli.auth import _load_auth_store
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    ...
    from mikasa_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _nous_picker_model_ids(curated: dict, force_fresh_nous_tier: bool) -> list:
    ...
    try:
        ...
        from mikasa_cli.auth import get_provider_auth_state
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from mikasa_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from mikasa_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from mikasa_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from mikasa_cli.auth import PROVIDER_REGISTRY
    ...
    for pid, overlay in MIKASA_OVERLAYS.items():
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from mikasa_cli.model_switch import _declared_model_ids
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    ...
    # MIKASA_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # provider IDs ("copilot").
    mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in MIKASA_OVERLAYS.items():
        hermes_slug = mdev_to_hermes.get(pid, pid)
        ...
        if hermes_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            from mikasa_cli.models import cached_provider_model_ids
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from mikasa_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from mikasa_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from mikasa_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from mikasa_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str) -> dict[str, list[str]]:
    ...
    from mikasa_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nous_model_ids
...
def list_authenticated_providers(
    ...
    from mikasa_cli.config import coerce_provider_id, stringify_provider_map
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from mikasa_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    from mikasa_cli.model_switch import list_authenticated_providers
    from mikasa_cli.models import fetch_openrouter_models
