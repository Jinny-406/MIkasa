...
from mikasa_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $MIKASA_HOME."""
    from mikasa_constants import get_mikasa_home
    return get_mikasa_home() / "desktop-build-stamp.json"
...
def _register_linux_desktop_entry() -> None:
    ...
    from mikasa_cli.main import PROJECT_ROOT
    try:
        from mikasa_cli.linux_desktop_entry import install_desktop_entry, is_supported
        if not is_supported():
            return
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from mikasa_cli.main import PROJECT_ROOT
    from mikasa_cli.main_web_build import _run_npm_install_deterministic
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    for attr, key in (
        ("fake_boot", "mikasa_DESKTOP_BOOT_FAKE"), ("ignore_existing", "mikasa_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "mikasa_root", None):
        env["mikasa_DESKTOP_HERMES_ROOT"] = str(Path(args.hermes_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["mikasa_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "mikasa_DESKTOP_DISABLE_GPU" not in os.environ:
        env["mikasa_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "mikasa_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["mikasa_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from mikasa_cli.main import PROJECT_ROOT
    from mikasa_cli.main_install_repair import _resolve_node_runtime_npm
    desktop_dir = PROJECT_ROOT / "apps" / "desktop"
