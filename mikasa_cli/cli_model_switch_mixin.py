...
def _merge_preflight_warning(cli, result, custom_providers) -> None:
    ...
    if cli.agent is None:
        return
    try:
        from mikasa_cli.context_switch_guard import merge_preflight_compression_warning
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from mikasa_cli.model_switch import format_model_for_display, resolve_display_context_length
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from mikasa_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from mikasa_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        from mikasa_cli.models import opencode_provider_family
        if opencode_provider_family(resolved_provider) is not None:
            from mikasa_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
    ...
    def _restore_session_model(self, session_meta: dict, *, quiet: bool = False) -> None:
        ...
        from mikasa_state import SessionDB as _SessionDB
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if not getattr(result, "success", False):
            return True
        try:
            from mikasa_cli.model_selection_guards import combined_selection_warning
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from mikasa_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from cli import _cprint
        from mikasa_cli import codex_runtime_switch as crs
    ...
    def _cmd_moa(self, cmd_original: str):
        ...
        from mikasa_cli.moa_config import moa_usage, normalize_moa_config
    ...
    def _cmd_race(self, cmd_original: str):
        """/race: Race a prompt across multiple models and return the best response (ULTRAPLINIAN)."""
        from cli import _cprint, _slash_args

        payload = _slash_args(cmd_original)
        if not payload:
            _cprint("  Usage: /race <prompt> [--models model1,model2] [--timeout 30]")
            _cprint("  Races your prompt across multiple models in parallel and returns the best response.")
            return True
        ...
        while i < len(args):
            if args[i] == "--models" and i + 1 < len(args):
                models_list = [m.strip() for m in args[i + 1].split(",")]
                i += 2
        ...
        if not prompt:
            _cprint("  Error: No prompt provided.")
            return True

        # Store race config for the next turn
        self._pending_race_config = {
            "prompt": prompt,
            "models": models_list,
            "timeout": timeout,
        }
        self._pending_agent_seed = prompt
        _cprint(f"  ULTRAPLINIAN race queued ({len(models_list) or 'default'} models, {timeout}s timeout)")
        return True
