...
from mikasa_cli.auth_codex import _pool_entries
from mikasa_cli.auth_constants import (
...
if TYPE_CHECKING:  # annotation-only; the runtime import would be a cycle
    from mikasa_cli.auth import ProviderConfig

# Log-record parity with the origin module (caplog tests pin "mikasa_cli.auth").
logger = logging.getLogger("mikasa_cli.auth")
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("mikasa_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
def _portal_entitlement_message(capability: str) -> str:
    ...
    from mikasa_cli.nous_account import (
...
def _nous_inference_env_override() -> Optional[str]:
    ...
    """
    from mikasa_cli.auth import _optional_base_url
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    return _optional_base_url(
        os.getenv("mikasa_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL"))
...
def _nous_invoke_jwt_status(
    ...
    from mikasa_cli.auth import _is_expiring
...
def _nous_invoke_jwt_is_usable(
    ...
    from mikasa_cli.auth import _nous_invoke_jwt_status
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from mikasa_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_nous_agent_key_from_invoke_jwt(
    ...
    from mikasa_cli.auth import _nonempty_str
...
def _nous_shared_auth_dir() -> Path:
    ...
    """
    override = os.getenv("mikasa_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from mikasa_constants import get_default_mikasa_root
    return get_default_mikasa_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from mikasa_constants import get_default_mikasa_root
        real_home_shared = (
            get_default_mikasa_root() / "shared" / NOUS_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    """
    from mikasa_cli.auth import _file_lock
    try:
        lock_path = _nous_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        yield  # No MIKASA_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
...
def _merge_shared_nous_oauth_state(state: Dict[str, Any]) -> bool:
    ...
    from mikasa_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_nous_state
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    """
    from mikasa_cli.auth import _nonempty_str, _write_private_file_atomic
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    """
    from mikasa_cli.auth import _auth_file_path
...
def _refresh_nous_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from mikasa_cli.auth import _refresh_access_token, _is_terminal_nous_refresh_error
    try:
...
def _apply_nous_refreshed_tokens(
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from mikasa_cli.auth import _coerce_ttl_seconds
...
def persist_nous_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    ...
    it.
    """
    from mikasa_cli.auth import _save_active_provider_state, _write_shared_nous_state
...
def _nous_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    """
    from mikasa_cli.auth import _NOUS_PORTAL_ALLOWED_HOSTS, _optional_base_url
    portal_url = (
        _optional_base_url(state.get("portal_base_url")) or os.getenv("mikasa_PORTAL_BASE_URL")
...
class _NousRuntimeResolve:
    ...
    def persist(self, reason: str) -> None:
        from mikasa_cli.auth import _save_provider_state_to_source, _write_shared_nous_state
        state = self.state
...
def _snapshot_nous_pool_status() -> Dict[str, Any]:
    ...
    """
    from mikasa_cli.auth import _parse_iso_timestamp
    try:
        from agent.credential_pool import load_pool
        pool = load_pool("nous")
...
def _compute_nous_auth_status() -> Dict[str, Any]:
    ...
    from mikasa_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def _nous_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    portal_base_url = (
        portal_base_url or os.getenv("mikasa_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _mirror_nous_state_best_effort(auth_state: Dict[str, Any]) -> None:
    ...
    from mikasa_cli.auth import _sync_nous_pool_from_auth_store, _write_shared_nous_state
    with suppress(Exception):
...
def _pick_nous_model_after_login(
    ...
    from mikasa_cli.models_pricing import (
...
def _offer_shared_nous_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    ...
    """
    from mikasa_cli.auth import _prompt_yes_no, _read_shared_nous_state
    shared = _read_shared_nous_state()
    if not shared:
        return None
    try:
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    ...
    from mikasa_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("mikasa_CA_BUNDLE")
