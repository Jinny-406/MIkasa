...
resolved through the module object so test patches on ``mikasa_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from mikasa_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'mikasa setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'mikasa setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'mikasa tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("mikasa setup", "          Re-run the full wizard"), ("mikasa setup model", "    Change model/provider"),
    ("mikasa setup terminal", " Change terminal backend"), ("mikasa setup gateway", "  Configure messaging"),
    ("mikasa setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("mikasa config", "         View current settings"), ("mikasa config edit", "    Open config in your editor"),
    ("mikasa config set <key> <value>", ""))
_READY_ROWS = (
    ("mikasa", "              Start chatting"), ("mikasa gateway", "      Start messaging gateway"),
    ("mikasa doctor", "       Check for issues"))
...
def _first_available_plugin_provider(registry: str, skip: str = None):
    ...
    try:
        import importlib
        from mikasa_cli.plugins import _ensure_plugins_discovered
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'mikasa setup' to configure")
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'mikasa setup terminal'")
...
def _print_setup_summary(config: dict, MIKASA_HOME):
    ...
    try:
        from mikasa_cli.auth import resolve_provider
        resolve_provider()
    except Exception:
        print()
        ...
        _setup._info("  Finish this one step with either of:",
              ...
              "    mikasa setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'mikasa setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {MIKASA_HOME}/cron/, sessions/, logs/")):
...
import mikasa_cli.setup as _setup  # noqa: E402  (bottom: hermes_cli.setup imports this module)
