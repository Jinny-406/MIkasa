...
def _root() -> Path:
    ...
    # MIKASA_HOME) cannot move the reader away from the helper's token.
    return get_default_mikasa_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    launches_module = (
        argv[1:3] == ["-m", "mikasa_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "mikasa_cli.main" in argv[2]))
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = (
        ...
        "runpy.run_module('mikasa_cli.main',run_name='__main__',alter_sys=True)")
...
def _probe(*_: str) -> dict[str, Any]:
    import platform
    return {"os": "Windows", "arch": platform.machine(), "hermesHome": str(get_default_mikasa_root()), "python": sys.executable}
