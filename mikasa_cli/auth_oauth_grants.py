...
from mikasa_cli.auth_constants import _decode_jwt_claims
...
def _is_pkce_row(row: Dict[str, Any]) -> bool:
    return str(row.get("source") or "").endswith("mikasa_pkce")
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    """
    from mikasa_cli.auth import _save_auth_store
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    """
    from mikasa_cli.auth import _oauth_heal_notices
...
class _HealPass:
    ...
    def heal_profile_singleton(self, profile_singleton: Optional[Path]) -> None:
        if profile_singleton is None or not profile_singleton.exists():
            return
        from mikasa_cli.auth import _is_same_auth_store
    ...
    def sync_root_singleton_with_pkce_row(self) -> None:
        ...
        pkce_idx = next(
            (i for i, r in enumerate(self.r_rows)
             if _is_oauth_pool_payload(r) and r.get("source") == "mikasa_pkce"), None)
        if pkce_idx is None:
            return
