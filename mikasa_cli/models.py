...
from mikasa_cli import __version__ as _HERMES_VERSION
from mikasa_cli.urllib_security import open_credentialed_url
from mikasa_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from mikasa_cli.models_reasoning_caps import (
    _OPENROUTER_CATALOG_URL,
    _seed_reasoning_caps)
from mikasa_cli.models_local import (
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from mikasa_cli.config import get_custom_provider_tls_settings
...
def _pricing_profile_key() -> str:
    ...
    from mikasa_constants import mikasa_home_key

    return mikasa_home_key()
...
def check_nous_free_tier(*, force_fresh: bool = False, cached_only: bool = False) -> bool:
    ...
    if cached_only:
        return False
    try:
        from mikasa_cli.nous_account import get_nous_portal_account_info
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_mikasa_home() / "cache" / "nous_recommended_cache.json"
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from mikasa_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from mikasa_cli.model_catalog import get_default_model_from_cache
...
def fetch_openrouter_models(
    ...
    try:
        from mikasa_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        from mikasa_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from mikasa_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from mikasa_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from mikasa_cli.auth import get_auth_status, has_usable_secret
...
def _resolve_static_model_alias(
    ...
    try:
        from mikasa_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from mikasa_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from mikasa_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from mikasa_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from mikasa_cli.codex_models import get_codex_model_ids
    ...
    try:
        from mikasa_cli.auth import resolve_codex_runtime_credentials
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from mikasa_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
...
def _api_key_credentials(normalized: str) -> tuple[str, str]:
    ...
    try:
        from mikasa_cli.auth import resolve_api_key_provider_credentials
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    from mikasa_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $MIKASA_HOME/provider_models_cache.json;
...
def _provider_models_cache_path() -> Path:
    from mikasa_constants import get_mikasa_home
    return get_mikasa_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    parts: list[str] = []
    try:
        from mikasa_cli.auth import PROVIDER_REGISTRY
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_mikasa_home() / rel)
    except Exception:
        pass
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    try:
        from mikasa_cli.copilot_auth import copilot_request_headers
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from mikasa_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
        "HTTP-Referer": "https://hermes-agent.nousresearch.com",
        "X-Title": "Mikasa Agent",
...
def probe_api_models(
    ...
    if isinstance(request_headers, dict):
        ...
        from mikasa_cli.config import normalize_extra_headers
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('mikasa_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('mikasa_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('mikasa_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('mikasa_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('mikasa_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('mikasa_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('mikasa_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('mikasa_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('mikasa_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('mikasa_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('mikasa_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('mikasa_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('mikasa_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('mikasa_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('mikasa_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('mikasa_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('mikasa_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('mikasa_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('mikasa_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('mikasa_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('mikasa_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('mikasa_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('mikasa_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('mikasa_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('mikasa_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('mikasa_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('mikasa_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('mikasa_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('mikasa_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('mikasa_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('mikasa_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
