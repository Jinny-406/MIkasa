...
Tokens live in ~/.hermes/auth.json, NOT ~/.codex/: Mikasa keeps its own Codex OAuth session
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from mikasa_cli.auth import _POOL_STATUS_FIELDS
...
def _codex_base_url() -> str:
    return os.getenv("mikasa_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from mikasa_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Mikasa auth store (~/.hermes/auth.json)."""
    from mikasa_cli.auth import _load_provider_state, _nonempty_str
...
def _sync_codex_pool_entries(
    ...
    ``device_code`` (the singleton-seeded entry from ``mikasa setup`` / the model picker) is always
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Mikasa auth store (~/.hermes/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Mikasa auth, if available."""
    from mikasa_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
...
def refresh_codex_oauth_pure(
    ...
    """Refresh Codex OAuth tokens without mutating Mikasa auth state."""
    from mikasa_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    from mikasa_cli.auth import _save_codex_tokens, refresh_codex_oauth_pure
    try:
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from mikasa_cli.auth import _codex_access_token_is_expiring
...
def resolve_codex_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    """Resolve runtime credentials from Mikasa's own Codex token store.
    ...
    refresh_timeout_seconds = env_float("mikasa_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from mikasa_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Mikasa will create its own session to avoid conflicts with Codex CLI / VS Code.")
            if _prompt_yes_no(
                ...
                print("Mikasa will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Mikasa gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Mikasa creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from mikasa_cli.auth import _utc_now_z
