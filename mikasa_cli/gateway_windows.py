...
_TASK_NAME_DEFAULT = "mikasa_Gateway"
_TASK_DESCRIPTION = "Mikasa Agent Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("mikasa_GATEWAY_DETACHED", "1"), ("mikasa_SUPERVISED_CHILD", "1"))
...
def _mikasa_home() -> Path:
    from mikasa_cli.config import get_mikasa_home

    return Path(get_mikasa_home())


def _preserve_mikasa_home_path(path: str | Path) -> str:
    r"""Render Hermes-owned paths under the configured MIKASA_HOME spelling.

    ``%LOCALAPPDATA%\hermes`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under MIKASA_HOME.
    """
    candidate = Path(path)
    try:
        home = _mikasa_home()
...
def _current_profile_cli_args() -> list[str]:
    ...
    from mikasa_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "mikasa_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"mikasa_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "mikasa_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "mikasa_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from mikasa_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<MIKASA_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _mikasa_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at MIKASA_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m mikasa_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "mikasa_cli.main"]
    if profile_arg:
...
def _launcher_settings() -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, MIKASA_HOME, profile_arg) for generated launchers."""
    from mikasa_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    MIKASA_HOME = str(_mikasa_home())
    return (
        _preserve_mikasa_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        MIKASA_HOME,
        _profile_arg(MIKASA_HOME),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_mikasa_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_mikasa_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, MIKASA_HOME: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "mikasa_HOME={MIKASA_HOME}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_mikasa_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, MIKASA_HOME: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('mikasa_HOME')}) = {q(MIKASA_HOME)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_mikasa_home_path(venv_dir))}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from mikasa_cli.gateway import PROJECT_ROOT

    python_path, working_dir, MIKASA_HOME, profile_arg = _launcher_settings()
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"mikasa_HOME": MIKASA_HOME, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_mikasa_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_mikasa_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (MIKASA_HOME,
    ...
    try:
        MIKASA_HOME = str(_mikasa_home().resolve())
    except Exception:
        MIKASA_HOME = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "mikasa_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if MIKASA_HOME:
        env_overlay["mikasa_HOME"] = MIKASA_HOME
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("mikasa_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("mikasa_GATEWAY_INSTALL_START_ON_LOGIN")
    if start_now is not None and start_on_login is not None:
        return start_now, start_on_login

    from mikasa_cli.setup import prompt_yes_no
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated Mikasa gateway install prompt.")
            if start_now:
...
def _confirm_gateway_stable(initial_pids: list[int], confirm_s: float, interval_s: float, all_profiles: bool = False) -> list[int]:
    ...
    if confirm_s <= 0:
        return initial_pids
    from mikasa_cli.gateway import find_gateway_pids
...
def _start_attestation_path() -> Path:
    return _mikasa_home().joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(pid: int) -> bool:
    ...
    try:
        ...
        data = json.loads(get_lifecycle_sentinel_path(_mikasa_home()).read_text(encoding="utf-8"))
    except Exception:
        return False
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated Mikasa gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: hermes gateway status")
                    return
