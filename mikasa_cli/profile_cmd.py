...
def _render_distribution_plan(plan) -> None:
    ...
    from mikasa_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _profile_status(args):
    ...
    from mikasa_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    profile_name = get_active_profile_name()
    dhh = display_mikasa_home()
...
def _profile_use(args):
    from mikasa_cli.profiles import set_active_profile
...
def _profile_delete(args):
    from mikasa_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_mikasa_home),
    everything else to its named directory."""
    from mikasa_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from mikasa_constants import get_mikasa_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from mikasa_cli import profile_describer as _pd
    if all_flag:
...
def _profile_rename(args):
    from mikasa_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from mikasa_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from mikasa_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="mikasa_dist_preview_") as tmp:
...
def _profile_update(args):
    from mikasa_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from mikasa_cli.profiles import get_profile_dir, normalize_profile_name
    try:
...
_INFO_FIELDS = (
    ...
    ("author", "Author:       "),
    ("license", "License:      "),
    ("mikasa_requires", "Requires:     Hermes "),
    ("source", "Source:       "),
...
def _profile_info(args):
    from mikasa_cli.profile_distribution import describe_distribution, DistributionError
    try:
