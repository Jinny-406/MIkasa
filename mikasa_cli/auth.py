"""Multi-provider authentication system for Mikasa Agent.
...
from mikasa_cli.config import (
    get_mikasa_home, get_config_path, read_raw_config, require_readable_config_before_write)
from mikasa_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
from mikasa_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from mikasa_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_nous import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_minimax import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_xai import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_codex import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from mikasa_cli.auth_qwen import (  # noqa: F401  re-exported
...
from mikasa_cli.auth_constants import (  # noqa: F401  re-exported
...
def get_anthropic_key() -> str:
    ...
    """
    from mikasa_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from mikasa_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def _auth_file_path() -> Path:
    path = get_mikasa_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch MIKASA_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".hermes" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set MIKASA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_mikasa_root()
    except Exception:
        return None
    return None if _same_path(get_mikasa_home(), global_root) else global_root / "auth.json"
...
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "mikasa_pkce", "manual"})
...
def _explicit_env_credentials_present(normalized: str) -> bool:
    ...
    if pconfig is None:
        from mikasa_cli.providers import get_provider
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        from mikasa_cli.config import validate_config_structure
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from mikasa_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run mikasa setup.",
        code="corrupt_config")
...
def _nous_portal_base_url(state: Dict[str, Any]) -> str:
    """mikasa_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted operator override and wins
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("mikasa_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
...
def _provider_is_keyless(provider_id: str) -> bool:
    ...
    try:
        from mikasa_cli.providers import MIKASA_OVERLAYS
        return bool(getattr(MIKASA_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    try:
        from mikasa_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from mikasa_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('mikasa_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('mikasa_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('mikasa_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('mikasa_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('mikasa_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NOUS_INFERENCE_INVOKE_SCOPE': ('mikasa_cli.auth_constants', 'NOUS_INFERENCE_INVOKE_SCOPE'),
    'NOUS_SHARED_STORE_FILENAME': ('mikasa_cli.auth_nous', 'NOUS_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('mikasa_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('mikasa_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('mikasa_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('mikasa_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('mikasa_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('mikasa_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('mikasa_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('mikasa_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('mikasa_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nous_oauth_pure': ('mikasa_cli.auth_nous', 'refresh_nous_oauth_pure'),
}
