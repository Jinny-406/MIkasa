...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("mikasa_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(MIKASA_HOME: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    if MIKASA_HOME is None:
        from mikasa_constants import get_mikasa_home

        MIKASA_HOME = get_mikasa_home()
    if _path_is_mounted(MIKASA_HOME):  # any error propagates to run_security_audit (= no finding)
        return None
    return (f"Running in a container but the data dir ({MIKASA_HOME}) is NOT on a persistent volume mount — "
            "sessions, memory, skills, and API keys are ephemeral and lost on container restart. Mount a host "
            "volume over the MIKASA_HOME data directory.")
...
def run_security_audit(*, MIKASA_HOME: Optional[Path] = None, config: Optional[dict] = None) -> list[str]:
    ...
    """
    findings: list[str] = []
    for check in (_running_as_root, _ssh_password_auth_enabled, lambda: _container_no_volume_mount(MIKASA_HOME),
...
def log_startup_security_warnings(
    *, MIKASA_HOME: Optional[Path] = None, config: Optional[dict] = None, force: bool = False
) -> list[str]:
    ...
    if _AUDIT_RAN and not force:
        return []
    _AUDIT_RAN = True
    findings = run_security_audit(MIKASA_HOME=MIKASA_HOME, config=config)
    if findings:
