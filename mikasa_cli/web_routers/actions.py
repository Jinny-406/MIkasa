...
from mikasa_cli.config import format_docker_update_message, recommended_update_command_for_method
...
detect_install_method = late("detect_install_method", "mikasa_cli.config")
get_mikasa_home = late("get_mikasa_home", "mikasa_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "mikasa_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "mikasa_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "mikasa_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "mikasa_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import mikasa_cli.web_server_gateway as gw
...
async def update_hermes():
    ...
    with http_failure("Failed to spawn hermes update", 500, "Failed to start update"):
        proc = _spawn_hermes_action(["update"], "hermes-update", env_overrides={"mikasa_ACTION_ID": action_id})
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        from mikasa_cli.banner import check_for_updates

        if force:
            with contextlib.suppress(OSError):
                (get_mikasa_home() / ".update_check").unlink()
