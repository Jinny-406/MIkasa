...
from mikasa_cli.web_server_config import _apply_main_model_assignment, _normalize_main_model_assignment
from mikasa_cli.web_server_gateway import _strip_session_list_rows
from mikasa_cli.web_server_profiles import (
    _fallback_profile_dicts, _hub_action_name, _write_profile_mcp_servers,
)
from mikasa_cli.web_server_sessions import _open_session_db_at_path
...
from mikasa_cli.web_models import (
    ...
    SessionPrScanBody)
from mikasa_cli.web_server_profiles import _mikasa_home_scope
...
_cron_profile_home = late("_cron_profile_home", "mikasa_cli.web_server_cron")
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (MIKASA_HOME-scoped);
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    from mikasa_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the MIKASA_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
