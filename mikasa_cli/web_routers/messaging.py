...
from mikasa_cli.config import OPTIONAL_ENV_VARS, get_env_path, redact_key
from mikasa_cli.web_deps import LateState, late
from mikasa_cli.web_server_gateway import _restart_gateway_after
from mikasa_cli.web_server_messaging import (
...
from mikasa_cli.web_routers._common import http_failure
from mikasa_cli.web_models import (
...
_profile_scope = late("_profile_scope", "mikasa_cli.web_server_profiles")
_resolve_profile_dir = late("_resolve_profile_dir", "mikasa_cli.web_server_profiles")
_restart_gateway_after_whatsapp_onboarding = late("_restart_gateway_after_whatsapp_onboarding", "mikasa_cli.web_server_messaging")
_telegram_onboarding_request_sync = late("_telegram_onboarding_request_sync", "mikasa_cli.web_server_messaging")
_whatsapp_session_path = late("_whatsapp_session_path", "mikasa_cli.web_server_messaging")
_write_platform_enabled = late("_write_platform_enabled", "mikasa_cli.web_server_messaging")
load_env = late("load_env", "mikasa_cli.config")
load_config = late("load_config", "mikasa_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
remove_env_value = late("remove_env_value", "mikasa_cli.config")
save_env_value = late("save_env_value", "mikasa_cli.config")
_gateway_subcommand = late("_gateway_subcommand", "mikasa_cli.web_server_gateway")
...
def _messaging_platform_payload(
    ...
    # and do NOT follow the MIKASA_HOME contextvar override, so without it messaging
...
def _platform_payloads(scoped_dir: Optional[Path], entries) -> list[dict[str, Any]]:
    ...
    MIKASA_HOME contextvar; the gateway status readers do not, hence the explicit path)."""
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    from mikasa_constants import find_node_executable, with_hermes_node_path
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Mikasa Agent").strip() or "Mikasa Agent"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    def _run():
        ...
        # credentials/state, not the root install's. load_env() honors the MIKASA_HOME contextvar override;
...
def _multiplex_port_binding_conflict(platform_id: str, requested_profile: Optional[str]) -> Optional[str]:
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (unrecognized MIKASA_HOME) is outside
