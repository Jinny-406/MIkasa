...
from mikasa_cli.config import redact_key
from mikasa_cli.web_deps import late
from mikasa_cli.web_server_files import _path_is_under
from mikasa_cli.web_server_gateway import _restart_gateway_after
from mikasa_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from mikasa_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
from mikasa_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("mikasa_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "mikasa_cli.web_server_memory")
...
save_config = late("save_config", "mikasa_cli.config")
...
async def list_webhooks():
    import mikasa_cli.webhook as wh
    ...
    def _run():
        try:
            ...
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from mikasa_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def get_memory_status():
    def _run():  # load_config(), stats and discovery are disk reads — off-loop
        ...
        mem_dir = get_mikasa_home() / "memories"
...
def _dashboard_backup_dir() -> Path:
    return get_mikasa_home() / "backups"
...
async def list_hooks():
    ...
    def _run():
        from mikasa_cli.config import load_config as _load_config
        from agent import shell_hooks

        valid_events = []
        with contextlib.suppress(Exception):
            from mikasa_cli.plugins import VALID_HOOKS
...
async def create_hook(body: HookCreate):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from mikasa_cli.plugins import VALID_HOOKS as valid_hooks
...
async def list_checkpoints():
    ...
    cp_dir = get_mikasa_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
