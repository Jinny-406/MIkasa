...
from mikasa_cli.web_deps import LateState, late
from mikasa_cli.web_server_gateway import _display_system_platform
...
from mikasa_cli.config import get_config_path, get_env_path
from mikasa_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from mikasa_cli.web_routers._common import scoped_to_thread
from pathlib import Path
from typing import Any, Dict, Optional

_log = logging.getLogger("mikasa_cli.web_server")
router = APIRouter()
# Mounted separately by web_server so /api/logs keeps its original route-table position.
logs_router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "mikasa_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "mikasa_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "mikasa_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "mikasa_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "mikasa_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "mikasa_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "mikasa_cli.web_server_lifecycle")
_spawn_hermes_action = late("_spawn_hermes_action", "mikasa_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "mikasa_cli.config")
get_mikasa_home = late("get_mikasa_home", "mikasa_cli.config")
...
load_config = late("load_config", "mikasa_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "mikasa_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    from mikasa_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from mikasa_cli.web_server import _SSH_OWNER_NONCE
...
# status) are written to the per-profile home, not the process-level MIKASA_HOME (see issue #69143). Plain
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from mikasa_cli.dashboard_auth import (
...
def _nous_session_validity() -> str:
    ...
    try:
        from mikasa_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from mikasa_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_mikasa_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from mikasa_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from mikasa_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_mikasa_home())
        ...
        if not auth["auth_required"]:
            status.update({
                "mikasa_home": str(get_mikasa_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "mikasa_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_mikasa_home()))
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from mikasa_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from mikasa_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    from mikasa_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_mikasa_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from mikasa_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
