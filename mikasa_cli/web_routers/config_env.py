...
from mikasa_cli.web_routers._common import http_failure, scoped_to_thread
from mikasa_cli.web_deps import LateState, late
from mikasa_cli.web_server_config import (
...
from mikasa_cli.web_server_profiles import (
    _approval_mode_of, _broadcast_gateway_session_info, _is_other_profile, _parse_model_ids,
)
from fastapi import HTTPException, Request
from mikasa_cli.config import DEFAULT_CONFIG, OPTIONAL_ENV_VARS, read_raw_config, custom_endpoint_key_env, coerce_provider_id, find_provider_entry, redact_key, _deep_merge
from mikasa_cli.web_models import ConfigUpdate, EnvVarUpdate, EnvVarDelete, EnvVarReveal, CustomEndpointUpdate
...
_channel_managed_env_keys = late("_channel_managed_env_keys", "mikasa_cli.web_server_messaging")
...
async def get_egress_status():
    ...
    from mikasa_cli.proxy_cli import format_status_text
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different MIKASA_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from mikasa_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level MIKASA_HOME (mirrors ``/api/config``).
    """
