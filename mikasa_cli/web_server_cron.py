...
from mikasa_cli.web_models import CronJobCreate
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from mikasa_cli.web_server_profiles import _fallback_profile_dicts
...
def _cron_default_profile() -> str:
    ...
    profile's job into ~/.hermes. ``custom`` (MIKASA_HOME outside the profiles tree) has no
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, MIKASA_HOME)."""
    from mikasa_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    return {
        **job,
        ...
        "mikasa_home": str(home),
        "is_default_profile": profile == "default"}


@contextlib.contextmanager
def _cron_store_scope(home: Path):
    """Point MIKASA_HOME and the cron.jobs store at one profile's home for the block.
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    if multiplex and profile != "default":
        ...
        listener_profile, listener_home = "default", get_default_mikasa_root()
        _log.info(
    ...
    try:
        ...
        from mikasa_constants import reset_mikasa_home_override, set_mikasa_home_override
        token = set_mikasa_home_override(str(listener_home))
        try:
