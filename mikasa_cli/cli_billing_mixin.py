...
_CHARGE_ERROR_COPY = {
    ...
    "cli_billing_disabled": "  Remote spending is off for this account — a billing admin can turn it on from the portal's Mikasa Agent page.",
...
_KILLSWITCH_REASON_OVERVIEW = "A billing admin can turn it on from the portal's Mikasa Agent page to add funds here."
_KILLSWITCH_REASON_BUY = "A billing admin can turn it on from the portal's Mikasa Agent page before adding funds."
...
class CLIBillingMixin:
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        if not url:
            return False
        try:
            from mikasa_cli.auth import _can_open_graphical_browser, _is_remote_session
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from mikasa_cli.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_apply(self, state, action, idempotency_key=None, *, allow_stepup=True):
        ...
        from cli import _cprint
        from mikasa_cli.nous_billing import (
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        try:
            from mikasa_cli import nous_billing as _nb
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {_RULE}")
        self._dim('By confirming, you allow Jinny to charge your card.')
        confirm_choices = [
    ...
    def _billing_submit_and_poll(self, state, amount, key, *, missing_msg, status_msg, on_scope=None):
        ...
        from cli import _cprint, _d
        from mikasa_cli.nous_billing import BillingError, BillingScopeRequired, post_charge
        try:
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        from mikasa_cli.nous_billing import BillingError, BillingTransient, get_charge_status
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from mikasa_cli.nous_billing import BillingTransient, BillingRemoteSpendingRevoked, BillingSessionRevoked
        code = exc.error
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Mikasa Agent page, then run /topup again.")
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        if reload_amt is None or threshold_amt is None or reload_amt <= threshold_amt:
            ...
            return
        self._dim(f"By confirming, you authorize Jinny to charge {card.masked} whenever your balance reaches {format_money(threshold_amt)}. Turn off any time here or on the portal.", lead=True)
    ...
    def _billing_patch_auto_top_up(self, state, **kwargs) -> bool:
        ...
        from mikasa_cli.nous_billing import BillingError, BillingScopeRequired, patch_auto_top_up
        try:
