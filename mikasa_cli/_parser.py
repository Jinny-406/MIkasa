...
# (it sets ``MIKASA_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "--provider", default=None, help=(
        ...
        "under model.provider — use `mikasa setup` or edit the file to change it."))
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Mikasa Agent")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="hermes", description="Mikasa Agent - AI assistant with tool-calling capabilities",
