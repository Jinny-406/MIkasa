...
(``$MIKASA_HOME/node``), installing in place with ``--prefix`` so ``bin/npm`` keeps resolving to
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    ...
    Symlinks are resolved first: ``~/.local/bin/npm`` → ``$MIKASA_HOME/node/bin/npm`` →
...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool = False) -> bool:
    ...
    into ``$MIKASA_HOME/node/etc/npmrc`` (so global installs land on PATH), and without the override
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision (or reuse) the managed tree under ``$MIKASA_HOME/node`` and return a satisfying npm.
