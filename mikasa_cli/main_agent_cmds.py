...
def _cmd_memory_reset(args):
    ...
    if not existing:
        print(f"\n  Nothing to reset — no memory files found in {display_mikasa_home()}/memories/\n")
        return
    ...
    print(f"  Files were in: {display_mikasa_home()}/memories/\n")
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from mikasa_cli.memory_setup import memory_command
        memory_command(args)
...
def cmd_skills(args):
    ...
    if action == "config":
        ...
        from mikasa_cli.skills_config import skills_command as skills_config_command
        skills_config_command(args)
    elif action in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from mikasa_cli.skills_hub import skills_command
        skills_command(args)
