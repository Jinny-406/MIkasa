...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_mikasa_home() / "hindsight" / "config.json"
...
def _post_setup_hook(provider, config: dict) -> bool:
    ...
    if hasattr(provider, "post_setup"):
        provider.post_setup(str(get_mikasa_home()), config)
        return True
    return False
...
def cmd_setup(args) -> None:
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, str(get_mikasa_home()))
        except Exception as e:
...
def _write_env_vars(
    env_writes: dict, MIKASA_HOME: str | os.PathLike[str] | None = None) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    ``save_env_value`` applies the shared gate (name regex, ``LD_PRELOAD``/``PYTHONPATH``/``MIKASA_HOME``
    denylist, CR/LF stripping, atomic 0o600 writes). ``ValueError`` is reported and skipped so one
    bad key doesn't sink the batch; filesystem errors propagate. ``MIKASA_HOME`` is applied via the
    ...
    token = set_mikasa_home_override(MIKASA_HOME) if MIKASA_HOME is not None else None
    try:
