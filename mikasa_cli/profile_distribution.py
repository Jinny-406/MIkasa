...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "mikasa_state.db", "response_store.db",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            hermes_requires=_str(data, "mikasa_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("mikasa_requires", self.hermes_requires),
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from mikasa_cli.profiles import _canon_valid, get_profile_dir
    from mikasa_cli import __version__ as hermes_version
...
def install_distribution(
    ...
    from mikasa_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="mikasa_dist_install_") as tmp:
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from mikasa_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    with tempfile.TemporaryDirectory(prefix="mikasa_dist_update_") as tmp:
