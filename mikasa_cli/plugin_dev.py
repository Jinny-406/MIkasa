...
def _doctor_runtime(plugin_path: Path):
    ...
    MIKASA_HOME with outbound socket connects blocked.
    """
    stack = ExitStack()
    try:
        ...
        stack.enter_context(patch.dict(os.environ, {
            ...
            "mikasa_BUNDLED_PLUGINS": str(bundled),
            "mikasa_ENABLE_PROJECT_PLUGINS": "0",
        }, clear=False))
...
def _is_plugin_module(name: str) -> bool:
    return name == "mikasa_plugins" or name.startswith("mikasa_plugins.")
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        candidates.append(get_mikasa_home() / "plugins" / raw)
        try:
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    import re as _re
    from mikasa_cli.plugins import SUPPORTED_MANIFEST_VERSION
    ...
    if schema:
        from mikasa_cli.plugins import _CONFIG_SCHEMA_TYPES
