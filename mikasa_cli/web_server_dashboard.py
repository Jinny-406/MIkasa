...
from mikasa_cli.config import cfg_get, get_process_mikasa_home
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    from mikasa_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    """
    from mikasa_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from mikasa_cli.web_deps import _server
    ...
    if os.environ.get("mikasa_SERVE_HEADLESS") == "1":
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_mikasa_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``MIKASA_HOME``.
    """
    themes_dir = get_process_mikasa_home() / "dashboard-themes"
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``MIKASA_HOME=<root>/profiles/<name>``) the
    ...
    # context-local MIKASA_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``MIKASA_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the hermes root (``~/.hermes/plugins``). Scan the default root as well (``get_default_mikasa_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``MIKASA_HOME`` unchanged when it
    ...
    # plugins.
    user_plugin_roots = [get_process_mikasa_home() / "plugins"]
    root_plugins = get_default_mikasa_root() / "plugins"
    ...
    # contract.
    if env_var_enabled("mikasa_ENABLE_PROJECT_PLUGINS"):
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from mikasa_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from mikasa_cli.web_server import _get_dashboard_plugins
    from mikasa_cli.config import get_mikasa_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from mikasa_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_mikasa_home() / "plugins").resolve()
...
def _mount_plugin_api_routes():
    ...
    """
    from mikasa_cli.web_server import _get_dashboard_plugins, app
    try:
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"mikasa_dashboard_plugin_{plugin['name']}"
