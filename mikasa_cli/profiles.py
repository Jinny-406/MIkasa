...
from mikasa_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
from mikasa_constants import clear_named_profile_deleted, mark_named_profile_deleted, named_profile_is_deleted
...
# stay visible; containers persist HOME state here). See mikasa_constants.get_subprocess_home().
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_mikasa_home().resolve():
...
# Allow-list for ``export_profile("default")``: when MIKASA_HOME equals the cwd
...
# when introduced in ``mikasa_constants``.
# See #58394.
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the hermes root (NOT the current MIKASA_HOME, which
    may itself be a profile) so ``coder profile list`` sees all profiles."""
    return _get_default_mikasa_home() / "profiles"


def _get_default_mikasa_home() -> Path:
    """Default (pre-profile) MIKASA_HOME: ``~/.hermes``, or MIKASA_HOME itself in
...
def _get_active_profile_path() -> Path:
    return _get_default_mikasa_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its MIKASA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_mikasa_home()
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    with contextlib.suppress(Exception):
        ...
        from mikasa_cli.config import check_config_version, migrate_config
        token = set_mikasa_home_override(str(profile_dir))
        try:
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    with contextlib.suppress(Exception):  # creation must not fail over this; `hermes model` sets it later
        ...
        from mikasa_constants import get_mikasa_home
        from mikasa_cli.config import read_user_config_raw
        source = get_mikasa_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating MIKASA_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from mikasa_cli.gateway import named_profile_served_by_running_multiplexer
...
def list_profiles() -> List[ProfileInfo]:
    ...
    profiles = []
    default_home = _get_default_mikasa_home()
...
def profiles_to_serve(multiplex: bool, profile_allowlist: Optional[List[str]] = None) -> List[Tuple[str, Path]]:
    """``(profile_name, MIKASA_HOME)`` pairs a gateway should serve — the single chokepoint
    ...
    *profile_allowlist* (invalid entries skipped, missing ones warned once)."""
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_mikasa_home())]
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from mikasa_cli.auth import strip_cloned_single_use_oauth_grants
...
def create_profile(
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from mikasa_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches MIKASA_HOME at
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "mikasa_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_mikasa_home() / ".env"
    for entry in _iter_named_profile_dirs():
        env_path = entry / ".env"
        if env_path.exists():
            continue
...
_HERMES_ARGV_MARKERS = ("mikasa_cli.main", "hermes-gateway", "tui_gateway")
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Bound to THIS profile by selector flag, or by MIKASA_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("mikasa_HOME", "")
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``MIKASA_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # MIKASA_HOME is set temporarily so _profile_suffix resolves the service name.
    old_home = os.environ.get("mikasa_HOME")
    try:
        os.environ["mikasa_HOME"] = str(profile_dir)
        from mikasa_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("mikasa_HOME", None)
        if old_home is not None:
            os.environ["mikasa_HOME"] = old_home
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's MIKASA_HOME stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded MIKASA_HOME "
...
def get_active_profile_name() -> str:
    """Profile name inferred from mikasa_HOME: ``"default"`` when unset or ``~/.hermes``, the
    name under ``~/.hermes/profiles/<name>``, ``"custom"`` for any other path."""
    from mikasa_constants import get_mikasa_home
    resolved = get_mikasa_home().resolve()
    if resolved == _get_default_mikasa_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when MIKASA_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_mikasa_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point MIKASA_HOME at its source checkout: use a sibling store,
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    MIKASA_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Hermes file the user may have lying alongside MIKASA_HOME (#58394). * **Universal exclusions
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="mikasa_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"mikasa_{old_name}"
    legacy_old_host = f"hermes.{old_name}"
    new_host = f"mikasa_{new_name}"
    candidates = [
        new_dir / "honcho.json", _get_default_mikasa_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MIKASA_HOME path string. Called early in the CLI entry
    point, before hermes modules are imported, to set MIKASA_HOME.

    When MIKASA_HOME is already set, the configured spelling IS the launch root (it may be a
    ...
    env_home = os.environ.get("mikasa_HOME", "").strip()
    if env_home:
