...
def _has_agent_browser() -> bool:
    ...
    from mikasa_constants import agent_browser_runnable
    ...
    try:
        ...
        # Hermes-managed Node dirs ($MIKASA_HOME/node, prepended to PATH at runtime but usually absent
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("mikasa_LOCAL_STT_COMMAND"):
        return True
    try:
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from mikasa_cli.setup import prompt_checklist
    except Exception:
        return set()
