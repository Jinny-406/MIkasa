...
def _run_fresh_recovery_process(
    ...
    command = [sys.executable, "-m", "mikasa_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
    env["mikasa_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_HERMES_GATEWAY", "mikasa_GATEWAY", "mikasa_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _recover_gateway_restart_after_abort(
    ...
    """
    from mikasa_cli.update_cmd import _gateway_recovery_partition
