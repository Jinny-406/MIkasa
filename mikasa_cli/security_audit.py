"""On-demand supply-chain audit for Mikasa Agent installs.
...
def _discover_plugins(MIKASA_HOME: Path) -> list[Component]:
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, MIKASA_HOME: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = MIKASA_HOME or Path(get_mikasa_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    components = _discover_components(MIKASA_HOME=home, **skips)
    ...
    try:
        findings = run_audit(MIKASA_HOME=home, components=components)
