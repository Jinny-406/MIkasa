...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("mikasa_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _pip_install(args: List[str], *, timeout: int = 300, capture_output: bool = True):
    ...
    # Managed uv first: $MIKASA_HOME/bin is never on PATH, so a bare which() misses the uv Hermes
...
def install_cua_driver(upgrade: bool = False, require_confirmed_update: bool = False,
    ...
    override = os.environ.get("mikasa_CUA_DRIVER_CMD", "").strip()
