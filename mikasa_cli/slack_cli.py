...
def _build_full_manifest(
    ...
    UI after pasting.
    """
    from mikasa_cli.commands_platforms import slack_app_manifest
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Mikasa agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    description = getattr(args, "description", None) or "Your Mikasa agent on Slack"
    ...
    if isinstance(write_target, bool) and write_target:  # bare --write → default location
        ...
        target = Path(get_mikasa_home()) / "slack-manifest.json"
    else:
