...
def _blank_slate_done(config: dict, MIKASA_HOME, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from mikasa_cli.setup import _info, _print_setup_summary, print_success
    print()
    ...
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         hermes skills opt-in --sync",
          "  Add MCP servers:     mikasa mcp add", *extra, "  Tune agent settings: mikasa setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from mikasa_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    from mikasa_cli.setup import _info
    try:
        from mikasa_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``mikasa setup --portal`` / ``mikasa portal``)."""
    from mikasa_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ⚕ Mikasa Setup — Nous Portal (one-shot)             │")
...
def _run_first_time_quick_setup(config: dict, MIKASA_HOME, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'mikasa setup gateway'",
    ], 0)
    if gateway_choice == 0:
    ...
    _info(None, "  Configure all settings:    mikasa setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  mikasa setup gateway")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from mikasa_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``mikasa setup agent``."""
...
def _run_blank_slate_setup(config: dict, MIKASA_HOME, is_existing: bool):
    ...
    if path != 0:
        _blank_slate_walkthrough(config, MIKASA_HOME)
        return
    save_config(config)
    ...
    _blank_slate_done(config, MIKASA_HOME, "  Enable tools:        hermes tools", "  Enable plugins:      hermes plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, MIKASA_HOME):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, MIKASA_HOME, "  Enable more tools:   hermes tools")


def _run_quick_setup(config: dict, MIKASA_HOME):
    ...
    from mikasa_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'mikasa setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Hermes to messaging apps to chat from anywhere.",
              "You can configure these later with 'mikasa setup gateway'.")
