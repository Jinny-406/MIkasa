...
import subprocess
import sys
from mikasa_cli.doctor_platform import _system_package_install_cmd
from mikasa_cli.doctor_report import Finding, _fail_and_issue, check_bool, check_info, check_ok, check_warn, doctor_check
from mikasa_cli.vercel_auth import describe_vercel_auth
from mikasa_constants import agent_browser_runnable, is_termux as _is_termux
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    return (item.get("name") == "kanban" and not os.environ.get("mikasa_KANBAN_TASK")
            and bool(tools) and all(str(tool).startswith("kanban_") for tool in tools))


def _doctor_tool_availability_detail(toolset: str) -> str:
    """Optional explanatory suffix for toolsets whose doctor status needs context."""
    if toolset == "kanban" and not os.environ.get("mikasa_KANBAN_TASK"):
...
def _apply_doctor_tool_availability_overrides(available: list[str], unavailable: list[dict]) -> tuple[list[str], list[dict]]:
    ...
    from mikasa_cli.doctor_state import _honcho_is_configured_for_doctor
...
def _check_terminal_backend(should_fix: bool, f: Finding) -> None:
    ...
    try:
        from mikasa_constants import is_container as _is_container
...
def _check_npm_audit(should_fix: bool, f: Finding) -> None:
    ...
    MIKASA_HOME mirror rather than the (possibly read-only) Docker install tree, hence the shared resolver.
    """
    from mikasa_cli.doctor import PROJECT_ROOT
    ...
    if npm_bin:
        try:
            ...
            # See #38772. The WhatsApp bridge may live under a writable MIKASA_HOME mirror instead of the
...
def _check_tool_availability(should_fix: bool, f: Finding) -> None:
    ...
    if api_disabled or any(status != "ok" for status, _, _ in web_rows):
        f.issues.append("Run 'mikasa setup' to configure missing API keys for full tool access")
