...
from mikasa_cli.subcommands._shared import add_json_flag


def _cu_install(args) -> int:
    from mikasa_cli.tools_config import _cua_driver_contract_status, install_cua_driver
...
def _cu_status(args) -> int:
    import os as _os
    import subprocess
    from mikasa_cli.tools_config import _cua_driver_contract_status
    ...
    override = _os.environ.get("mikasa_CUA_DRIVER_CMD", "").strip()
    if not path:
    ...
    version = ""
    try:
        from mikasa_cli.tools_config import _cua_driver_env
        ...
        pass
    from mikasa_cli.tools_config import _cua_version_summary
    ...
    origin = " [custom binary from mikasa_CUA_DRIVER_CMD]" if override else ""
