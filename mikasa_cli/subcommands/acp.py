...
from mikasa_cli.subcommands._shared import add_accept_hooks_flag


def build_acp_parser(subparsers, *, cmd_acp: Callable) -> None:
    """Attach the ``acp`` subcommand to ``subparsers``."""
    acp_parser = subparsers.add_parser(
        "acp", help="Run Mikasa Agent as an ACP (Agent Client Protocol) server",
        description="Start Mikasa Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
