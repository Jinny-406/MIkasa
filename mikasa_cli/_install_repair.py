...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``mikasa_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``mikasa_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from mikasa_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (mikasa_cli.main's version lives behind imports)."""
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (mikasa_constants is stdlib-only)."""
    # mikasa_constants is stdlib-only, so the canonical layout helpers are safe to use from this
    # corrupted-venv repair path (#76105: never open-code the Scripts/bin split).
    from mikasa_constants import project_venv_dir, venv_bin_dir
...
def _default_hermes_root() -> Path | None:
    """The DEFAULT Hermes root (not ``get_mikasa_home()``, which under ``hermes -p <name>`` is
    ...
    try:
        return Path(get_default_mikasa_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_mikasa_home()``
    would point inside ``profiles\\<name>`` under ``hermes -p``, so the anchor here is
    :func:`mikasa_constants.get_default_mikasa_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    targets: list[Path] = []
    # Runs at every mikasa_cli.main process start, so the healthy path must stay a few stat calls.
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``MIKASA_HOME\\bin`` launcher layout (``hermes update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from mikasa_constants import venv_bin_dir
