...
from mikasa_cli.config import get_env_path, get_mikasa_home, get_project_root
...
_DHH = display_mikasa_home()  # user-facing display path (e.g. ~/.hermes or ~/.hermes/profiles/coder)

# Load environment variables from ~/.hermes/.env so API key checks work
_env_path = get_env_path()
load_mikasa_dotenv(MIKASA_HOME=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from mikasa_cli.colors import Colors, color
from mikasa_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from mikasa_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from mikasa_cli.doctor_tools import _safe_which

from mikasa_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from mikasa_cli.doctor_platform import (
    _check_certificates,
...
from mikasa_cli.doctor_tools import (
    _check_git_and_rg,
...
from mikasa_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from mikasa_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
    ...
    with warn_on_error(""):  # xAI OAuth separately, so an import failure cannot disrupt the rows already printed
        from mikasa_cli.auth import get_xai_oauth_auth_status
...
def _ack_advisory(ack_target: str) -> None:
    ...
    from mikasa_cli.security_advisories import ADVISORIES, ack_advisory
...
def run_doctor(args):
    ...
    os.environ.setdefault("mikasa_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from mikasa_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('mikasa_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('mikasa_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('mikasa_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('mikasa_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('mikasa_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('mikasa_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('mikasa_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('mikasa_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('mikasa_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('mikasa_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('mikasa_cli.vercel_auth', 'describe_vercel_auth'),
    ...
    'managed_scope_check': ('mikasa_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('mikasa_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('mikasa_cli.doctor_config', 'report_deprecated_config_and_env'),
}
