...
class CLITerminalMixin:
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        if getattr(self, "_terminal_io_broken", False):
            return
        try:
            from mikasa_cli.curses_ui import flush_stdin
            flush_stdin()
        except Exception:
            pass
        self._force_full_redraw()
    ...
    def _recover_after_resize(self, app, original_on_resize) -> None:
        ...
        handled by ``_mikasa_call_output_screen_diff`` (#83874). Suppression is cleared by
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        from mikasa_cli.clipboard import save_clipboard_image
        self._image_counter += 1
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        img_path = get_mikasa_home() / "images" / f"clip_{ts}_{self._image_counter}.png"
