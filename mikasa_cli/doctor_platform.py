...
from mikasa_cli.config import is_nix_install_method, recommended_update_command_for_method
...
from mikasa_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from mikasa_cli.doctor import PROJECT_ROOT
    from mikasa_cli.config import detect_install_method
...
def _hermes_database_paths(MIKASA_HOME: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Hermes-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from mikasa_cli.backup import _QUICK_STATE_FILES
    entries = [(name, MIKASA_HOME / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((MIKASA_HOME / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(MIKASA_HOME)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    """
    from mikasa_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_journal_modes(MIKASA_HOME: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from mikasa_cli.doctor import mikasa_HOME
    from mikasa_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    try:
        databases = _hermes_database_paths(MIKASA_HOME if MIKASA_HOME is not None else MIKASA_HOME)
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    try:
        from mikasa_cli import __version__ as init_version
    except Exception:
        return
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from mikasa_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from mikasa_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from mikasa_cli import macos_tcc_anchor as tcc
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from mikasa_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from mikasa_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
