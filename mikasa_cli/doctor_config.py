...
import os
import shutil
from mikasa_cli.doctor_report import (
...
def _has_provider_env_config(content: str) -> bool:
    ...
    from mikasa_cli.doctor import _PROVIDER_ENV_HINTS
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ("mikasa_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("mikasa_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(raw_config: dict | None, env_map: dict | None) -> list[tuple[str, str]]:
    ...
    from mikasa_cli.relay_plugin_cutover import (LEGACY_RELAY_EXPORT_ENV_VARS, RELAY_PLUGINS_CONFIG_ENV,
...
def managed_scope_check() -> None:
    ...
    if os.environ.get("mikasa_MANAGED_DIR", "").strip():
...
def _check_env_file(should_fix: bool, f: Finding) -> None:
    ...
    from mikasa_cli.doctor import mikasa_HOME, PROJECT_ROOT, _DHH
    managed_scope_check()
    ...
    if env_path.exists():
        ...
        if not check_bool(_has_provider_env_config(content), "API key or custom endpoint configured", f"No API key found in {_DHH}/.env"):
            f.issues.append("Run 'mikasa setup' to configure API keys")
        ...
        if should_fix:
            ...
            check_info("Run 'mikasa setup' to configure API keys")
            f.fixed += 1
        else:
            check_info("Run 'mikasa setup' to create one")
            f.issues.append("Run 'mikasa setup' to create .env")
...
def _known_provider_ids(cfg: dict) -> tuple[set, list, object, object, object]:
    ...
    with warn_on_error(""):
        from mikasa_cli.auth import PROVIDER_REGISTRY, resolve_provider as resolve_auth
        known = set(PROVIDER_REGISTRY.keys()) | {"openrouter", "custom", "auto", "moa"}
    with warn_on_error(""):
        from mikasa_cli.config import get_compatible_custom_providers
        from mikasa_cli.providers import custom_provider_aliases as aliases, normalize_provider as normalize, resolve_provider_full as resolve_full
        with warn_on_error(""):
            custom_providers = get_compatible_custom_providers(cfg)
    user_providers = cfg.get("providers")
    if isinstance(user_providers, dict):
        from mikasa_cli.config import is_provider_enabled
...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    if runtime_provider and runtime_provider not in ("auto", "custom"):
        from mikasa_cli.doctor import _DHH
        with warn_on_error(""):
            if not _provider_has_credentials(runtime_provider):
                _fail_and_issue(f"model.provider '{runtime_provider}' is set but no API key is configured",
                                "(check ~/.hermes/.env or run 'mikasa setup')",
                                f"No credentials found for provider '{runtime_provider}'. Run 'mikasa setup' or set the provider's "
...
def _check_config_file(should_fix: bool, f: Finding) -> None:
    ...
    from mikasa_cli.doctor import mikasa_HOME, PROJECT_ROOT, _DHH
    config_path = MIKASA_HOME / 'config.yaml'
    if config_path.exists():
        ...
        if example_config.exists():
            ...
            from mikasa_cli.config import DEFAULT_CONFIG, save_config
...
def _drift_config_version(f: Finding, should_fix: bool, config_path) -> None:
    ...
    if check_bool(current_ver >= latest_ver, f"Config version up to date (v{current_ver})", outdated):
        return
    if not should_fix:
        f.issues.append("Run 'mikasa doctor --fix' or 'mikasa setup' to migrate config")
        return
    try:
        ...
        f.issues.append("Run 'mikasa setup' to migrate config")
...
def _drift_stale_root_keys(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from mikasa_cli.config import atomic_config_write, read_user_config_raw
...
def _drift_max_iterations_ghost(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from mikasa_cli.config import load_env, read_user_config_raw, remove_env_value
    raw_config = read_user_config_raw(config_path)
    ...
    if cfg_max_turns is None or env_ghost is None or str(cfg_max_turns).strip() == str(env_ghost).strip():
        return
    check_warn(f"mikasa_MAX_ITERATIONS={env_ghost} in .env shadows agent.max_turns={cfg_max_turns} in config.yaml",
               "(stale ghost from an earlier `mikasa setup` run)")
    if not should_fix:
        f.issues.append("Stale HERMES_MAX_ITERATIONS in .env shadows config.yaml — run 'hermes doctor --fix'")
    elif remove_env_value("mikasa_MAX_ITERATIONS"):
...
def _drift_deprecations(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from mikasa_cli.config import load_env, read_user_config_raw
...
def _check_config_drift(should_fix: bool, f: Finding) -> None:
    ...
    """
    from mikasa_cli.doctor import mikasa_HOME
    config_path = MIKASA_HOME / 'config.yaml'
...
def _check_xai_retirement(should_fix: bool, f: Finding) -> None:
    from mikasa_cli.config import load_config
    from mikasa_cli.xai_retirement import MIGRATION_GUIDE_URL, find_retired_xai_refs, format_issue
...
def _check_plugin_compat(should_fix: bool, f: Finding) -> None:
    from mikasa_cli.plugin_compat import ALLOW_KEY, COMPAT_REMOVAL, compat_report, removal_in_effect
