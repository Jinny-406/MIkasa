...
def resolve_pack_plugins(pack: PluginPack) -> List[ResolvedPackPlugin]:
    ...
    for entry in pack.plugins:
        if entry.install_identifier is not None:
            ...
            continue
        try:
            from mikasa_cli.plugin_index import load_index, resolve_name
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-hermes-pack") -> tuple[str, List[str]]:
    ...
    import yaml
    from mikasa_cli.plugins_cmd import _get_enabled_set, _plugins_dir, _read_install_metadata
...
def _load_and_review(console, source: str):
    ...
    from mikasa_cli.plugins_cmd import _fail
    try:
...
def cmd_pack_show(source: str) -> None:
    ...
    from mikasa_cli.plugins_cmd import _console
    console = _console()
...
def cmd_pack_install(source: str, *, force: bool = False) -> None:
    ...
    from mikasa_cli.plugins_cmd import _ask_yes, _console, _fail, _is_tty
    console = _console()
    pack, resolved = _load_and_review(console, source)
...
def pack_command(args) -> None:
    ...
    if handler is None:
        from mikasa_cli.plugins_cmd import _console, _fail
