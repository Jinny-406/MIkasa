"""Single source of truth for provider identity in Mikasa Agent."""
...
MIKASA_OVERLAYS: Dict[str, HermesOverlay] = {
    ...
    "qwen-oauth": HermesOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="mikasa_QWEN_BASE_URL"),
...
HERMES_OVERLAYS = MIKASA_OVERLAYS  # backward compat alias
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    overlay = MIKASA_OVERLAYS.get(canonical)
    ...
    # MIKASA_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    ...
    try:
        from mikasa_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
