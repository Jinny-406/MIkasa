"""Unified tool configuration for Mikasa Agent."""
...
from mikasa_cli.cli_output import print_info as _print_info
from mikasa_cli.colors import Colors, color
from mikasa_cli.config import cfg_get, load_config, save_config, get_env_value
from mikasa_cli.nous_subscription import (
    NousSubscriptionFeatures, apply_nous_managed_defaults, get_nous_subscription_features)
from mikasa_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from mikasa_cli.toolset_scope import (
...
from mikasa_cli.tools_config_cua import (  # noqa: F401
...
from mikasa_cli.tools_config_post_setup import (  # noqa: F401
...
from mikasa_cli.tools_config_providers import (  # noqa: F401
...
from mikasa_cli.tools_config_mcp import (  # noqa: F401
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from mikasa_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from mikasa_cli.plugins import get_plugin_toolset_keys_nowait
...
_LANGFUSE_PUBLIC = ("mikasa_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("mikasa_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("mikasa_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from mikasa_cli.plugins import get_portable_mcp_server_names_nowait
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from mikasa_cli.setup import setup_telemetry
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `mikasa tools` / `mikasa setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    print(color(f"  Tool configuration saved to {display_mikasa_home()}/config.yaml", Colors.DIM))
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('mikasa_cli.nous_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    ...
    'format_nous_portal_entitlement_message': ('mikasa_cli.nous_account', 'format_nous_portal_entitlement_message'),
