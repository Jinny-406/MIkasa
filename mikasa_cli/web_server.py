"""Mikasa Agent — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m mikasa_cli.main web [--port 8080]``.
"""
...
from mikasa_cli.install_identity import get_install_id as _shared_get_install_id
from mikasa_cli.pty_session import run_reaper
...
from mikasa_cli import __version__
from mikasa_cli.config import load_config
...
WEB_DIST = Path(os.environ["mikasa_WEB_DIST"]) if "mikasa_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)


from mikasa_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from mikasa_cli.profiles import profiles_to_serve
            ...
            if len(profile_homes) > 1:
                ...
                from mikasa_cli.profiles import _check_gateway_running

                start_kwargs["profile_gate"] = lambda _name, home: not _check_gateway_running(Path(home))
                from mikasa_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # import mikasa_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    if os.getenv("mikasa_DESKTOP") == "1":
        ...
        try:
            from mikasa_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from mikasa_cli.config import load_config
            from mikasa_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    try:
        ...
        try:
            from mikasa_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="Mikasa Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from mikasa_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
def _resolve_session_token() -> str:
    return os.environ.get("mikasa_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
from mikasa_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from mikasa_cli.dashboard_auth.prefix import resolve_public_url
...
def _desktop_loopback_auth_exempt(
    ...
    return (
        ...
        and os.environ.get("mikasa_DESKTOP") == "1"
        and bool(os.environ.get("mikasa_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from mikasa_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from mikasa_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from mikasa_cli.dashboard_auth.token_auth import token_auth_middleware
...
from mikasa_cli import web_server_gateway as _gateway_mod  # noqa: E402
from mikasa_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from mikasa_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from mikasa_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from mikasa_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Hermes home (not the profile MIKASA_HOME) so every
...
from mikasa_cli.web_routers import (  # noqa: E402
...
from mikasa_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    try:
        from mikasa_cli.config import load_config as _load_cfg
        from mikasa_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
...
def _configure_auth_gate(
    ...
    if app.state.auth_required:
        ...
        from mikasa_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from mikasa_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from mikasa_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from mikasa_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from mikasa_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    def _register_identity() -> None:
        from mikasa_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    ready_token = "mikasa_BACKEND_READY" if headless else "mikasa_DASHBOARD_READY"
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from mikasa_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    from mikasa_cli.resource_limits import apply_nofile_soft_limit
    ...
    try:
        from mikasa_cli.nous_auth_keepalive import start_nous_auth_keepalive
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('mikasa_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('mikasa_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('mikasa_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('mikasa_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('mikasa_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('mikasa_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('mikasa_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('mikasa_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('mikasa_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('mikasa_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('mikasa_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('mikasa_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('mikasa_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('mikasa_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('mikasa_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('mikasa_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('mikasa_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('mikasa_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('mikasa_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('mikasa_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('mikasa_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('mikasa_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('mikasa_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('mikasa_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('mikasa_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('mikasa_cli.web_models', 'HookCreate'),
    'HookDelete': ('mikasa_cli.web_models', 'HookDelete'),
    'ImportRequest': ('mikasa_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('mikasa_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('mikasa_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('mikasa_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('mikasa_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('mikasa_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('mikasa_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('mikasa_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('mikasa_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('mikasa_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('mikasa_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('mikasa_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('mikasa_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('mikasa_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('mikasa_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('mikasa_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('mikasa_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('mikasa_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('mikasa_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('mikasa_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('mikasa_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('mikasa_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('mikasa_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('mikasa_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('mikasa_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('mikasa_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('mikasa_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('mikasa_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('mikasa_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('mikasa_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('mikasa_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('mikasa_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('mikasa_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('mikasa_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('mikasa_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('mikasa_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('mikasa_cli.web_models', 'SessionImport'),
    'SessionPrune': ('mikasa_cli.web_models', 'SessionPrune'),
    'SessionRename': ('mikasa_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('mikasa_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('mikasa_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('mikasa_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('mikasa_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('mikasa_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('mikasa_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('mikasa_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('mikasa_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('mikasa_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('mikasa_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('mikasa_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('mikasa_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('mikasa_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('mikasa_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('mikasa_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('mikasa_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('mikasa_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('mikasa_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('mikasa_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('mikasa_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('mikasa_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('mikasa_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('mikasa_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('mikasa_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('mikasa_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('mikasa_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('mikasa_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('mikasa_cli.web_routers.mcp', 'auth_mcp_server'),
    'build_cron_model_impact': ('mikasa_cli.config', 'build_cron_model_impact'),
    'bulk_delete_sessions_endpoint': ('mikasa_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('mikasa_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('mikasa_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('mikasa_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('mikasa_cli.config', 'cfg_get'),
    'check_config_version': ('mikasa_cli.config', 'check_config_version'),
    'check_hermes_update': ('mikasa_cli.web_routers.actions', 'check_hermes_update'),
    'clear_model_endpoint_credentials': ('mikasa_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('mikasa_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('mikasa_cli.config', 'coerce_provider_id'),
    'console_ws': ('mikasa_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('mikasa_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('mikasa_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('mikasa_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('mikasa_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('mikasa_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('mikasa_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('mikasa_cli.web_routers.ops', 'create_webhook'),
    'cron_fire_webhook': ('mikasa_cli.web_routers.cron', 'cron_fire_webhook'),
    'custom_endpoint_key_env': ('mikasa_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('mikasa_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('mikasa_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('mikasa_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('mikasa_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('mikasa_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('mikasa_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('mikasa_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('mikasa_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('mikasa_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('mikasa_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('mikasa_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('mikasa_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('mikasa_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('mikasa_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('mikasa_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('mikasa_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('mikasa_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('mikasa_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('mikasa_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('mikasa_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('mikasa_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('mikasa_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('mikasa_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('mikasa_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('mikasa_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('mikasa_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('mikasa_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('mikasa_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('mikasa_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('mikasa_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('mikasa_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('mikasa_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('mikasa_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('mikasa_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('mikasa_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('mikasa_cli.config', 'get_config_path'),
    'get_config_raw': ('mikasa_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('mikasa_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('mikasa_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('mikasa_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('mikasa_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('mikasa_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('mikasa_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('mikasa_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('mikasa_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('mikasa_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('mikasa_cli.config', 'get_env_path'),
    'get_env_vars': ('mikasa_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('mikasa_cli.web_routers.status', 'get_health'),
    'get_mikasa_home': ('mikasa_cli.config', 'get_mikasa_home'),
    'get_learning_graph': ('mikasa_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('mikasa_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('mikasa_cli.web_routers.status', 'get_logs'),
    'get_media': ('mikasa_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('mikasa_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('mikasa_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('mikasa_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('mikasa_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('mikasa_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('mikasa_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('mikasa_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('mikasa_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('mikasa_cli.web_routers.status', 'get_portal_status'),
    'get_process_mikasa_home': ('mikasa_cli.config', 'get_process_mikasa_home'),
    'get_profile_setup_command': ('mikasa_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('mikasa_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('mikasa_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('mikasa_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('mikasa_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('mikasa_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('mikasa_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('mikasa_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('mikasa_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('mikasa_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('mikasa_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('mikasa_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('mikasa_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('mikasa_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('mikasa_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('mikasa_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('mikasa_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('mikasa_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('mikasa_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('mikasa_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('mikasa_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('mikasa_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('mikasa_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('mikasa_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('mikasa_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('mikasa_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('mikasa_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('mikasa_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('mikasa_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('mikasa_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('mikasa_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('mikasa_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('mikasa_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('mikasa_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('mikasa_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('mikasa_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('mikasa_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('mikasa_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('mikasa_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('mikasa_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('mikasa_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('mikasa_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('mikasa_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('mikasa_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('mikasa_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('mikasa_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('mikasa_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('mikasa_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('mikasa_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('mikasa_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('mikasa_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('mikasa_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('mikasa_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('mikasa_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('mikasa_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('mikasa_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('mikasa_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('mikasa_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('mikasa_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('mikasa_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('mikasa_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('mikasa_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('mikasa_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('mikasa_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('mikasa_cli.config', 'load_env'),
    'mcp_oauth_callback': ('mikasa_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('mikasa_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('mikasa_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('mikasa_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('mikasa_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('mikasa_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('mikasa_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('mikasa_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('mikasa_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('mikasa_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('mikasa_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('mikasa_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('mikasa_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('mikasa_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('mikasa_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('mikasa_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('mikasa_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('mikasa_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('mikasa_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('mikasa_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('mikasa_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('mikasa_cli.config', 'remove_env_value'),
    'remove_env_var': ('mikasa_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('mikasa_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('mikasa_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('mikasa_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('mikasa_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('mikasa_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('mikasa_cli.web_routers.ops', 'reset_memory'),
    'resolve_cron_model_drift_defaults': ('mikasa_cli.config', 'resolve_cron_model_drift_defaults'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('mikasa_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('mikasa_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('mikasa_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('mikasa_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('mikasa_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('mikasa_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('mikasa_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('mikasa_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('mikasa_cli.doctor', 'run_doctor'),
    'run_dump': ('mikasa_cli.dump', 'run_dump'),
    'run_import': ('mikasa_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('mikasa_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('mikasa_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('mikasa_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('mikasa_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('mikasa_cli.config', 'save_config'),
    'save_env_value': ('mikasa_cli.config', 'save_env_value'),
    'save_toolset_env': ('mikasa_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('mikasa_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('mikasa_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('mikasa_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('mikasa_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('mikasa_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('mikasa_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('mikasa_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('mikasa_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('mikasa_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('mikasa_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('mikasa_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('mikasa_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('mikasa_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('mikasa_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('mikasa_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('mikasa_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('mikasa_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('mikasa_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('mikasa_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('mikasa_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('mikasa_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('mikasa_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('mikasa_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('mikasa_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('mikasa_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('mikasa_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('mikasa_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('mikasa_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('mikasa_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('mikasa_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('mikasa_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('mikasa_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('mikasa_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('mikasa_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('mikasa_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('mikasa_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('mikasa_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('mikasa_cli.web_routers.cron', 'update_cron_job'),
    'update_hermes': ('mikasa_cli.web_routers.actions', 'update_hermes'),
    'update_learning_node': ('mikasa_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('mikasa_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('mikasa_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('mikasa_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('mikasa_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('mikasa_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('mikasa_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('mikasa_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('mikasa_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('mikasa_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('mikasa_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('mikasa_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('mikasa_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('mikasa_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('mikasa_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('mikasa_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('mikasa_cli.config', 'write_platform_config_field'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from mikasa_cli.plugin_compat import warn_once
