...
# env-var passthrough (MIKASA_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a MIKASA_HOME pointing there burned into
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    MIKASA_HOME passes through only IF SET, read from os.environ (not get_mikasa_home()): when
    unset the codex subprocess must inherit its launcher's runtime MIKASA_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched MIKASA_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched hermes finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # MIKASA_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_mikasa_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever MIKASA_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's MIKASA_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("mikasa_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    MIKASA_HOME = os.environ.get("mikasa_HOME") or ""
    if MIKASA_HOME and not _looks_like_test_tempdir(MIKASA_HOME):
        env["mikasa_HOME"] = MIKASA_HOME
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["mikasa_QUIET"] = "1"
    env["mikasa_REDACT_SECRETS"] = env.get("mikasa_REDACT_SECRETS", "true")
    return {
