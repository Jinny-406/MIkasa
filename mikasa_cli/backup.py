...
from mikasa_constants import (
    _get_platform_default_mikasa_home, get_default_mikasa_root, get_mikasa_home, display_mikasa_home,
)
from utils import (
...
# dependency/cache entries matter: one plugin venv or pip/uv cache under MIKASA_HOME walked
...
# on demand and routinely tens to hundreds of GB. Matched ONLY at the root of MIKASA_HOME and at
...
# Reserved archive subtree for memory-provider state OUTSIDE MIKASA_HOME (e.g. ~/.honcho, via
...
@contextmanager
def _backup_operation_lock(MIKASA_HOME: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = MIKASA_HOME / ".backup.lock"
...
def _iter_backup_files(hermes_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # copy data from outside MIKASA_HOME; never archive the output zip into itself.
...
def run_backup(args) -> None:
    ...
    hermes_root = get_default_mikasa_root()
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    print(f"Scanning {display_mikasa_home()} ...")
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_mikasa_home()}.")
    if skipped_external:
...
def _import_members(
    ...
    for member in members:
        ...
        # NOT under MIKASA_HOME; provider configs commonly hold credentials, so tighten to 0600.
...
def run_import(args) -> None:
    ...
    # ``get_default_mikasa_root()`` would silently retarget a profile restore at the live root.
    hermes_root = get_mikasa_home()
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if restored_external:
            print(f"\n  Restored {restored_external} memory-provider file(s) to "
                  f"their original location(s) outside {display_mikasa_home()}.")
        if errors:
            _print_capped(f"\n  Warnings ({len(errors)} files skipped):", errors, "  ")
        if db_shrunk:
...
def _revive_gateway_after_import(hermes_root: Path) -> None:
    ...
    """
    native_default = _get_platform_default_mikasa_home()
    if hermes_root != native_default and any(
        ...
        return
    try:
        from mikasa_cli.gateway import ensure_gateway_service, _is_service_running
...
# Critical state files (relative to MIKASA_HOME) for quick snapshots; everything else is
...
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside MIKASA_HOME.
...
def _quick_snapshot_root(MIKASA_HOME: Optional[Path] = None) -> Path:
    home = MIKASA_HOME or get_mikasa_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None, MIKASA_HOME: Optional[Path] = None, keep: Optional[int] = None,
...
def list_quick_snapshots(limit: int = 20, MIKASA_HOME: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(MIKASA_HOME)):
...
def restore_quick_snapshot(snapshot_id: str, MIKASA_HOME: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, MIKASA_HOME: Optional[Path] = None) -> Optional[Dict[str, Any]]:
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_mikasa_home()
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", MIKASA_HOME=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, MIKASA_HOME: Optional[Path] = None) -> Optional[Dict[str, Any]]:
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, MIKASA_HOME=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, MIKASA_HOME=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, MIKASA_HOME: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(MIKASA_HOME)), keep, shutil.rmtree, "snapshot")
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_mikasa_home()}/state-snapshots/\n"
...
def _create_prefixed_full_backup(
    MIKASA_HOME: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<MIKASA_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    Returns the path, or ``None`` if nothing to back up or the write failed. Never raises."""
    hermes_root = MIKASA_HOME or get_default_mikasa_root()
    if not hermes_root.is_dir():
        return None
...
def create_pre_update_backup(
    MIKASA_HOME: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    ...
    return _create_prefixed_full_backup(MIKASA_HOME, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    MIKASA_HOME: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    ...
    return _create_prefixed_full_backup(
        MIKASA_HOME, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
