...
def _cmd_login(args) -> int:
    ...
    Reuses the exact wiring behind ``mikasa setup --portal`` so the commands stay in lockstep.
    """
    from mikasa_cli.setup import _run_portal_one_shot
...
# `mikasa setup --portal`). `status` kept as a back-compat alias for `info`.
_SUBCOMMANDS = {
    None: _cmd_login,
    "": _cmd_login,
    "login": _cmd_login,
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        ...
        description=(
            "Run `mikasa portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `mikasa auth add "
            "nous --type oauth`, identical to `mikasa setup --portal`). "
