...
from mikasa_cli.sqlite_runtime import (
...
def managed_uv_path() -> Path:
    """Path of Hermes' own uv binary (``$MIKASA_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_mikasa_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def _uv_self_update_stamp() -> Path:
    from mikasa_constants import get_mikasa_home
    return get_mikasa_home() / "cache" / ".uv_self_update_stamp"
...
def _reload_mikasa_constants():
    """Re-execute ``mikasa_constants`` from disk (the imported one may predate venv_python_path)."""
    import mikasa_constants
    return importlib.reload(mikasa_constants)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("mikasa_cli.main")
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            f"      cd {root}",
            "      <system Python> -m mikasa_cli.main update",
...
def _install_uv(target: Path) -> None:
    ...
    into ``$MIKASA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
