...
from mikasa_cli.tools_config_cua import (
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Hermes-managed ``$MIKASA_HOME/bin`` copy does."""
...
def _post_setup_spotify() -> None:
    ...
    try:
        from mikasa_cli.auth import login_spotify_command
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        from mikasa_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from mikasa_cli.setup import prompt_choice, prompt as _setup_prompt
        from mikasa_cli.setup_tts import _run_xai_oauth_login_from_setup
...
def _agent_browser_installed() -> bool:
    ...
    *and* the Chromium build it drives (or the Lightpanda engine, which needs no Chromium), so "Run
    ...
    from mikasa_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from mikasa_cli.nous_subscription import _has_agent_browser
