...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from mikasa_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"hermes {candidate}", f"mikasa_cli.main {candidate}", f"mikasa_cli/main.py {candidate}")
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    ...
    log_path = get_mikasa_home() / "logs" / "dashboard-restart.log"
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # mikasa_cli.config.get_mikasa_home to simulate setup failure.
        from mikasa_cli.config import get_mikasa_home as _get_mikasa_home
        logs_dir = _get_mikasa_home() / "logs"
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from mikasa_cli.web_server import should_require_dashboard_auth
    ...
    if choice == "2":
        ...
        print(
            ...
            "mikasa_DASHBOARD_OAUTH_CLIENT_ID into ~/.hermes/.env for you.\n"
    ...
    try:
        ...
        from mikasa_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from mikasa_cli.windows_ssh_runtime import read_token
    ...
    # desktop-ssh, independent of MIKASA_HOME and the active profile. Anchor
    # validation there, NOT get_mikasa_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_mikasa_home() and would reject every token.
    # See #69551.
...
def _route_named_profile_dashboard(
    ...
    if (
        ...
        or os.environ.get("mikasa_DESKTOP") == "1"
    ):
        return
    ...
    reexec_argv = [
        sys.executable, "-m", "mikasa_cli.main",
        "-p", "default",
    ...
    # MIKASA_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping MIKASA_HOME: in the Docker layout the root is /opt/data, and an
    # unset MIKASA_HOME would fall back to $HOME/.hermes = /opt/data/.hermes — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_mikasa_root() strips a trailing profiles/<name> for both layouts.
    try:
        from mikasa_constants import get_default_mikasa_root
        env["mikasa_HOME"] = str(get_default_mikasa_root())
    except Exception:
        env.pop("mikasa_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    from mikasa_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["mikasa_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "mikasa_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["mikasa_WEB_DIST"])
            if "mikasa_WEB_DIST" in os.environ
            else PROJECT_ROOT / "mikasa_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            _recoverable = "mikasa_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # path).
        _dist_root = Path(os.environ["mikasa_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
        ...
        os.environ["mikasa_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from mikasa_WEB_DIST: {_dist_root}")
