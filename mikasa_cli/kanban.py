...
from mikasa_cli import kanban_swarm as ks
from mikasa_cli.kanban_output import (
...
from mikasa_cli.kanban_boards import _dispatch_boards
from mikasa_cli.kanban_ops import (
    _cmd_daemon, _kanban_config, _cmd_dispatch, _cmd_gc, _cmd_repair, _cmd_tail, _cmd_watch,
)
from mikasa_cli.kanban_parser import build_parser  # noqa: F401  (re-exported: mikasa_cli.main, run_slash)
...
def _check_dispatcher_presence(MIKASA_HOME: Optional[Path] = None) -> tuple[bool, str]:
    """``(running, message)`` for the "will anything dispatch this?" warning: True when a gateway is
    alive for this MIKASA_HOME with ``kanban.dispatch_in_gateway`` on, else False + human guidance.
    Fails OPEN (probe/config errors -> ``(True, "")``) — a missed warning beats crying wolf.
    ``MIKASA_HOME`` scopes the probe to a profile dir (dashboard backend); CLI callers pass None.

    The dashboard plugin API passes it because the dashboard backend process can be running under a
    different MIKASA_HOME than the profile the request targets, which otherwise produced a "no gateway is
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(profile_dir=MIKASA_HOME, use_cache=False)
    except Exception:
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    with board_scope:
        ...
        # "no such table: tasks" on first use from a fresh MIKASA_HOME.
        try:
            kb.init_db()
...
def _profile_author() -> str:
    ...
    for env in ("mikasa_PROFILE_NAME", "mikasa_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("mikasa_DELEGATED_CHILD_CONTEXT"))
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    from mikasa_cli import kanban_diagnostics as kd
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    raw = os.environ.get("mikasa_KANBAN_RUN_ID")
    if os.environ.get("mikasa_KANBAN_TASK") != task_id or not raw:
        return None
    try:
        return int(raw)
    except ValueError:
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from mikasa_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from mikasa_cli import kanban_decompose as decomp
