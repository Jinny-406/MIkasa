...
- **gateway profiles**, relaunched through the existing per-profile
  ``mikasa_cli.main -p <profile> gateway restart`` command; and
...
_RECOVERY_ENV = "mikasa_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_HERMES_GATEWAY", "mikasa_GATEWAY", "mikasa_GATEWAY_MODE")
...
def _run_profile_restart(profile: str, *, run: Callable[..., Any]) -> bool:
    ...
    argv = [sys.executable, "-m", "mikasa_cli.main", "-p", profile, "gateway", "restart"]
