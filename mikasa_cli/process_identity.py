...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    if project_root is None:
        try:
            from mikasa_constants import PROJECT_ROOT as _root
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_mikasa_root()) / LEDGER_FILENAME
    except Exception:
        from mikasa_cli.config import get_mikasa_home

        return Path(get_mikasa_home()) / LEDGER_FILENAME
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    ...
    try:
        spawner_pid = int(os.environ.get("mikasa_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
    marker = os.environ.get("mikasa_PARENT_START_MARKER", "")
