...
logger = logging.getLogger("mikasa_cli.main")
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from mikasa_cli.main import _resolve_last_session
    target = (
...
def _tui_selected_workspace_keys(tui_dir: Path, ws_root: Path) -> set:
    ...
    from mikasa_cli.main import _is_termux_startup_environment
    try:
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    force = (os.environ.get("mikasa_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    from mikasa_cli.main import PROJECT_ROOT
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("mikasa_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "mikasa_HOME": MIKASA_HOME},
    ...
    extras += [Path(MIKASA_HOME) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("mikasa_QUIET"):
...
def _tui_node_bin(bin: str) -> str:
    ...
    ``find_node_executable()`` sees the managed ``$MIKASA_HOME/node`` tree a bare which() misses."""
    if bin == "node":
        env_node = os.environ.get("mikasa_NODE")
        if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
            return env_node
    from mikasa_constants import find_node_executable
    path = find_node_executable(bin)
    if not path and bin == "node":
        with contextlib.suppress(Exception):
            from mikasa_cli.dep_ensure import ensure_dependency
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        from mikasa_constants import with_hermes_node_path
    ...
    if result.returncode != 0:
        ...
        from mikasa_cli.npm_engine import maybe_repair_npm_engine
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    ext_dir = os.environ.get("mikasa_TUI_DIR")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from mikasa_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from mikasa_cli.main import PROJECT_ROOT
    src_root = str(env.get("mikasa_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["mikasa_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("mikasa_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["mikasa_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("mikasa_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["mikasa_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["mikasa_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    env["mikasa_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        wt_info = _setup_tui_worktree()
        env["mikasa_CWD"] = wt_info["path"]
    ...
    for key, value in (
        ("mikasa_MODEL", model), ("mikasa_INFERENCE_MODEL", model),
        ("mikasa_TUI_PROVIDER", provider), ("mikasa_INFERENCE_PROVIDER", provider),
        ("mikasa_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("mikasa_TUI_SKILLS", skills_value),
        ("mikasa_TUI_QUERY", query), ("mikasa_TUI_IMAGE", image),
        ("mikasa_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("mikasa_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("mikasa_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("mikasa_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("mikasa_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    env.pop("mikasa_TUI_RESUME", None)
    if resume_session_id:
        env["mikasa_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    ...
    """
    if os.environ.get("mikasa_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from mikasa_cli.kanban_db import get_current_board
        os.environ["mikasa_KANBAN_BOARD"] = get_current_board()
...
def _resolve_use_tui(args) -> bool:
    ...
    try:
        ...
        return False
    if os.environ.get("mikasa_TUI") == "1":
        return True
    try:
