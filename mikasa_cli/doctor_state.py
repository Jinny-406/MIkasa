"""mikasa_HOME state checks for hermes doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
from mikasa_cli.sizefmt import format_bytes as _human_bytes
from mikasa_state_common import FTS_STORAGE_VERSION
...
def _doctor_memory_config(MIKASA_HOME: Path | None = None) -> dict:
    ...
    try:
        ...
        config_path = (MIKASA_HOME if MIKASA_HOME is not None else MIKASA_HOME) / "config.yaml"
...
def _memory_store_flags(MIKASA_HOME: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(MIKASA_HOME)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """mikasa_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from mikasa_cli.doctor import mikasa_HOME, _DHH
    MIKASA_HOME = MIKASA_HOME
    ensure_dir(f, should_fix, MIKASA_HOME, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(MIKASA_HOME)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, MIKASA_HOME / subdir_name, f"{_DHH}/{subdir_name}/ exists",
                   f"Created {_DHH}/{subdir_name}/", f"{_DHH}/{subdir_name}/ not found")
    # SOUL.md persona file
    soul_path = MIKASA_HOME / "SOUL.md"
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text("# Mikasa Agent Persona\n\n<!-- Edit this file to customize how Mikasa communicates. -->\n\n"
    ...
    memories_dir = MIKASA_HOME / "memories"
    if not memory_on:
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    from mikasa_state_repair import repair_state_db_schema
...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from mikasa_state_repair import _db_opens_cleanly
        ...
        from mikasa_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from mikasa_state_dbfile import collect_state_db_stats, count_db_holders
...
def _check_state_db(should_fix: bool, f: Finding) -> None:
    ...
    from mikasa_cli.doctor import mikasa_HOME, _DHH
    state_db_path = MIKASA_HOME / "state.db"
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    from mikasa_cli.doctor import mikasa_HOME, _DHH
    hub_dir = MIKASA_HOME / "skills" / ".hub"
...
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    from mikasa_cli.doctor import mikasa_HOME
    name = _doctor_memory_config(MIKASA_HOME).get("provider", "")
    if not name:
...
def _check_profiles(should_fix: bool, f: Finding) -> None:
    from mikasa_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
    import re as _re
