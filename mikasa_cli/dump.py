...
from mikasa_cli.config import get_mikasa_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    if value:
        return value
    try:
        from mikasa_cli.build_info import get_build_sha  # deferred: keeps dump cheap on non-dump paths
...
def _gateway_status() -> str:
    ...
    try:
        from mikasa_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(MIKASA_HOME: Path) -> int:
...
def _cron_summary(MIKASA_HOME: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = MIKASA_HOME / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_mikasa_dotenv(MIKASA_HOME=get_env_path().parent, project_env=get_project_root() / ".env")
    project_root, MIKASA_HOME = get_project_root(), get_mikasa_home()
    try:
        config = load_config()
    except Exception:
        config = {}
    ...
    lines = [
        ...
        f"profile:          {profile}",
        f"mikasa_home:      {display_mikasa_home()}",
        f"model:            {model}",
        f"provider:         {provider}",
        ...
        f"  cron_jobs:          {_cron_summary(MIKASA_HOME)}",
        f"  skills:             {_count_skills(MIKASA_HOME)}",
    ]
