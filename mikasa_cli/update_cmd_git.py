...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from mikasa_cli.update_cmd import _git_run
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_mikasa_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from mikasa_constants import get_mikasa_home
        (get_mikasa_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from mikasa_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from mikasa_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped MIKASA_HOME), then profile home as a fallback for custom layouts.

    The Hermes-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped MIKASA_HOME (``<root>/profiles/<name>``), so a profile-scoped ``hermes update`` must look
    there (monerostar review, #87876).
    """
    from mikasa_cli.update_cmd import get_default_mikasa_root, get_mikasa_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_mikasa_root(), Path(get_mikasa_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from mikasa_cli.update_cmd import _locate_real_git
