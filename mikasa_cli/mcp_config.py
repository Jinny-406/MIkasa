...
from mikasa_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_mikasa_home,  # noqa: F401 — used by test mocks
)
...
def cmd_mcp_add(args):
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_mikasa_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def mcp_command(args):
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from mikasa_cli import mcp_picker
