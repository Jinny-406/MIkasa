...
try:
    import mikasa_bootstrap  # noqa: F401
...
from mikasa_cli._subprocess_compat import suppress_platform_ver_console
...
# is hermes_cli/, not the repo root) can import mikasa_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from mikasa_cli import _startup_fast  # noqa: E402
...
# the mikasa_cli.config/env_loader imports further down would then crash before
...
# #57828.
from mikasa_cli import _early_recovery as _early_recovery_mod
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from mikasa_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from mikasa_cli.oneshot import run_oneshot
...
# mikasa_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("mikasa_HOME")
        if home:
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("mikasa_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("mikasa_TUI_NO_EARLY_DISABLE") == "1":
        return
...
from mikasa_cli.subcommands.cron import build_cron_parser
from mikasa_cli.subcommands.sync import build_sync_parser
from mikasa_cli.subcommands.gateway import build_gateway_parser
from mikasa_cli.subcommands.profile import build_profile_parser
from mikasa_cli.subcommands.model import build_model_parser
from mikasa_cli.subcommands.setup import build_setup_parser

from mikasa_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from mikasa_cli.subcommands.slack import build_slack_parser
from mikasa_cli.subcommands.login import build_login_parser
from mikasa_cli.subcommands.logout import build_logout_parser
from mikasa_cli.subcommands.auth import build_auth_parser
from mikasa_cli.subcommands.status import build_status_parser
from mikasa_cli.subcommands.pause import build_pause_parser
from mikasa_cli.subcommands.webhook import build_webhook_parser
from mikasa_cli.subcommands.hooks import build_hooks_parser
from mikasa_cli.subcommands.doctor import build_doctor_parser
from mikasa_cli.subcommands.verify import build_verify_parser
from mikasa_cli.subcommands.security import build_security_parser
from mikasa_cli.subcommands.approvals import build_approvals_parser
from mikasa_cli.subcommands.dump import build_dump_parser
from mikasa_cli.subcommands.debug import build_debug_parser
from mikasa_cli.subcommands.backup import build_backup_parser
from mikasa_cli.subcommands.import_cmd import build_import_cmd_parser
from mikasa_cli.subcommands.import_agent import build_import_agent_parser
from mikasa_cli.subcommands.config import build_config_parser
from mikasa_cli.subcommands.skin import build_skin_parser
from mikasa_cli.subcommands.console import build_console_parser
from mikasa_cli.subcommands.update import build_update_parser
from mikasa_cli.subcommands.uninstall import build_uninstall_parser
from mikasa_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from mikasa_cli.subcommands.gui import build_gui_parser
from mikasa_cli.subcommands.logs import build_logs_parser
from mikasa_cli.subcommands.prompt_size import build_prompt_size_parser
from mikasa_cli.subcommands.memory import build_memory_parser
from mikasa_cli.subcommands.acp import build_acp_parser
from mikasa_cli.subcommands.tools import build_tools_parser
from mikasa_cli.subcommands.insights import build_insights_parser
from mikasa_cli.subcommands.monitoring import build_monitoring_parser
from mikasa_cli.subcommands.skills import build_skills_parser
from mikasa_cli.subcommands.pairing import build_pairing_parser
from mikasa_cli.subcommands.plugins import build_plugins_parser
from mikasa_cli.subcommands.mcp import build_mcp_parser
from mikasa_cli.subcommands.claw import build_claw_parser
from mikasa_cli.subcommands.moa import build_moa_parser
from mikasa_cli.subcommands.fallback import build_fallback_parser
from mikasa_cli.subcommands.worktree import build_worktree_parser
from mikasa_cli.subcommands.browser import build_browser_parser
from mikasa_cli.subcommands.secrets import build_secrets_parser
from mikasa_cli.subcommands.egress import build_egress_parser
from mikasa_cli.subcommands.migrate import build_migrate_parser
from mikasa_cli.subcommands.checkpoints import build_checkpoints_parser
from mikasa_cli.subcommands.bundles import build_bundles_parser
from mikasa_cli.subcommands.curator import build_curator_parser
from mikasa_cli.subcommands.pets import build_pets_parser
from mikasa_cli.subcommands.journey import build_journey_parser
from mikasa_cli.subcommands.computer_use import build_computer_use_parser
from mikasa_cli.subcommands.sessions import build_sessions_parser
from mikasa_cli.subcommands.completion import build_completion_parser
...
# cache MIKASA_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# MIKASA_HOME set, and the flag stripped so argparse never sees it. Falls back
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    """
    from mikasa_cli._parser import top_level_value_flag_sets
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin MIKASA_HOME to the profile dir; a bare invocation
    means "the root MIKASA_HOME profile". If a supervised default-profile
    ...
    """
    if os.environ.get("mikasa_SUPERVISED_CHILD") or os.environ.get("mikasa_S6_SUPERVISED_CHILD"):
        return True
    ...
    return os.environ.get(
        "mikasa_GATEWAY_EXTERNAL_SUPERVISOR", ""
    ).strip().lower() in {"1", "true", "yes", "on"}


def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set MIKASA_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # MIKASA_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the hermes root (systemd hardcodes MIKASA_HOME=/root/.hermes)
    # we must still read active_profile — the user may have run
    # `hermes profile use` and the gateway should honour it (#22502).
    MIKASA_HOME_env = os.environ.get("mikasa_HOME", "")
    if profile_name is None and MIKASA_HOME_env and Path(MIKASA_HOME_env).parent.name == "profiles":
        return

    if profile_name is None and not _under_gateway_supervisor(argv):
        try:
            from mikasa_constants import get_default_mikasa_root

            active_path = get_default_mikasa_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from mikasa_cli.profiles import resolve_profile_env

        MIKASA_HOME = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        MIKASA_HOME = _resolve_sudo_user_profile_env(profile_name)
        if not MIKASA_HOME:
        ...
        return
    os.environ["mikasa_HOME"] = MIKASA_HOME
...
# app's ``python -m mikasa_cli.main`` spawn. Gates fail toward inaction. Sits
...
if sys.platform == "win32":
    try:
        from mikasa_cli import _install_repair as _install_repair_mod
...
from mikasa_cli.config import get_mikasa_home
from mikasa_cli.env_loader import load_mikasa_dotenv
...
try:
    ...
    from mikasa_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_mikasa_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from mikasa_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "mikasa_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["mikasa_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from mikasa_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from mikasa_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if mikasa_constants not importable yet

import logging
import threading
from datetime import datetime

from mikasa_cli import __version__, __release_date__

from mikasa_cli.model_setup_flows import (
...
from mikasa_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from mikasa_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from mikasa_cli.main_dashboard import (
...
from mikasa_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from mikasa_cli.main_provider_setup import (
...
from mikasa_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from mikasa_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from mikasa_cli.main_desktop import (
    cmd_gui,
)
from mikasa_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from mikasa_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from mikasa_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from mikasa_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_mikasa_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("mikasa_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("mikasa_TERMUX_PREFETCH_UPDATES") == "1"
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from mikasa_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    """
    from mikasa_cli.config import DEFAULT_CONFIG, get_env_path, get_mikasa_home, load_config
    from mikasa_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from mikasa_cli.config import split_model_config_default
    ...
    if _auth_store_logged_in(get_mikasa_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from mikasa_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    ...
    db = None
    try:
        from mikasa_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    hermes_bin = container_info["mikasa_bin"]
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from mikasa_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from mikasa_cli.foreign_sessions import import_foreign_session, pick_foreign_session
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from mikasa_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from mikasa_cli.config import load_config as _load_config_for_xai_check
...
def _start_chat_background_prefetch() -> None:
    ...
    if _termux_should_prefetch_update_check():
        try:
            from mikasa_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from mikasa_cli.config import get_mikasa_home
            skills_dir = Path(get_mikasa_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import mikasa_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `mikasa setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like Hermes isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  mikasa setup")
    print()

    from mikasa_cli.setup import (
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'mikasa setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["mikasa_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["mikasa_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older mikasa_cli.config) crash
        ...
        from mikasa_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from mikasa_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from mikasa_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
cmd_setup = _forward_command("cmd_setup", "mikasa_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "mikasa_cli.auth", "login_command", doc='Authenticate Hermes CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "mikasa_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "mikasa_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "mikasa_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "mikasa_cli.cron", "cron_command", doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "mikasa_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "mikasa_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "mikasa_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "mikasa_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "mikasa_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "mikasa_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "mikasa_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "mikasa_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "mikasa_cli.backup", "run_import", doc='Restore a Hermes backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "mikasa_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "mikasa_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "mikasa_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "mikasa_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "mikasa_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "mikasa_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "mikasa_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "mikasa_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from mikasa_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from mikasa_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from mikasa_cli.auth import AuthError, format_auth_error, resolve_provider
    from mikasa_cli.config import get_compatible_custom_providers, get_env_value
    from mikasa_cli.providers import custom_provider_aliases, resolve_provider_full
...
def select_provider_and_model(args=None):
    ...
    effective_provider = config_provider or os.getenv("mikasa_INFERENCE_PROVIDER") or "auto"
    ...
    from mikasa_cli.models import _PROVIDER_LABELS
    ...
    # mikasa_cli.main keep intercepting.
...
# on mikasa_cli.main; importing update_cmd eagerly would cost every ``hermes``
...
def cmd_verify(args):
    ...
    from mikasa_cli.verify_cmd import run_verify_command
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from mikasa_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    ...
    from mikasa_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from mikasa_cli.config import config_command
...
def cmd_uninstall(args):
    """Uninstall Mikasa Agent (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from mikasa_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from mikasa_cli.uninstall import run_gui_uninstall
    ...
    from mikasa_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from mikasa_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from mikasa_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Mikasa Agent")
        return True
    ...
    if getattr(args, "plan", False):
        ...
        from mikasa_cli.update_inventory import (
    ...
    from mikasa_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from mikasa_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Mikasa Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from mikasa_cli.update_lock import (
    ...
    from mikasa_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from mikasa_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
from mikasa_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        ...
        from mikasa_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    """
    if os.environ.get("mikasa_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("mikasa_WEB_DIST", "")):
            os.environ.pop("mikasa_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("mikasa_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    try:
        from mikasa_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        # #61115, #65696.
        from mikasa_cli.config import apply_terminal_config_to_env
    ...
    # dashboard_auth, …).
    try:
        from mikasa_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("mikasa_DESKTOP") == "1"
    if not mcp_discovery_after_bind:
        try:
            from mikasa_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard(args):
    ...
    from mikasa_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from mikasa_cli import completion
...
def cmd_logs(args):
    ...
    from mikasa_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from mikasa_cli.console_engine import run_console_repl
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("mikasa_TUI") == "1":
        return True
...
def _prepare_agent_startup(args) -> None:
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from mikasa_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from mikasa_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from mikasa_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["mikasa_SAFE_MODE"] = "1"
    os.environ["mikasa_IGNORE_USER_CONFIG"] = "1"
    os.environ["mikasa_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from mikasa_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("mikasa_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from mikasa_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("mikasa_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("mikasa_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["mikasa_DEFER_AGENT_STARTUP"] = "1"
            os.environ["mikasa_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["mikasa_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _advertise_agent_env() -> None:
    ...
    os.environ.setdefault("mikasa_AGENT", "true")
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from mikasa_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    ...
    from mikasa_cli.sessions_cmd import cmd_sessions
...
def _build_cli_parser():
    ...
    from mikasa_cli.send_cmd import register_send_subparser
    ...
    from mikasa_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from mikasa_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    from mikasa_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from mikasa_cli.projects_cmd import build_parser as _build_project_parser
...
def main():
    ...
    try:
        from mikasa_cli.stdio import configure_windows_stdio
    ...
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from mikasa_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('mikasa_cli.cli_output', 'line_input'),
}
