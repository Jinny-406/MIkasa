"""mikasa import-agent — import Claude Code / Codex CLI setups into Mikasa.
...
class AgentImporter:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → MIKASA_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``hermes import-agent`` (invoked from mikasa_cli.main)."""
    from mikasa_cli.config import get_config_path, load_config, save_config
    from mikasa_constants import get_mikasa_home
    from mikasa_cli.setup import (Colors, color, print_header, print_info, print_success,
    ...
    print(color("│          ⚕ Mikasa — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
    ...
    print_info(f"Target:      {MIKASA_HOME}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'mikasa setup' for credentials")
    ...
    def run_import(execute: bool, phase: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            return AgentImporter(agent, source_dir.resolve(), MIKASA_HOME.resolve(),
    ...
    print_info("API keys and credentials were NOT imported — run 'mikasa setup' "
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    from mikasa_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped := report.get("stripped_secrets"):
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'mikasa setup' or ~/.hermes/.env.")
        print()
