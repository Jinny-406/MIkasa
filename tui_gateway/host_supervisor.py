...
class HostSupervisor:
    ...
    def __init__(
        ...
        expected_mikasa_home: str | None = None, autostart: bool = True) -> None:
        self.registry_path = (
            Path(registry_path) if registry_path is not None
            else get_mikasa_home() / "state" / _REGISTRY_NAME)
        ...
        self.expected_mikasa_home = (
            str(get_mikasa_home()) if expected_mikasa_home is None else expected_mikasa_home)
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        env["mikasa_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
        root = str(_repo_root())
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("mikasa_home") or "")
        if got_home and got_home != self.expected_mikasa_home:
            raise RuntimeError(
                f"compute host MIKASA_HOME mismatch: {got_home} != {self.expected_mikasa_home}")
