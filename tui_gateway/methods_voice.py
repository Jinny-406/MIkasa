...
def _voice_mode_enabled() -> bool:
    return os.environ.get("mikasa_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    return os.environ.get("mikasa_VOICE_TTS", "").strip() == "1"
...
def _end_voice_chat(*, stop_loop: bool, stop_tts: bool) -> None:
    ...
    os.environ["mikasa_VOICE"] = os.environ["mikasa_VOICE_TTS"] = "0"
    if stop_loop:
        with contextlib.suppress(Exception):
            from mikasa_cli.voice import stop_continuous
            stop_continuous()
    if stop_tts:
        with contextlib.suppress(Exception):
...
def _speak_text_with_barge(text: str) -> None:
    ...
    from mikasa_cli.voice import speak_text
    stop, done = threading.Event(), threading.Event()
    with _fd_listener_lock:
...
def _(rid, params: dict) -> dict:
    ...
    from mikasa_cli.active_sessions import PER_SESSION_EXCLUSIVE_SUBMIT
...
def _voice_toggle_mode(rid, params: dict) -> dict:
    ...
    os.environ["mikasa_VOICE"] = "1" if enabled else "0"
    stop_hint = ""
    if enabled:
...
def _set_voice_tts(on: bool) -> None:
    ...
    os.environ["mikasa_VOICE_TTS"] = "1" if on else "0"
    if not on:
        _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from mikasa_cli.voice import start_continuous
        ...
        with contextlib.suppress(Exception):
            from mikasa_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        import mikasa_cli.voice  # noqa: F401  (a missing module must answer 5026, not die in a thread)
    except Exception as e:
