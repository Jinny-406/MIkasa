...
from mikasa_constants import display_mikasa_home as _display_mikasa_home
...
def _cfg_get_provider(params):
    from mikasa_cli.models import list_available_providers, normalize_provider
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_mikasa_home), "display": _display_mikasa_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's MIKASA_HOME + ``.env`` secret scope (ContextVars: concurrent checks
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from mikasa_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
        from mikasa_cli.diagnostics_upload import share_to_nous
