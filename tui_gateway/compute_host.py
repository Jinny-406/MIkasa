...
class ComputeHost:
    ...
    def __init__(
        ...
        self._heartbeat_secs = (
            ...
            else float(os.environ.get("mikasa_COMPUTE_HOST_HEARTBEAT_SECS") or "15"))
    ...
    def _build_server_session(self, server: Any, frame: dict[str, Any], sid: str) -> dict:
        ...
        try:
            if profile_home:
                from mikasa_constants import set_mikasa_home_override
            ...
            if home_token is not None:
                with contextlib.suppress(Exception):
                    from mikasa_constants import reset_mikasa_home_override
...
def _default_workers() -> int:
    try:
        return max(2, int(os.environ.get("mikasa_TUI_RPC_POOL_WORKERS") or "8"))
    except (TypeError, ValueError):
        return 8


def run_host(stdin: Any = None, stdout: Any = None) -> None:
    os.environ["mikasa_COMPUTE_HOST_CHILD"] = "1"
    ...
    host.emit({
        ...
        "mikasa_home": os.environ.get("mikasa_HOME", "")})
