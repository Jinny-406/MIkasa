...
def _profile_build_scope(profile_home):
    """Bind MIKASA_HOME + secret scope for an agent build (home alone leaves get_secret() on the LAUNCH .env)."""
    if not profile_home:
        yield
        return
...
def _billing_call(rid, fn, extra: dict | None = None) -> dict:
    ...
    from mikasa_cli.nous_billing import BillingError
    try:
        return _ok(rid, fn())
...
def _persist_branch(db, new_key: str, parent_key: str, title: str, history: list, *, source, cwd, profile_name,
    ...
    try:
        ...
        from mikasa_state_errors import is_disk_full_error
...
def _(rid, params: dict) -> dict:
    ...
    # ``profile`` (app-global remote mode): stored so the build and every turn re-bind MIKASA_HOME.
...
def _resume_guard(ctx: _Resume) -> dict | None:
    ...
    from mikasa_state import SessionResumeTooLargeError, resolved_max_resume_messages
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if (session := _sessions.get(sid)) is not None:
                ...
                # Each turn re-binds MIKASA_HOME (mid-turn memory/skills reads); lease claimed lazily on turn 1.
...
def _(rid, params: dict) -> dict:
    ...
    if not (raw := _str_param(params, "cwd")):
        return _err(rid, 4016, "cwd required")
    from mikasa_constants import translate_cwd_for_wsl_backend
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        try:
            home = Path(profile_home) if profile_home is not None else get_mikasa_home()
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    from agent.pet import store
    from mikasa_cli.pets import _clear_active_if
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    if new_slug != slug:
        from mikasa_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    from mikasa_cli.pets import _set_enabled
    _set_enabled(False)
...
def _(rid, params: dict) -> dict:
    ...
    from mikasa_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    from mikasa_cli.nous_billing import post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    def call():
        from mikasa_cli.auth import step_up_nous_billing_scope
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    lines = [
        "Hermes TUI Status", "", f"Session ID: {key}", f"Path: {display_mikasa_home()}",
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    saved_dir = get_mikasa_home() / "sessions" / "saved"
    try:
        saved_dir.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        return _err(rid, 5011, f"failed to create save directory {saved_dir}: {e}")
    path = saved_dir / f"mikasa_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with session["history_lock"]:
        messages = list(session.get("history", []))
