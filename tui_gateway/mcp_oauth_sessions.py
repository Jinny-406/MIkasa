...
def _probe_with_rollback(
    server_name: str, cfg: dict, MIKASA_HOME: str, flow, reconnect_live: bool) -> None:
    """Run the OAuth probe; on ANY failure restore the prior token file + manager entry."""
    from mikasa_cli.mcp_config import _oauth_tokens_present, _probe_single_server, _save_mcp_server
    ...
    previous_entry = None
    try:
        previous_entry = manager.remove(server_name, MIKASA_HOME=MIKASA_HOME)
        ...
        manager.restore_entry(server_name, previous_entry, MIKASA_HOME=MIKASA_HOME)
        raise


def _worker(
        session_id: str, MIKASA_HOME: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    try:
        ...
        home_token = set_mikasa_home_override(MIKASA_HOME)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(MIKASA_HOME)))
        try:
            with force_interactive_oauth(), dashboard_oauth_flow(flow):
                _probe_with_rollback(server_name, cfg, MIKASA_HOME, flow, reconnect_live)
        finally:
...
def start_flow(
    MIKASA_HOME: str, server_name: str, cfg: dict, *, reconnect_live: bool = False,
    ...
    with _sessions_lock:
        ...
        if any(r["server_name"] == server_name and r["mikasa_home"] == MIKASA_HOME for r in active):
            raise RuntimeError(f"MCP OAuth for '{server_name}' is already in progress")

    session_id = secrets.token_urlsafe(24)
    flow = DashboardOAuthFlow(
        flow_id=session_id, server_name=server_name, profile=None, MIKASA_HOME=MIKASA_HOME,
    ...
    rec = {
        "session_id": session_id, "server_name": server_name, "mikasa_home": MIKASA_HOME,
        "flow": flow, "httpd": httpd, "created_at": time.time()}
    with _sessions_lock:
        _sessions[session_id] = rec
    threading.Thread(
        target=_worker, args=(session_id, MIKASA_HOME, server_name, dict(cfg), reconnect_live),
