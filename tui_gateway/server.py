...
from mikasa_constants import (
    get_mikasa_home, get_mikasa_home_override, reset_mikasa_home_override, set_mikasa_home_override)
from mikasa_cli.env_loader import load_mikasa_dotenv
from utils import is_truthy_value
from tools.environments.local import mikasa_subprocess_env
...
load_mikasa_dotenv(MIKASA_HOME=_mikasa_home, project_env=Path(__file__).parent.parent / ".env")
...
_CRASH_LOG = os.path.join(_mikasa_home, "logs", "tui_gateway_crash.log")
...
with contextlib.suppress(Exception):
    from mikasa_cli.banner import prefetch_update_check
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("mikasa_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("mikasa_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("mikasa_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("mikasa_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "mikasa_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("mikasa_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    managed_bin = ""
    with contextlib.suppress(Exception):
        managed_bin = str(Path(get_mikasa_home()) / "bin")
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # launch MIKASA_HOME (#40677). The override goes through the build_subprocess_env factory's `extra`
        # (applied last, always wins) instead of a hand-rolled env["mikasa_HOME"] assignment.
        ...
        env = _prepend_tool_paths(build_subprocess_env(
            ...
            inherit_profile_home=False, extra={"mikasa_HOME": str(profile_home)} if profile_home else None))
...
_SESSION_TTL_S = max(0.0, env_float("mikasa_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("mikasa_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("mikasa_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
# backend, so calls carry ``profile`` → open that profile's db and bind its MIKASA_HOME (ContextVar
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    try:
        ...
        return None
    if home.resolve() == Path(_mikasa_home).resolve() or not home.exists():
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s MIKASA_HOME around a handler (pets/projects resolve via
    ``get_mikasa_home``, so app-global remote mode still hits the focused profile). No-op for launch.
...
def _bind_build_profile_scopes(profile_home: str) -> "_TurnScopes":
    """Bind a session profile's MIKASA_HOME / secret / terminal scopes for an agent build. Fail-open per
    ...
    scopes = _TurnScopes()
    scopes.home = set_mikasa_home_override(profile_home)
    with contextlib.suppress(Exception):
...
def _release_build_profile_scopes(scopes: "_TurnScopes") -> None:
    if scopes.home is not None:
        reset_mikasa_home_override(scopes.home)
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's MIKASA_HOME and hand the agent that profile's db —
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _mikasa_home) / "config.yaml"
...
def _env_model_seed() -> str:
    ...
    return (os.environ.get("mikasa_MODEL", "") or os.environ.get("mikasa_INFERENCE_MODEL", "")).strip()
...
def _resolve_model() -> str:
    ...
    with contextlib.suppress(Exception):
        from mikasa_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    desktop = is_truthy_value(os.environ.get("mikasa_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("mikasa_DESKTOP_TERMINAL")) else "tui"
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("mikasa_TUI_PROVIDER", "").strip():
        return model, explicit_provider
    if not (explicit_model := _env_model_seed()):
        return model, None
    with contextlib.suppress(Exception):
        from mikasa_cli.models import detect_static_provider_for_model
        cfg = _load_cfg().get("model") or {}
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("mikasa_INFERENCE_PROVIDER", "").strip().lower() or "auto")
...
from mikasa_state import _BARE_BILLING_PROVIDERS


def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from mikasa_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if provider and not _is_routable_provider(provider):
        healed = None
        try:
            from mikasa_cli.runtime_provider import canonical_custom_identity
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    if live is None or not hasattr(live[0], "_build_system_prompt") or not hasattr(live[2], "update_system_prompt"):
        return
    agent, session_key, db = live
    # Re-bind the session's profile MIKASA_HOME (the build's finally reset it → root profile's SOUL.md/skills)
    ...
    home_token = set_mikasa_home_override(profile_home) if profile_home else None
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`mikasa_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("mikasa_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    ...
    try:  # (enabled, disabled) MCP server names from raw config; both empty on any failure
        ...
        from mikasa_cli.tools_config import _parse_enabled_flag
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [item.strip() for item in os.environ.get("mikasa_TUI_TOOLSETS", "").split(",") if item.strip()]
...
def _get_usage(agent) -> dict:
    ...
    if is_truthy_value(os.environ.get("mikasa_DEV_CREDITS")):
        with contextlib.suppress(Exception):
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            with contextlib.suppress(Exception):
                from mikasa_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from mikasa_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if live_agent:
        ...
        with contextlib.suppress(Exception):
            from mikasa_cli.banner import get_available_skills
    ...
    with contextlib.suppress(Exception):
        from mikasa_cli.banner import get_update_result
        from mikasa_cli.config import recommended_update_command
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    ...
    from mikasa_cli.config import resolve_ephemeral_system_prompt_from_config
...
def _make_agent(
    ...
    for _mod in ("mikasa_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("mikasa_IGNORE_RULES"))
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("mikasa_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("mikasa_TUI_PASS_SESSION_ID")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped MIKASA_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str) -> tuple[list, list, list]:
    ...
    from mikasa_state import SessionResumeTooLargeError
    prefix_fits = True
...
def _pet_gen_root():
    ...
    root = get_mikasa_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("mikasa_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
def _spawn_trees_root():
    root = get_mikasa_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _resolve_name(name: str) -> str:
    with contextlib.suppress(Exception):
        from mikasa_cli.commands import resolve_command
