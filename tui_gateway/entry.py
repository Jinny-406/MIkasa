...
import mikasa_bootstrap

mikasa_bootstrap.harden_import_path()
...
def _install_sidecar_publisher() -> None:
    ...
    url = os.environ.get("mikasa_TUI_SIDECAR_URL")
    if not url:
        return
...
def _shutdown_grace_seconds() -> float:
    value = env_float("mikasa_TUI_GATEWAY_SHUTDOWN_GRACE_S", _DEFAULT_SHUTDOWN_GRACE_S)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # CALLER's profile context (agent build binds the session profile's MIKASA_HOME first), so a launch
...
def _has_configured_mcp_servers() -> bool:
    ...
    from mikasa_cli.mcp_startup import _has_configured_mcp_servers as configured
    return configured()
...
def ensure_mcp_discovery_started() -> None:
    ...
    profile's MIKASA_HOME. MCP registration is process-global: the FIRST profile wins.

    WebSocket/Desktop entrypoints can accept sessions without running ``main()``, so the agent-build path
    (``server._start_agent_build``) also calls it AFTER binding the session profile's MIKASA_HOME override —
