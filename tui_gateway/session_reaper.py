...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from mikasa_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from mikasa_cli.active_sessions import coerce_max_concurrent_sessions
        cfg = _load_cfg() or {}
...
_HEARTBEAT_REFRESH_S = max(0.0, env_float("mikasa_GATEWAY_HEARTBEAT_REFRESH_S", 60.0))
