...
def _is_repo_junk(root: str) -> bool:
    ...
    MIKASA_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    from mikasa_constants import get_mikasa_home
    real = os.path.realpath(root)
    MIKASA_HOME = os.path.realpath(str(get_mikasa_home()))
    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == MIKASA_HOME
        or real.startswith(MIKASA_HOME + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of MIKASA_HOME may be an
    intentional prose/data workspace, so only MIKASA_HOME itself is excluded here."""
    if not cwd:
        return True
    from mikasa_constants import get_mikasa_home
    real = os.path.normcase(os.path.realpath(cwd))
    MIKASA_HOME = os.path.normcase(os.path.realpath(str(get_mikasa_home())))
    return real in _non_workspace_dirs() or real == MIKASA_HOME
