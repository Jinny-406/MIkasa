...
def _(rid, params: dict) -> dict:
    ...
    if not text.startswith("/"):
        return _ok(rid, {"items": []})
    from mikasa_cli.commands_completion import SlashCommandCompleter
...
@_catch(5033)
def _(rid, params: dict) -> dict:
    from mikasa_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    from mikasa_cli.credential_lifecycle import save_provider_env_credential  # also rotates stale config.yaml mirrors
...
def _(rid, params: dict) -> dict:
    ...
    from mikasa_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
