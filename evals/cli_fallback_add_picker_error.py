...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from mikasa_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="mikasa_test_fallback_") as home:
        hh = Path(home) / ".hermes"
        hh.mkdir()
        ...
        proc = subprocess.Popen([sys.executable, "-m", "mikasa_cli.main", "fallback", "add"],
