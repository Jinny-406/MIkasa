...
def _snap(pid: int, db_path: str) -> dict:
    ...
    return {
        ...
        "tcp": int(tcp or 0), "pyright": kids.count("pyright"), "kernels": kids.count("mikasa_kernel_runner"),
...
def main() -> None:
    ...
    home = tempfile.mkdtemp(prefix="mikasa_bench_home_")
    os.environ["mikasa_HOME"] = home
    ...
    wts = []
    base = tempfile.mkdtemp(prefix="mikasa_bench_wt_")
    for w in range(a.worktrees):
        d = os.path.join(base, f"wt{w}")
        os.makedirs(os.path.join(d, "mikasa_cli"))
        subprocess.run(["git", "init", "-q", d], check=True)
        open(os.path.join(d, "pyproject.toml"), "w", encoding="utf-8").write("[project]\nname='b'\n")
        open(os.path.join(d, "mikasa_cli", "__init__.py"), "w", encoding="utf-8").write("")
        wts.append(d)
    ...
    tasks = [{"goal": json.dumps({"file": os.path.join(wts[i % len(wts)], "mikasa_cli", f"bench_{i}.py")}),
