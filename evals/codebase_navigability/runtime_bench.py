...
Each probe runs in a FRESH subprocess with an isolated MIKASA_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "mikasa_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "mikasa_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("mikasa_SESSION", "mikasa_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "mikasa_cli.main", "gateway.run", "tools.registry", "mikasa_state", "tui_gateway.server", "mikasa_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
CLI = {"version": ["mikasa_cli/main.py", "--version"], "help": ["mikasa_cli/main.py", "--help"], "config_get": ["mikasa_cli/main.py", "config", "get", "model"], "tools_list": ["mikasa_cli/main.py", "tools", "--help"], "skills_help": ["mikasa_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    import mikasa_state, tempfile, uuid
