...
for key in list(os.environ):
    if key.startswith('mikasa_') or key.endswith(('_API_KEY', '_TOKEN')):
        os.environ.pop(key, None)
home = tempfile.TemporaryDirectory(prefix='review-goaldup-')
os.environ['mikasa_HOME'] = home.name
os.environ['NO_PROXY'] = '*'
os.environ['TZ'] = 'UTC'
...
from mikasa_cli import cli_commands_mixin, goals
...
server._mikasa_home = Path(home.name)
...
with patch('mikasa_cli.goals.draft_contract', return_value=None), contextlib.redirect_stdout(io.StringIO()):
