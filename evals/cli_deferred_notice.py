...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="mikasa_test_notice_") as home:
        hh = Path(home) / ".hermes"
        hh.mkdir()
        ...
        from mikasa_cli.banner import VERSION
        ...
        if early:
            ...
            os.mkfifo(cache)
        env = {"PATH": os.environ["PATH"], "HOME": home, "mikasa_HOME": str(hh),
               "PYTHONPATH": str(root), "PYTHONUNBUFFERED": "1",
               "TERM": "xterm-256color", "LANG": "C.UTF-8",
               "OPENAI_API_KEY": "local-not-used", "PROMPT_TOOLKIT_NO_CPR": "1"}
        master, slave = pty.openpty()
        fcntl.ioctl(slave, termios.TIOCSWINSZ, struct.pack("HHHH", 40, 120, 0, 0))
        bootstrap = ("import mikasa_cli.main as m; import mikasa_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "mikasa_cli/banner.py") in text,
