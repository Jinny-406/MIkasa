...
os.environ.pop("mikasa_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
MIKASA_HOME = os.path.join(tmp_root, ".hermes")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(MIKASA_HOME)
os.makedirs(workspace)
with open(os.path.join(MIKASA_HOME, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
ctx = {
    "workspace": workspace, "mikasa_home": MIKASA_HOME,
