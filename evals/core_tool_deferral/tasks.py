...
ctx keys: workspace, MIKASA_HOME, events (desktop emit records),
...
def _grep_home(ctx, needle):
    """Binary-safe grep of the entire MIKASA_HOME tree (sqlite included)."""
    nb = needle.encode()
    for root, _dirs, files in os.walk(ctx["mikasa_home"]):
        for fn in files:
            try:
