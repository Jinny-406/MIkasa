...
try:
    ...
    def get_mikasa_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("mikasa_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".hermes"

DEFAULT_TUI_DIR = Path(
    os.environ.get("mikasa_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("mikasa_PERF_LOG", str(get_mikasa_home() / "perf.log")))
DEFAULT_STATE_DB = get_mikasa_home() / "state.db"
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["mikasa_DEV_PERF"] = "1"
    env["mikasa_DEV_PERF_MS"] = str(args.threshold_ms)
    env["mikasa_DEV_PERF_LOG"] = str(log)
    env["mikasa_TUI_RESUME"] = sid
    ...
    node = os.environ.get("mikasa_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="mikasa_DEV_PERF_MS (0 = capture all)")
