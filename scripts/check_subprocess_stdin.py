...
# ``get_mikasa_home() / "plugins"`` (user) and ``./.hermes/plugins/`` (project,
...
# this script runs as a standalone subprocess — ``mikasa_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import mikasa_constants (this script
    ...
    # plugins in get_mikasa_home()/plugins/ and project plugins in
    # ./.hermes/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_mikasa_home() / "plugins"]
    if os.environ.get("mikasa_ENABLE_PROJECT_PLUGINS"):
