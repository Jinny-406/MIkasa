...
def run_one(scenario: Dict[str, Any], mode: str, rep: int, out_dir: Path) -> Dict[str, Any]:
    ...
    MIKASA_HOME = base.setup_isolated_home(enabled, listing=("auto" if mode == "listing" else "off"))
    ...
    try:
        ...
        from mikasa_cli.plugins import get_plugin_manager, discover_plugins
    ...
    shutil.rmtree(Path(os.environ["mikasa_HOME"]).parent, ignore_errors=True)
    return rec
