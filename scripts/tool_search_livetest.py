...
ORIGINAL_HOME = os.environ.get("mikasa_HOME")
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """
    home_dir = Path(tempfile.mkdtemp(prefix="mikasa_ts_live_"))
    MIKASA_HOME = home_dir / ".hermes"
    MIKASA_HOME.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, MIKASA_HOME / "auth.json")
    ...
    if real_env_file.exists():
        shutil.copy(real_env_file, MIKASA_HOME / ".env")
        ...
        load_mikasa_dotenv(MIKASA_HOME=str(Path.home() / ".hermes"))
    ...
    (MIKASA_HOME / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return MIKASA_HOME
...
def reset_module_state():
    """Drop cached modules so the new MIKASA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "mikasa_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["mikasa_HOME"] = str(home)
...
def main():
    ...
    # Restore original MIKASA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["mikasa_HOME"] = ORIGINAL_HOME
    else:
