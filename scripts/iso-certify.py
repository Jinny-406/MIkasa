...
1. Spawns a SCRATCH dashboard (``hermes dashboard``) bound to loopback on a
   free port, with an ISOLATED ``MIKASA_HOME`` (temp dir, minimal seeded state).
...
the scratch MIKASA_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
_READY_RE = re.compile(r"mikasa_(?:DASHBOARD|BACKEND)_READY port=(\d+)")
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        env["mikasa_HOME"] = str(self.home)
        env["HOME"] = str(self.home.parent) if str(self.home.parent) else env.get("HOME", "")
        env["mikasa_HOME"] = str(self.home)
        env["PYTHONPATH"] = str(REPO_ROOT) + os.pathsep + env.get("PYTHONPATH", "")
        env["mikasa_ISO_CERTIFY_SYNTH_TURN"] = "1"
        cmd = [
            python, "-m", "mikasa_cli.main", "dashboard",
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with ScratchDashboard(
            ...
            env_extra={"mikasa_DASHBOARD_SESSION_TOKEN": token},
        ) as dash:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch MIKASA_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch MIKASA_HOME on exit")
