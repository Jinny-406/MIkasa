...
5. **Killed gateway:** terminate one canary, verify missing-series detection,
  ...
  Mikasa Agent-owned Relay transport health remains in scope. A separate gateway
...
The `hermes monitoring` CLI intentionally exposes `status` only. This first
  release covers only Mikasa Agent-owned service-health and operational-diagnostic
  signals, including Mikasa Agent-owned Relay transport health. Team Gateway's
