...
import { MinimalBanner } from './minimalBanner.js'
...
export function Banner({ maxWidth, t, version }: { maxWidth?: number; t: Theme; version?: string }) {
  const term = useStdout().stdout?.columns ?? 80
  const cols = Math.max(1, Math.min(term, maxWidth ?? term))

  if (t.brand.bannerStyle === 'minimal') {
    return <MinimalBanner columns={Math.max(1, cols - 2)} t={t} version={version} />
  }
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const wide = t.brand.bannerStyle !== 'minimal' && cols >= 90 && leftW + 40 < cols
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · {t.brand.tagline ?? 'Nous Research'}</Text>
      </Text>
