...
const INDICATOR_PRESETS = {
  ascii: { frames: ASCII_FRAMES, intervalMs: SPINNER_TICK_MS },
  emoji: { frames: EMOJI_FRAMES, intervalMs: SPINNER_TICK_MS * 6 },
  pulse: { frames: PULSE_FRAMES, intervalMs: 170 }
} as const
...
export const renderIndicator = (style: IndicatorStyle, tick: number): IndicatorRender => {
  ...
  if (style in INDICATOR_PRESETS) {
    const preset = INDICATOR_PRESETS[style as keyof typeof INDICATOR_PRESETS]

    return { frame: preset.frames[tick % preset.frames.length]!, intervalMs: preset.intervalMs, showVerb: true }
  }
...
const indicatorFrameWidth = (style: IndicatorStyle): number => {
  ...
  if (style === 'pulse') {
    return stringWidth(PULSE_FRAMES[0]!)
  }
