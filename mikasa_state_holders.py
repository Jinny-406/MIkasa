...
_MIKASA_PYTHON_MODULES = frozenset({"acp_adapter", "mikasa_cli.main"})
_MIKASA_PYTHON_SCRIPTS = frozenset({"mikasa_cli/main.py", "run_agent.py"})
...
def _looks_like_hermes(argv: Sequence[str]) -> bool:
    ...
    if kind == "module":
        return normalized in _MIKASA_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _MIKASA_PYTHON_SCRIPTS
    )
