...
# Defaults for the in-agent progress-aware wrap; mirror mikasa_cli.config.DEFAULT_CONFIG["compression"] keys.
...
def _rebind_session_context(session_id: str) -> None:
    ...
    with contextlib.suppress(Exception):
        from mikasa_logging import set_session_context
...
def _parent_deliberately_ended(session_db: Any, session_id: str) -> bool:
    ...
    if not callable(reader):
        return False
    try:
        from mikasa_state_common import is_automatic_end_reason
...
def _carry_session_state_to_child(agent: Any, old_session_id: str, old_title: Any) -> None:
    ...
    with _swallow('Could not migrate goal on compression: %s'):
        ...
        from mikasa_cli.goals import migrate_goal_to_session
        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
    with _swallow('Could not migrate heartbeat on compression: %s'):
        from mikasa_cli.heartbeat import migrate_heartbeat_to_session
...
def _publish_rotated_compaction(
    ...
    # publish also COALESCEs from the parent row for threads lacking MIKASA_HOME.
    ...
    agent._session_db.publish_compression_child(
        ...
        source=agent.platform or os.environ.get("mikasa_SESSION_SOURCE", "cli"), model=agent.model,
...
def _shrink_data_url(url: str, *, max_dimension: int, resize_fn: Any) -> tuple:
    ...
    try:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="mikasa_shrink_", suffix=_IMAGE_SUFFIX_BY_MIME.get(mime, ".jpg"), delete=False
        )
        try:
            tmp.write(raw)
            tmp.close()
