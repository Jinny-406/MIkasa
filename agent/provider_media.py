"""``$MIKASA_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$MIKASA_HOME/cache/<kind>/``, creating parents as needed."""
    from mikasa_constants import get_mikasa_home
    path = get_mikasa_home() / "cache" / kind
