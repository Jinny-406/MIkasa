...
def _guarded_global_root(global_path: Optional[Path]) -> Optional[Path]:
    ...
    never write the real user's ``~/.hermes/auth.json`` even when MIKASA_HOME
...
def _singleton_target_for_entry(pool: "CredentialPool", entry: "PooledCredential") -> Optional[Path]:
    ...
    if entry.source != "mikasa_pkce" or entry.id not in getattr(pool, "_borrowed_root_ids", ()):
        return None
    try:
...
_REFRESH_TIMEOUT_ENV_VARS = {
    "openai-codex": "mikasa_CODEX_REFRESH_TIMEOUT_SECONDS",
    "xai-oauth": "mikasa_XAI_REFRESH_TIMEOUT_SECONDS",
}
...
class CredentialPool:
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        env_var = _REFRESH_TIMEOUT_ENV_VARS.get(self.provider, "mikasa_ANTHROPIC_REFRESH_TIMEOUT_SECONDS")
    ...
    def _refresh_anthropic(self, entry: PooledCredential) -> PooledCredential:
        ...
        refreshed = refresh_anthropic_oauth_pure(entry.refresh_token, use_json=entry.source.endswith("mikasa_pkce"))
        updated = replace(
            entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            synced.refresh_token, use_json=synced.source.endswith("mikasa_pkce"),
                        )
...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    ...
    if _env_val("ANTHROPIC_API_KEY") and not anthropic_oauth_env:
        ...
        seed.changed |= _retain_sources_not_in(seed.entries, {"mikasa_pkce", "claude_code"})
        return
    ...
    for source_name, creds in (
        ("mikasa_pkce", read_hermes_oauth_credentials()),
...
def _seed_copilot_singleton(seed: _Seeder) -> None:
    ...
    try:
        from mikasa_cli.copilot_auth import (
            COPILOT_ENV_VARS,
...
def _seed_qwen_singleton(seed: _Seeder) -> None:
    ...
    try:
        from mikasa_cli.auth import resolve_qwen_runtime_credentials
...
def _env_payload(*, env_var: str, token: str, base_url: str) -> Dict[str, Any]:
    ...
    try:
        from mikasa_cli.env_loader import get_secret_source
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        return is_borrowed_credential_source(entry.source, entry.provider) or entry.source == "mikasa_pkce"
