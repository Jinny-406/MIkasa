...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("mikasa_KANBAN_TASK"):
            kanban_db = spawn_env.get("mikasa_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("mikasa_HOME", os.path.expanduser("~/.hermes")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("mikasa_KANBAN_ROOT", default_root)
            cmd += [
