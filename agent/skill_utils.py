...
from mikasa_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("mikasa_KANBAN_TASK") or os.getenv("mikasa_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no mikasa_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at MIKASA_HOME; absolute paths pass through."""
    from mikasa_constants import get_mikasa_home
    return p if p.is_absolute() else get_mikasa_home() / p
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    resolved_platform = platform or os.getenv("mikasa_PLATFORM") or get_session_env("mikasa_SESSION_PLATFORM")
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to MIKASA_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to MIKASA_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_mikasa_home()}/skills/"
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (MIKASA_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under MIKASA_HOME, never inside the repo); "dangerous" excludes the
...
# high-confidence findings only. The scan cache lives under MIKASA_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from mikasa_constants import get_mikasa_home
        cache_dir = get_mikasa_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped MIKASA_HOME), so normalization
