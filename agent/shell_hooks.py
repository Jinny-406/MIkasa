...
from mikasa_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, split_command_line, windows_hide_flags
...
def _home_key() -> str:
    return str(get_mikasa_home().expanduser().resolve())
...
def register_from_config(cfg: Optional[Dict[str, Any]], *, accept_hooks: bool = False) -> List[ShellHookSpec]:
    ...
    if env_var_enabled("mikasa_SAFE_MODE"):  # hooks are user customizations too — fire zero user-configured code
        logger.info("mikasa_SAFE_MODE=1 — shell-hook registration skipped")
        return []
    effective_accept = _resolve_effective_accept(cfg, accept_hooks)
    specs = _parse_hooks_block(cfg.get("hooks"))
    if not specs:
        return []
    from mikasa_cli.plugins import get_plugin_manager  # lazy: avoids import cycle
    ...
    for spec in specs:
        ...
        if not already_allowlisted and not _prompt_and_record(spec.event, spec.command, accept_hooks=effective_accept):
            logger.warning("shell hook for %s (%s) not allowlisted — skipped. Use --accept-hooks / "
                           "mikasa_ACCEPT_HOOKS=1 / hooks_auto_accept: true, or approve at the TTY prompt next run.",
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    from mikasa_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def allowlist_path() -> Path:
    ...
    return get_mikasa_home() / ALLOWLIST_FILENAME
...
def _resolve_effective_accept(cfg: Dict[str, Any], accept_hooks_arg: bool) -> bool:
    ...
    if accept_hooks_arg or os.environ.get("mikasa_ACCEPT_HOOKS", "").strip().lower() in _TRUTHY:
        return True
