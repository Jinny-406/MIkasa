...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    if not base_url:
        return False
    try:
        from mikasa_cli.models import _is_model_free
        from mikasa_cli.models_pricing import peek_cached_pricing
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    ...
    name = os.environ.get("mikasa_DEV_CREDITS_FIXTURE", "").strip()
    if not name or not is_truthy_value(os.environ.get("mikasa_DEV_CREDITS")):
        return None
