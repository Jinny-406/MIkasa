...
from mikasa_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    ...
    if not content:
        return content
    values = {
        "mikasa_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "mikasa_SESSION_ID": str(session_id) if session_id else None,
    }
