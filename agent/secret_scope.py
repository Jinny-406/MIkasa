...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "mikasa_HOME", "mikasa_PROFILE", "mikasa_GATEWAY_LOCK_DIR",
    "mikasa_MAX_ITERATIONS", "mikasa_MAX_TOKENS", "mikasa_API_TIMEOUT",
    "mikasa_REDACT_SECRETS", "mikasa_NOUS_TIMEOUT_SECONDS",
    "_HERMES_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "mikasa_KANBAN_DB", "mikasa_KANBAN_WORKSPACES_ROOT", "mikasa_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "mikasa_KANBAN_",
    "mikasa_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    from mikasa_cli.config import _parse_env_value
...
def build_profile_secret_scope(MIKASA_HOME: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(MIKASA_HOME) / ".env")
    try:
        from mikasa_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(MIKASA_HOME))
    except Exception:
        external_secrets = {}
