...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    ...
    _plines(
        ...
        f"     • Verify stored credentials: {display_mikasa_home()}/auth.json",
...
def recover_after_classification(
    ...
    if (
        ...
        try:
            from mikasa_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from mikasa_cli.model_normalize import suggest_prefixed_model_id
