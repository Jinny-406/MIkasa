...
from agent.prompt_builder import (
    ...
    MIKASA_AGENT_HELP_GUIDANCE, MIKASA_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE, MEMORY_GUIDANCE,
...
from mikasa_constants import get_default_mikasa_root, get_mikasa_home
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("mikasa_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the MIKASA_HOME
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from mikasa_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from mikasa_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound MIKASA_HOME ContextVar override wins (the gateway multiplexes
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses ``get_default_mikasa_root()`` (NOT ``get_mikasa_home()``): on a bound
    ...
    try:
        ...
        rel = home.resolve().relative_to((get_default_mikasa_root() / "profiles").resolve())
...
def _active_profile_line(agent: Any) -> str:
    ...
    if active_profile == "default":
        ...
        # ROOT (get_mikasa_home() on a bound profile session is the PROFILE dir).
        # Without one, keep the ambient (patchable) resolution byte-identical.
        _root_str = str(get_default_mikasa_root() if _agent_home_path is not None else get_mikasa_home())
        return (
    ...
    profile_home = str(_agent_home_path) if _agent_home_path is not None else str(get_mikasa_home())
    ...
    # default profile's data sits at the ROOT (get_default_mikasa_root()), which in ambient profile mode is
    # NOT get_mikasa_home().
...
def _timestamp_line(agent: Any) -> str:
    ...
    from mikasa_time import get_timezone as _hermes_tz, now as _hermes_now
    now = _hermes_now()
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    stable_parts.append(MIKASA_AGENT_HELP_GUIDANCE_NO_SKILLS)
    ...
    if "skill_view" in (agent.valid_tool_names or set()) and "- hermes-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = MIKASA_AGENT_HELP_GUIDANCE
