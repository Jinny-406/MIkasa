...
``hermes pause`` writes a sentinel at ``$MIKASA_HOME/ESTOP``; ``hermes resume``
...
from agent.file_safety import _mikasa_home_path as _mikasa_home, _hermes_root_path as _canonical_root
...
def sentinel_path() -> Path:
    ...
    return _mikasa_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (MIKASA_HOME=~/.hermes/profiles/<n>) must still honor an operator's ~/.hermes/ESTOP."""
