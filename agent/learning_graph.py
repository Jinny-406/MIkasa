...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        try:
            return json.loads((get_mikasa_home() / "skills" / ".usage.json").read_text(encoding="utf-8"))
        except Exception:
            return {}
...
def build_learning_graph() -> dict[str, Any]:
    ...
    roots = [("base", Path(__file__).resolve().parent.parent / "skills"), ("profile", get_mikasa_home() / "skills")]
    learned_skills = {
