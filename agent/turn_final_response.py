...
def finish_text_response(
    ...
    # --- GODMODE: Refusal detection + auto-route + coaching ---
    godmode_enabled = getattr(agent, "_godmode_enabled", True)
    if godmode_enabled and final_response:
        try:
            from agent.godmode import godmode_hook_with_auto_route
            final_response, _godmode_modified, _strategy = godmode_hook_with_auto_route(
                user_message or "",
                final_response,
                agent,
                enabled=True,
                auto_route=True,
                coaching_enabled=getattr(agent, "_godmode_coaching", True),
                max_retries=getattr(agent, "_godmode_max_retries", 2),
            )
            if _godmode_modified:
                agent._emit_status(f"GODMODE: Bypassed via {_strategy}")
        except Exception as _gm_err:
            logger.debug("GODMODE hook failed: %s", _gm_err)
