...
def kanban_stop_nudge_enabled() -> bool:
    ...
    if (os.environ.get("mikasa_KANBAN_STOP_NUDGE") or "").strip().lower() in {"0", "false", "no", "off"}:
        return False
    return bool((os.environ.get("mikasa_KANBAN_TASK") or "").strip())
...
def build_kanban_stop_nudge(
    *,
    ...
    tid = (task_id or os.environ.get("mikasa_KANBAN_TASK") or "").strip() or "this task"
    return (
