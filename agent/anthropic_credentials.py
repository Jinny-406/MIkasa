...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "mikasa_pkce": lambda: _get_hermes_oauth_file()
}
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from mikasa_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("mikasa_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
...
def _get_hermes_oauth_file() -> Path:
    return get_mikasa_home() / ".anthropic_oauth.json"
...
def _root_hermes_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        return None if root.resolve(strict=False) == get_mikasa_home().resolve(strict=False) else root / ".anthropic_oauth.json"
    except Exception:
        return None
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from mikasa_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def _write_hermes_oauth_credentials(
    ...
    instead of forking a copy under its own MIKASA_HOME; without this write-through the next ``load_pool()``
