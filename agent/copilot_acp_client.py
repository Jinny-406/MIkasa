...
def _resolve_command() -> str:
    return os.getenv("mikasa_COPILOT_ACP_COMMAND", "").strip() or os.getenv("COPILOT_CLI_PATH", "").strip() or "copilot"


def _resolve_args() -> list[str]:
    return shlex.split(os.getenv("mikasa_COPILOT_ACP_ARGS", "").strip()) or ["--acp", "--stdio"]
...
def _build_subprocess_env() -> dict[str, str]:
    from mikasa_constants import apply_subprocess_home_env
    ...
    def _spawn(self) -> subprocess.Popen[str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                "mikasa_COPILOT_ACP_COMMAND / HERMES_COPILOT_ACP_ARGS to a working pair."
            )
        try:
            from mikasa_cli._subprocess_compat import windows_hide_flags  # hide the Windows console flash (#56747); pipes intact for the ACP wire
            ...
            raise RuntimeError(f"Could not start Copilot ACP command '{self._acp_command}'. Install GitHub Copilot CLI or set "
                               "mikasa_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.") from exc
