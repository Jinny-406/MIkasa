...
into ``<MIKASA_HOME>/bin``; CA, ``proxy.yaml``, ``mappings.json``, pidfile and logs live in
``<MIKASA_HOME>/proxy``.  Failures warn and never block agent startup.
"""
...
_MGMT_API_KEY_ENV = "mikasa_IRON_PROXY_MGMT_KEY"
...
_HERMES_IRON_PROXY_NONCE_ENV = "mikasa_IRON_PROXY_NONCE"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<MIKASA_HOME>/bin`` copy first, then PATH; optionally auto-install."""
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<MIKASA_HOME>/proxy/proxy.yaml`` (safe_dump, no Python tags)."""
