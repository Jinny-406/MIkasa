...
from mikasa_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from mikasa_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _provider_preferences_for_agent(agent) -> Dict[str, Any]:
    ...
    with contextlib.suppress(Exception):
        ...
        from mikasa_constants import resolve_per_model_provider_routing
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("mikasa_STREAM_STALE_GIVEUP", 5)
...
def _configured_stale_base(agent) -> float:
    ...
    return cfg if cfg is not None else env_float("mikasa_STREAM_STALE_TIMEOUT", 180.0)
...
def _managed_local_load_notice(agent, api_kwargs: dict) -> "Optional[str]":
    ...
    try:
        ...
        from mikasa_cli.local_runtime.load_progress import get_loading_progress, get_prefill_progress
...
def _resolve_nonstream_watchdogs(agent, api_kwargs: dict) -> _NonStreamWatchdogs:
    ...
    if codex and openai_codex_backend:
        ...
        hard_timeout = env_float("mikasa_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
        if hard_timeout > 0:
    ...
    ttfb_enabled = codex
    ttfb_timeout = env_float("mikasa_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if ttfb_timeout <= 0:
        ttfb_enabled = False
        ...
        disable_above = env_float("mikasa_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        strict = os.environ.get("mikasa_CODEX_TTFB_STRICT", "").strip().lower() in {"1", "true", "yes", "on"}
        ...
        ttfb_cap = env_float("mikasa_CODEX_TTFB_MAX_SECONDS", 120.0)
    ...
    idle_timeout = env_float("mikasa_CODEX_EVENT_STALE_TIMEOUT_SECONDS", idle_default)
...
def _is_anthropic_wire_url(url: str) -> bool:
    ...
    from mikasa_cli.providers import host_mandated_api_mode
...
class _StreamingCall:
    ...
    def _stream_timeouts(self) -> tuple[float, float, float]:
        ...
        base = cfg if cfg is not None else env_float("mikasa_API_TIMEOUT", 1800.0)
        if cfg is not None:
            return base, cfg, min(base, 60.0)
        read = env_float("mikasa_STREAM_READ_TIMEOUT", 120.0)
    ...
    def _call(self):
        _max_stream_retries = env_int("mikasa_STREAM_RETRIES", 2)
        try:
    ...
    def _resolve_stale_timeout(self) -> None:
        ...
        if base == 180.0 and self.agent.base_url and is_local_endpoint(self.agent.base_url):
            ...
            self._stream_stale_timeout = env_float("mikasa_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
