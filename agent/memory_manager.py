...
def _ctx_bound(fn: Callable[[], Any]) -> Callable[[], Any]:
    ...
    ContextVar-scoped MIKASA_HOME override, and an unbound worker would silently use the default profile."""
...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        """Initialize all providers, injecting ``MIKASA_HOME`` so they resolve profile-scoped paths."""
        if "mikasa_home" not in kwargs:
            from mikasa_constants import get_mikasa_home
            kwargs["mikasa_home"] = str(get_mikasa_home())
