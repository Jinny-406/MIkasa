...
from mikasa_cli.config import cfg_get
from mikasa_cli.route_identity import normalize_route_base_url
from mikasa_cli.timeouts import get_provider_request_timeout
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from mikasa_cli.providers import MIKASA_OVERLAYS, get_provider
        overlay = MIKASA_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from mikasa_cli.models import normalize_provider as normalize_model_provider
        from mikasa_cli.providers import normalize_provider as normalize_registry_provider
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$MIKASA_HOME`` is profile-scoped; not a config key)."""
    return get_mikasa_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if api_mode in _EXPLICIT_API_MODES:
        ...
        try:
            ...
            from mikasa_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    with suppress(Exception):
        from mikasa_cli.model_normalize import (
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from mikasa_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from mikasa_logging import setup_logging, setup_verbose_logging
    setup_logging(MIKASA_HOME=_ra()._mikasa_home)
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from mikasa_cli.auth import build_minimax_oauth_token_provider
...
def _init_bedrock_client(agent, base_url):
    ...
    with suppress(Exception):
        from mikasa_cli.config import load_config_readonly as _load_br_cfg
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("mikasa_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "mikasa_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched MIKASA_HOME since model_tools was imported;
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        if not delegated_child:
            os.environ["mikasa_SESSION_ID"] = session_id
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    agent.logs_dir = get_mikasa_home() / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot is opt-in (sessions.write_json_snapshots); state.db is canonical.
    agent._session_json_enabled = False
    with suppress(Exception):
        from mikasa_cli.config import load_config_readonly as _load_sess_cfg
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from mikasa_cli.auth import resolve_provider as resolve_auth_provider
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from mikasa_cli.plugins import get_plugin_context_engine
...
def _warn_nonagentic_hermes_model(agent):
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from mikasa_cli.model_switch import _check_hermes_model_warning
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, MIKASA_HOME=str(get_mikasa_home()),
...
def init_agent(
    ...
    try:
        from mikasa_cli.config import load_config_readonly as _load_agent_config
