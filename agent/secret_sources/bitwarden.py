...
``bws`` is auto-installed into ``<MIKASA_HOME>/bin/bws`` (one pinned version,
...
# <MIKASA_HOME>/cache/bws_cache.json holds only secret VALUES (never the access
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<MIKASA_HOME>/bin/bws`` first, then PATH, then optional auto-install."""
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_mikasa_dotenv()`` calls after the .env
