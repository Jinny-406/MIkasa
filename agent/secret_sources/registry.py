...
def _merged(scope: Optional[str]) -> Dict[str, SecretSource]:
    ...
    merged.update(_SCOPED_SOURCES.get(scope or MIKASA_HOME_key(), {}))
    return merged
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(MIKASA_HOME_key(), {}))
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(secrets_cfg, scope=MIKASA_HOME_key(home_path))
    if not enabled:
        return report
