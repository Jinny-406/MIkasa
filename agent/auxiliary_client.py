...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    try:
        ...
        from mikasa_cli.config import get_custom_provider_tls_settings, load_config_readonly
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    with contextlib.suppress(Exception):
        from mikasa_cli.models import OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER, opencode_zen_free_headers
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from mikasa_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_nous:
            ...
            try:
                from mikasa_cli.models_pricing import _resolve_nous_pricing_credentials
        ...
        _nous_kwargs = {}
        if is_nous:
            from mikasa_cli.models_pricing import _NOUS_CATALOG_TTL_SECONDS
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from mikasa_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    env_cache = os.environ.get("mikasa_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("mikasa_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_AUTH_JSON_PATH = get_mikasa_home() / "auth.json"
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from mikasa_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("mikasa_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _maybe_wrap_anthropic(
    ...
    if (
        ...
        or _client_declares(client_obj, "mikasa_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _nous_api_key(provider: dict) -> str:
    ...
    from mikasa_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from mikasa_cli.auth import _agent_key_is_usable
        pool = load_pool("nous")
    except Exception as exc:
    ...
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    try:
        ...
        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("mikasa_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from mikasa_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(os.getenv("mikasa_XAI_BASE_URL", ""))
    ...
    try:
        from mikasa_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from mikasa_cli.auth import _read_codex_tokens
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            with contextlib.suppress(ImportError):
                from mikasa_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from mikasa_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from tools.xai_http import mikasa_xai_default_headers
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from mikasa_cli.config import cfg_get, load_config_readonly
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from mikasa_cli import config as _cfg_mod
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        from mikasa_cli.moa_config import resolve_moa_preset
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from mikasa_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _refresh_copilot_credentials() -> bool:
    from mikasa_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_nous_credentials() -> bool:
    ...
    return _creds_have_api_key(resolve_nous_runtime_credentials(
        timeout_seconds=env_float("mikasa_NOUS_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "mikasa_SKIP_ASYNC_WRAP"):
        return sync_client, model
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from mikasa_cli.model_normalize import normalize_model_for_provider
...
def _resolve_nous_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    from mikasa_cli.providers import nous_api_mode
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from mikasa_cli.models import opencode_zen_free_runtime as _oc_free_rt
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from mikasa_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("mikasa_cli.auth not available for provider %s", provider)
        return None, None
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from mikasa_cli.local_runtime.supervisor import state_path
