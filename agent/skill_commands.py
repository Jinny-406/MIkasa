...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    try:
        ...
        resolved_platform = os.getenv("mikasa_PLATFORM") or get_session_env("mikasa_SESSION_PLATFORM")
    except Exception:
        resolved_platform = os.getenv("mikasa_PLATFORM")
...
def _resolve_skill_commands_home() -> str:
    ...
    ``set_mikasa_home_override``), but the module-level scan only tracked
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    try:
        ...
        if not resolved:
            return
        parts.append("")
        parts.append(f"[Skill config (from {display_mikasa_home()}/config.yaml):")
