...
Installs go to a Hermes-owned staging dir, ``<MIKASA_HOME>/lsp/bin/``, so the
...
# ``bin`` in ``<MIKASA_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $MIKASA_HOME/node isn't on an arbitrary process's
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <MIKASA_HOME>/lsp/
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "mikasa_lsp_bin_dir"]
