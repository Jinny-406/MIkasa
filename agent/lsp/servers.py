...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<MIKASA_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_mikasa_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <MIKASA_HOME>/lsp/PowerShellEditorServices."
)
...
def hermes_lsp_session_dir() -> str:
    ...
    from mikasa_constants import get_mikasa_home
    d = os.path.join(str(get_mikasa_home()), "lsp", "pses")
