...
from mikasa_cli._subprocess_compat import IS_WINDOWS, harden_git_argv, noninteractive_git_env, windows_hide_flags
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    home, MIKASA_HOME = Path(os.path.expanduser("~")).resolve(), get_mikasa_home().resolve()
    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES} | {MIKASA_HOME / ".env"}
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS] + [MIKASA_HOME / rel for rel in _SENSITIVE_HERMES_DIRS]
