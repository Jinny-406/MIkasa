...
def _constants_path(getter_name: str) -> Path:
    """Call ``mikasa_constants.<getter_name>()`` (local import avoids cycles); ``~/.hermes`` on any failure."""
    try:
        import mikasa_constants

        return getattr(mikasa_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.hermes"))


def _mikasa_home_path() -> Path:
    """Active MIKASA_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_mikasa_home")


def _hermes_root_path() -> Path:
    """Hermes root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_mikasa_root")


def _hermes_dirs() -> list[Path]:
    """Resolved active MIKASA_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (MIKASA_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_mikasa_home_path(), _hermes_root_path()))))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    paths = [
        ...
        *(str(base / f) for f in hermes_files for base in (_mikasa_home_path(), _hermes_root_path())),
...
def get_safe_write_roots() -> set[str]:
    ...
    roots: set[str] = set()
    for path in filter(None, os.getenv("mikasa_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# MIKASA_HOME / root subpaths that the agent's generic file tools must not
...
# Exact-file credential stores under MIKASA_HOME / <root>. The agent never
...
# Directory-prefix read denies under MIKASA_HOME / <root>: (subdir, message for
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    stores under MIKASA_HOME and the global root (exact files, plus anything
...
def _resolve_active_profile_name() -> str:
    """Active profile name from mikasa_HOME: ``~/.hermes`` -> ``"default"``,
    ``~/.hermes/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _mikasa_home_path().resolve().relative_to(_hermes_root_path().resolve() / "profiles").parts
...
#   <MIKASA_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.hermes/...
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "authoritative file is likely {inner_path!r} under the real MIKASA_HOME.",
