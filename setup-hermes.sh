...
# ============================================================================
# Mikasa PATH setup — symlink mikasa into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up mikasa command..."

MIKASA_BIN="$SCRIPT_DIR/venv/bin/mikasa"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MIKASA_BIN" "$COMMAND_LINK_DIR/mikasa"
echo -e "${GREEN}✓${NC} Symlinked mikasa → $COMMAND_LINK_DISPLAY_DIR/mikasa"
...
# ============================================================================
# Seed bundled skills into ~/.mikasa/skills/
# ============================================================================

MIKASA_SKILLS_DIR="${MIKASA_HOME:-$HOME/.mikasa}/skills"
mkdir -p "$MIKASA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.mikasa/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MIKASA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  mikasa status        # Check configuration"
if is_termux; then
    echo "  mikasa gateway       # Run gateway in foreground"
else
    echo "  mikasa gateway install # Install gateway service (messaging + cron)"
fi
echo "  mikasa cron list     # View scheduled jobs"
echo "  mikasa doctor        # Diagnose issues"
echo ""
