...
``~/.hermes`` before the per-test ``MIKASA_HOME`` redirect, which makes tests
...
def test_conftest_does_not_import_hermes_state_at_collection():
    ...
    def guard(name, *args, **kwargs):
        if name == "mikasa_state" or name.startswith("mikasa_state."):
            blocked.append(name)
    ...
    saved = sys.modules.pop("mikasa_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["mikasa_state"] = saved
    assert not blocked
