...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    # Some modules cache get_mikasa_home; clear if needed.
    import mikasa_constants
    if hasattr(mikasa_constants, "_mikasa_home_CACHE"):
        mikasa_constants._mikasa_home_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(MIKASA_HOME, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from mikasa_cli.env_loader import _apply_external_secret_sources
