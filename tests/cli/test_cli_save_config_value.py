...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_mikasa_home(), so
        # point MIKASA_HOME at the temp dir (the _mikasa_home import-time
        # constant is no longer consulted).
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        monkeypatch.setattr("cli._mikasa_home", MIKASA_HOME)
        return config_path
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "mikasa_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in MIKASA_HOME/config.yaml
    ...
    install whose MIKASA_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read MIKASA_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh MIKASA_HOME with NO config.yaml (managed/desktop first launch).
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
