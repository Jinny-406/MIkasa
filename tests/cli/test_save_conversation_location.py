...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    # Clear any cached MIKASA_HOME computation
    import mikasa_constants
    if hasattr(mikasa_constants, "_mikasa_home_cache"):
        mikasa_constants._mikasa_home_cache = None
    return home
...
def test_save_conversation_writes_under_mikasa_home(MIKASA_HOME, tmp_path, monkeypatch, capsys):
    ...
    # Import fresh to pick up the MIKASA_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "mikasa_constants"]:
...
def test_save_conversation_empty_history_does_nothing(MIKASA_HOME, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "mikasa_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(MIKASA_HOME, capsys):
...
def test_save_conversation_bad_format_shows_usage(MIKASA_HOME, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "mikasa_constants"]:
        sys.modules.pop(mod, None)
    import cli
