...
def MIKASA_HOME(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("mikasa_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves MIKASA_HOME each test.
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, MIKASA_HOME):
        ...
        with patch("mikasa_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, MIKASA_HOME):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, MIKASA_HOME):
