...
def MIKASA_HOME(tmp_path, monkeypatch):
    """Point MIKASA_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_load_mappings_handles_corrupt_json(MIKASA_HOME):
...
def test_verify_checksums_signature_skips_without_gpg(MIKASA_HOME, monkeypatch, tmp_path):
...
def test_start_proxy_idempotent_when_already_running(MIKASA_HOME, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_subprocess_env_strips_unrelated_secrets(MIKASA_HOME, monkeypatch):
...
def test_ca_key_created_with_0o600(MIKASA_HOME, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(MIKASA_HOME, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(MIKASA_HOME, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_proxy_state_dir_is_0o700(MIKASA_HOME):
    state = ip._proxy_state_dir()
...
def test_mappings_roundtrip_preserves_headers_and_aliases(MIKASA_HOME):
    m = ip.TokenMapping(
...
def test_ensure_management_token_persists_and_is_stable(MIKASA_HOME):
...
def test_reload_proxy_refuses_when_not_running(MIKASA_HOME, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(MIKASA_HOME, monkeypatch):
...
def test_start_proxy_injects_management_key_env(MIKASA_HOME, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=MIKASA_HOME / "ca.crt",
        ca_key=MIKASA_HOME / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (MIKASA_HOME / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = MIKASA_HOME / "bin" / "iron-proxy"
...
def test_docker_egress_node_options_uses_sentinel(MIKASA_HOME, monkeypatch):
...
def test_persisted_nonce_roundtrip(MIKASA_HOME, monkeypatch):
...
def test_get_status_probes_configured_bind_host(MIKASA_HOME, monkeypatch):
