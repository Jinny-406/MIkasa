"""Tests for mikasa_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import mikasa_logging
# Use whatever RotatingFileHandler class mikasa_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. mikasa_logging
...
from mikasa_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    mikasa_logging._logging_initialized = False
    ...
    mikasa_logging._reset_queued_handlers()
    ...
    mikasa_logging._install_session_record_factory()
    yield
    ...
    mikasa_logging._logging_initialized = False
    mikasa_logging.clear_session_context()


@pytest.fixture
def MIKASA_HOME(tmp_path, monkeypatch):
    """Provide an isolated MIKASA_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_mikasa_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["mikasa_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, MIKASA_HOME):
        log_dir = mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)
        assert log_dir == MIKASA_HOME / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)
        root = logging.getLogger()

        agent_handlers = [
            h for h in mikasa_logging._queued_file_handlers
    ...
    def test_idempotent_no_duplicate_handlers(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)

        test_logger = logging.getLogger("test_mikasa_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        mikasa_logging.flush_log_queue()

        agent_log = MIKASA_HOME / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, MIKASA_HOME, tmp_path):
        ...
        profile_home.mkdir()
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)
        assert mikasa_logging.enable_profile_log_routing(
            [MIKASA_HOME, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            MIKASA_HOME / "logs" / "agent.log"
        ).read_text()




    def test_explicit_params_override_config(self, MIKASA_HOME):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (MIKASA_HOME / "config.yaml").write_text(yaml.dump(config))

        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, MIKASA_HOME):
        ...
        gw_log = MIKASA_HOME / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, MIKASA_HOME):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME, mode="gui")

        logging.getLogger("mikasa_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        mikasa_logging.flush_log_queue()

        gui_log = MIKASA_HOME / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, MIKASA_HOME):
        """When session context is set, log lines include [session_id]."""
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)
        mikasa_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = mikasa_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)
        mikasa_logging.setup_verbose_logging()
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        mikasa_logging._add_rotating_handler(
            log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in mikasa_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        mikasa_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("mikasa_cli.config.is_managed", return_value=True):
                mikasa_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = mikasa_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert mikasa_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not mikasa_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, MIKASA_HOME):
        level, max_size, backup = mikasa_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, MIKASA_HOME):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> mikasa_logging._ManagedRotatingFileHandler:
        handler = mikasa_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that mikasa_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        mikasa_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME, mode="gateway")
        gw_path = MIKASA_HOME / "logs" / "gateway.log"
        rotated = MIKASA_HOME / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = mikasa_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, MIKASA_HOME):
        mikasa_logging.setup_logging(MIKASA_HOME=MIKASA_HOME)
        root = logging.getLogger()
        ...
        assert any(
            ...
            for h in mikasa_logging._queued_file_handlers
        )
