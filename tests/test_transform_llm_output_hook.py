...
from mikasa_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(MIKASA_HOME: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <MIKASA_HOME>/plugins/<name> and opt it in."""
    plugin_dir = MIKASA_HOME / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("mikasa_HOME", str(tmp_path / "mikasa_empty"))
