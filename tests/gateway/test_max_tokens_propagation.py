...
def isolated_home(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME with a writable config.yaml and a clean module cache.
    ...
    monkeypatch.delenv("mikasa_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("mikasa_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (MIKASA_HOME / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("mikasa_cli", "gateway")):
                del sys.modules[mod]
