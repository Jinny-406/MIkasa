...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    # get_mikasa_home() prefers the context-local override over the env
    # var, so a set_mikasa_home_override() leaked by ANY earlier test in
...
class TestCliResumeRestartsWork:
    def test_resume_after_budget_exhaustion_queues_continuation(self, MIKASA_HOME):
    ...
    def test_resume_without_goal_queues_nothing(self, MIKASA_HOME):
...
class TestGatewayResumeRestartsWork:
    ...
    @pytest.mark.asyncio
    async def test_resume_without_goal_enqueues_nothing(self, MIKASA_HOME):
        runner, adapter = _make_runner()
