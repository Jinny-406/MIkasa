...
closure the PR changed, against a real temp ``MIKASA_HOME``.
"""
...
def _fake_switch_result():
    ...
    from mikasa_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_mikasa_home() / config.yaml`` — point it here.
    monkeypatch.setattr("mikasa_constants.get_mikasa_home", lambda: MIKASA_HOME)
    monkeypatch.setattr("mikasa_cli.config.get_mikasa_home", lambda: MIKASA_HOME)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_mikasa_home", default_home)
    _stub_picker_dependencies(monkeypatch)
