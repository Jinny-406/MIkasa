...
(/opt/mikasa/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable MIKASA_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_mikasa_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable MIKASA_HOME."""
    ...
    monkeypatch.setattr(
        "mikasa_constants.get_mikasa_home", lambda: MIKASA_HOME
    )
    ...
    expected = MIKASA_HOME / "scripts" / "whatsapp-bridge"
