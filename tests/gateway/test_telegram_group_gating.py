...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="mikasa_bot")
...
class _IdentityBot:
    ...
    def __init__(self, bot_id=999, cached="mikasa_bot", server=None):
        self.id = bot_id
        self._cached = cached
...
def test_bot_never_adopts_another_accounts_username():
    ...
    adapter = _make_adapter(require_mention=True)
    adapter._bot = _IdentityBot(cached="mikasa_bot")
    ...
    assert adapter._current_bot_username() == "mikasa_bot"
