...
os.environ["mikasa_HOME"] = sys.argv[2]
...
def test_named_pipe_identify_status_and_fleet_consumer(live_server, monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.profiles._get_default_mikasa_home", lambda: home)
    monkeypatch.setattr(
        "mikasa_cli.profiles._get_profiles_root", lambda: home / "no-profiles"
    )
