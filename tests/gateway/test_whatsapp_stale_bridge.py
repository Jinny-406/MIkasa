...
class TestCacheDirEnvPassthrough:
    ...
    async def test_bridge_spawn_env_has_cache_dirs(self, tmp_path):
        ...
        assert env["mikasa_IMAGE_CACHE_DIR"] == str(get_image_cache_dir())
        assert env["mikasa_AUDIO_CACHE_DIR"] == str(get_audio_cache_dir())
        assert env["mikasa_DOCUMENT_CACHE_DIR"] == str(get_document_cache_dir())
