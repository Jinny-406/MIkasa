...
import mikasa_startup_watchdog as sw
...
def _isolate(tmp_path, monkeypatch):
    """Every test gets a fresh singleton and its own MIKASA_HOME."""
...
class TestContracts:
    ...
    def test_all_documented_entry_points_arm_the_watchdog(self):
        ...
        arm_sites = {
            ...
            "mikasa_cli/main.py": "argv fast-path (standard `hermes gateway run`)",
            "mikasa_cli/gateway.py": "run_gateway() config-bridge re-arm",
        ...
        for rel, reason in arm_sites.items():
            ...
            aliases = set()
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module == "mikasa_startup_watchdog":
...
class TestProgressLease:
    ...
    def test_schema_init_declares_lease(self):
        """mikasa_state_schema._init_schema must hold a progress lease so
