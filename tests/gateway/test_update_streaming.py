...
def _make_runner(MIKASA_HOME=None):
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.2)
            (MIKASA_HOME / ".update_response").write_text("y")
        ...
        assert not (MIKASA_HOME / ".update_prompt.json").exists()
        assert not (MIKASA_HOME / ".update_response").exists()
...
class TestRestoreStashWithInputFn:
    ...
    def test_uses_input_fn_when_provided(self, tmp_path):
        ...
        from mikasa_cli.update_cmd import _restore_stashed_changes
...
class TestWatchUpdateProgress:
    ...
    async def test_streams_output_to_adapter(self, tmp_path):
        ...
        # Write output
        (MIKASA_HOME / ".update_output.txt").write_text("→ Fetching updates...\n", encoding="utf-8")
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (MIKASA_HOME / ".update_pending.json").write_text(json.dumps(pending))
        (MIKASA_HOME / ".update_output.txt").write_text("output\n")
        ...
        async def simulate_prompt_cycle():
            ...
            (MIKASA_HOME / ".update_prompt.json").write_text(json.dumps(prompt))
            # Simulate user responding
            await asyncio.sleep(0.2)
            (MIKASA_HOME / ".update_response").write_text("y")
            (MIKASA_HOME / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.2)
            (MIKASA_HOME / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_is_recovered_after_watcher_restart(self, tmp_path):
        ...
        with patch("gateway.run._mikasa_home", MIKASA_HOME):
            ...
            assert (MIKASA_HOME / ".update_prompt.json").exists()
...
class TestUpdatePromptInterception:
    ...
    async def test_recognized_slash_command_bypasses_pending_update_prompt(self, tmp_path):
        ...
        (MIKASA_HOME / ".update_prompt.json").write_text(json.dumps({"prompt": "test"}))
        ...
        response_path = MIKASA_HOME / ".update_response"
