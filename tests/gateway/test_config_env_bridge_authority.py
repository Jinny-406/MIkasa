...
def _run_gateway_import(MIKASA_HOME: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "mikasa_AGENT_TIMEOUT",
            "mikasa_AGENT_TIMEOUT_WARNING",
            "mikasa_TURN_LEASE_TIMEOUT",
            "mikasa_SESSION_STALL_TIMEOUT",
            "mikasa_GATEWAY_BUSY_INPUT_MODE",
            "mikasa_GATEWAY_BUSY_TEXT_MODE",
            "mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT",
            "mikasa_TIMEZONE",
        ):
            v = os.environ.get(k)
            if v is not None:
    ...
    env["mikasa_HOME"] = str(MIKASA_HOME)
...
@pytest.fixture
def MIKASA_HOME(tmp_path: Path) -> Path:
    home = tmp_path / ".hermes"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(MIKASA_HOME: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(MIKASA_HOME, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
        "session_stall_timeout": 300,
    })
    _write_env(MIKASA_HOME, {
        "mikasa_AGENT_TIMEOUT": "60",
        "mikasa_AGENT_TIMEOUT_WARNING": "30",
        "mikasa_SESSION_STALL_TIMEOUT": "15",
    })

    env = _run_gateway_import(MIKASA_HOME, initial_env={})

    assert env.get("mikasa_AGENT_TIMEOUT") == "1800"
    assert env.get("mikasa_AGENT_TIMEOUT_WARNING") == "900"
    assert env.get("mikasa_SESSION_STALL_TIMEOUT") == "300"


def test_config_turn_lease_timeout_wins_over_stale_env(MIKASA_HOME: Path) -> None:
    ...
    _write_env(
        MIKASA_HOME,
        {"mikasa_TURN_LEASE_TIMEOUT": "60"},
    )

    env = _run_gateway_import(MIKASA_HOME, initial_env={})

    assert env.get("mikasa_TURN_LEASE_TIMEOUT") == "600"
...
def test_default_turn_lease_timeout_overrides_stale_env_when_key_is_omitted(
    ...
    assert env.get("mikasa_TURN_LEASE_TIMEOUT") == "5"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(MIKASA_HOME: Path) -> None:
    ...
    _write_config(MIKASA_HOME, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(MIKASA_HOME, initial_env={})

    assert env.get("mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(MIKASA_HOME: Path) -> None:
    ...
    env = _run_gateway_import(
        MIKASA_HOME,
        initial_env={"mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
