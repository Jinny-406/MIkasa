...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    MIKASA_HOME.mkdir()
    monkeypatch.setattr(base, "_mikasa_home", MIKASA_HOME)
    monkeypatch.setattr(base, "_HERMES_ROOT", MIKASA_HOME)
    path = MIKASA_HOME / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("mikasa_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("mikasa_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("mikasa_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("mikasa_KANBAN_HOME", str(tmp_path / "hermes"))
        monkeypatch.setenv("mikasa_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("mikasa_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("mikasa_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("mikasa_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        ...
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._mikasa_home",
            hermes_dir,
        )
        monkeypatch.setattr(
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the MIKASA_HOME root (google_token.json)
        ...
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._mikasa_home", hermes_dir)
    ...
    def test_denylist_blocks_non_cache_file_under_mikasa_home(self, tmp_path, monkeypatch):
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose MIKASA_HOME is symlinked
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        ...
        home copy."""
        MIKASA_HOME = tmp_path / ".hermes"
        cache = MIKASA_HOME / "cache" / "images"
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "mikasa_HUMAN_DELAY_MODE": "natural",
            "mikasa_HUMAN_DELAY_MIN_MS": "oops",
            "mikasa_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_tolerates_malformed_env_vars(self):
        env = {
            "mikasa_HUMAN_DELAY_MODE": "custom",
            "mikasa_HUMAN_DELAY_MIN_MS": "oops",
            "mikasa_HUMAN_DELAY_MAX_MS": "still-bad",
        }
        with patch.dict(os.environ, env):
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"mikasa_MEDIA_DELIVERY_STRICT": "1",
                                     "mikasa_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_MEDIA_DELIVERY_STRICT", "0")
