...
def test_session_store_default_db_uses_runtime_mikasa_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime MIKASA_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    hermes_state before a fixture redirected MIKASA_HOME used to pin every
    default SessionDB() at the developer's real ~/.hermes/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_mikasa_home"
    fake_home.mkdir()
    monkeypatch.setenv("mikasa_HOME", str(fake_home))
