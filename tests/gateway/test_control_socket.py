...
def test_default_identify_payload_shape(home: Path, monkeypatch):
    ...
    for key in ("mikasa_home", "supervisor", "kind", "start_time"):
        assert key in ident
...
def test_collect_fleet_versions_prefers_socket(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.profiles._get_profiles_root", lambda: tmp_path / "no-profiles"
    )
...
def test_runtime_inventory_dedupes_same_pid_across_homes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.profiles._get_profiles_root", lambda: profiles_root
    )
    monkeypatch.setattr(
        "mikasa_cli.gateway._get_service_pids", lambda all_profiles=False: set()
    )
    monkeypatch.setattr(
        "mikasa_cli.gateway.find_profile_gateway_processes", lambda: []
    )
    monkeypatch.setattr(
        "gateway.control_socket.identify_gateway",
