...
async def test_gateway_boot_discovers_mcp_under_every_profile_home(
    ...
    def fake_discover() -> list[str]:
        seen.append((get_mikasa_home(), threading.current_thread().name))
        return []
...
async def test_reload_mcp_only_touches_requesting_profile(
    ...
    worker_scope = MIKASA_HOME_key(worker_home)
    ...
    monkeypatch.setattr(
        ...
        {"default-srv": MIKASA_HOME_key(tmp_path), "worker-srv": worker_scope},
    )
    seen: list[tuple] = []

    def fake_shutdown(*, scope=None) -> None:
        seen.append(("shutdown", scope, get_mikasa_home()))

    def fake_discover() -> list[str]:
        seen.append(("discover", get_mikasa_home()))
        return []
...
def test_deregister_scope_kwarg_targets_overlay_and_keeps_plugin_confinement() -> None:
    ...
    reg._plugin_module_scopes["mikasa_plugins.p"] = {"/home/p1"}
    reg._caller_module = staticmethod(lambda: "mikasa_plugins.p")
