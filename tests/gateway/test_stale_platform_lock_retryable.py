...
``gateway run --replace`` only kills same-MIKASA_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "mikasa_home": "/opt/data/profiles/lead-gen-outreach",
    }
...
def test_lock_conflict_infers_profile_from_legacy_mikasa_home(adapter):
    """Locks written before the profile field existed still attribute via MIKASA_HOME."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "mikasa_home": "/opt/data/profiles/lead-gen-outreach",
