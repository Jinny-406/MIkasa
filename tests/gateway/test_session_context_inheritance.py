...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "mikasa_SESSION_CHAT_ID": env.get("mikasa_SESSION_CHAT_ID"),
        "mikasa_SESSION_THREAD_ID": env.get("mikasa_SESSION_THREAD_ID"),
        "mikasa_SESSION_KEY": env.get("mikasa_SESSION_KEY"),
    }
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("mikasa_SESSION_CHAT_ID", "mikasa_SESSION_THREAD_ID", "mikasa_SESSION_KEY"):
    ...
    assert captured["bound"]["mikasa_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["mikasa_SESSION_KEY"] == FOREIGN["session_key"]
