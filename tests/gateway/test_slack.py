...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "mikasa_feedback" in created_apps[0].registered_actions
        assert "mikasa_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("mikasa_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("mikasa_clarify_choice")
            for pattern in clarify_choice_patterns
        )
