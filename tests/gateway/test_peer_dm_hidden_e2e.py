...
SQLite ``state.db`` (its own tmp MIKASA_HOME) containing a hidden
...
from mikasa_cli.subcommands import peer as peer_cmd
...
def peer_gateway(tmp_path, monkeypatch):
    """A real api_server gateway (own MIKASA_HOME + state.db) on a real socket."""
