...
class TestRuntimeProviderUsesScope:
    """mikasa_cli.runtime_provider._getenv resolves through the secret scope."""


    def test_getenv_two_profiles_isolated(self, monkeypatch):
        from mikasa_cli.runtime_provider import _getenv
...
def test_turn_scoped_dotenv_reload_does_not_pollute_process_env(tmp_path, monkeypatch):
    ...
    ``load_mikasa_dotenv`` has several lazy-import and cron call sites beyond
    ...
    with _profile_runtime_scope(profile_a):
        ...
        assert load_mikasa_dotenv(MIKASA_HOME=get_mikasa_home()) == []
