...
class TestOrgPullIsWiredIn:
    ...
    def test_no_internal_jargon_in_user_facing_strings(self):
        ...
        targets = [
            root / "mikasa_cli" / "subcommands" / "sync.py",
            root / "mikasa_cli" / "subcommands" / "skills.py",
        ]
        banned = re.compile(
...
class TestSkillSyncIsOneCommand:
    ...
    def test_sync_usage_lists_propose(self):
        main_src = self._src("mikasa_cli", "main_platform_setup.py")
...
class TestLocalEditsSurviveOrgUpdates:
    ...
    def test_auto_propose_defaults_off(self, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_SYNC_ORG_AUTO_PROPOSE", raising=False)
        monkeypatch.setattr(
            "mikasa_cli.config.load_config", lambda: {}, raising=False
        )
    ...
    def test_auto_propose_can_be_enabled_by_env(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_SYNC_ORG_AUTO_PROPOSE", "1")
