...
Real imports, real temp MIKASA_HOME root + named profile, real auth.json I/O.
...
def fleet(tmp_path, monkeypatch):
    """Root MIKASA_HOME with an expired-but-refreshable Anthropic pool row."""
    ...
    mikasa_constants._default_hermes_root_memo = None  # type: ignore[attr-defined]
    ...
    import mikasa_cli.auth as _auth_mod
...
def test_borrower_prune_never_deletes_root_singleton_grant(fleet, tmp_path):
    ...
    assert [e.source for e in root_rows] == ["mikasa_pkce"]

    kid = _profile(fleet, "kid")
    fleet["use"](kid)
    pool = load_pool("anthropic")
    assert [e.source for e in pool.entries()] == ["mikasa_pkce"], "borrowed root grant was pruned"
    assert fleet["rows"](root) and fleet["rows"](root)[0]["source"] == "mikasa_pkce"
...
def test_heal_consolidates_existing_forks_to_the_live_copy(fleet, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="mikasa_cli.auth"):
        fleet["use"](forge)
        sel = load_pool("anthropic").select()
...
def test_heal_is_idempotent_and_logs_once(fleet, caplog):
    import logging
    from agent.credential_pool import load_pool
    from mikasa_cli.auth import consume_oauth_heal_notices, heal_forked_single_use_oauth_grants
...
def test_heal_is_a_noop_in_classic_mode(fleet):
    from mikasa_cli.auth import heal_forked_single_use_oauth_grants
    fleet["use"](fleet["root"])
