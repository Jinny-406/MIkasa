...
thread that did not bind the MIKASA_HOME ContextVar.
...
active-profile line resolved the home via the ambient ``get_mikasa_home()``,
...
def test_skills_prompt_scoped_to_override_not_ambient_home(tmp_path, monkeypatch):
    """An explicit skills_dir_override wins over ambient MIKASA_HOME, on a
    ...
    def build():
        # No set_mikasa_home_override on THIS thread — ambient resolves to
...
def test_profile_name_correct_on_bound_profile_session(tmp_path, monkeypatch):
    ...
    ``get_mikasa_home()/profiles`` as the root would never match and every
    profile would misreport as \"default\". The name must derive from the
    hermes ROOT (get_default_mikasa_root)."""
    from agent import system_prompt

    bot_home = tmp_path / "profiles" / "mybot"
    bot_home.mkdir(parents=True)

    # Bound session: MIKASA_HOME env points at the profile dir itself.
    monkeypatch.setenv("mikasa_HOME", str(bot_home))
...
def test_profile_name_correct_when_ambient_is_another_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("mikasa_HOME", str(other_home))
