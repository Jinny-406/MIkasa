...
def MIKASA_HOME(tmp_path, monkeypatch):
    """Real on-disk MIKASA_HOME so ``load_pool()`` re-reads what it persisted."""
...
def test_persisted_claude_code_row_carries_no_token_material(
    MIKASA_HOME, claude_credentials
):
    ...
    rows = [r for r in _persisted_rows(MIKASA_HOME) if r.get("source") == "claude_code"]
    assert len(rows) == 1
...
def test_pool_store_sync_never_adopts_a_borrowed_row(MIKASA_HOME, claude_credentials):
...
def test_empty_oauth_entry_is_never_leased(MIKASA_HOME, claude_credentials):
