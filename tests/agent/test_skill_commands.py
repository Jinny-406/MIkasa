...
class TestScanSkillCommands:
    ...
    def test_get_skill_commands_rescans_when_platform_scope_changes(self, tmp_path):
        ...
        def _disabled_skills():
            platform = os.getenv("mikasa_PLATFORM")
        ...
        with (
            ...
            with patch.dict(os.environ, {"mikasa_PLATFORM": "telegram"}):
            ...
            with patch.dict(os.environ, {"mikasa_PLATFORM": "discord"}):
    ...
    def test_get_skill_commands_rescans_when_session_platform_changes(self, tmp_path):
        ...
        def _disabled_skills():
            platform = (
                os.getenv("mikasa_PLATFORM")
                or get_session_env("mikasa_SESSION_PLATFORM")
            )
            if platform == "telegram":
                return {"telegram-only"}
            if platform == "discord":
    ...
    def test_get_skill_commands_rescans_when_profile_home_changes(self, tmp_path):
        ...
        the same platform scope, so only ``MIKASA_HOME`` moves. Each profile
        ...
        with (
            ...
            token = set_mikasa_home_override(profile_a)
            try:
    ...
    def test_get_skill_commands_rescans_when_leaving_platform_scope(self, tmp_path, monkeypatch):
        ...
        def _disabled_skills():
            if os.getenv("mikasa_PLATFORM") == "telegram":
                return {"telegram-only"}
            return set()
