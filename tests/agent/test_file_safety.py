...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = MIKASA_HOME / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._mikasa_home_path", return_value=MIKASA_HOME):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        hub = MIKASA_HOME / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_mikasa_home(self, tmp_path):
        """The env basename guard does not depend on MIKASA_HOME resolution."""
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        cache = MIKASA_HOME / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
