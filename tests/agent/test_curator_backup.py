...
def backup_env(monkeypatch, tmp_path):
    """Isolate MIKASA_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_mikasa_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd mikasa_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under MIKASA_HOME. Returns the path.
