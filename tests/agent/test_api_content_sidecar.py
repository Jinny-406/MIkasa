...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("mikasa_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated MIKASA_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="mikasa_api_content_")
    os.makedirs(os.path.join(test_home, ".hermes"))
    prev_home = os.environ.get("mikasa_HOME")
    os.environ["mikasa_HOME"] = os.path.join(test_home, ".hermes")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["mikasa_HOME"] = prev_home
