...
from mikasa_cli import auth as A
...
def test_hermes_pkce_refresh_writes_back_to_singleton(tmp_path, monkeypatch):
    ...
    oauth_file = MIKASA_HOME / ".anthropic_oauth.json"
    oauth_file.write_text(
        json.dumps({"accessToken": "sk-ant-oat-rt0", "refreshToken": "rt0", "expiresAt": 0}),
        encoding="utf-8",
    )
    _write_store(MIKASA_HOME / "auth.json", {"version": 1, "providers": {}})
    ...
    reloaded_entries = [e for e in reloaded.entries() if e.source.endswith("mikasa_pkce")]
    assert reloaded_entries, "mikasa_pkce entry should still be present after reload"
