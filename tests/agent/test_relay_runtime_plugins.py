...
def test_legacy_exporter_env_without_plugins_toml_warns_and_stays_disabled(
    ...
    monkeypatch.delenv(relay_runtime.RELAY_PLUGINS_CONFIG_ENV, raising=False)
    monkeypatch.setenv("mikasa_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("mikasa_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S", "30")
    relay = _FakeRelay()
    ...
    try:
        ...
        assert "mikasa_NEMO_RELAY_ATOF_ENABLED" in caplog.text
        assert "mikasa_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S" in caplog.text
    finally:
        host.shutdown()
