...
class TestLocalStreamReadTimeout:
    ...
    def test_local_endpoint_bumps_read_timeout(self, base_url):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("mikasa_STREAM_READ_TIMEOUT", None)
            _base_timeout = float(os.getenv("mikasa_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("mikasa_STREAM_READ_TIMEOUT", 120.0))
    ...
    def test_user_override_respected_for_local(self):
        ...
        with patch.dict(os.environ, {"mikasa_STREAM_READ_TIMEOUT": "300"}, clear=False):
            _base_timeout = float(os.getenv("mikasa_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("mikasa_STREAM_READ_TIMEOUT", 120.0))
