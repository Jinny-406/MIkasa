...
class CopilotACPClientSafetyTests(unittest.TestCase):
    ...
    def test_write_text_file_respects_safe_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"mikasa_WRITE_SAFE_ROOT": str(safe_root)}, clear=False):
                response = self._dispatch(
                    {
                        "jsonrpc": "2.0",
                        "id": 5,
        ...
        self.assertIn("mikasa_WRITE_SAFE_ROOT", str(response["error"]))
...
def test_run_prompt_preserves_real_home_when_profile_home_available(monkeypatch, tmp_path):
    ...
    (MIKASA_HOME / "home").mkdir(parents=True)
    ...
    assert captured["kwargs"]["env"]["mikasa_REAL_HOME"] == str(real_home)
