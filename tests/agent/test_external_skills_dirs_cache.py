...
@pytest.fixture
def MIKASA_HOME_with_config(tmp_path, monkeypatch):
...
def test_cache_invalidates_on_mtime_change(MIKASA_HOME_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = MIKASA_HOME_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different MIKASA_HOMEs keep separate cache entries."""
