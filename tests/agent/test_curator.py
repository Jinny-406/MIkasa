...
def curator_env(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from mikasa_HOME at write time, so a
    ...
    # thread here, while MIKASA_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from mikasa_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from mikasa_cli.main_provider_setup import _AUX_TASKS
    from mikasa_cli.web_server_config import _AUX_TASK_SLOTS
