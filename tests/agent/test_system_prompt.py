...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_mikasa_home``,
    ``get_default_mikasa_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``MIKASA_HOME`` and no resolver mocks.
    """

    def test_real_mikasa_home_under_profiles_renders_correct_paths(
        ...
        assert get_default_mikasa_root() == root
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """mikasa_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".hermes"
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "mikasa_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "mikasa_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_mikasa_home", lambda: Path("/hermes"))

    # Production renders this as str(get_mikasa_home()) + "/profiles/<name>/",
    ...
    with (
        ...
        patch("mikasa_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("mikasa_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under MIKASA_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from mikasa_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("mikasa_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from mikasa_time import now as hermes_now
