...
from mikasa_cli.observability.shared_metrics_contract import MODEL_ROUTE_METRIC
from mikasa_cli.observability.shared_metrics_subscriber import SharedMetricsSubscriber


@pytest.fixture()
def relay_turn(tmp_path, monkeypatch):
    monkeypatch.setenv("mikasa_HOME", str(tmp_path / "profile"))
...
def test_auxiliary_provider_fallback_records_one_terminal_model_route(
    ...
    assert snapshot[0]["resource"]["mikasa_version"] == "test-version"
