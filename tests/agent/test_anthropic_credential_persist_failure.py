...
def MIKASA_HOME(tmp_path, monkeypatch):
    """Real on-disk MIKASA_HOME so ``load_pool()`` re-reads what we persisted."""
    ...
    monkeypatch.setattr(
        "mikasa_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_hermes_oauth_writer_raises_instead_of_swallowing(MIKASA_HOME, monkeypatch):
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    MIKASA_HOME, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
def test_pool_hermes_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "mikasa_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
