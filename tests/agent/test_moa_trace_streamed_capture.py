...
with real file I/O against a temp MIKASA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point MIKASA_HOME at a temp dir and turn moa.save_traces on."""
    MIKASA_HOME = tmp_path / ".hermes"
    MIKASA_HOME.mkdir()
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))

    # save_moa_turn reads config via mikasa_cli.config.load_config; stub it to
