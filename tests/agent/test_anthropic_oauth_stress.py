...
def _process_claude_code_refresh_worker(
    ...
    os.environ["mikasa_HOME"] = profile_home
...
def MIKASA_HOME(tmp_path, monkeypatch):
    """Real, throwaway MIKASA_HOME so _auth_store_lock and
...
def test_distinct_profiles_share_one_claude_refresh_without_duplicate_post(
    ...
    """
    shared_credentials_path = MIKASA_HOME / "shared-claude-credentials.json"
    ...
    server_state_path = MIKASA_HOME / "fake-token-server.json"
    server_state_path.write_text(
        json.dumps({"calls": [], "spent": [], "rotation": 0}),
        encoding="utf-8",
    )

    profile_homes = [MIKASA_HOME / "profile-a", MIKASA_HOME / "profile-b"]
