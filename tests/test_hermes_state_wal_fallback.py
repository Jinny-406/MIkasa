...
Without this fallback, users on NFS-mounted ``MIKASA_HOME`` silently lose
...
from mikasa_state import SessionDB, format_session_db_unavailable, get_last_init_error
from mikasa_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("mikasa_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
