...
from mikasa_cli import proxy_cli


@pytest.fixture
def MIKASA_HOME(tmp_path, monkeypatch):
    """Point MIKASA_HOME at a temp dir so the wizard doesn't touch the
...
def test_cmd_install_success_returns_0(MIKASA_HOME, monkeypatch):
    monkeypatch.setattr(ip, "install_iron_proxy", lambda **kw: MIKASA_HOME / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "v0.39.0-test")
    rc = proxy_cli.cmd_install(_args())
    assert rc == 0


def test_cmd_install_failure_returns_1(MIKASA_HOME, monkeypatch):
    def boom(**kw):
...
def test_cmd_setup_from_bitwarden_refuses_on_empty_vault(MIKASA_HOME, monkeypatch):
    ...
    monkeypatch.setattr(ip, "find_iron_proxy", lambda **kw: MIKASA_HOME / "iron-proxy")
    monkeypatch.setattr(ip, "iron_proxy_version", lambda b: "test")
    monkeypatch.setattr(
        ip, "ensure_ca_cert",
        lambda **kw: (MIKASA_HOME / "ca.crt", MIKASA_HOME / "ca.key"),
    )
...
def test_cmd_restart_propagates_start_failure(MIKASA_HOME, monkeypatch):
...
def test_cmd_status_returns_0(MIKASA_HOME, monkeypatch):
...
def test_cmd_disable_uses_public_status_pid_not_private_read_pid(
    ...
    # mutation only.
    from mikasa_cli.config import load_config as _lc
    cfg2 = _lc()
    assert cfg2["proxy"]["enabled"] is False


def test_cmd_config_returns_0_when_present(MIKASA_HOME, monkeypatch):
    fake = ip.ProxyStatus()
    fake.config_path = MIKASA_HOME / "proxy.yaml"
...
def test_cmd_start_refuses_when_bitwarden_mode_but_disabled(MIKASA_HOME, monkeypatch):
...
def test_cmd_setup_audit_log_failure_is_warning_not_abort(MIKASA_HOME, monkeypatch):
