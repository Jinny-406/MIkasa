...
CASES = {
    ...
    "product python → nix": (["mikasa_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every mikasa_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["mikasa_state.py"], _lanes(python=True, scan=True)),
