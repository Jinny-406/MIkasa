...
from mikasa_cli.config import ensure_mikasa_home, get_env_path, load_env
from mikasa_cli.memory_setup import _write_env_vars
...
def test_denylisted_key_does_not_block_other_writes(capsys):
    ...
    _write_env_vars({
        ...
        "mikasa_LANGFUSE_PUBLIC_KEY": "pk-test-123",
        "OPENROUTER_API_KEY": "sk-or-test-456",
    })

    assert "LD_PRELOAD" not in _env_file_keys()
    env = load_env()
    assert env["mikasa_LANGFUSE_PUBLIC_KEY"] == "pk-test-123"
...
def test_legitimate_hermes_integration_key_still_writable():
    ...
    the convention."""
    _write_env_vars({
        "mikasa_LANGFUSE_PUBLIC_KEY": "pk-lf-789",
        "mikasa_QWEN_BASE_URL": "https://example.com/v1",
    })

    env = load_env()
    assert env["mikasa_LANGFUSE_PUBLIC_KEY"] == "pk-lf-789"
    assert env["mikasa_QWEN_BASE_URL"] == "https://example.com/v1"
...
def test_explicit_mikasa_home_writes_to_that_env_file(tmp_path):
    ...
    _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"}, MIKASA_HOME=home)
