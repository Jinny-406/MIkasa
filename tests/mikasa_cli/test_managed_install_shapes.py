...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("mikasa_MANAGED", raising=False)
    return home
...
def test_env_var_names_the_managing_system(MIKASA_HOME, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("mikasa_MANAGED", raising=False)
    else:
        monkeypatch.setenv("mikasa_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    MIKASA_HOME, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the HERMES_MANAGED of the service."""
    (MIKASA_HOME / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("mikasa_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(MIKASA_HOME, monkeypatch):
    (MIKASA_HOME / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("mikasa_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    MIKASA_HOME, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("mikasa_MANAGED", managed_value)
...
def test_unmanaged_install_offers_no_update_command(MIKASA_HOME, monkeypatch):
    monkeypatch.delenv("mikasa_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(MIKASA_HOME, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = MIKASA_HOME / ".managed"
