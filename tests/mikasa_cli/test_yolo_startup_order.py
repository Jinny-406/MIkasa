...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("mikasa_YOLO_MODE")

    monkeypatch.setattr(
        "mikasa_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("mikasa_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("mikasa_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from mikasa_cli.main import main as cli_main
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    ...
    assert result == "1", (
        "mikasa_YOLO_MODE was not '1' when _prepare_agent_startup was "
