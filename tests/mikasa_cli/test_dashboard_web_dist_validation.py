...
@pytest.fixture()
def main_mod():
    import mikasa_cli.main as main
    return main
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "mikasa_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "mikasa_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )


def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """mikasa_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("mikasa_WEB_DIST", str(empty_dist))

    started = []
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.web_server",
        types.SimpleNamespace(start_server=lambda **k: started.append(k)),
    )
    builds = []
    monkeypatch.setattr(
        "mikasa_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "mikasa_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("mikasa_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "mikasa_cli" / "web_dist"
    ...
    monkeypatch.setattr("mikasa_cli.main_web_build._build_web_ui", fake_build)
