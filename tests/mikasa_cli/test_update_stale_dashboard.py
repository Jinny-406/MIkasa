...
from mikasa_cli.main_dashboard import _find_stale_dashboard_pids
from mikasa_cli.dashboard_procs import _kill_stale_dashboard_processes
from mikasa_cli import dashboard_procs
from mikasa_cli import main_dashboard
from mikasa_cli import update_cmd
from mikasa_cli.update_cmd import _finish_dashboard_update_cleanup
from mikasa_cli.main_dashboard import _restart_managed_dashboard_service
from mikasa_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "hermesPath": "/opt/mikasa/bin/hermes",
    ...
    monkeypatch.delenv("mikasa_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "mikasa_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m mikasa_cli.main dashboard"),
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("mikasa_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("mikasa_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("mikasa_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("mikasa_cli._subprocess_compat.pid_is_hermes", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "mikasa_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("mikasa_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m mikasa_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("mikasa_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("mikasa_cli.dashboard_procs._mikasa_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        live = self._live()
        monkeypatch.setenv("mikasa_HOME", str(tmp_path / ".hermes"))
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from mikasa_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_dedupes_identical_normalized_cmdlines(self):
        ...
        a = ["/usr/bin/python3", "-m", "mikasa_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "mikasa_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_mikasa_home(self):
        from mikasa_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another MIKASA_HOME is never respawned (#94030).
    ...
    def test_matching_mikasa_home_is_kept(self):
        from mikasa_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_symlinked_mikasa_home_compares_equal(self, tmp_path):
        from mikasa_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable MIKASA_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from mikasa_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_mikasa_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mikasa_constants, "get_mikasa_home", lambda: Path("/home/u/.hermes")
        )
        argv = ["hermes", "serve", "--port", "9118"]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00mikasa_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "mikasa_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``from mikasa_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("mikasa_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import mikasa_cli._subprocess_compat as compat
        from mikasa_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["mikasa_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["mikasa_cli._subprocess_compat"])
            importlib.reload(sys.modules["mikasa_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "mikasa_cli._subprocess_compat" in reloaded
        assert "mikasa_cli.dashboard_procs" in reloaded
