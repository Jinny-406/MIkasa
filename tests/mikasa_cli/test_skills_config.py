...
class TestSaveDisabledSkills:
    @patch("mikasa_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("mikasa_cli.config.load_config")
    ...
    @patch("mikasa_cli.config.load_config")
    @patch.dict("os.environ", {"mikasa_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """mikasa_SESSION_PLATFORM should be used when HERMES_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_hermes_platform_takes_precedence(self, tmp_path, monkeypatch):
        """mikasa_PLATFORM should win over HERMES_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("mikasa_HOME", str(tmp_path))
        monkeypatch.setenv("mikasa_PLATFORM", "telegram")
        monkeypatch.setenv("mikasa_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from mikasa_cli.skills_config import _get_categories
        skills = [
