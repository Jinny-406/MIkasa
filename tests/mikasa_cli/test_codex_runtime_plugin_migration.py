...
from mikasa_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from mikasa_cli import codex_runtime_plugin_migration as crpm
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "mikasa_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: MIKASA_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``MIKASA_HOME``
    ...
    def test_real_mikasa_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate MIKASA_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("mikasa_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("mikasa_HOME") == real_path

    def test_unset_mikasa_home_omits_env_key(self, monkeypatch):
        """When MIKASA_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever MIKASA_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert "mikasa_HOME" not in env, (
            f"mikasa_HOME should not be set when env var is unset, got: "
            f"{env.get('mikasa_HOME')!r}"
        )
