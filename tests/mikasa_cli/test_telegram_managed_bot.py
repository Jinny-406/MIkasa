...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "mikasa_abcdefghijklmnop_bot",
        ...
        with patch(
            "mikasa_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "mikasa_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "mikasa_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("mikasa_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("mikasa_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("mikasa_cli.telegram_managed_bot.time.sleep"):
                result = poll_for_setup_result(
...
class TestSetupTelegramAuto:
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from mikasa_cli import setup

        seen = {}
        profile_home = tmp_path / ".hermes" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_mikasa_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "mikasa_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from mikasa_cli.setup_platforms import _profile_name_from_mikasa_home

        assert (
            _profile_name_from_mikasa_home(
