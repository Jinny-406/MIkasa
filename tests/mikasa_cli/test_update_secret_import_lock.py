...
""",
    ...
    dispatch = (
        "mikasa_main.cmd_update = lambda _args: 0\n"
        "mikasa_main.main()\n"
        if run_main
        else ""
    )
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import mikasa_cli.main as hermes_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "mikasa_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_dotenv_loading_is_preserved_when_external_secrets_are_skipped(
    ...
    loaded = env_loader.load_mikasa_dotenv(
        MIKASA_HOME=home,
