...
import mikasa_state
import mikasa_state_wal
from mikasa_cli import kanban_db as kb
from mikasa_cli import kanban_db_connect as kbc
from mikasa_cli import kanban_db_dispatch as kbd
from mikasa_cli import kanban_db_workspace as kbw


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import mikasa_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import mikasa_cli.kanban_db as _kb
    from mikasa_cli import kanban_db_dispatch as _kbd

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("mikasa_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("mikasa_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
# where `kanban_db_path()` resolved to the active profile's MIKASA_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's MIKASA_HOME."""

    def _set_home(self, monkeypatch, tmp_path, MIKASA_HOME):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        monkeypatch.delenv("mikasa_KANBAN_HOME", raising=False)
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # MIKASA_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile MIKASA_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        env = captured["env"]
        assert env["mikasa_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["mikasa_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["mikasa_KANBAN_TASK"] == "t_dispatch_env"
        assert env["mikasa_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "mikasa_SESSION_SOURCE":
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import mikasa_state_wal as _hermes_state_wal
    monkeypatch.setattr(
    ...
    with _patch("mikasa_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="mikasa_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "mikasa_cli.kanban_db.sqlite3.connect",
...
def test_sqlite_connect_closes_tracked_conn_on_setup_failure(tmp_path, monkeypatch):
    ...
    from mikasa_cli import sqlite_safe_read
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from mikasa_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m mikasa_cli.main`.
    ...
    import shutil
    import sys
    import mikasa_cli.kanban_db as kb
    from mikasa_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("mikasa_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kbd._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "mikasa_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `mikasa_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m mikasa_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("mikasa_BIN", None)
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="mikasa_cli.kanban_db"):
        with kbc.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from mikasa_cli.kanban_db_connect import connect
    from mikasa_cli.sqlite_safe_read import file_length_matches_header
