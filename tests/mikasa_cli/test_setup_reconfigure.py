...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("mikasa_cli.setup.ensure_mikasa_home", {}),
        ("mikasa_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("mikasa_cli.config.is_managed", {"return_value": False}),
        ("mikasa_cli.setup.load_config", {"return_value": {}}),
        ("mikasa_cli.setup.save_config", {}),
        ("mikasa_cli.setup.get_env_value", {"return_value": None}),
        ("mikasa_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("mikasa_cli.setup._print_setup_summary", {}),
        ("mikasa_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("mikasa_cli.setup.load_config", {"return_value": {}}),
        ("mikasa_cli.setup.save_config", {}),
        ("mikasa_cli.auth.get_active_provider", {"return_value": None}),
        ("mikasa_cli.setup.get_env_value", {"return_value": None}),
        ("mikasa_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="mikasa_cli.setup.prompt_choice",
                quick="mikasa_cli.setup._run_quick_setup",
                model="mikasa_cli.setup.setup_model_provider",
                terminal="mikasa_cli.setup.setup_terminal_backend",
                agent="mikasa_cli.setup.setup_agent_settings",
                gateway="mikasa_cli.setup.setup_gateway",
                tools="mikasa_cli.setup.setup_tools",
            )
            from mikasa_cli.setup import run_setup_wizard
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from mikasa_cli.setup import run_setup_wizard
            from mikasa_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("mikasa_cli.setup.prompt_choice", {"return_value": 0}),
                first="mikasa_cli.setup_quick._run_first_time_quick_setup",
            )
            from mikasa_cli.setup import run_setup_wizard
            from mikasa_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("mikasa_cli.setup.prompt_choice", {"return_value": 2}),
                blank="mikasa_cli.setup_quick._run_blank_slate_setup",
            )
            from mikasa_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "mikasa_cli.setup.run_setup_wizard",
