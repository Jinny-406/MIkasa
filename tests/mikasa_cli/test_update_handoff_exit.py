...
def _run_cmd_update(monkeypatch, impl, *, reexec: bool):
    ...
    monkeypatch.setattr("mikasa_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("mikasa_cli.update_lock.UpdateLock", lambda: _FakeLock(events["order"]))
    ...
    monkeypatch.setattr("mikasa_cli.update_receipt.finalize_pending_update_receipt", fake_receipt)
    monkeypatch.setattr("os._exit", fake_exit)
    if reexec:
        monkeypatch.setenv("mikasa_UPDATE_REEXEC", "1")
    else:
        monkeypatch.delenv("mikasa_UPDATE_REEXEC", raising=False)
