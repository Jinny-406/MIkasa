...
mikasa_cli.config._load_config_impl, so the managed-scope merge has to be
...
def _load_cli_config(home):
    ...
    cli.py binds ``_mikasa_home = get_mikasa_home()`` at import time (module
    singleton), so monkeypatching MIKASA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap MIKASA_HOME.)
    """
    import cli

    cli._mikasa_home = home
