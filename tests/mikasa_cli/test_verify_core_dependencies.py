...
def temp_pyproject(tmp_path, monkeypatch):
    """Point mikasa_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("mikasa_cli.main_install_repair._resolve_install_target_python", return_value=py), \
             patch("mikasa_cli.main_install_repair.subprocess.run", side_effect=fake_subprocess_run), \
             patch("mikasa_cli.main_install_repair._run_install_with_heartbeat"):

            from mikasa_cli.main_install_repair import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("mikasa_cli.main_install_repair._resolve_install_target_python") as mock_resolve, \
             patch("mikasa_cli.main_install_repair._run_install_with_heartbeat") as mock_install:
            from mikasa_cli.main_install_repair import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        with patch("mikasa_cli.main_install_repair._is_windows", return_value=True):
            from mikasa_cli.main_install_repair import _resolve_install_target_python
