...
def test_presets_stale_when_a_staged_model_has_no_section(MIKASA_HOME):
    from mikasa_cli.local_runtime.bootstrap import _presets_stale

    _stage(MIKASA_HOME, "model-a")
    _stage(MIKASA_HOME, "model-b")
    _write_presets(MIKASA_HOME, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(MIKASA_HOME):
    from mikasa_cli.local_runtime.bootstrap import _presets_stale
...
def test_no_models_is_never_stale(MIKASA_HOME):
    from mikasa_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(MIKASA_HOME, monkeypatch):
    ...
    import mikasa_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "mikasa_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "mikasa_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(MIKASA_HOME, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(MIKASA_HOME, monkeypatch):
    import mikasa_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "mikasa_cli.local_runtime.endpoint._state_endpoint", lambda: None)
