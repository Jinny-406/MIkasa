...
from mikasa_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    MIKASA_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def MIKASA_HOME(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("mikasa_cli.config") or mod == "mikasa_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, MIKASA_HOME, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, MIKASA_HOME, capsys, monkeypatch):
