...
from mikasa_cli.config import DEFAULT_CONFIG, load_config
from mikasa_cli.main_provider_setup import _AUX_TASKS, _DELEGATION_TASK_KEY, _delegation_cfg_as_task, _format_aux_current, _reset_aux_to_auto, _save_aux_choice
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from mikasa_cli.config import load_config as _lc, save_config
...
def test_leave_unchanged_replaces_cancel_label(tmp_path, monkeypatch):
    ...
    from mikasa_cli import main as main_mod
