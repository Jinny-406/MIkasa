...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from mikasa_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("mikasa_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "mikasa_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["hermes", "chat", "--provider", "gmi"])

        from mikasa_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("mikasa_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("mikasa_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("mikasa_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            from mikasa_cli.model_setup_flows import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_mikasa_home() / "config.yaml").read_text()) or {}
