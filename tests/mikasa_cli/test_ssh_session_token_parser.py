...
from mikasa_cli.main import cmd_dashboard
from mikasa_cli.main_dashboard import _read_ssh_session_token_file
...
def test_token_anchor_is_os_home_not_active_profile(tmp_path, monkeypatch):
    ...
    $HOME). A non-default sticky profile re-homes ``get_mikasa_home()`` to
    ``<root>/profiles/<name>``, and a Docker-style ``MIKASA_HOME`` can point
    ...
    # A sticky profile and a custom (Docker) root both point get_mikasa_home()
    # away from $HOME/.hermes; the anchor must ignore both.
    for elsewhere in (home / ".hermes" / "profiles" / "coder", tmp_path / "opt" / "data"):
        token_path.write_text("b" * 64)
        token_path.chmod(0o600)
        override = set_mikasa_home_override(elsewhere)
        try:
            assert _read_ssh_session_token_file(str(token_path)) == "b" * 64
            assert not token_path.exists()
        finally:
...
def test_token_under_profile_desktop_ssh_is_rejected(tmp_path, monkeypatch):
    ...
    token_path.write_text("b" * 64)
    token_path.chmod(0o600)
    override = set_mikasa_home_override(profile_home)
    try:
...
def test_token_file_rejects_symlink(tmp_path, monkeypatch):
    ...
    override = set_mikasa_home_override(home / ".hermes")
    try:
...
def test_windows_runtime_root_stays_at_machine_root_for_named_profile(tmp_path, monkeypatch):
    from mikasa_cli import windows_ssh_runtime

    machine_root = tmp_path / "custom-hermes-root"
    monkeypatch.setenv("mikasa_HOME", str(machine_root / "profiles" / "writer_2"))
