...
from mikasa_cli import send_cmd
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    MIKASA_HOME.mkdir()
    (MIKASA_HOME / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_mikasa_home() to re-resolve under the patched env.
    from importlib import reload

    import mikasa_cli.config as _hc_config
    reload(_hc_config)
...
def test_load_hermes_env_bomless_utf8_still_loads(tmp_path, monkeypatch):
    ...
    MIKASA_HOME = tmp_path / ".hermes"
    MIKASA_HOME.mkdir()
    (MIKASA_HOME / ".env").write_bytes(b"SEND_PLAIN_TOKEN=plain-val\n")
...
def test_load_hermes_env_latin1_fallback_overrides_shell(tmp_path, monkeypatch):
    ...
    (MIKASA_HOME / ".env").write_bytes(b"SEND_OVR_TOKEN=caf\xe9-file\n")
...
def test_load_hermes_env_fallback_read_error_is_swallowed(tmp_path, monkeypatch):
    ...
    (MIKASA_HOME / ".env").write_bytes(b"SEND_ERR_TOKEN=caf\xe9\n")
...
def test_load_hermes_env_bom_only_env_is_noop(tmp_path, monkeypatch):
    ...
    MIKASA_HOME = tmp_path / ".hermes"
    MIKASA_HOME.mkdir()
    (MIKASA_HOME / ".env").write_bytes(b"\xef\xbb\xbf")
