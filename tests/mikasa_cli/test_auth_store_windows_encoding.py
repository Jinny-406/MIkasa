...
import mikasa_cli.auth as auth
import mikasa_cli.auth_codex as auth_codex
...
def MIKASA_HOME(tmp_path, monkeypatch):
    """Point MIKASA_HOME at a tmp dir so we never touch the real auth store.
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, MIKASA_HOME, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, MIKASA_HOME, windows_default_encoding):
        ...
        assert not (MIKASA_HOME / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, MIKASA_HOME):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, MIKASA_HOME):
        auth_path = MIKASA_HOME / "auth.json"
...
class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, MIKASA_HOME, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(MIKASA_HOME / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, MIKASA_HOME, windows_default_encoding, monkeypatch):
        ...
        # MIKASA_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", MIKASA_HOME / "auth.json")
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """mikasa_cli.auth._read_shared_nous_state must read a non-ASCII store.
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, MIKASA_HOME, monkeypatch, windows_default_encoding
    ):
        """mikasa_cli.main._has_any_provider_configured reads auth.json.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nous_state(
        self, MIKASA_HOME, windows_default_encoding
    ):
