...
def _run_opencode_switch(
    ...
    with (
        patch("mikasa_cli.model_switch.resolve_alias", return_value=None),
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        # Re-import with the new MIKASA_HOME so config cache is fresh.
        import mikasa_cli.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import mikasa_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import mikasa_cli.model_switch as _ms_mod
