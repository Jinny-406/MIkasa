...
def config_home(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME with a minimal config."""
...
class TestCustomProviderModelSwitch:
    ...
    def test_custom_endpoint_switch_prunes_stale_model_config_pool_entry(
        ...
        from mikasa_cli.auth import read_credential_pool, write_credential_pool
        from mikasa_cli.model_setup_flows import _model_flow_custom
        ...
        with patch(
            ...
            return_value={
             ...
             patch("mikasa_cli.secret_prompt.masked_secret_prompt", return_value="sk-new"), \
             patch("mikasa_cli.main_provider_setup._prompt_custom_api_mode_selection", return_value=""), \
             patch(
                 "builtins.input",
                 side_effect=[
    ...
    def test_env_template_api_key_is_preserved_in_model_config(self, config_home, monkeypatch):
        ...
        with patch("mikasa_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]) as mock_fetch, \
             patch("mikasa_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
    ...
    def test_key_env_custom_provider_persists_reference_not_secret(self, config_home, monkeypatch):
        ...
        with patch("mikasa_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]), \
             patch("mikasa_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_env_ref_base_url_preserves_api_key_ref_through_picker(
        ...
        with patch("mikasa_cli.main._prompt_provider_choice",
                   side_effect=_pick_neuralwatt), \
             patch("mikasa_cli.models.fetch_api_models",
    ...
    def test_key_env_providers_dict_entry_does_not_add_api_key(
        ...
        monkeypatch.setenv("mikasa_CRS_HENKEE_KEY", "cr_live_secret_xyz")
        ...
        assert entry["key_env"] == "mikasa_CRS_HENKEE_KEY"
    ...
    def test_picker_recognizes_current_provider_alias_when_name_differs(
        ...
        with patch(
            "mikasa_cli.main._prompt_provider_choice",
...
class TestCustomProviderDiscoverModels:
    ...
    def test_discover_false_with_only_singular_model_skips_probe(self, config_home):
        ...
        with patch("mikasa_cli.models.fetch_api_models") as mock_fetch, \
             patch("mikasa_cli.models_local.fetch_ollama_local_models") as mock_ollama, \
             patch("mikasa_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_probe_empty_falls_back_to_configured_list(self, config_home):
        ...
        with patch("mikasa_cli.models.fetch_api_models", return_value=[]), \
             patch("mikasa_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="2"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
