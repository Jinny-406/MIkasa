...
These tests run against a fake $MIKASA_HOME under tmp_path; no real
...
from mikasa_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "mikasa_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under MIKASA_HOME/profiles/<name>/."""
    p = MIKASA_HOME / "profiles" / name
...
def _seed_default_root(
    ...
    *,
    ...
    MIKASA_HOME root (the implicit default profile)."""
    if state is not None:
        (MIKASA_HOME / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (MIKASA_HOME / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (MIKASA_HOME / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        MIKASA_HOME=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from mikasa_cli import container_boot
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            "top",
            "/opt/mikasa/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "0.0.0.0",
            "--port",
