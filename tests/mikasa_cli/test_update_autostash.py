...
from mikasa_cli import config as hermes_config
...
def _patch_gateway_discovery():
    ...
    phase's fresh ``from mikasa_cli.gateway import ...`` then loads an
    ...
    with patch("mikasa_cli.gateway.find_gateway_pids", return_value=[]), \
         ...
         patch("mikasa_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("mikasa_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         ...
         patch("mikasa_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.memory_setup._install_dependencies",
...
def test_reload_updated_runtime_modules_restores_new_mikasa_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import mikasa_constants

    monkeypatch.delattr(mikasa_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(mikasa_constants, "apply_subprocess_home_env")

    hermes_main._reload_updated_runtime_modules()

    assert callable(mikasa_constants.apply_subprocess_home_env)
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "mikasa_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    import subprocess
    from mikasa_cli import update_cmd
    import mikasa_cli.update_cmd_deps as update_cmd_deps
...
def test_restore_stays_parked_when_untracked_baseline_is_unknown(
    ...
    from mikasa_cli import update_cmd
    import mikasa_cli.update_cmd_stash as update_cmd_stash
