...
before that symbol existed. The function-level `from mikasa_cli.gateway
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_hermes_modules()
    assert sys.modules.get("mikasa_cli.update_cmd") is update_cmd
    assert sys.modules.get("mikasa_cli.main") is cli_main
    assert "mikasa_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import mikasa_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_hermes_modules()
        post_purge_receipt = importlib.import_module("mikasa_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["mikasa_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_hermes_modules()
    finally:
        sys.modules.pop("mikasa_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from mikasa_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from mikasa_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_hermes_modules()
    from mikasa_cli.update_inventory import RuntimeRecord as after
