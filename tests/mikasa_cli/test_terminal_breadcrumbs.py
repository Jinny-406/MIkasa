...
write/read roundtrip under a temp MIKASA_HOME, stale-session fallback
...
from mikasa_cli import terminal_breadcrumbs as tb
...
def test_breadcrumb_roundtrip(MIKASA_HOME, monkeypatch, no_terminal_env):
    ...
    files = list((MIKASA_HOME / "terminal-sessions").iterdir())
    assert [f.name for f in files] == ["tty-dev-pts-7"]


def test_write_skipped_without_terminal_identity(MIKASA_HOME, monkeypatch, no_terminal_env):
    _fake_no_tty(monkeypatch)
    tb.write_breadcrumb("20260815_120000_abc123")
    assert not (MIKASA_HOME / "terminal-sessions").exists()
    assert tb.read_breadcrumb() is None


def test_two_terminals_do_not_clobber(MIKASA_HOME, monkeypatch, no_terminal_env):
...
def test_stale_breadcrumb_ignored_and_pruned(MIKASA_HOME, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = MIKASA_HOME / "terminal-sessions"
...
def test_corrupt_breadcrumb_returns_none(MIKASA_HOME, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/1")
    directory = MIKASA_HOME / "terminal-sessions"
    directory.mkdir(parents=True)
...
def test_resolve_picks_this_terminals_session(MIKASA_HOME, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(MIKASA_HOME, "20260815_100000_aaaaaa")
    _make_session(MIKASA_HOME, "20260815_110000_bbbbbb")  # newer, other terminal
    tb.write_breadcrumb("20260815_100000_aaaaaa")
    assert tb.resolve_breadcrumb_session() == "20260815_100000_aaaaaa"


def test_resolve_falls_back_when_session_deleted(MIKASA_HOME, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(MIKASA_HOME, "20260815_110000_bbbbbb")
    tb.write_breadcrumb("20260815_100000_deleted")  # never existed / deleted
    assert tb.resolve_breadcrumb_session() is None


def test_resolve_projects_through_compression_chain(MIKASA_HOME, monkeypatch, no_terminal_env):
    _fake_tty(monkeypatch, "/dev/pts/5")
    _make_session(MIKASA_HOME, "20260815_100000_parent")
...
def test_config_gate_off_disables_writes_and_resolution(
    MIKASA_HOME, monkeypatch, no_terminal_env
):
