...
from mikasa_constants import venv_bin_dir, venv_python_path
...
def test_no_open_coded_venv_layout_remains_in_hermes_cli():
    ...
    assert not offenders, (
        "open-coded venv layout found (use mikasa_constants.venv_bin_dir):\n"
        + "\n".join(offenders)
    )
...
def test_top_level_files_are_swapped_atomically(tmp_path):
    ...
    mikasa_constants.py, ...). Covering only directories would leave exactly
...
def test_update_via_zip_wires_discard_into_the_commit_failure_path():
    ...
    from mikasa_cli import update_cmd_zip
