...
1. ``hermes_cli.web_server_dashboard._discover_dashboard_plugins`` opted into
   ...
   ``os.environ.get("mikasa_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
class TestProjectPluginsEnvGate:
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("mikasa_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("mikasa_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"mikasa_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "mikasa_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "mikasa_dashboard_plugin_evil" not in sys.modules
