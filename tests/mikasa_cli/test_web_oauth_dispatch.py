...
import mikasa_cli.web_routers.oauth as _rt_oauth
import mikasa_cli.web_server_oauth as _web_server_oauth
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "mikasa_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "mikasa_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "mikasa_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from mikasa_cli import web_server as ws
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    with patch("mikasa_cli.auth.get_auth_status", return_value=fake_status):
