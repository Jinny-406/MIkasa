...
def test_doctor_restores_global_tool_policy_and_module_state(tmp_path: Path) -> None:
    ...
    before_modules = {
        ...
        if name == "mikasa_plugins" or name.startswith("mikasa_plugins.")
    }
...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from mikasa_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    a temporary MIKASA_HOME — running it from ``$HOME`` copied the home
    directory, cloud-storage placeholders included.
    """
    import os
    import shutil

    from mikasa_cli import plugin_dev
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = MIKASA_HOME / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_mikasa_home", lambda: MIKASA_HOME)
