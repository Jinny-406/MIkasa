...
Runs a real subprocess with a temp MIKASA_HOME whose config.yaml contains a
...
Subprocess (not in-process monkeypatching) so module-level ``_mikasa_home``
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    code = textwrap.dedent(
        ...
        import os
        from pathlib import Path
        saved = Path(server._mikasa_home, "config.yaml").read_text(encoding="utf-8")
    ...
    out = _run_py(
        code,
        {
            ...
            "mikasa_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['mikasa_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"mikasa_HOME": str(home)}, tmp_path)
