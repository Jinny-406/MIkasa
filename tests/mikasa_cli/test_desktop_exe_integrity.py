...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors mikasa_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    with patch("mikasa_cli.main_desktop._windows_native_machine", return_value="AMD64"):
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("mikasa_cli.main_desktop._purge_electron_build_cache", return_value=[]), \
         patch("mikasa_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    ...
    with patch("mikasa_cli.main_desktop.shutil.which", return_value="/usr/bin/npm"), \
         patch("mikasa_cli.main_install_repair._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("mikasa_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("mikasa_cli.main_desktop._write_desktop_build_stamp") as mock_stamp, \
         patch("mikasa_cli.main_desktop._windows_native_machine", return_value="AMD64"), \
         patch("mikasa_cli.main_desktop.subprocess.run", side_effect=pack_into_staging), \
