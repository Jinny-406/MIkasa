...
def _client():
    ...
    # Keep the state DB under the isolated MIKASA_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_mikasa_home):
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from mikasa_cli.mcp_config import _get_mcp_servers
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_mikasa_home):
        ...
        (get_mikasa_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_mikasa_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "mikasa_version"):
...
class TestOfficialSkillsCatalogEndpoint:
    ...
    def test_lists_full_catalog_with_installed_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mikasa_cli.web_routers.skills._installed_hub_identifiers",
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mikasa_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import mikasa_cli.banner as banner
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        import mikasa_cli.debug as dbg
        ...
        monkeypatch.setattr("mikasa_cli.dump.run_dump", lambda a: None)
...
def test_spawn_hermes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    assert captured["env"]["mikasa_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_mikasa_home
):
    ...
    # web_server imports the reaper lazily from mikasa_cli.gateway, so patch it
    # on that module.
    import mikasa_cli.gateway as g
