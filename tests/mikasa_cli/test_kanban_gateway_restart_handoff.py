...
def test_managed_gateway_worker_is_spawned_in_restart_safe_scope(
    ...
    assert captured_env["mikasa_KANBAN_TASK"] == task.id
    assert captured_env["mikasa_KANBAN_RUN_ID"] == "23"
...
def test_real_user_systemd_scope_preserves_worker_context(
    ...
    script = (
        ...
        "'task': os.environ.get('mikasa_KANBAN_TASK'), "
        "'run': os.environ.get('mikasa_KANBAN_RUN_ID'), "
