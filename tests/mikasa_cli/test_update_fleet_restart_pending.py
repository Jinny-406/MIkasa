...
import mikasa_cli.main_web_build as main_web_build
import mikasa_cli.main_install_repair as main_install_repair
from mikasa_cli import update_cmd
import mikasa_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    import mikasa_cli.gateway as hermes_gateway
    ...
    monkeypatch.setattr(
        "mikasa_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_mikasa_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_mikasa_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_mikasa_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "mikasa_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(hermes_main, "_purge_stale_hermes_modules", lambda: None)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    latest = get_mikasa_home() / "logs" / "update_receipts" / "latest.json"
