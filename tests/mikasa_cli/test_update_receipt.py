...
def receipt_home(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME for receipt writes."""
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setattr(
        "mikasa_cli.config.get_mikasa_home", lambda: home, raising=False
    )
...
class TestCommandBoundaryFinalization:
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        import mikasa_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "mikasa_cli.profiles._get_default_mikasa_home", lambda: home
        )
        monkeypatch.setattr(
            "mikasa_cli.profiles._get_profiles_root",
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("mikasa_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
