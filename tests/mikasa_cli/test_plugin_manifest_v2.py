...
def MIKASA_HOME(tmp_path, monkeypatch):
    home = tmp_path / "mikasa_home"
    (home / "plugins").mkdir(parents=True)
    monkeypatch.setenv("mikasa_HOME", str(home))
    monkeypatch.setenv("mikasa_ENABLE_PROJECT_PLUGINS", "0")
    monkeypatch.setenv(
        "mikasa_BUNDLED_PLUGINS", str(tmp_path / "empty-bundled")
    )
...
class TestV1Regression:
    def test_v1_manifest_parses_with_defaults(self, MIKASA_HOME):
        _write_plugin(MIKASA_HOME / "plugins", "oldie")
        _enable(MIKASA_HOME, ["oldie"])
        mgr = PluginManager()
    ...
    def test_v1_unknown_fields_do_not_warn_loudly(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "oldie",
...
class TestV2Parsing:
    def test_v2_fields_parse(self, MIKASA_HOME):
        _write_plugin(
            MIKASA_HOME / "plugins", "modern",
            manifest_extra={
                "manifest_version": 2,
                "api_version": 1,
                "license": "MIT",
    ...
    def test_unknown_field_in_v2_warns_but_loads(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "modern",
    ...
    def test_future_manifest_version_warns_but_loads(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "fromfuture",
            manifest_extra={
                "manifest_version": SUPPORTED_MANIFEST_VERSION + 5,
            },
        )
        _enable(MIKASA_HOME, ["fromfuture"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_malformed_v2_fields_warn_and_degrade(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "sloppy",
            manifest_extra={
                "manifest_version": 2,
                "api_version": "banana",
...
class TestDependencyOrder:
    def test_dep_registers_before_dependent(self, MIKASA_HOME):
        ...
        _write_plugin(
            MIKASA_HOME / "plugins", "aaa-consumer",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            MIKASA_HOME / "plugins", "zzz-base",
            register_body="import sys; sys._m2_order.append('zzz-base')",
        )
        _enable(MIKASA_HOME, ["aaa-consumer", "zzz-base"])
        import sys
    ...
    def test_missing_dep_warns_but_loads(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "needy",
            manifest_extra={
                "manifest_version": 2,
                "requires_plugins": [{"id": "ghost-plugin"}],
            },
        )
        _enable(MIKASA_HOME, ["needy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_cycle_warns_and_falls_back_alpha(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "cyc-a",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(MIKASA_HOME, ["cyc-a", "cyc-b"])
        import sys
...
class TestConfigSchema:
    def test_type_mismatch_warns_but_loads(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "cfgd",
            manifest_extra={
                "manifest_version": 2,
                "config_schema": {
                    "api_url": {"type": "str"},
    ...
    def test_unknown_declared_type_skips_check(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "weird",
            manifest_extra={
                "manifest_version": 2,
...
class TestPythonDependenciesSeam:
    def test_missing_pip_dep_surfaced_with_hint_not_installed(
        self, MIKASA_HOME, caplog, monkeypatch
    ):
        calls = []
        import subprocess
    ...
    def test_satisfied_pip_dep_is_quiet(self, MIKASA_HOME, caplog):
        _write_plugin(
            MIKASA_HOME / "plugins", "pipok",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": ["pyyaml>=5,<7"],
            },
        )
        _enable(MIKASA_HOME, ["pipok"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestCtxHasPlugin:
    def test_has_plugin_probe(self, MIKASA_HOME):
        _write_plugin(MIKASA_HOME / "plugins", "probe-target")
        _write_plugin(
            MIKASA_HOME / "plugins", "prober",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(MIKASA_HOME, ["probe-target", "prober"])
        import sys
