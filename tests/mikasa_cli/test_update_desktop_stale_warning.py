...
import mikasa_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_constants.with_hermes_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "mikasa_constants.display_mikasa_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_mikasa_home", lambda: tmp_path)
