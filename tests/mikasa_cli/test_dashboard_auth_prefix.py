...
from mikasa_cli.dashboard_auth import prefix as prefix_mod
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``mikasa_cli.config.load_config`` with a stub
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "mikasa_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("mikasa_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        patch_config(None)
        monkeypatch.setenv("mikasa_DASHBOARD_PUBLIC_URL", "hermes.domain.com")
        ...
        assert any(
            "mikasa_DASHBOARD_PUBLIC_URL" in m
...
class TestCookiePathRespectsPrefix:
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from mikasa_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("mikasa_session_pkce=") for c in cookies), (
