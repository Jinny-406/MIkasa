...
def container_env(tmp_path, monkeypatch):
    """Set up a fake MIKASA_HOME with .container-mode file."""
    MIKASA_HOME = tmp_path / ".hermes"
    MIKASA_HOME.mkdir()
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    monkeypatch.delenv("mikasa_DEV", raising=False)

    container_mode = MIKASA_HOME / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=hermes-agent\n"
        "exec_user=hermes\n"
        "mikasa_bin=/data/current-package/bin/hermes\n"
    )
    return MIKASA_HOME
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("mikasa_constants.is_container", return_value=False):
    ...
    assert info["mikasa_bin"] == "/data/current-package/bin/hermes"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from mikasa_cli.main import _exec_in_container
