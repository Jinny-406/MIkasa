...
from mikasa_cli.cron import cron_command
...
class TestProfileFlagGatewayLifecycle:
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry MIKASA_HOME/HERMES_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("mikasa_PROFILE", "zeus")
        monkeypatch.delenv("mikasa_PROFILE_NAME", raising=False)
    ...
    def test_mikasa_home_derived_profile(self, monkeypatch):
        # Without HERMES_PROFILE the guard falls back to the MIKASA_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("mikasa_PROFILE", raising=False)
        monkeypatch.delenv("mikasa_PROFILE_NAME", raising=False)
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # MIKASA_HOME/scripts so the scheduler — and the guard — resolve it.
...
class TestLifecycleGuardModule:
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under MIKASA_HOME/scripts (the
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_mikasa_home()` falls back to Path.home(), which raises
        RuntimeError when neither MIKASA_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "sqlite3 ./stats.db \"SELECT restart_reason from mikasa_gateway_restarts\"",
    ])
