...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_mikasa_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from mikasa_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from mikasa_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "mikasa_state", mod("mikasa_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.config",
        mod("mikasa_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.models",
        mod("mikasa_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.runtime_provider",
        mod(
            "mikasa_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.tools_config",
        mod("mikasa_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["mikasa_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["mikasa_MODEL"] == "nous/hermes-test"
    assert env["mikasa_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["mikasa_TUI_PROVIDER"] == "nous"
    assert env["mikasa_INFERENCE_PROVIDER"] == "nous"
    assert env["mikasa_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["mikasa_TUI_ACTIVE_SESSION_FILE"])
...
def test_make_tui_argv_dev_prebuilds_hermes_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("mikasa_TUI_DIR", raising=False)
