...
repair that runs BEFORE mikasa_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  mikasa_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
from mikasa_cli import _early_recovery as er
...
def _run_lifecycle_subprocess(tmp_path: Path, *, repair: bool) -> subprocess.CompletedProcess:
    ...
    script.write_text(
        textwrap.dedent(
            ...
            import mikasa_cli._early_recovery as er
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing mikasa_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"mikasa_cli"}
            ...
            import mikasa_cli._early_recovery  # noqa: F401
...
def test_core_marker_triggers_install_before_any_native_import(
    ...
    from mikasa_cli import _install_repair as ir
    ...
    import mikasa_cli._install_repair  # noqa: F401  (pre-import for patch)
...
def test_core_marker_marks_attempts_and_keeps_marker_on_install_failure(
    ...
    import mikasa_cli._install_repair  # noqa: F401
