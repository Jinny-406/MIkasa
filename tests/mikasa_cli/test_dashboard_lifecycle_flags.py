...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("mikasa_cli.dashboard_procs._scan_dashboard_processes", return_value=[]), \
    ...
    def test_status_with_processes(self, capsys):
        ...
        processes = [
            ...
            (12346, "python -m mikasa_cli.main dashboard --host 0.0.0.0 --port 9120"),
            (12347, "hermes serve --host 100.94.65.93 --port 9119"),
        ]
        with patch("mikasa_cli.dashboard_procs._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("mikasa_cli.main_dashboard._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("mikasa_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("mikasa_cli.dashboard_procs._kill_stale_dashboard_processes") as mock_kill, \
             pytest.raises(SystemExit) as exc:
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("mikasa_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("mikasa_cli.dashboard_procs._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("mikasa_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"mikasa_cli.web_server": fake_ws}), \
...
class TestArgparseWiring:
    ...
    def test_flags_are_registered(self):
        from mikasa_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("mikasa_cli.main")
