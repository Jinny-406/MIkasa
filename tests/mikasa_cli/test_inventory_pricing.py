...
import mikasa_cli.inventory as inv
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (MIKASA_HOME_key(), dict(scope))
        started[label].set()
    ...
    try:
        for label in ("a", "b"):
            home = tmp_path / label
            home_token = set_mikasa_home_override(str(home))
        ...
        assert observed["a"] == (
            MIKASA_HOME_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            MIKASA_HOME_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    token = set_mikasa_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
