...
def _write_plugin(MIKASA_HOME: Path) -> None:
    plugin_dir = MIKASA_HOME / "plugins" / "ledger_probe"
...
def _write_profile_probe(MIKASA_HOME: Path, marker: str) -> None:
    plugin_dir = MIKASA_HOME / "plugins" / "profile_probe"
    plugin_dir.mkdir(parents=True)
    (plugin_dir / "plugin.yaml").write_text(
        yaml.safe_dump(
            {
...
def test_reverse_unload_restores_an_overridden_platform_registration():
    ...
    from gateway.platform_registry import platform_registry
    from mikasa_cli.plugins import PluginContext, PluginManager, PluginManifest
...
def test_scoped_plugin_cannot_deregister_a_process_global_tool():
    ...
    module_name = "mikasa_plugins.scoped_cleanup"
...
def test_shared_entrypoint_module_uses_the_active_profile_scope(tmp_path):
    ...
    token = set_mikasa_home_override(home_a)
    try:
        with patch.object(ToolRegistry, "_caller_module", return_value=module_name):
...
def test_entrypoint_policy_uses_the_most_specific_module_prefix(tmp_path):
    ...
    token = set_mikasa_home_override(narrow_scope)
    try:
        registry.register(
...
def test_direct_plugin_platform_registration_infers_immutable_scope(tmp_path):
    ...
    token = set_mikasa_home_override(home_b)
    try:
        platform_registry.register(
            PlatformEntry(
                name=name,
...
def test_same_name_tool_and_platform_are_isolated_by_mikasa_home(
    tmp_path,
    monkeypatch,
):
    ...
    token_a = set_mikasa_home_override(home_a)
    try:
    ...
    token_b = set_mikasa_home_override(home_b)
    try:
...
def test_manager_discovery_uses_its_home_not_the_ambient_profile(
    ...
    ambient = set_mikasa_home_override(home_b)
    try:
        manager_a.discover_and_load()
    finally:
        reset_mikasa_home_override(ambient)
