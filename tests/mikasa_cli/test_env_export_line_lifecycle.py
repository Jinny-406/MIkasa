...
def test_classic_pat_save_via_endpoint_succeeds(MIKASA_HOME):
...
def test_remove_export_prefixed_token(MIKASA_HOME):
    """DELETE must clear an ``export KEY=...`` line, not 404 on it."""
    _write_env_raw(MIKASA_HOME, f"export GITHUB_TOKEN={OLD_PAT}\n")
    ...
    env_text = MIKASA_HOME.joinpath(".env").read_text(encoding="utf-8")
...
def test_update_export_prefixed_token_does_not_duplicate(MIKASA_HOME):
...
def test_plain_line_save_and_remove_still_work(MIKASA_HOME):
    """Sanity: the ordinary KEY= path is unchanged."""
    from mikasa_cli.config import load_env, remove_env_value, save_env_value
...
def test_export_line_with_comment_untouched(MIKASA_HOME):
...
def test_remove_token_written_with_spaces_around_equals(MIKASA_HOME):
    ...
    """
    _write_env_raw(MIKASA_HOME, f"GITHUB_TOKEN = {OLD_PAT}\n")
...
def test_spaced_token_rotate_then_delete_does_not_resurrect(MIKASA_HOME):
...
def test_writer_matches_exactly_what_load_env_accepts(MIKASA_HOME):
    """Parity: the writers must recognise a line iff ``load_env()`` does."""
    from mikasa_cli.config import _env_line_defines_key, load_env
    ...
    for line, expected in [
        ...
        _write_env_raw(MIKASA_HOME, line + "\n")
