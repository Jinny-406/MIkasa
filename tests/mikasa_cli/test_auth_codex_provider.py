...
def _setup_hermes_auth(MIKASA_HOME: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(MIKASA_HOME, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (MIKASA_HOME / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((MIKASA_HOME / "auth.json").read_text())
...
def test_resolve_returns_hermes_auth_store_source(tmp_path, monkeypatch):
    MIKASA_HOME = tmp_path / "hermes"
    _setup_hermes_auth(MIKASA_HOME)
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from mikasa_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from mikasa_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    ...
    monkeypatch.setattr("mikasa_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
