...
def _install_pid_sequence(monkeypatch, snapshots):
    ...
    monkeypatch.setattr("mikasa_cli.gateway.find_gateway_pids", _fake)
    return calls
...
def test_report_gateway_start_failure_is_loud_not_checkmark(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "mikasa_cli.config.get_mikasa_home", lambda: str(tmp_path)
    )
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "mikasa_Gateway_x")
...
def attest_home(monkeypatch, tmp_path):
    monkeypatch.setattr("mikasa_cli.config.get_mikasa_home", lambda: str(tmp_path))
...
def test_attestation_warning_includes_schtasks_recovery(monkeypatch, attest_home):
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(
        gateway_windows, "get_task_name", lambda: "mikasa_Gateway_arthur_tutor"
    )
...
def test_breakaway_fallback_warns_even_on_success(monkeypatch, attest_home, capsys):
    ...
    monkeypatch.setattr(gateway_windows, "is_task_registered", lambda: True)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "mikasa_Gateway")
