...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        with _patch.dict(os.environ, {"mikasa_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("mikasa_cli.tools_config_cua.shutil.which", side_effect=which_side_effect), \
             patch("mikasa_cli.tools_config_cua.subprocess.run", fake_run):
