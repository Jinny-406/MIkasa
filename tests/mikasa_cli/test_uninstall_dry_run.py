...
from mikasa_cli import uninstall
...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_mikasa_home", lambda: MIKASA_HOME)
    monkeypatch.setattr(uninstall, "_is_default_mikasa_home", lambda home: False)
    ...
    assert str(MIKASA_HOME) in output
    assert project_root.exists()
    assert MIKASA_HOME.exists()
