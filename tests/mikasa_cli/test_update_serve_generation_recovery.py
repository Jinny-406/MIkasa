...
from mikasa_cli import update_abort_recovery as abort_recovery
from mikasa_cli import update_cmd
from mikasa_cli import update_restart_recovery as recovery
...
def _identity_module():
    return __import__("mikasa_cli.process_identity", fromlist=["ledger_entries"])
...
def test_serve_coverage_reaches_the_persisted_receipt(tmp_path, monkeypatch):
    from mikasa_cli import update_receipt
    ...
    update_receipt.record_gateway_restart(
        restarted_services=[],
        incomplete=True,
        phase_error="cannot import name 'line_input' from 'mikasa_cli.cli_output'",
        fresh_recovery={
            "requested": ["default"],
            "verified": ["default"],
            "relaunch_attempted": [],
...
def test_recovery_module_reports_serve_units_in_a_real_process():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "mikasa_cli.update_restart_recovery", "--stdin"],
        input=json.dumps(
...
def _stub_dashboard_helpers(monkeypatch, **helpers):
    """Stub the ``mikasa_cli.main_dashboard`` helpers the dashboard-cleanup path reads at call time."""
