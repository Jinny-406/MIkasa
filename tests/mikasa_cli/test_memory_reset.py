...
- Profile-scoped reset (uses MIKASA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake MIKASA_HOME with memory files."""
    MIKASA_HOME = tmp_path / ".hermes"
    memories = MIKASA_HOME / "memories"
...
class TestMemoryReset:
    ...
    def test_reset_all_with_yes_flag(self, memory_env):
        ...
        MIKASA_HOME, memories = memory_env
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        (MIKASA_HOME / "memories").mkdir(parents=True)
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
