...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import mikasa_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("mikasa_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from mikasa_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    the per-MIKASA_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root MIKASA_HOME, no ``-p``) and named-profile
    render paths.
    """
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = MIKASA_HOME / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside MIKASA_HOME the way
