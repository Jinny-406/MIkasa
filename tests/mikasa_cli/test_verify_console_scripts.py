...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        ...
        with patch("mikasa_cli.main_install_repair._is_windows", return_value=True), \
             patch("mikasa_cli.main_install_repair._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("mikasa_cli.main_install_repair._run_quarantined_install") as mock_install:
            from mikasa_cli.main_install_repair import _verify_console_scripts_installed
