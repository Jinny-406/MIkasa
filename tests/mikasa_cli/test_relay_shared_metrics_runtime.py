...
from mikasa_cli import lifecycle, plugins
from agent import relay_runtime
from mikasa_cli.observability import relay_shared_metrics
...
def direct_runtime(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.config.read_raw_config_readonly",
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    monkeypatch.setattr(
        "mikasa_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from mikasa_cli import config, managed_scope
    ...
    monkeypatch.setenv("mikasa_MANAGED_DIR", str(managed))
    config._LOAD_CONFIG_CACHE.clear()
    config._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()

    token = set_mikasa_home_override(profile)
    try:
        assert (
