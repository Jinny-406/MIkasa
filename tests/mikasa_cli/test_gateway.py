...
import mikasa_cli.gateway as gateway
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # sandboxes ``MIKASA_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``MIKASA_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        diag_path = pathlib.Path(os.environ["mikasa_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            ...
            "mikasa_GATEWAY_DETACHED": "1",
            "mikasa_GATEWAY_EXIT_DIAG": "1",
            "mikasa_GATEWAY_MAX_STARTS": "0",
    ...
    from mikasa_cli._subprocess_compat import windows_detach_flags_without_breakaway
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["mikasa_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "mikasa_HOME": str(tmp_path),
        "mikasa_GATEWAY_EXIT_DIAG": "0",
        "mikasa_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("mikasa_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mikasa_constants.is_container", lambda: True)
    monkeypatch.setattr("mikasa_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("mikasa_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_mikasa_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test MIKASA_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="mikasa_HOME=/home/alice/.hermes"\n'
        ),
    )
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "mikasa_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "mikasa_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "mikasa_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        """mikasa_Gateway_* is Running => reaper returns False, kills nothing."""
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        ...
        import mikasa_cli.gateway_windows as gateway_windows

        monkeypatch.setattr(
            gateway_windows, "get_task_name", lambda: "mikasa_Gateway_testprof"
        )
        queried = []
        ...
        assert result is False
        assert killed_pids == []
        assert queried == ["mikasa_Gateway_testprof"]
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("mikasa_Gateway") is expected, state
