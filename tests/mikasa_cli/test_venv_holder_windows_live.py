...
def _detect() -> list[tuple[int, str, str]]:
    from mikasa_cli.update_cmd import _detect_venv_python_processes
...
class TestDetection:
    def test_detects_hermes_argv_process(self):
        """Baseline: a live process running `-m mikasa_cli.main serve` with
        cwd under the install root is detected as a venv holder."""
        proc = _spawn(["-m", "mikasa_cli.main", "serve"])
        try:
            matches = _detect()
            ...
            assert "mikasa_cli.main" in cmdline
        finally:
            _kill(proc)
    ...
    def test_long_runtime_path_gateway_detected_with_full_argv(self):
        ...
        proc = _spawn([padding, "-m", "mikasa_cli.main", "gateway", "run"])
        try:
            matches = _detect()
...
class TestClassification:
    def test_pausable_exemption_sees_long_path_gateway(self):
        ...
        from mikasa_cli.update_cmd import _leftover_pausable_gateway_pids
...
class TestHolderMessage:
    ...
    def test_dashboard_not_labeled_desktop_backend(self):
        from mikasa_cli.update_cmd import _format_venv_python_holders_message

        proc = _spawn(["-m", "mikasa_cli.main", "dashboard"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
    ...
    def test_substring_subcommand_not_mislabeled(self):
        ...
        proc = _spawn(["-m", "mikasa_cli.main", "kanban", "--preserve-cache"])
        try:
            matches = [m for m in _detect() if m[0] == proc.pid]
...
class TestAncestorExclusion:
    ...
    def test_gateway_parent_visible_to_child_scan(self, tmp_path):
        ...
        # `... -m mikasa_cli.main gateway run`, not a multi-line -c blob).
...
class TestConcurrentGateClassification:
    ...
    def test_live_gateway_process_classified_gateway(self):
        """A real process whose argv carries `-m mikasa_cli.main gateway run`
        classifies as ``gateway`` via real psutil against the live table."""
        from mikasa_cli.update_cmd import _classify_concurrent_instance

        proc = _spawn(["-m", "mikasa_cli.main", "gateway", "run"])
        try:
    ...
    def test_live_non_gateway_processes_keep_the_abort(self):
        ...
        repl = _spawn(["-m", "mikasa_cli.main"])
        mgmt = _spawn(["-m", "mikasa_cli.main", "gateway", "status"])
        try:
    ...
    def test_live_filter_drops_only_the_gateway(self):
        ...
        gw = _spawn(["-m", "mikasa_cli.main", "gateway", "run"])
        backend = _spawn(["-m", "mikasa_cli.main", "serve", "--port", "8127"])
        dead = _spawn([])
...
class TestUpdaterOwnedBackendDeferral:
    ...
    def _ledger_write(pid: int, purpose: str, spawner_pid: int | None,
        ...
        from mikasa_cli.process_identity import (
            LedgerEntry,
            _append_entry,
            install_id,
        )
    ...
    def test_dead_spawner_serve_is_deferred_live(self, tmp_path, monkeypatch):
        ...
        backend = _spawn(["-m", "mikasa_cli.main", "serve", "--host", "127.0.0.1"])
        try:
            self._ledger_write(backend.pid, "serve", spawner.pid, spawner_create)

            from mikasa_cli._scan_venv_blockers import _is_updater_owned_backend
    ...
    def test_handoff_desktop_ancestor_spawner_is_deferred_live(self, tmp_path, monkeypatch):
        ...
        parent_file.write_text(
            ...
            f"sys.path.insert(0, {str(PROJECT_ROOT)!r})\n"
            "import psutil\n"
            "from mikasa_cli.process_identity import LedgerEntry, _append_entry, install_id\n"
        ...
        try:
            result = subprocess.run(
                ...
                cwd=str(PROJECT_ROOT),
                timeout=120,
                env={**os.environ, "mikasa_HOME": str(tmp_path)},
            )
            line = result.stdout.strip().splitlines()[-1] if result.stdout.strip() else "{}"
