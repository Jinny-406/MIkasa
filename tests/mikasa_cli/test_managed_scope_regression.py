...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("mikasa_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(MIKASA_HOME, monkeypatch):
    from mikasa_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(MIKASA_HOME, monkeypatch):
    from mikasa_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_mikasa_dotenv(MIKASA_HOME=str(home))
