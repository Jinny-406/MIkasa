...
def _runtime_info(
    ...
    from mikasa_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from mikasa_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    ...
    def test_posix(self, tmp_path):
        with patch("mikasa_cli.managed_uv.get_mikasa_home", return_value=tmp_path):
            from mikasa_cli.managed_uv import managed_uv_path
...
class TestMacOSManagedPythonSigning:
    def test_signs_with_stable_identifier_and_verifies(self, tmp_path, monkeypatch):
        import mikasa_cli.managed_uv as managed_uv
...
class TestResolveUv:
    ...
    def test_existing_executable(self, tmp_path):
        ...
        with patch("mikasa_cli.managed_uv.get_mikasa_home", return_value=tmp_path):
            from mikasa_cli.managed_uv import resolve_uv
            result = resolve_uv()
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("mikasa_cli.managed_uv.get_mikasa_home", return_value=tmp_path), \
             patch("mikasa_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("mikasa_cli.managed_uv._install_uv") as mock_install:
            # Simulate the installer creating the binary
            def fake_install(target):
                _make_executable(target)
            mock_install.side_effect = fake_install

            from mikasa_cli.managed_uv import ensure_uv
            path = ensure_uv()
            assert path == str(tmp_path / "bin" / "uv")
            mock_install.assert_called_once()

    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from mikasa_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "mikasa_cli.managed_uv.get_mikasa_home",
            return_value=tmp_path,
        ), patch(
            "mikasa_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "mikasa_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestEnsureUvUpdateBoundary:
    ...
    ``mikasa_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("mikasa_cli.managed_uv.get_mikasa_home", return_value=tmp_path), \
             patch("mikasa_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("mikasa_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from mikasa_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from mikasa_cli.managed_uv import _UvResult
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("mikasa_cli.managed_uv.get_mikasa_home", return_value=tmp_path), \
             patch("mikasa_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
            from mikasa_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        from mikasa_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / "uv"
        _make_executable(uv)
        # Fresh stamp under the isolated MIKASA_HOME.
        import mikasa_constants
        stamp = mikasa_constants.get_mikasa_home() / "cache" / ".uv_self_update_stamp"
        stamp.parent.mkdir(parents=True, exist_ok=True)
        stamp.touch()

        with patch("mikasa_cli.managed_uv.get_mikasa_home", return_value=tmp_path), \
             patch("mikasa_cli.managed_uv.subprocess.run") as mock_run, \
             patch(
                 "mikasa_cli.managed_uv.repair_vulnerable_runtime",
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from mikasa_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from mikasa_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("mikasa_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("mikasa_HOME", str(tmp_path / "profiles" / "beta"))
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from mikasa_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "mikasa_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "mikasa_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
    ...
    def test_stage_candidate_sync_keeps_uv_project_config(self, tmp_path):
        from mikasa_cli.managed_uv import _stage_candidate_venv
        ...
        with patch("mikasa_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch(
                 "mikasa_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        with patch(
             ...
             patch(
                 "mikasa_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from mikasa_cli.managed_uv import _acquire_repair_lock, _release_repair_lock
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from mikasa_cli.managed_uv import _cut_over_candidate
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("mikasa_cli.managed_uv._install_uv_posix") as mock_posix:
            from mikasa_cli.managed_uv import _install_uv
            _install_uv(target)
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from mikasa_cli.managed_uv import _runtime_request
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("mikasa_cli.managed_uv.get_mikasa_home", return_value=tmp_path), \
             patch("mikasa_cli.managed_uv._install_uv"), \
             patch(
                 "mikasa_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "mikasa_cli.managed_uv._refresh_managed_uv_catalog",
...
class TestDefaultLiveVenv:
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from mikasa_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a mikasa_constants predating its symbol.

    ``hermes update`` imports mikasa_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from mikasa_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'mikasa_constants'
        (~/.hermes/hermes-agent/mikasa_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from mikasa_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(mikasa_constants, "venv_python_path", raising=False)
        ...
        expected = Path("/opt/mikasa/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/mikasa/venv/bin/python")
        assert _venv_python(Path("/opt/mikasa/venv")) == expected

    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through mikasa_constants, not open-code it.
        ...
        assert _venv_python(Path("/opt/mikasa/venv")) == sentinel
...
class TestWindowsRuntimeSelfLock:
    ...
    def test_self_lock_defers_repair_before_provisioning(
        ...
        from mikasa_cli import managed_uv
        from mikasa_cli.managed_uv import repair_vulnerable_runtime
        ...
        with patch(
                 "mikasa_cli.managed_uv._windows_runtime_holders",
                 return_value=(False, ""),
             ), \
             patch(
                 "mikasa_cli.managed_uv.probe_sqlite_runtime",
