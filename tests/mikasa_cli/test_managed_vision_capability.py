...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    importlib.reload(mikasa_constants)
    yield home
    importlib.reload(mikasa_constants)


def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp MIKASA_HOME IS the root here).
    from mikasa_cli.local_runtime.bootstrap import models_dir
...
def test_not_ours_returns_none(MIKASA_HOME):
    from mikasa_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(MIKASA_HOME):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from mikasa_cli.local_runtime.bootstrap import assets_dir
    from mikasa_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(MIKASA_HOME, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(MIKASA_HOME):
...
def test_lookup_chain_consults_managed_runtime(MIKASA_HOME, monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(MIKASA_HOME, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(MIKASA_HOME, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(MIKASA_HOME, monkeypatch, tmp_path):
