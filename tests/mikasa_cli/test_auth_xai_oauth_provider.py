...
def _setup_hermes_auth(
    ...
    *,
    ...
    auth_file = MIKASA_HOME / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("mikasa_cli.auth.httpx.Client", _factory)
    return holder
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    MIKASA_HOME.mkdir(parents=True, exist_ok=True)
    (MIKASA_HOME / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_refresh_xai_oauth_tokens_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(MIKASA_HOME, access_token=near, refresh_token="rt-old")
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))

    auth_path = MIKASA_HOME / "auth.json"
    ...
    monkeypatch.setattr("mikasa_cli.auth.refresh_xai_oauth_pure", _fake_pure)
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def _seed_xai_oauth_state(
    MIKASA_HOME: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    MIKASA_HOME.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (MIKASA_HOME / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    MIKASA_HOME = tmp_path / "hermes"
    _seed_xai_oauth_state(MIKASA_HOME, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    ...
    monkeypatch.setattr("mikasa_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((MIKASA_HOME / "auth.json").read_text())
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from mikasa_cli.auth import _xai_oauth_discovery
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(MIKASA_HOME, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
...
def test_credential_pool_device_code_seed_respects_suppression(tmp_path, monkeypatch):
    from agent.credential_pool import load_pool
    from mikasa_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool
    from mikasa_cli.auth_commands import auth_remove_command
    ...
    # nothing to import.
    raw_after = json.loads((MIKASA_HOME / "auth.json").read_text())
...
def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    ...
    new_access = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    monkeypatch.setattr(
        "mikasa_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
                "access_token": new_access,
    ...
    monkeypatch.setattr(
        "mikasa_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(MIKASA_HOME, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    ...
    monkeypatch.setattr("mikasa_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_runtime_provider_uses_pool_entry_for_xai_oauth(tmp_path, monkeypatch):
    ...
    MIKASA_HOME = tmp_path / "hermes"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_hermes_auth(MIKASA_HOME, access_token=fresh)
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    monkeypatch.delenv("mikasa_XAI_BASE_URL", raising=False)
    monkeypatch.delenv("XAI_BASE_URL", raising=False)
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(MIKASA_HOME, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    ...
    def _fake_refresh(access_token, refresh_token, **kwargs):
        ...
        (MIKASA_HOME / "auth.json").write_text(json.dumps(raw))
        raise AuthError(
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(MIKASA_HOME, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(MIKASA_HOME, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
