...
script. Its canonical home is the managed binary dir ``MIKASA_HOME\\bin`` —
...
parameters (same pattern as ``mikasa_constants.venv_bin_dir``), so these
...
from mikasa_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: MIKASA_HOME/hermes-agent/venv/Scripts + launchers."""
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under MIKASA_HOME gains nothing anywhere."""
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``hermes -p <name>`` MIKASA_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("mikasa_HOME", str(home / "profiles" / "work"))
