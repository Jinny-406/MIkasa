...
import pytest
import mikasa_cli.web_server_mcp as _web_server_mcp
...
def test_hosted_auth_start_returns_public_authorization_url(monkeypatch):
    ...
    with patch(
        "mikasa_cli.dashboard_auth.prefix.resolve_public_url",
...
def test_hosted_auth_allows_same_server_name_in_different_profiles(tmp_path, monkeypatch):
    ...
    existing = DashboardOAuthFlow(
        ...
        server_name="reports",
        profile=None,
        MIKASA_HOME=str(tmp_path / "default"),
    ...
    with patch("mikasa_cli.mcp_config._get_mcp_servers", return_value={"reports": {"url": "https://mcp.example"}}), \
