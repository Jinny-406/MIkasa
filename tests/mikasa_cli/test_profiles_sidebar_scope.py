...
@pytest.fixture
def profiles_on_disk(tmp_path, monkeypatch, _isolate_mikasa_home):
...
def client(monkeypatch, profiles_on_disk):
    ...
    import mikasa_state
    from mikasa_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    from mikasa_constants import get_mikasa_home
...
def _seed_project(home, name, folder):
    from mikasa_cli import projects_db
...
class TestCrossProfileProjectTree:
    ...
    def test_each_profile_contributes_its_own_projects_db(self, client, profiles_on_disk, tmp_path):
        ...
        filters through ``get_mikasa_home()``. If the fan-out failed to rebind
    ...
    def test_a_profile_that_cannot_be_read_does_not_sink_the_rest(
        ...
        def explode_for_worker(db, **kwargs):
            ...
            if get_mikasa_home().name == "worker":
