...
def fresh_home(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test MIKASA_HOME plus a path-init cache reset
    ...
    # Also reset mikasa_constants cache so get_default_mikasa_root() re-reads.
    try:
        import mikasa_constants
        mikasa_constants._cached_default_hermes_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_KANBAN_DB", str(forced))
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_KANBAN_BOARD", "envwin")
        with kbc.connect() as conn:
...
class TestWorkerSpawnEnv:
    ...
    def test_default_spawn_sets_env_vars(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["mikasa_KANBAN_BOARD"] == "spawntest"
        assert env["mikasa_KANBAN_TASK"] == "t_abc"
        # DB path should match the per-board DB, not the legacy default.
        expected_db = fresh_home / "kanban" / "boards" / "spawntest" / "kanban.db"
        assert env["mikasa_KANBAN_DB"] == str(expected_db)
        expected_ws = fresh_home / "kanban" / "boards" / "spawntest" / "workspaces"
        assert env["mikasa_KANBAN_WORKSPACES_ROOT"] == str(expected_ws)
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "mikasa_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
