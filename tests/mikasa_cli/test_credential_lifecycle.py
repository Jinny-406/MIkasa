...
MIKASA_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def MIKASA_HOME(monkeypatch, tmp_path):
    """Fresh MIKASA_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(MIKASA_HOME):
    _write_env(MIKASA_HOME, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(MIKASA_HOME, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = MIKASA_HOME / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(MIKASA_HOME):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(MIKASA_HOME, OPENAI_API_KEY=old)
    _write_config(
        MIKASA_HOME,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = MIKASA_HOME.joinpath("config.yaml").read_text(encoding="utf-8")
...
def test_put_api_env_materializes_credential_pool_entry(MIKASA_HOME):
...
def test_put_api_env_writes_auth_json_for_provider(MIKASA_HOME):
...
def test_update_rotates_keyed_providers_mirror(MIKASA_HOME):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(MIKASA_HOME, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(MIKASA_HOME):
    old = "sk-kp-" + "p" * 24
    _write_env(MIKASA_HOME, OPENAI_API_KEY=old)
    _write_config(
        MIKASA_HOME,
...
def test_scrub_never_touches_providers_base_url_alias(MIKASA_HOME):
