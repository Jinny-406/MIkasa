...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("mikasa_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from mikasa_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("mikasa_cli.main_provider_setup._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("mikasa_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("mikasa_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndNousUnchanged:
    ...
    def test_openrouter_does_not_call_merge(self):
        ...
        with patch(
            "mikasa_cli.models._merge_with_models_dev",
