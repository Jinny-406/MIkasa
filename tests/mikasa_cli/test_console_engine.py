...
from mikasa_cli.console_engine import HermesConsoleEngine, run_console_repl
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_mikasa_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_mikasa_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_mikasa_home):
...
def test_capture_output_surfaces_string_exit_code_as_command_error():
    from mikasa_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_mikasa_home):
    result = HermesConsoleEngine().execute(
...
def test_console_checkpoints_prune_does_not_reprompt_for_orphans(
    _isolate_mikasa_home, monkeypatch
):
