...
def test_install_id_stable_and_path_scoped():
    a = pi.install_id(Path("/opt/mikasa"))
    assert a == pi.install_id(Path("/opt/mikasa"))
...
def test_register_self_falls_back_to_desktop_parent_env(tmp_path):
    ...
    fake = _fake_psutil({999: 50.0})
    env = {
        "mikasa_PARENT_PID": "888",
        "mikasa_PARENT_START_MARKER": "winms:1755689000123",
    }
    with patch.dict(sys.modules, {"psutil": fake}), \
         patch.dict(pi.os.environ, env, clear=False), \
...
def _holders(*pids):
    return [(p, "python.exe", f"python.exe -m mikasa_cli.main --profile p{p} serve") for p in pids]
