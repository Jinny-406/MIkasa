...
from mikasa_cli import plugin_index
from mikasa_cli.plugin_index import (
    PluginIndexEntry,
    _parse_entries,
    load_index,
    resolve_name,
...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(plugin_index, "get_mikasa_home", lambda: tmp_path)
    return tmp_path
...
class TestLoadIndex:
    def test_fresh_cache_wins_without_network(self, MIKASA_HOME, monkeypatch):
        _write_cache(MIKASA_HOME, SAMPLE)
    ...
    def test_expired_cache_triggers_remote(self, MIKASA_HOME, monkeypatch):
        _write_cache(MIKASA_HOME, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_remote_failure_falls_back_to_stale_cache(self, MIKASA_HOME, monkeypatch):
        _write_cache(MIKASA_HOME, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_no_cache_no_remote_falls_back_to_seed(self, MIKASA_HOME, monkeypatch):
        monkeypatch.setattr(plugin_index, "_fetch_remote", lambda: None)
        entries, source = load_index()
        assert source == "seed"
        assert len(entries) >= 3

    def test_refresh_bypasses_fresh_cache(self, MIKASA_HOME, monkeypatch):
        _write_cache(MIKASA_HOME, SAMPLE)
    ...
    def test_offline_skips_network(self, MIKASA_HOME, monkeypatch):
    ...
    def test_corrupt_cache_ignored(self, MIKASA_HOME, monkeypatch):
        cache = MIKASA_HOME / "cache" / "plugin_index.json"
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_fetch_writes_cache(self, MIKASA_HOME, monkeypatch):
...
class TestInstallResolution:
    def test_bare_name_detection(self):
        from mikasa_cli.plugins_cmd import _looks_like_bare_index_name
    ...
    def test_install_resolves_name_and_pins_ref(self, MIKASA_HOME, monkeypatch):
        from mikasa_cli import plugins_cmd
    ...
    def test_install_explicit_ref_beats_index_pin(self, MIKASA_HOME, monkeypatch):
        from mikasa_cli import plugins_cmd
    ...
    def test_install_ambiguous_name_lists_candidates_and_exits(
        self, MIKASA_HOME, monkeypatch, capsys
    ):
        from mikasa_cli import plugins_cmd
    ...
    def test_install_unknown_name_exits(self, MIKASA_HOME, monkeypatch, capsys):
        from mikasa_cli import plugins_cmd
    ...
    def test_owner_repo_passthrough_skips_index(self, MIKASA_HOME, monkeypatch):
...
class TestCmdSearch:
    def test_json_output(self, MIKASA_HOME, monkeypatch, capsys):
        from mikasa_cli import plugins_cmd
    ...
    def test_no_results_message(self, MIKASA_HOME, monkeypatch, capsys):
        from mikasa_cli import plugins_cmd
    ...
    def test_dispatch_routes_search(self, MIKASA_HOME, monkeypatch):
        from mikasa_cli import plugins_cmd
