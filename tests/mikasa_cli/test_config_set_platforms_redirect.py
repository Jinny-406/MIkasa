...
def _write_config(MIKASA_HOME: Path, data: dict) -> Path:
...
def _set(monkeypatch, MIKASA_HOME, key, value, force=False):
    """Isolated call to set_config_value against a temp MIKASA_HOME."""
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    # set_config_value resolves the home live via get_config_path()/get_mikasa_home()
...
class TestPerPlatformDisplayRedirect:
    def test_streaming_redirects_to_display_platforms(self, MIKASA_HOME, monkeypatch):
        """platforms.telegram.streaming must land under display.platforms."""
        _set(monkeypatch, MIKASA_HOME, "platforms.telegram.streaming", "false")

        result = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
    ...
    def test_show_reasoning_redirects(self, MIKASA_HOME, monkeypatch):
        _set(monkeypatch, MIKASA_HOME, "platforms.telegram.show_reasoning", "false")
        result = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
        assert result["display"]["platforms"]["telegram"]["show_reasoning"] is False

    def test_tool_progress_redirects(self, MIKASA_HOME, monkeypatch):
        ...
        # is the bool.
        _set(monkeypatch, MIKASA_HOME, "platforms.discord.tool_progress", "off")
        result = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
        assert result["display"]["platforms"]["discord"]["tool_progress"] is False

    def test_connection_key_not_redirected(self, MIKASA_HOME, monkeypatch):
        """A real connection key (token) stays in top-level platforms.<name>."""
        _set(monkeypatch, MIKASA_HOME, "platforms.telegram.token", "new-token")
        result = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
...
class TestRedirectSiblingSurfaces:
    ...
    def test_get_mirrors_gateway_resolution_after_set(self, MIKASA_HOME, monkeypatch, capsys):
        from gateway.display_config import resolve_display_setting
        from mikasa_cli.config import get_config_value
        ...
        raw = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
        assert resolve_display_setting(raw, "telegram", "streaming") is False

    def test_unset_removes_the_redirected_leaf(self, MIKASA_HOME, monkeypatch):
    ...
    def test_unset_missing_redirected_leaf_exits_nonzero(self, MIKASA_HOME, monkeypatch):
        from mikasa_cli.config import unset_config_value
    ...
    def test_set_prints_redirect_note(self, MIKASA_HOME, monkeypatch, capsys):
        _set(monkeypatch, MIKASA_HOME, "platforms.telegram.streaming", "false")
    ...
    def test_redirect_helper_only_touches_known_display_keys(self):
        ...
        from mikasa_cli.config import _redirect_platform_display_key
