...
import mikasa_cli.doctor as doctor
import mikasa_constants
from mikasa_cli import config as config_mod
import mikasa_cli.gateway as gateway_cli
from mikasa_cli import doctor as doctor_mod
from mikasa_cli.doctor_config import _has_provider_env_config
import shutil
from mikasa_cli import doctor_tools
from mikasa_cli import doctor_state
from mikasa_cli import doctor_platform
from mikasa_cli import doctor_config
...
class TestDoctorPlatformHints:
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert mikasa_constants.is_termux() is True
    ...
    def test_sqlite_upgrade_hint_preserves_nix_guidance_as_prose(self):
        from mikasa_cli.config import recommended_update_command_for_method
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        env_path = MIKASA_HOME / ".env"
        env_path.write_text(
            "OPENAI_API_KEY=sk-test  # em-dash here — should not crash\n",
            encoding="utf-8",
        )

        monkeypatch.setattr(doctor_mod, "mikasa_HOME", MIKASA_HOME)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("mikasa_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_mikasa_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal MIKASA_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_mikasa_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "mikasa_HOME", home)
        ...
        try:
            from mikasa_cli import auth as _auth_mod
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp MIKASA_HOME.

        ``run_doctor`` reads the module-level ``MIKASA_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("mikasa_HOME")`` alone
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from mikasa_cli.doctor import run_doctor
        import io, contextlib
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from mikasa_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("mikasa_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "mikasa_cli.doctor", raising=False)
        from mikasa_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"mikasa_MAX_ITERATIONS={ghost}\n")
        (MIKASA_HOME / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "mikasa_HOME", MIKASA_HOME)
        monkeypatch.setattr(doctor_mod, "get_mikasa_home", lambda: MIKASA_HOME)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("mikasa_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("mikasa_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (MIKASA_HOME / ".env").read_text(encoding="utf-8")
        assert "mikasa_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_hermes_tool_progress_warning_says_unsupported_since_floor(self):
        """mikasa_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"mikasa_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["mikasa_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({"mikasa_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["mikasa_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir(parents=True)
        (MIKASA_HOME / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (MIKASA_HOME / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_config.report_deprecated_config_and_env(
            ...
            {"mikasa_TOOL_PROGRESS_MODE": "verbose"},
        )
