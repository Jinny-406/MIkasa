...
Path.home() and MIKASA_HOME are redirected to tmp_path so nothing touches
the real ~/.hermes.
"""
...
from mikasa_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and MIKASA_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, MIKASA_HOME -> tmp/.hermes."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".hermes"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("mikasa_HOME", str(default_home))
    return tmp_path


@pytest.fixture()
def MIKASA_HOME(profile_env):
...
def run_import(agent, source, MIKASA_HOME, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=MIKASA_HOME,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, MIKASA_HOME, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, MIKASA_HOME)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, MIKASA_HOME):
        before = snapshot_tree(MIKASA_HOME)
        report = run_import("claude-code", claude_tree, MIKASA_HOME, execute=False)
        assert snapshot_tree(MIKASA_HOME) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, MIKASA_HOME):
        preview = run_import("claude-code", claude_tree, MIKASA_HOME, execute=False)
        applied = run_import("claude-code", claude_tree, MIKASA_HOME, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, MIKASA_HOME):
        return run_import("claude-code", claude_tree, MIKASA_HOME, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, MIKASA_HOME):
        config = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, MIKASA_HOME):
        return run_import("codex", codex_tree, MIKASA_HOME, execute=True)


    def test_mcp_servers_from_config_toml(self, report, MIKASA_HOME):
        config = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, MIKASA_HOME):
        assert (MIKASA_HOME / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, MIKASA_HOME):
        run_import("claude-code", claude_tree, MIKASA_HOME, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in MIKASA_HOME.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, MIKASA_HOME):
        run_import("codex", codex_tree, MIKASA_HOME, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in MIKASA_HOME.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, MIKASA_HOME):
        report = run_import("claude-code", claude_tree, MIKASA_HOME, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, MIKASA_HOME):
        run_import("claude-code", claude_tree, MIKASA_HOME, execute=True)
        config = yaml.safe_load((MIKASA_HOME / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, MIKASA_HOME):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, MIKASA_HOME, execute=True)
        ...
        assert "still importable" in (
            MIKASA_HOME / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, MIKASA_HOME):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, MIKASA_HOME, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, MIKASA_HOME):
        dest = MIKASA_HOME / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, MIKASA_HOME):
        run_import("claude-code", claude_tree, MIKASA_HOME, execute=True)
        first = (MIKASA_HOME / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, MIKASA_HOME, execute=True)
        assert (MIKASA_HOME / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, MIKASA_HOME):
        memory = MIKASA_HOME / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, MIKASA_HOME):
        return MIKASA_HOME / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, MIKASA_HOME, config_path):
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, MIKASA_HOME,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in MIKASA_HOME.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, MIKASA_HOME, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, MIKASA_HOME, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from mikasa_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, MIKASA_HOME, capsys):
        """End-to-end through import_agent_command with --dry-run."""
        import types
        from mikasa_cli.agent_import import import_agent_command
        ...
        assert not (MIKASA_HOME / "memories" / "MEMORY.md").exists()
        assert not (MIKASA_HOME / "skills" / "claude-code-imports").exists()
        config_text = (MIKASA_HOME / "config.yaml").read_text(encoding="utf-8") \
            if (MIKASA_HOME / "config.yaml").exists() else ""
