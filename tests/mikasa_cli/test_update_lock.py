...
def test_marker_path_follows_process_mikasa_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* MIKASA_HOME; a profile-scoped path would
...
class TestAncestryHandoff:
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("mikasa_cli.update_lock._pid_alive", lambda pid: True)
