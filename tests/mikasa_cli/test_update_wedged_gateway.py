...
def tmp_path():
    ...
    The loop-tick witness tests bind real UNIX sockets under MIKASA_HOME.
...
def _launchd_harness(monkeypatch, tmp_path, pid):
    ...
    monkeypatch.setattr(
        "gateway.shutdown_watchdog._process_mikasa_home", lambda: tmp_path
    )
    return events
