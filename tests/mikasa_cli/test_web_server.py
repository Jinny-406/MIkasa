...
import mikasa_cli.config as _cfg_mod
import mikasa_cli.web_routers.chat_ws as _rt_chat_ws
import mikasa_cli.web_routers.status as _rt_status
import mikasa_cli.web_server_chat as _web_server_chat
import mikasa_cli.web_server_config as _web_server_config
import mikasa_cli.web_server_dashboard as _web_server_dashboard
import mikasa_cli.web_server_files as _web_server_files
import mikasa_cli.web_server_gateway as _web_server_gateway
import mikasa_cli.web_server_lifecycle as _web_server_lifecycle
import mikasa_cli.web_server_memory as _web_server_memory
import mikasa_cli.web_server_messaging as _web_server_messaging
import mikasa_cli.web_server_sessions as _web_server_sessions
...
@pytest.fixture
def _install_example_plugin(_isolate_mikasa_home):
    """Drop the example-dashboard fixture into the per-test MIKASA_HOME
    ...
    from mikasa_cli import web_server

    user_plugins_dir = get_mikasa_home() / "plugins"
    ...
    # enable example`.
    from mikasa_cli.config import load_config, save_config
    _cfg = load_config()
    ...
    #      and start serving requests against a torn-down MIKASA_HOME.
    app = web_server.app
...
class TestSessionTokenInjection:
    ...
    def test_honors_injected_token(self, monkeypatch):
        import mikasa_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("mikasa_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mikasa_home):
        """Create a TestClient and isolate the state DB under the test MIKASA_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import mikasa_state
        from mikasa_constants import get_mikasa_home
        from mikasa_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(hermes_state, "DEFAULT_DB_PATH", get_mikasa_home() / "state.db")
    ...
    def test_get_sessions_poll_preserves_pending_wal(self):
        ...
        db_path = get_mikasa_home() / "state.db"
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import mikasa_cli.web_server as web_server
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        MIKASA_HOME contextvar override (#56986), so /api/messaging/platforms
        ...
        """
        import mikasa_cli.web_server as web_server
        from mikasa_cli import profiles as profiles_mod
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from mikasa_constants import get_mikasa_home
        from mikasa_cli.config import load_env
        ...
        config_path = get_mikasa_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from mikasa_constants import get_mikasa_home
        from mikasa_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide MIKASA_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from mikasa_constants import get_mikasa_home

        path = get_mikasa_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_mikasa_home() / "honcho.json").read_text(encoding="utf-8"))
    ...
    def test_get_media_requires_auth(self):
        from mikasa_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        # method through mikasa_cli.config directly.
        monkeypatch.setattr(
            "mikasa_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
    ...
    def test_update_hermes_returns_apt_guidance_without_spawning(self, monkeypatch):
        ...
        monkeypatch.setattr(_web_server_files, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through mikasa_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "mikasa_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
    ...
    def test_update_hermes_spawns_with_action_id(self, monkeypatch):
        ...
        assert calls == [
            (["update"], "hermes-update", {"mikasa_ACTION_ID": "a" * 32})
        ]
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        """
        from mikasa_cli.web_server_messaging import _MESSAGING_KEYS_PAGE_KEYS, _build_catalog_entry, _channel_managed_env_keys
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from mikasa_cli.config import save_env_value
        from mikasa_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from mikasa_cli.config import save_env_value
        from mikasa_cli.web_server import _SESSION_TOKEN
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "mikasa_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "mikasa_pair_restart_fails_bot",
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from mikasa_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from mikasa_cli.web_server_profiles import _parse_model_ids
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from mikasa_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from mikasa_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from mikasa_cli import profiles as profiles_mod
        from mikasa_cli.config import custom_endpoint_key_env
        from mikasa_constants import get_mikasa_home

        default_home = get_mikasa_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from mikasa_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from mikasa_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from mikasa_cli.web_server_config import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from mikasa_cli.web_server_config import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from mikasa_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_mikasa_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from mikasa_constants import get_mikasa_home
        import mikasa_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import mikasa_cli.tools_config as tools_config
        from mikasa_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import mikasa_cli.tools_config as tools_config
        from mikasa_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "mikasa_cli.nous_subscription.get_nous_portal_account_info",
        ...
        import mikasa_cli.tools_config_post_setup as tools_config_post_setup
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_DESKTOP", "1")
        monkeypatch.setenv("mikasa_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_DASHBOARD_SESSION_TOKEN", "tok")
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from mikasa_cli.web_server_config import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from mikasa_cli.web_server_config import _denormalize_config_from_web
        from mikasa_cli.config import save_config
...
class TestStatusInstallId:
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_mikasa_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """mikasa_HOME=<root> and MIKASA_HOME=<root>/profiles/<name> resolve to
        ...
        monkeypatch.setenv("mikasa_HOME", str(profile_home))
        monkeypatch.setattr(ws, "_INSTALL_ID_CACHE", {"value": None})
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from mikasa_cli.web_server_dashboard import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        from mikasa_constants import (
            reset_mikasa_home_override,
            set_mikasa_home_override,
        )
        from mikasa_cli import web_server

        token = set_mikasa_home_override(str(other))
        try:
            results = _web_server_dashboard._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(MIKASA_HOME, name="ocean"):
        themes_dir = MIKASA_HOME / "dashboard-themes"
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mikasa_home):
        ...
        monkeypatch.setattr(
            hermes_state, "DEFAULT_DB_PATH", get_mikasa_home() / "state.db"
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mikasa_home):
        ...
        # Pin the SessionDB to the isolated MIKASA_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_mikasa_home, _install_example_plugin):
        ...
        installs it into the per-test ``MIKASA_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into MIKASA_HOME by the class-level
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME override scopes a request to another profile."""
        from mikasa_constants import (
            reset_mikasa_home_override,
            set_mikasa_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets MIKASA_HOME=<root>/profiles/<name>) must still discover user
...
import sys
from mikasa_cli import main_tui_launch
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        import mikasa_cli.main as main_mod
        ...
        env = {
            "mikasa_CWD": str(tmp_path),
            "mikasa_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_tui_launch._apply_tui_python_env(env)

        assert env["mikasa_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from mikasa_cli.pty_bridge import PtyUnavailableError
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from mikasa_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import mikasa_cli.main_tui_launch as tui_launch
    import mikasa_cli.web_server as ws
    ...
    gateway_url = env.get("mikasa_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mikasa_home, _install_example_plugin):
        ...
        the per-test ``MIKASA_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_mikasa_home):
        import threading
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("mikasa_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        _web_server_dashboard.mount_spa(spa_app)
...
class TestHeadlessServeTokenPage:
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("mikasa_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
