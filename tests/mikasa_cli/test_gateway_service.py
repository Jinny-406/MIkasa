...
import mikasa_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``MIKASA_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` MIKASA_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir MIKASA_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="mikasa_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="mikasa_HOME=/tmp/hermes-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="mikasa_HOME={tmp_path}/hermes-home"\n'
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("mikasa_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("mikasa_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("mikasa_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        import mikasa_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_mikasa_home", lambda: tmp_path)
...
class TestSystemUnitHermesHome:
    """mikasa_HOME in system units must reference the target user, not root."""
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        monkeypatch.setenv("mikasa_HOME", str(root_hermes))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        )
        monkeypatch.setattr(gateway_cli, "get_mikasa_home", lambda: root_hermes)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "mikasa_constants.iter_hermes_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "mikasa_constants.hermes_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'mikasa_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's MIKASA_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        MIKASA_HOME = str(gateway_cli.get_mikasa_home().resolve())
        assert f'mikasa_HOME={MIKASA_HOME}' in unit


class TestSystemUnitRefreshSyncsHermesHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.hermes."""

    def test_refresh_adopts_unit_mikasa_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        ...
        # Correct installed unit (operator's MIKASA_HOME + drain timeout).
        monkeypatch.setenv("mikasa_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited MIKASA_HOME (falls back to root).
        monkeypatch.setenv("mikasa_HOME", str(root_hermes))
        ...
        assert os.environ["mikasa_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned MIKASA_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_mikasa_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_mikasa_home_from_systemd_unit",
...
class TestHermesHomeForTargetUser:
    """Unit tests for _mikasa_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("mikasa_HOME", raising=False)

        result = gateway_cli._mikasa_home_for_target_user("/home/alice")
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_mikasa_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        ...
        assert f'mikasa_HOME={target_home / ".hermes" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".hermes" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("mikasa_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_mikasa_home", lambda: profile_dir)
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("mikasa_HOME", str(root_home / ".hermes"))
        monkeypatch.setattr(gateway_cli, "get_mikasa_home", lambda: root_home / ".hermes")
        ...
        # WorkingDirectory is anchored at the target user's MIKASA_HOME (stable,
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m mikasa_cli.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m mikasa_cli.main gateway run
        ...
        variants = [
            "ExecStart=/venv/bin/python -m mikasa_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/mikasa/hermes_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/hermes gateway run --replace",
            "ExecStart=/venv/bin/python /opt/mikasa/gateway/run.py",
        ]
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import mikasa_cli.main as cli_main
        ...
        if project_root is None:
            import mikasa_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "mikasa_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "mikasa_cli.main", "gateway", "status", "-l", "--help"],
...
class TestServiceWorkingDirIsStable:
    ...
    (MIKASA_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_mikasa_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_mikasa_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_mikasa_home(self, tmp_path, monkeypatch):
        import re
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        ...
        monkeypatch.setenv("mikasa_HOME", str(hermes))
        monkeypatch.delenv("mikasa_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("mikasa_CRON_DRAIN_TIMEOUT", raising=False)
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
        monkeypatch.setattr(
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"mikasa_CRON_DRAIN_TIMEOUT": "200"},
        )
