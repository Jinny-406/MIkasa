    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                ...
                # mikasa_constants._get_platform_default_mikasa_home.
...
class TestEnsureHermesHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from mikasa_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from mikasa_cli import config as cfg_mod
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from mikasa_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"mikasa_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"mikasa_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"mikasa_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"mikasa_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("mikasa_cli.config.sanitize_env_file", return_value=2),
            patch(
                "mikasa_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("mikasa_cli.config.read_raw_config", return_value={}),
            patch("mikasa_cli.config.get_missing_env_vars", return_value=[]),
            patch("mikasa_cli.config.get_missing_config_fields", return_value=[]),
            patch("mikasa_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from mikasa_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """mikasa_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).
        ...
        """
        from mikasa_cli.config import OPTIONAL_ENV_VARS
        assert "mikasa_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestConfigSupportFloor:
    ...
    def test_floor_message_uses_display_mikasa_home(self):
        from mikasa_cli.config_migrations import support_floor_message
        from mikasa_constants import display_mikasa_home

        msg = support_floor_message()
        assert f"{display_mikasa_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from mikasa_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    def _mikasa_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "mikasa_LANGFUSE_PUBLIC_KEY",
            "mikasa_SPOTIFY_CLIENT_ID",
            "mikasa_QWEN_BASE_URL",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "mikasa_COPILOT_ACP_COMMAND",
            "mikasa_COPILOT_ACP_ARGS",
            "mikasa_YOLO_MODE",
            "mikasa_ACCEPT_HOOKS",
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from mikasa_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"mikasa_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["mikasa_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("mikasa_Yolo_Mode", "mikasa_YOLO_MODE"),
            ("mikasa_Optional_Mcps", "mikasa_OPTIONAL_MCPS"),
            ("mikasa_Copilot_Acp_Command", "mikasa_COPILOT_ACP_COMMAND"),
            ("mikasa_Copilot_Acp_Args", "mikasa_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from mikasa_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from mikasa_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from mikasa_cli.config import _persist_migration, read_raw_config
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import mikasa_cli.config as cfg_mod
