...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (MIKASA_HOME / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from mikasa_cli.auth_commands import auth_add_command
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((MIKASA_HOME / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("mikasa_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from mikasa_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from mikasa_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from mikasa_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from mikasa_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from mikasa_cli.auth import clear_provider_auth
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from mikasa_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (MIKASA_HOME / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (MIKASA_HOME / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["mikasa_pkce"]},
    }))
    ...
    assert entries == []
    assert "mikasa_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from mikasa_cli.auth import is_source_suppressed
    from mikasa_cli.auth_commands import auth_remove_command

    with patch(
        "mikasa_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "mikasa_cli.copilot_auth.get_copilot_api_token",
...
def test_qwen_oauth_login_marks_active_through_moved_owner(monkeypatch):
    ...
    import mikasa_cli.auth_commands as auth_commands
    import mikasa_cli.auth_qwen as auth_qwen
