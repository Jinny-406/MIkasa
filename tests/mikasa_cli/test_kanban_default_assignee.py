...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh MIKASA_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("mikasa_HOME", test_home)
    # Force-reimport so the fresh MIKASA_HOME is picked up.
    for mod in list(sys.modules.keys()):
        if mod.startswith("mikasa_cli") or mod.startswith("mikasa_state") or mod == "mikasa_constants":
            del sys.modules[mod]
    from mikasa_cli import kanban_db
    yield kanban_db, test_home
    # Cleanup is best-effort; tempfile dir survives but pytest isolation
    # gives each test its own monkeypatched MIKASA_HOME so no cross-test
    # contamination.
