"""Regression tests for _apply_profile_override MIKASA_HOME guard (issue #22502).

When MIKASA_HOME is set to the hermes root (e.g. systemd hardcodes
MIKASA_HOME=/root/.hermes), _apply_profile_override must still read
active_profile and update MIKASA_HOME to the profile directory.

When MIKASA_HOME is already a profile directory (.../profiles/<name>),
...
def _run_apply_profile_override(
    tmp_path, monkeypatch, *, MIKASA_HOME: str | None, active_profile: str | None,
    argv: list[str] | None = None, extra_env: dict[str, str] | None = None,
):
    """Run _apply_profile_override in isolation.

    Returns the value of os.environ["mikasa_HOME"] after the call,
    or None if unset.
    """
    ...
    if MIKASA_HOME is not None:
        monkeypatch.setenv("mikasa_HOME", MIKASA_HOME)
    else:
    ...
    for var in (
        "mikasa_SUPERVISED_CHILD",
        "mikasa_S6_SUPERVISED_CHILD",
        "INVOCATION_ID",
    ...
    from mikasa_cli.main import _apply_profile_override
    _apply_profile_override()

    return os.environ.get("mikasa_HOME")


class TestApplyProfileOverrideHermesHomeGuard:
    """Regression guard for issue #22502.

    Verifies that MIKASA_HOME pointing to the hermes root does NOT suppress
    the active_profile check, while MIKASA_HOME already pointing to a
    profile directory IS trusted as-is.
    """

    def test_mikasa_home_at_root_with_active_profile_is_redirected(
        self, tmp_path, monkeypatch
    ):
        """mikasa_HOME=/root/.hermes + active_profile=coder must redirect
        MIKASA_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets MIKASA_HOME to the hermes root
        ...
        result = _run_apply_profile_override(
            tmp_path,
            monkeypatch,
            MIKASA_HOME=str(hermes_root),
            active_profile="coder",
        )

        assert result is not None, "mikasa_HOME must be set after profile redirect"
        assert "profiles" in result, (
            f"Expected MIKASA_HOME to point into profiles/ dir, got: {result!r}"
        )
        assert result.endswith("coder"), (
            f"Expected MIKASA_HOME to end with 'coder', got: {result!r}"
        )
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        assert os.environ.get("mikasa_HOME") == str(profile_dir)
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``hermes gateway run`` (no ``-p``) to mean "the root MIKASA_HOME
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.delenv("mikasa_HOME", raising=False)
        monkeypatch.setenv("mikasa_S6_SUPERVISED_CHILD", "1")
        monkeypatch.setattr(sys, "argv", ["hermes", "-p", "coder", "gateway", "run"])

        from mikasa_cli.main import _apply_profile_override
        _apply_profile_override()

        result = os.environ.get("mikasa_HOME")
        assert result is not None
        assert result.endswith("coder")
...
class TestGeneralizedSupervisorMarkers:
    ...
    profile identity via the unit's MIKASA_HOME (root home for the default
    ...
    def test_supervised_child_marker_skips_active_profile(
        ...
        """mikasa_SUPERVISED_CHILD=1 + root MIKASA_HOME must keep the
        ...
        result = _run_apply_profile_override(
            ...
            argv=["hermes", "gateway", "run"],
            extra_env={"mikasa_SUPERVISED_CHILD": "1"},
        )
    ...
    def test_external_supervisor_marker_skips_active_profile(
        ...
        result = _run_apply_profile_override(
            ...
            active_profile="telegram_nick",
            argv=["hermes", "gateway", "run"],
            extra_env={"mikasa_GATEWAY_EXTERNAL_SUPERVISOR": "1"},
        )
        assert result == str(hermes_root)
    ...
    def test_generated_systemd_unit_exports_supervised_marker(
        ...
        monkeypatch.setenv("mikasa_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
        from mikasa_cli.gateway import generate_systemd_unit

        unit = generate_systemd_unit()
        assert 'Environment="mikasa_SUPERVISED_CHILD=1"' in unit
