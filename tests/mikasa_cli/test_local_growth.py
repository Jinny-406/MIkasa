...
def test_overrides_roundtrip_and_clear(MIKASA_HOME):
    from mikasa_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(MIKASA_HOME):
    from mikasa_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(MIKASA_HOME):
    ...
    from mikasa_cli.local_runtime.growth import maybe_grow_window
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    from mikasa_cli.local_runtime.context_policy import growth_decision
    from mikasa_cli.local_runtime.estimator import (
        HardwareBudget,
        LayerKind,
        ModelProfile,
    )
...
def _tiny_profile(model_id: str):
    from mikasa_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(MIKASA_HOME, tmp_path, monkeypatch):
    ...
    import mikasa_cli.local_runtime.presets as presets_mod

    from mikasa_cli.local_runtime.catalog import CATALOG
    from mikasa_cli.local_runtime.estimator import HardwareBudget
...
def test_preset_restores_grown_window_capped_at_native(MIKASA_HOME, tmp_path, monkeypatch):
    ...
    from mikasa_cli.local_runtime.estimator import HardwareBudget
    from mikasa_cli.local_runtime.growth import save_window_override
...
def test_preset_ignores_override_below_launch_window(MIKASA_HOME, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(MIKASA_HOME, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import mikasa_cli.local_runtime.presets as presets_mod

    from mikasa_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from mikasa_cli.local_runtime.growth import save_window_override
...
def test_sampling_ladder_file_beats_catalog_beats_nothing(MIKASA_HOME, tmp_path, monkeypatch):
