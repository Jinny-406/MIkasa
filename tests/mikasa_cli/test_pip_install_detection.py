...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $MIKASA_HOME.

    Models a host git install whose $MIKASA_HOME is shared with (and stamped
    ...
    with patch("mikasa_cli.config.get_managed_system", return_value=None), \
         patch("mikasa_cli.config.get_mikasa_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("mikasa_cli.config.get_managed_system", return_value=None), \
         patch("mikasa_cli.config.get_mikasa_home", return_value=tmp_path), \
         patch("mikasa_constants.is_container", return_value=True):
        from mikasa_cli.config import detect_install_method
