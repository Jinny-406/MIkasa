...
import mikasa_cli.gui_uninstall as gu


def _make_agent(MIKASA_HOME: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = MIKASA_HOME / "hermes-agent"
    (agent_root / "mikasa_cli").mkdir(parents=True)
    (agent_root / "mikasa_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(MIKASA_HOME: Path) -> None:
    """Create the source-built GUI artifacts a `hermes desktop` run produces."""
    desktop = MIKASA_HOME / "hermes-agent" / "apps" / "desktop"
    ...
    (MIKASA_HOME / "hermes-agent" / "node_modules").mkdir(parents=True)
    (MIKASA_HOME / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(MIKASA_HOME: Path) -> None:
    (MIKASA_HOME / "config.yaml").write_text("x: 1\n")
    (MIKASA_HOME / ".env").write_text("KEY=secret\n")
    (MIKASA_HOME / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(MIKASA_HOME)
    _make_gui_build(MIKASA_HOME)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(MIKASA_HOME)
    ...
    assert summary["mikasa_home"] == str(MIKASA_HOME)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    ...
    from mikasa_cli import linux_desktop_entry as lde
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    MIKASA_HOME = tmp_path / ".hermes"
    _make_agent(MIKASA_HOME)
    icon = lde.icon_path(MIKASA_HOME / "hermes-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(MIKASA_HOME)
    ...
    assert lde.icon_path(MIKASA_HOME / "hermes-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (MIKASA_HOME / "hermes-agent" / "mikasa_cli").is_dir()
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import mikasa_cli.uninstall as uninstall
