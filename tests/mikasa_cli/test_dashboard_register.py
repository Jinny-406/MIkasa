...
import mikasa_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "mikasa_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("mikasa_cli.config.is_managed", return_value=False):
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        def fake_get_env(key):
            if key == "mikasa_DASHBOARD_OAUTH_CLIENT_ID":
                return existing_client_id
            return None

        with patch(
            "mikasa_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("mikasa_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "mikasa_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "mikasa_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["mikasa_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
        assert "mikasa_DASHBOARD_PORTAL_URL" not in saved
...
class TestIdempotentRerun(TestHappyPath):
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["mikasa_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "mikasa_DASHBOARD_PORTAL_URL":
                return existing_portal
            return None

        with patch(
            "mikasa_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("mikasa_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "mikasa_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "mikasa_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            ...
            dr.os.environ.pop("mikasa_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
...
class TestPublicUrlPersistence:
    ...
    def _run(self, *, args, existing_public=None):
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "mikasa_DASHBOARD_PUBLIC_URL":
    ...
    def test_no_redirect_flag_does_not_overwrite_existing(self, capsys):
        ...
        assert "mikasa_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        with patch(
            ...
            "mikasa_cli.config.get_env_value", return_value=None
        ), patch(
            "mikasa_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
        ...
        assert saved["mikasa_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["mikasa_DASHBOARD_PUBLIC_URL"] == "https://hermes.example.com"
