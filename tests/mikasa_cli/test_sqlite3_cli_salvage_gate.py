...
import mikasa_state_repair

from mikasa_cli.session_lost_and_found import (
    _parse_sqlite3_cli_version,
    _wal_reset_vulnerable,
    find_sqlite3_cli,
    find_sqlite3_cli_refusal,
)
from mikasa_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable
...
class TestFindSqlite3CliRefusal:
    def test_missing_binary_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "mikasa_cli.session_lost_and_found.shutil.which", lambda _: None
        )
    ...
    def test_no_dbpage_refusal(self, monkeypatch):
        monkeypatch.setattr(
            "mikasa_cli.session_lost_and_found.shutil.which",
            lambda _: "/usr/bin/sqlite3",
        )
        monkeypatch.setattr(
            "mikasa_cli.session_lost_and_found._cli_supports_recover",
            lambda _: False,
        )
        assert find_sqlite3_cli() is None
    ...
    def test_wal_reset_vulnerable_refusal(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mikasa_cli.session_lost_and_found._parse_sqlite3_cli_version",
            lambda _: (3, 45, 1),
        )
        assert find_sqlite3_cli() is None
...
class TestParseSqlite3CliVersion:
    def test_parses_modern_output(self):
        ...
        with patch(
            "mikasa_cli.session_lost_and_found.subprocess.run",
            return_value=Probe(),
        ):
...
class TestGuidanceNeverNamesLiveDb:
    ...
    def test_repair_budget_error_names_safe_lane(self, tmp_path: Path):
        import mikasa_state_repair as hermes_state  # helper lives in the split-out repair module
    ...
    def test_kanban_manual_recovery_warns_about_live_db(self):
        import mikasa_cli.kanban_ops as kanban  # ``_cmd_repair`` lives here (split from mikasa_cli.kanban)
...
class TestEmittedCommandsSatisfyCliContract:
    ...
    def test_old_v1_shape_is_still_rejected(self, tmp_path):
        ...
        import mikasa_cli.sessions_cmd as sc
