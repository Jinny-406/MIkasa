...
def test_register_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # reached on host.
    monkeypatch.setattr(
        "mikasa_cli.service_manager.get_service_manager",
        lambda: _HostManager(),
    )
...
def test_register_passes_start_now_false(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mgr = _S6Manager()
    monkeypatch.setattr(
        "mikasa_cli.service_manager.get_service_manager", lambda: mgr,
    )
...
def test_register_silent_when_detect_throws(
    ...
    monkeypatch.setattr(
        "mikasa_cli.service_manager.detect_service_manager", _broken_detect,
    )
