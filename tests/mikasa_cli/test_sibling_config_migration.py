...
context-local MIKASA_HOME override.
...
import mikasa_cli.update_cmd_config as update_cmd_config
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        mikasa_constants, "get_process_mikasa_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_mikasa_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_mikasa_home_override() == before
