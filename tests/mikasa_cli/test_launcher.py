...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `mikasa_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("mikasa_cli.main")

    def fake_main():
        called.append("mikasa_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "mikasa_cli.main", fake_main_module)
