...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("mikasa_IGNORE_USER_CONFIG", "mikasa_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<MIKASA_HOME>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._mikasa_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        ...
        """
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("mikasa_IGNORE_USER_CONFIG", "1")
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("mikasa_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("mikasa_IGNORE_RULES") == "1"
...
class TestCmdChatWiring:
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("mikasa_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("mikasa_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("mikasa_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("mikasa_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "mikasa_IGNORE_USER_CONFIG" not in os.environ
        assert "mikasa_IGNORE_RULES" not in os.environ
