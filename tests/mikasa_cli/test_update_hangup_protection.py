...
from mikasa_cli.main_dashboard import _UpdateOutputStream, _finalize_update_output, _install_hangup_protection
from mikasa_cli.update_cmd import _log_only_write, _print_update_completion, _run_logged_subprocess


def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("mikasa_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("mikasa_cli.update_cmd._branch_head_suffix", lambda: "")
...
def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("mikasa_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("mikasa_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import mikasa_cli.config as _cfg
        if hasattr(_cfg, "_mikasa_home_CACHE"):
            _cfg._mikasa_home_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_mikasa_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "mikasa_cli.config.get_mikasa_home", _boom, raising=True
        )
