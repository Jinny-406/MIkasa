...
def _isolate_kanban_board_env():
    ...
    """
    prev = os.environ.get("mikasa_KANBAN_BOARD")
    os.environ.pop("mikasa_KANBAN_BOARD", None)
    try:
        yield
    finally:
        if prev is None:
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("mikasa_KANBAN_BOARD") == "preset"
