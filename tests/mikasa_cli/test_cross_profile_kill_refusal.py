...
A poisoned/contaminated ``gateway.pid`` inside one profile's MIKASA_HOME can
truthfully name ANOTHER profile's live gateway (its ``MIKASA_HOME`` stamp
...
def _pid_record(proc: subprocess.Popen, script: Path, owner_home: Path) -> dict:
    ...
    return {
        ...
        "mikasa_home": str(owner_home),
    }


class TestRecordedGatewayHomeConflicts:
    def test_conflicting_home_detected(self, tmp_path, monkeypatch):
        monkeypatch.setenv("mikasa_HOME", str(tmp_path / "profiles" / "tim"))
        record = {"pid": 1, "mikasa_home": str(tmp_path)}
    ...
    def test_same_home_accepted(self, tmp_path, monkeypatch):
        ...
        record = {"pid": 1, "mikasa_home": str(home)}
    ...
    def test_legacy_record_without_home_proves_nothing(self, tmp_path, monkeypatch):
        ...
        assert recorded_gateway_home_conflicts({"pid": 1, "mikasa_home": "  "}) is False
...
class TestCrossProfileStopRefusal:
    def test_stop_profile_gateway_refuses_other_profiles_pid(
        ...
        monkeypatch.setenv("mikasa_HOME", str(tim_home))
        ...
        try:
            ...
            from mikasa_cli import gateway as gateway_cli
...
class TestProfileDeleteStopRefusal:
    def test_stop_gateway_process_refuses_other_profiles_pid(
        ...
        try:
            ...
            from mikasa_cli.profiles import _stop_gateway_process
