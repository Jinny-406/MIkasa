...
@pytest.fixture
def MIKASA_HOME(tmp_path, monkeypatch):
    """Set up an isolated MIKASA_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from mikasa_cli.debug import _upload_paste_rs
        ...
        with patch("mikasa_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "mikasa_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from mikasa_cli.debug import upload_to_pastebin

        with patch("mikasa_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("mikasa_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, MIKASA_HOME, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = MIKASA_HOME / "logs" / "agent.log"
        from mikasa_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, MIKASA_HOME):
        """When truncation lands on a line boundary, keep the first full line."""
        from mikasa_cli.debug import _capture_log_snapshot
        ...
        (MIKASA_HOME / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    def test_backend_written_log_reports_plain_absence(self, MIKASA_HOME):
        from mikasa_cli.debug import _capture_log_snapshot

        (MIKASA_HOME / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, MIKASA_HOME):
        from mikasa_cli.debug import _capture_log_snapshot

        (MIKASA_HOME / "logs" / "desktop.log").unlink()
        ...
        assert str(MIKASA_HOME / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, MIKASA_HOME):
    ...
    def test_empty_client_log_is_empty_not_absent(self, MIKASA_HOME):
        """An empty file means the app ran and logged nothing — a different fact."""
        from mikasa_cli.debug import _capture_log_snapshot

        (MIKASA_HOME / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, MIKASA_HOME):
        """The uploaded report — what people paste into support — must explain it."""
        from mikasa_cli.debug import collect_debug_report
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def MIKASA_HOME_with_secret(self, tmp_path, monkeypatch):
        """Isolated MIKASA_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setenv("mikasa_HOME", str(home))
        ...
        monkeypatch.delenv("mikasa_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, MIKASA_HOME_with_secret):
        from mikasa_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, MIKASA_HOME_with_secret):
        from mikasa_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, MIKASA_HOME_with_secret, monkeypatch
    ):
        ...
        monkeypatch.setenv("mikasa_REDACT_SECRETS", "false")

        from mikasa_cli.debug import _capture_log_snapshot

        assert os.environ.get("mikasa_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, MIKASA_HOME_with_secret
    ):
        from mikasa_cli.debug import _capture_log_snapshot

        log_path = MIKASA_HOME_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, MIKASA_HOME_with_secret):
        from mikasa_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from mikasa_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, MIKASA_HOME):
        from mikasa_cli.debug import collect_debug_report

        with patch("mikasa_cli.dump.run_dump") as mock_dump:
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, MIKASA_HOME, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from mikasa_cli.debug import run_debug_share
        ...
        with patch("mikasa_cli.dump.run_dump"), \
             patch("mikasa_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("mikasa_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, MIKASA_HOME, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, MIKASA_HOME_with_secret, capsys
    ):
        ...
        with patch("mikasa_cli.dump.run_dump"), \
             patch("mikasa_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("mikasa_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        from mikasa_cli.debug import run_debug
    ...
    def test_share_subcommand_routes(self, MIKASA_HOME):
        from mikasa_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from mikasa_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from mikasa_cli.debug import delete_paste
        ...
        with patch("mikasa_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_records_pending_to_json(self, MIKASA_HOME):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from mikasa_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, MIKASA_HOME):
        """Same URL recorded twice → one entry with the later expire_at."""
        from mikasa_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, MIKASA_HOME):
        from mikasa_cli.debug import (
        ...
        with patch("mikasa_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, MIKASA_HOME):
        from mikasa_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("mikasa_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, MIKASA_HOME):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, MIKASA_HOME):
        from mikasa_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("mikasa_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from mikasa_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("mikasa_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, MIKASA_HOME, capsys):
        from mikasa_cli.debug import run_debug_share
        ...
        with patch("mikasa_cli.dump.run_dump"), \
             patch("mikasa_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("mikasa_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, MIKASA_HOME):
        from mikasa_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (MIKASA_HOME / "logs" / "agent.log").write_text(
        ...
        with patch("mikasa_cli.dump.run_dump"), patch(
            "mikasa_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("mikasa_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, MIKASA_HOME):
        from mikasa_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, MIKASA_HOME):
        from mikasa_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, MIKASA_HOME):
        from mikasa_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, MIKASA_HOME):
        import gzip
        import json as _json

        from mikasa_cli.debug import build_nous_bundle
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, MIKASA_HOME, capsys):
        from mikasa_cli.debug import run_debug_share
        ...
        with patch("mikasa_cli.dump.run_dump"), patch(
            "mikasa_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, MIKASA_HOME, capsys):
        from mikasa_cli.debug import run_debug_share

        with patch("mikasa_cli.dump.run_dump"), patch(
            "mikasa_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, MIKASA_HOME):
        from mikasa_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("mikasa_cli.dump.run_dump"), patch(
            "mikasa_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("mikasa_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    def _captured(self, cmd_original):
        ...
        with patch("mikasa_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    ...
    def test_non_interactive_requires_yes(self, MIKASA_HOME, capsys, monkeypatch):
        ...
        with patch("mikasa_cli.dump.run_dump"), \
             patch("mikasa_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, MIKASA_HOME, capsys, monkeypatch):
