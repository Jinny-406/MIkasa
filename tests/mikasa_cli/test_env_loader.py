...
def test_recovered_update_retry_skips_external_secret_sources(tmp_path, monkeypatch):
    ...
    import mikasa_cli.env_loader as env_loader
    ...
    loaded = load_mikasa_dotenv(MIKASA_HOME=home)
...
def test_latin1_fallback_stream_honors_override(tmp_path, monkeypatch):
    ...
    from mikasa_cli.env_loader import _load_dotenv_with_fallback
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from mikasa_cli.env_loader import _sanitize_env_file_if_needed

    env_file = tmp_path / ".env"
    content = "mikasa_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
    raw = codecs.BOM_UTF32_LE + content.encode("utf-32-le")
    env_file.write_bytes(raw)

    with caplog.at_level(logging.WARNING, logger="mikasa_cli.env_loader"):
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("mikasa_ACP_AUTH_METHOD", "cursor_login")
    ...
    load_mikasa_dotenv(MIKASA_HOME=home)
    ...
    assert "mikasa_ACP_AUTH_METHOD" not in os.environ
...
def test_empty_assignment_in_user_env_is_preserved(tmp_path, monkeypatch):
    ...
    home = tmp_path / "hermes"
    home.mkdir()
    (home / ".env").write_text("mikasa_ACP_AUTH_METHOD=\n", encoding="utf-8")
    ...
    assert "mikasa_ACP_AUTH_METHOD" in os.environ
    assert os.environ["mikasa_ACP_AUTH_METHOD"] == ""
    # COPILOT_CLI_PATH is absent from .env → cleared
    assert "COPILOT_CLI_PATH" not in os.environ


def test_no_user_env_does_not_clear_anything(tmp_path, monkeypatch):
    """When no profile .env exists (bare profile), load_mikasa_dotenv must not
    ...
    assert os.getenv("mikasa_ACP_AUTH_METHOD") == "cursor_login"
...
def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "mikasa_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    ...
    assert os.getenv("mikasa_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from mikasa_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
#
# load_mikasa_dotenv loads .env with override=True, so a stale
...
# the shared mikasa_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
...
def _seed_terminal_home(tmp_path, monkeypatch, *, config_yaml=None, env_text=None):
    ...
    # The bridge is scoped to the process MIKASA_HOME (a different profile's
...
def test_other_profile_home_does_not_bridge_process_config(tmp_path, monkeypatch):
    ...
    load_mikasa_dotenv(MIKASA_HOME=other_home)
