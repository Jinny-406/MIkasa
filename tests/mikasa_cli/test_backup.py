...
def _no_real_gateway_service(monkeypatch):
    ...
    assert the wiring."""
    import mikasa_cli.gateway as gateway_mod
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    ...
    import mikasa_cli.backup as _backup
...
def _make_hermes_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "mikasa_state.db"):
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        from mikasa_cli.backup import _should_exclude
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from mikasa_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the MIKASA_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        from mikasa_cli.backup import _iter_backup_files
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        _make_hermes_tree(MIKASA_HOME)

        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        ...
        import mikasa_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = MIKASA_HOME / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, MIKASA_HOME)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside MIKASA_HOME."""
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        _make_hermes_tree(MIKASA_HOME)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(MIKASA_HOME / "skills" / "outside-link.txt", outside)
        ...
        from mikasa_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME.mkdir()
        _make_hermes_tree(MIKASA_HOME)
        with sqlite3.connect(MIKASA_HOME / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from mikasa_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(MIKASA_HOME=MIKASA_HOME)
        assert snap_id and (MIKASA_HOME / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from mikasa_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_auto_installs_gateway_service(self, tmp_path, monkeypatch):
        ...
        from mikasa_cli.backup import run_import
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        (MIKASA_HOME / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (MIKASA_HOME / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (MIKASA_HOME / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            MIKASA_HOME / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (MIKASA_HOME / "gateway.pid").write_text("4242")
        (MIKASA_HOME / "processes.json").write_text('{"live": true}')
        ...
        assert (MIKASA_HOME / "gateway.pid").read_text() == "4242"
        assert (MIKASA_HOME / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (MIKASA_HOME / "cron.pid").exists()
        assert not (MIKASA_HOME / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (MIKASA_HOME / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from mikasa_cli.backup import run_backup, run_import
        ...
        monkeypatch.setenv("mikasa_HOME", str(dst_home))
...
class TestFormatSize:
    def test_bytes(self):
        from mikasa_cli.sizefmt import format_bytes as _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from mikasa_cli.backup import _detect_prefix
...
class TestBackupEdgeCases:
    ...
    def test_empty_mikasa_home(self, tmp_path, monkeypatch):
        """Backup handles empty hermes home (no files to back up)."""
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        # Only excluded dirs, no actual files
        (MIKASA_HOME / "__pycache__").mkdir()
        (MIKASA_HOME / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        (MIKASA_HOME / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = MIKASA_HOME / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        (MIKASA_HOME / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (MIKASA_HOME / "config.yaml").exists()
        assert (MIKASA_HOME / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    ...
    tests pin the invariant for both restore branches: the MIKASA_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (MIKASA_HOME / "config.yaml").write_text(original)
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (MIKASA_HOME / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(MIKASA_HOME.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside MIKASA_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        MIKASA_HOME = dst_home / ".hermes"
        MIKASA_HOME.mkdir()
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        target = MIKASA_HOME / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "mikasa_cli.backup._preserve_file_owner",
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from mikasa_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from mikasa_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from mikasa_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def MIKASA_HOME(self, tmp_path):
        """Create a fake MIKASA_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, MIKASA_HOME):
        from mikasa_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(MIKASA_HOME=MIKASA_HOME)
        db_copy = MIKASA_HOME / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, MIKASA_HOME, monkeypatch, capsys):
    ...
    def test_restore_refused_db_is_not_counted(self, MIKASA_HOME, monkeypatch):
        ...
        import mikasa_cli.backup as backup_mod
        from mikasa_cli.backup import create_quick_snapshot, restore_quick_snapshot
        ...
        restore_quick_snapshot(snap_id, MIKASA_HOME=MIKASA_HOME)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(MIKASA_HOME) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, MIKASA_HOME):
        ...
        live_conn = sqlite3.connect(str(MIKASA_HOME / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, MIKASA_HOME=MIKASA_HOME)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, MIKASA_HOME):
        ...
        (MIKASA_HOME / "platforms" / "pairing").mkdir(parents=True)
        (MIKASA_HOME / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (MIKASA_HOME / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (MIKASA_HOME / "pairing").mkdir()
        (MIKASA_HOME / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (MIKASA_HOME / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = MIKASA_HOME / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, MIKASA_HOME, capsys):
        ...
        """
        import json
        from mikasa_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", MIKASA_HOME=MIKASA_HOME)
        assert first_id is not None
        first_dir = MIKASA_HOME / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", MIKASA_HOME=MIKASA_HOME, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = MIKASA_HOME / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, MIKASA_HOME=MIKASA_HOME)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, MIKASA_HOME):
        ...
        board_dir = MIKASA_HOME / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(MIKASA_HOME=MIKASA_HOME)
        copy = (
            MIKASA_HOME / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, MIKASA_HOME=MIKASA_HOME) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, MIKASA_HOME, monkeypatch):
        ...
        import mikasa_cli.backup as bk
        from mikasa_cli.backup import create_quick_snapshot

        board = MIKASA_HOME / "kanban" / "boards" / "work"
        ...
        copy = MIKASA_HOME / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, MIKASA_HOME):
        """Pre-update backup should include the same user data that
        ``hermes backup`` would, and should exclude the same directories."""
        from mikasa_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(MIKASA_HOME=MIKASA_HOME)
    ...
    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, MIKASA_HOME):
        ...
        snap_id = create_quick_snapshot(label="pre-update", MIKASA_HOME=MIKASA_HOME)
        assert snap_id and (MIKASA_HOME / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, MIKASA_HOME):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(MIKASA_HOME=MIKASA_HOME, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (MIKASA_HOME / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, MIKASA_HOME, tmp_path):
        """Pre-update backups must not dereference symlinks outside MIKASA_HOME."""
        from mikasa_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def MIKASA_HOME(self, tmp_path, monkeypatch):
        root = tmp_path / ".hermes"
        root.mkdir()
        _make_hermes_tree(root)
        # Point MIKASA_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("mikasa_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve MIKASA_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(MIKASA_HOME, value):
        import yaml
        (MIKASA_HOME / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(MIKASA_HOME):
        d = MIKASA_HOME / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(MIKASA_HOME):
        d = MIKASA_HOME / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, MIKASA_HOME, capsys):
        ...
        self._set_mode(MIKASA_HOME, "off")
        from mikasa_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(MIKASA_HOME)
        assert not self._zips(MIKASA_HOME)



    def test_config_full_mode(self, MIKASA_HOME, capsys):
        self._set_mode(MIKASA_HOME, "full")
        from mikasa_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(MIKASA_HOME)) == 1
...
class TestPreMigrationBackup:
    ...
    def test_restorable_with_hermes_import(self, MIKASA_HOME, tmp_path):
        """The zip produced by pre-migration backup must be a valid Hermes
        backup — `hermes import` should accept it."""
        from mikasa_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(MIKASA_HOME=MIKASA_HOME)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, MIKASA_HOME):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from mikasa_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(MIKASA_HOME=MIKASA_HOME, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(MIKASA_HOME=MIKASA_HOME, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    ...
    def _make_snapshot(self, MIKASA_HOME: Path, label="pre-update"):
        from mikasa_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, MIKASA_HOME=MIKASA_HOME, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from mikasa_cli.backup import restore_cron_jobs_if_emptied
        MIKASA_HOME = tmp_path / ".hermes"
        jobs_path = MIKASA_HOME / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(MIKASA_HOME)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, MIKASA_HOME=MIKASA_HOME)
        assert result is not None
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, MIKASA_HOME: Path) -> Path:
        MIKASA_HOME.mkdir(parents=True, exist_ok=True)
        cfg = MIKASA_HOME / "config.yaml"
        cfg.write_text(self.USER_CONFIG, encoding="utf-8")
        return cfg

    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        import yaml
        from mikasa_cli.backup import restore_config_model_settings_if_rewritten

        MIKASA_HOME = tmp_path / ".hermes"
        cfg = self._seed(MIKASA_HOME)
        snap_id = self._make_snapshot(MIKASA_HOME)
        assert snap_id
        ...
        result = restore_config_model_settings_if_rewritten(
            snap_id, MIKASA_HOME=MIKASA_HOME
        )
        assert result is not None
        assert result["restored"] is True
        assert result["snapshot_id"] == snap_id
    ...
    def test_noop_when_user_never_set_protected_keys(self, tmp_path):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir(parents=True)
        cfg = MIKASA_HOME / "config.yaml"
...
# location, NOT under MIKASA_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, MIKASA_HOME: Path) -> None:
        MIKASA_HOME.mkdir(parents=True, exist_ok=True)
        (MIKASA_HOME / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (MIKASA_HOME / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (MIKASA_HOME / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        self._make_min_tree(MIKASA_HOME)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under MIKASA_HOME,
        ...
        # External state did NOT leak into MIKASA_HOME.
        assert not (MIKASA_HOME / "_external").exists()


# ---------------------------------------------------------------------------
# run_import: MIKASA_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------
...
class TestImportHonorsHermesHomeOverride:
    ...
    Resolving the target through get_default_mikasa_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """mikasa_HOME=<root>/profiles/<name> must restore INTO the profile,
        ...
        monkeypatch.setenv("mikasa_HOME", str(profile))
        from mikasa_constants import get_mikasa_home

        assert get_mikasa_home() == profile
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("mikasa_HOME", str(sandbox))

        import argparse

        import mikasa_constants
        from mikasa_cli import backup as backup_mod

        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_mikasa_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "mikasa_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "mikasa_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("mikasa_HOME", str(native_default))
