...
has ``mikasa_cli.config.load_config`` patched. With a module-level
...
``mikasa_cli.config`` attributes at call time. These tests pin exactly
that property — a patch on ``mikasa_cli.config.<fn>`` must be visible
...
def test_load_config_patch_is_visible_and_releases():
    ...
    with patch("mikasa_cli.config.load_config", return_value=sentinel):
...
def test_sibling_delegates_are_late_bound_too():
    ...
    with patch(
        "mikasa_cli.config.get_compatible_custom_providers", return_value=marker
    ):
        assert rp.get_compatible_custom_providers({}) is marker
    assert rp.get_compatible_custom_providers({}) is not marker

    hdrs = {"X-Marker": "1"}
    with patch("mikasa_cli.config.normalize_extra_headers", return_value=hdrs):
