"""Regression tests for #34107 — Docker UID/GID handling in ensure_mikasa_home.

When Hermes runs in Docker with ``HERMES_UID=1000`` / ``HERMES_GID=911``,
the entrypoint chowns the top-level ``MIKASA_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_mikasa_home()`` — especially
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("mikasa_UID", "1000")
        monkeypatch.setenv("mikasa_GID", "911")
        from mikasa_cli.config import _resolve_hermes_uid_gid
...
class TestChownToHermesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("mikasa_UID", "1000")
        monkeypatch.setenv("mikasa_GID", "911")
        from mikasa_cli import config as cfg
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("mikasa_UID", raising=False)
        monkeypatch.delenv("mikasa_GID", raising=False)
        from mikasa_cli import config as cfg
