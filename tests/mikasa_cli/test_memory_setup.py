...
import mikasa_cli.memory_setup as memory_setup
from mikasa_cli.memory_setup import _CANCELLED, _curses_select
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_mikasa_home", lambda: tmp_path)
    monkeypatch.setattr("mikasa_cli.config.load_config", lambda: {"memory": {}})
...
# (env_writes, MIKASA_HOME=None) \u2014 these three tests pinned the prior direct
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "mikasa_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )
