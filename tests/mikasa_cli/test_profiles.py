...
from mikasa_cli import profiles
from mikasa_cli.profiles import (
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and MIKASA_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * MIKASA_HOME  -> tmp_path/.hermes  (so get_mikasa_home() agrees)
...
class TestGetProfileDir:
    ...
    def test_default_returns_mikasa_home(self, profile_env):
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("mikasa_cli.profiles._cleanup_gateway_service"), \
             patch("mikasa_cli.profiles.time.sleep"), \
             patch("mikasa_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "mikasa_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "mikasa_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "mikasa_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "mikasa_cli.main", "--profile", "coder", "serve"]),
        ]
...
class TestGetActiveProfileName:
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom MIKASA_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("mikasa_HOME", str(custom))
        # With Docker-aware roots, a custom MIKASA_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("mikasa_cli.profiles.shutil.which", lambda name: "/opt/mikasa/bin/hermes")
        from mikasa_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/mikasa/bin/hermes -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from mikasa_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from mikasa_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from mikasa_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("mikasa_cli.profiles.check_alias_collision", return_value="skip"):
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert cfg["hosts"]["mikasa_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["mikasa_heimdall"]["peerName"] == "user-peer"
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_mikasa_home()."""



    def test_default_mikasa_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_mikasa_home() returns MIKASA_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("mikasa_HOME", str(docker_home))
        home = _get_default_mikasa_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under MIKASA_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("mikasa_HOME", str(docker_home))
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with MIKASA_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("mikasa_HOME", str(get_profile_dir("coder")))
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_mikasa_home()
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured MIKASA_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("mikasa_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No MIKASA_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("mikasa_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_mikasa_home()
