...
~/.local/bin pointing into $MIKASA_HOME/node and prepends ~/.local/bin to
...
def _make_hermes_node(MIKASA_HOME: Path) -> Path:
    """Create a fake $MIKASA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = MIKASA_HOME / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    MIKASA_HOME = fake_home / ".hermes"
    _make_hermes_node(MIKASA_HOME)
    ...
    removed = uninstall.remove_node_symlinks(MIKASA_HOME)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    MIKASA_HOME = fake_home / ".hermes"
    node_bin = _make_hermes_node(MIKASA_HOME)
