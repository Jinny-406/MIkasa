...
The installer clones into ``$MIKASA_HOME/plugins/<name>/`` (flat), provider
...
def test_flat_installed_model_provider_plugins_are_discovered_alongside_nested_ones(MIKASA_HOME):
    _write_plugin(MIKASA_HOME / "plugins" / "installed-acp", name="installed-acp",
                  manifest='name: installed-acp\nkind: "model-provider"\n')
    _write_plugin(MIKASA_HOME / "plugins" / "model-providers" / "nested-acp", name="nested-acp",
...
def test_other_plugins_in_the_flat_directory_are_left_to_the_plugin_manager(MIKASA_HOME):
    _write_plugin(MIKASA_HOME / "plugins" / "other-standalone", name="other-standalone",
                  manifest="name: other-standalone\nkind: standalone\n")
    _write_plugin(MIKASA_HOME / "plugins" / "manifestless", name="manifestless", manifest=None)
    _write_plugin(MIKASA_HOME / "plugins" / "broken-manifest", name="broken-manifest",
