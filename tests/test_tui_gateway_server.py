...
from mikasa_cli.active_sessions import active_session_registry_snapshot
from mikasa_cli.browser_connect import ChromeDebugLaunch
...
def test_session_slot_is_claimed_on_first_turn_not_on_create(monkeypatch, tmp_path):
    ...
    token = set_mikasa_home_override(home)
    ...
    try:
        ...
        server._cfg_mtime = None
        server._cfg_path = None
        reset_mikasa_home_override(token)
...
def test_default_config_seeds_dashboard_process_isolation_keys():
    from mikasa_cli.config import DEFAULT_CONFIG
...
def test_compute_host_turn_end_updates_metadata_mirror(monkeypatch):
    ...
    import mikasa_cli.banner as _banner
...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    """MCP discovery must start under the selected profile's MIKASA_HOME."""
    from mikasa_cli import mcp_startup
    ...
    monkeypatch.setenv("mikasa_HOME", str(tmp_path / "default"))
    token = set_mikasa_home_override(str(profile_home))
    ...
    monkeypatch.setattr(
        ...
        lambda: seen.append(str(get_mikasa_home())),
    )
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["mikasa_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("mikasa_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_VOICE", "1")
...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_VOICE", "1")
    monkeypatch.setenv("mikasa_VOICE_TTS", "1")
    ...
    assert os.environ["mikasa_VOICE"] == "0"
    assert os.environ["mikasa_VOICE_TTS"] == "0"
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("mikasa_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import mikasa_cli.config as config_mod
    ...
    from mikasa_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("mikasa_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "mikasa_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mikasa_state_registry.acquire", lambda db_path=None: profile_db)
    monkeypatch.setattr(server, "_get_db", lambda: launch_db)
    monkeypatch.setattr(server, "_enable_gateway_prompts", lambda: None)
    monkeypatch.setattr(server, "_set_session_context", lambda target: [])
    monkeypatch.setattr(server, "_clear_session_context", lambda tokens: None)
...
def test_stored_session_runtime_overrides_skips_bare_billing_provider(monkeypatch):
    ...
    import mikasa_cli.runtime_provider as rp

    monkeypatch.setattr(rp, "load_config", lambda: cfg)
    monkeypatch.setattr("mikasa_cli.config.load_config", lambda: cfg)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("mikasa_MODEL", raising=False)
    monkeypatch.setenv("mikasa_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("mikasa_MODEL", raising=False)
    monkeypatch.delenv("mikasa_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("mikasa_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    ...
    # permanently.
    monkeypatch.setenv("mikasa_MODEL", "one-shot/model")
    monkeypatch.setenv("mikasa_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("mikasa_MODEL", "seed/model")
    monkeypatch.setenv("mikasa_INFERENCE_MODEL", "seed/model")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "mikasa_cli.model_switch.resolve_persist_behavior",
    ...
    monkeypatch.setattr(
        "mikasa_cli.model_cost_guard.expensive_model_warning",
        lambda *a, **k: None,
    )
    session = {"agent": None}
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("mikasa_MODEL", "nous/hermes-test")
    monkeypatch.setenv("mikasa_TUI_PROVIDER", "nous")
    monkeypatch.delenv("mikasa_INFERENCE_PROVIDER", raising=False)
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("mikasa_MODEL", "nous/hermes-test")
    monkeypatch.delenv("mikasa_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("mikasa_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "mikasa_cli.models.detect_static_provider_for_model",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("mikasa_MODEL", "sonnet")
    monkeypatch.delenv("mikasa_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("mikasa_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "mikasa_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.delenv("mikasa_INFERENCE_MODEL", raising=False)
    monkeypatch.delenv("mikasa_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("mikasa_DESKTOP", raising=False)
    monkeypatch.delenv("mikasa_DESKTOP_TERMINAL", raising=False)
    monkeypatch.setattr(
        server,
        "_load_cfg",
        lambda: {
    ...
    monkeypatch.setattr(
        "mikasa_cli.runtime_provider.resolve_runtime_provider",
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "mikasa_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "mikasa_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_superseded_by_resume_is_recoverable_end_reason():
    from mikasa_state_common import _RECOVERABLE_END_REASONS
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mikasa_state_registry.acquire", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_mikasa_home", tmp_path)
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    monkeypatch.setattr(server, "_mikasa_home", tmp_path)
    # Point the canonical resolver (load_config → env MIKASA_HOME) at the
    ...
    monkeypatch.setenv("mikasa_HOME", str(tmp_path))
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # tools.approval, which reads via mikasa_cli.config.load_config —
    # that path resolves MIKASA_HOME from the environment, not
    # server._mikasa_home.
    monkeypatch.setenv("mikasa_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_get_approval_mode_normalizes_yaml_off(tmp_path, monkeypatch):
    ...
    # load_config, which resolves MIKASA_HOME from the environment.
    monkeypatch.setenv("mikasa_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    monkeypatch.setattr(server, "_mikasa_home", tmp_path)
    # config.set writes via server._mikasa_home, but the post-write
    # session.info emit resolves the effective mode through the canonical
    # tools.approval resolver (load_config → env MIKASA_HOME).
    monkeypatch.setenv("mikasa_HOME", str(tmp_path))
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("mikasa_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("mikasa_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("mikasa_EXEC_ASK", raising=False)
    monkeypatch.delenv("mikasa_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("mikasa_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["mikasa_GATEWAY_SESSION"] == "1"
    assert server.os.environ["mikasa_EXEC_ASK"] == "1"
    assert server.os.environ["mikasa_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("mikasa_cli.main._has_any_provider_configured", lambda **_kw: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("mikasa_cli.main._has_any_provider_configured", lambda **_kw: True)
    monkeypatch.setattr(
        "mikasa_cli.runtime_provider.resolve_runtime_provider",
...
def test_setup_readiness_scopes_to_requested_profile(monkeypatch, tmp_path):
    ...
    assert Path(str(get_mikasa_home())).resolve() != bot_home.resolve()
...
def test_setup_readiness_unknown_profile_never_answers_for_launch_profile(monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.profiles.profile_exists", lambda name: False)
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "mikasa_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setattr("mikasa_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_recovers_failed_profile_resume_after_build_completes(
    ...
    monkeypatch.setenv("mikasa_HOME", str(launch_home))
    monkeypatch.delenv("mikasa_MODEL", raising=False)
    monkeypatch.delenv("mikasa_INFERENCE_MODEL", raising=False)
    ...
    monkeypatch.setattr("mikasa_cli.model_switch.switch_model", fake_switch_model)
    monkeypatch.setattr(
        "mikasa_cli.model_selection_guards.combined_selection_warning",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr("mikasa_state_registry.acquire", FakeDb)
    monkeypatch.setattr(server, "_make_agent", fake_make_agent)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("mikasa_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "mikasa_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["mikasa_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "mikasa_TUI_PROVIDER" not in os.environ
        assert "mikasa_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("mikasa_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("mikasa_MODEL", raising=False)
    monkeypatch.delenv("mikasa_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    try:
        ...
        assert os.environ["mikasa_TUI_PROVIDER"] == "openai-codex"
        assert "mikasa_MODEL" not in os.environ
        assert "mikasa_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("mikasa_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("mikasa_MODEL", "old/model")
    monkeypatch.setattr(
        "mikasa_cli.model_switch.switch_model",
    ...
    try:
        ...
        assert os.environ["mikasa_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["mikasa_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("mikasa_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_personality_preserves_history_and_returns_info(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args: emits.append(args))
    # Persistence now flows through the single owner (mikasa_cli.personality),
    # never _write_config_key / agent.system_prompt.
    import mikasa_cli.personality as personality_mod
...
def test_commands_catalog_includes_plugin_commands(monkeypatch):
    monkeypatch.setattr(
        "mikasa_cli.plugins.get_plugin_commands",
        lambda: {
            "lcm": {
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("mikasa_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "mikasa_cli.commands_completion.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("mikasa_state")
    ...
    monkeypatch.setitem(sys.modules, "mikasa_state", fake_mod)
    fake_registry = types.ModuleType("mikasa_state_registry")
    fake_registry.acquire = _broken_shared
    monkeypatch.setitem(sys.modules, "mikasa_state_registry", fake_registry)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_mikasa_home
    # fixture pins MIKASA_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("mikasa_state_registry.acquire", ProfileDB)
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("mikasa_state_registry.acquire", ProfileDB2)
...
def test_handoff_request_uses_session_profile_home(monkeypatch, tmp_path):
    ...
    assert get_mikasa_home() != profile_home
...
def test_session_branch_installs_parent_profile_secret_scope(monkeypatch, tmp_path):
    ...
    session.branch already binds the parent's MIKASA_HOME and state.db, but the
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    with patch(
        "mikasa_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("mikasa_cli.models.provider_model_ids") as live_fetch:
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from mikasa_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "mikasa_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "mikasa_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "mikasa_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr(
        "mikasa_cli.auth.is_provider_explicitly_configured",
        lambda _slug: False,
    )
    monkeypatch.setattr("mikasa_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("mikasa_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "mikasa_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr("mikasa_cli.config.is_managed", lambda: False)
    save_credential = Mock()
    monkeypatch.setattr(
        "mikasa_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_verification_status_returns_recorded_evidence(tmp_path, monkeypatch):
    ...
    token = set_mikasa_home_override(profile_home)
    project = tmp_path / "project"
    project.mkdir()
    (project / ".git").mkdir()
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"mikasa_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool_lifecycle": fake}):
        ...
        with (
            patch(
                "mikasa_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("mikasa_cli.browser_connect.local_port_in_use", return_value=False),
            patch("mikasa_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "mikasa_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool_lifecycle": fake}):
        with (
            patch("mikasa_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("mikasa_cli.browser_connect.local_port_in_use", return_value=True),
            patch("mikasa_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_hermes_cli_reload_env(monkeypatch):
    ...
    with patch.dict(sys.modules, {"mikasa_cli.config": fake}):
...
def test_session_save_writes_under_mikasa_home_with_system_prompt(monkeypatch, tmp_path):
    ...
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("mikasa_HOME", str(home))
    ...
    assert not list(work.glob("mikasa_conversation_*.json"))
...
def test_reap_idle_sessions_calls_periodic_trim(monkeypatch):
    ...
    # `from mikasa_cli.mem_trim import trim_memory` at call time.
    import mikasa_cli.mem_trim as mem_trim
...
def test_billing_error_serialization_preserves_server_code(
    ...
    import mikasa_cli.nous_billing as nb
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from get_mikasa_home() (live
    # env var), always targeting MIKASA_HOME/config.yaml — point it at tmp_path.
    monkeypatch.setenv("mikasa_HOME", str(tmp_path))
    monkeypatch.setattr(cli, "_mikasa_home", tmp_path)
...
class TestResolveRuntimeWithFallback:
    ...
    def test_auth_error_tries_fallback_chain(self, monkeypatch):
        ...
        from mikasa_cli.auth import AuthError
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_VOICE_TTS", "1")
    monkeypatch.setenv("mikasa_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_full_duplex_stop_phrase_mid_generation_ends_voice_chat(monkeypatch, tmp_path):
    ...
    assert os.environ.get("mikasa_VOICE") == "0"  # voice chat ended
...
def test_prompt_submit_releases_old_history_before_heap_trim(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(server, "render_message", lambda _t, _c: "")
        monkeypatch.setattr(server, "_emit", lambda *a: None)
        monkeypatch.setattr(server, "set_mikasa_home_override", lambda _home: object())
        monkeypatch.setattr(
            server,
            "reset_mikasa_home_override",
            lambda _token: cleanup_order.append("reset_home"),
        )
        monkeypatch.setattr("mikasa_cli.mem_trim.trim_memory", _inspect_trim_frame)
...
def test_persist_live_session_system_prompt_uses_profile_home(monkeypatch, tmp_path):
    ...
    MIKASA_HOME to the session's profile before rebuilding the system
    ...
    from mikasa_constants import get_mikasa_home_override
    assert get_mikasa_home_override() is None
...
def test_persist_live_session_system_prompt_restores_pre_existing_override(tmp_path):
    """reset_mikasa_home_override() restores the previous ContextVar state,
    ...
    class FakeAgent:
        ...
        def _build_system_prompt(self, system_message=None):
            from mikasa_constants import get_mikasa_home
            built_homes.append(str(get_mikasa_home()))
            return "inner prompt"
    ...
    outer_token = set_mikasa_home_override(outer_home)
    try:
        server._persist_live_session_system_prompt(session)
        ...
        # to None.
        assert get_mikasa_home_override() == str(outer_home)
    finally:
        reset_mikasa_home_override(outer_token)
    assert get_mikasa_home_override() is None
