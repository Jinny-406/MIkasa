...
def server():
    ...
    # e.g. hermes_cli.active_sessions would bind the mocked get_mikasa_home
    ...
    with patch.dict("sys.modules", {
        "mikasa_constants": MagicMock(get_mikasa_home=MagicMock(return_value="/tmp/hermes_test")),
        "mikasa_cli.env_loader": MagicMock(),
        "mikasa_cli.banner": MagicMock(),
        "mikasa_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("mikasa_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient MIKASA_HOME
    ...
    # The gateway's own MIKASA_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    ...
    import mikasa_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_mikasa_home", lambda: tmp_path)
