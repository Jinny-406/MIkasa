...
def test_slash_worker_accepts_profile_home():
    ...
    # hermes_state evaluates get_mikasa_home() / "state.db" at import time, so
    # the mock must return a Path (a bare str raises TypeError under per-file
    # subprocess isolation).
    with patch.dict("sys.modules", {
        "mikasa_constants": MagicMock(
            get_mikasa_home=MagicMock(return_value=Path("/tmp/hermes_test")),
        ),
    }):
        with patch("subprocess.Popen") as mock_popen:
            ...
            # Check that MIKASA_HOME was set in the environment
            call_kwargs = mock_popen.call_args[1]
            assert "env" in call_kwargs
            assert call_kwargs["env"]["mikasa_HOME"] == "/home/luke/.hermes/profiles/work"
