...
def live_home(monkeypatch):
    """A REAL isolated MIKASA_HOME with a config.yaml + state.db on disk."""
    ...
    # mikasa_constants caches the resolved home at first read — the env var
    ...
    import mikasa_cli.config as hconfig
    ...
    monkeypatch.setattr(server, "_mikasa_home", str(home), raising=False)
    yield home, server
