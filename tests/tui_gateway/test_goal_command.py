...
def MIKASA_HOME(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves MIKASA_HOME.
...
def server(MIKASA_HOME, monkeypatch):
    ...
    # Pin config resolution to the isolated MIKASA_HOME. Sibling test
    ...
    # module-level ``_mikasa_home = get_mikasa_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, MIKASA_HOME):
    _write_moa_config(MIKASA_HOME, """
moa:
