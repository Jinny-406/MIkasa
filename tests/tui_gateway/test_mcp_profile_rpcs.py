...
``MIKASA_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
def hermes_root(tmp_path, monkeypatch):
    """A temp MIKASA_HOME root with two named profiles: 'work' and 'other'.

    Pointing MIKASA_HOME at a dir outside ~/.hermes makes it the profile ROOT
    (get_default_mikasa_root's Docker/custom branch), so named profiles live at
...
def test_default_profile_add_when_profile_omitted(hermes_root):
    ...
    # Omitted profile → launch/default profile == MIKASA_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
