...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: MIKASA_HOME isolated, HERMES_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("mikasa_HOME", str(home))
    monkeypatch.setenv("mikasa_PROFILE", "test-worker")
    monkeypatch.delenv("mikasa_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("mikasa_KANBAN_TASK", tid)
    return tid
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("mikasa_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated MIKASA_HOME with one claimed goal_mode task,
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from mikasa_PROFILE in the fixture, not the
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("mikasa_HOME", str(home))
    monkeypatch.setenv("mikasa_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def multi_board_env(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("mikasa_KANBAN_DB", raising=False)
    monkeypatch.delenv("mikasa_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("mikasa_KANBAN_TASK", raising=False)
    monkeypatch.setenv("mikasa_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def _list_subs_for_task(task_id):
    from mikasa_cli import kanban_db as kb
    from mikasa_cli import kanban_db_connect as kbc
    from mikasa_cli import kanban_db_notify as kbn
    conn = kbc.connect()
    try:
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("mikasa_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("mikasa_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("mikasa_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("mikasa_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("mikasa_SESSION_CHAT_TYPE", "forum")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("mikasa_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("mikasa_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    monkeypatch.setenv("mikasa_SESSION_CHAT_ID", "channel-1")
...
def allow_private_urls(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_ALLOW_PRIVATE_URLS", "true")
...
def default_url_guard(monkeypatch):
    ...
    monkeypatch.delenv("mikasa_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
