...
def test_engaged_unset_contextvar_strips_foreign_session_key(monkeypatch):
    ...
    os.environ["mikasa_SESSION_KEY"], this task's ContextVar is unset, and the
    ...
    assert "mikasa_SESSION_KEY" not in env, (
        "Foreign concurrent session key leaked into subprocess env: "
        f"{env.get('mikasa_SESSION_KEY')!r}"
    )
...
def test_set_session_vars_engages_and_overrides_foreign_global(monkeypatch):
    ...
    assert env.get("mikasa_SESSION_KEY") == "agent:main:discord:group:MY_BUGS_ROOT:111"
...
def test_unengaged_process_preserves_os_environ_fallback(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_SESSION_KEY", "cli-session-key")
    monkeypatch.setenv("mikasa_SESSION_ID", "cli-session-id")

    env = _make_run_env({})

    assert env.get("mikasa_SESSION_KEY") == "cli-session-key"
    assert env.get("mikasa_SESSION_ID") == "cli-session-id"
...
def test_explicit_empty_thread_id_overrides_stale_value(monkeypatch):
    ...
    """
    monkeypatch.setenv("mikasa_SESSION_THREAD_ID", "stale-thread-from-prior-turn")
    ...
    assert env.get("mikasa_SESSION_THREAD_ID") == "", (
        "Bound-empty thread id did not override the stale value: "
        f"{env.get('mikasa_SESSION_THREAD_ID')!r}"
    )
    assert env.get("mikasa_SESSION_KEY") == "mm:chan"
...
def test_sanitize_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "mikasa_SESSION_KEY": "agent:main:discord:thread:FOREIGN_BG:FOREIGN_BG",
        "mikasa_SESSION_THREAD_ID": "FOREIGN_BG",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert "mikasa_SESSION_KEY" not in sanitized, (
        f"Background subprocess inherited foreign key: {sanitized.get('mikasa_SESSION_KEY')!r}"
    )
    assert "mikasa_SESSION_THREAD_ID" not in sanitized
...
def test_sanitize_subprocess_env_set_contextvar_wins_when_engaged():
    ...
    assert sanitized.get("mikasa_SESSION_KEY") == "agent:main:discord:group:REAL_BG:222"
...
def test_hermes_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """mikasa_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
...
def test_hermes_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    ...
    monkeypatch.setenv("mikasa_SESSION_KEY", "cli-fallback-key")
    # not engaged (autouse fixture leaves _session_context_engaged False)
    env = hermes_subprocess_env()
    assert env.get("mikasa_SESSION_KEY") == "cli-fallback-key"
