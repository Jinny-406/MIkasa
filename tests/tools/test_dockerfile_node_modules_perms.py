...
Older images made /opt/mikasa/ui-tui and /opt/mikasa/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/mikasa/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hermes:hermes /opt/mikasa/ui-tui" not in text
    assert "chown -R hermes:hermes /opt/mikasa/node_modules" not in text
