...
def _fake_hooks(monkeypatch, results):
    ...
    monkeypatch.setattr("mikasa_cli.plugins.invoke_hook", _invoke)
    return captured
...
def _no_hooks(monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.plugins.has_hook", lambda name: False)
    monkeypatch.setattr("mikasa_cli.plugins.invoke_hook", _boom)
...
def test_real_fixture_plugins_thread_prompt_in_registration_order(
    ...
    MIKASA_HOME = Path(os.environ["mikasa_HOME"])
    plugin_dir = MIKASA_HOME / "plugins" / "stt_vocab"
