...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("mikasa_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_SESSION_KEY", session_key)
        monkeypatch.setenv("mikasa_EXEC_ASK", "1")
        monkeypatch.delenv("mikasa_CRON_SESSION", raising=False)
        monkeypatch.setattr(
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"mikasa_SESSION_KEY": "bob"}, clear=False):
...
class TestHermesConfigWriteProtection:
    ...
    def test_write_idioms_against_config(self):
        for command in (
            ...
            "echo x | tee $MIKASA_HOME/config.yaml",
...
class TestWindowsAbsolutePathFolding:
    ...
    The OS-specific ``Path.home()`` / ``get_mikasa_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/MIKASA_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("mikasa_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.hermes/hermes-agent && source venv/bin/activate && python -m mikasa_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m mikasa_cli.main gateway run --replace &",
            "nohup python -m mikasa_cli.main gateway run --replace",
        ):
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_SESSION_PLATFORM", "webhook")
    ...
    def test_all_unattended_platforms_return_false(self, monkeypatch):
        ...
        for platform in _UNATTENDED_APPROVAL_PLATFORMS:
            monkeypatch.setenv("mikasa_SESSION_PLATFORM", platform)
    ...
    def test_non_webhook_gateway_session_returns_true(self, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_CRON_SESSION", raising=False)
        monkeypatch.setenv("mikasa_GATEWAY_SESSION", "1")
        monkeypatch.setenv("mikasa_SESSION_PLATFORM", "telegram")
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("mikasa_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("mikasa_INTERACTIVE", raising=False)
        monkeypatch.setenv("mikasa_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("mikasa_SESSION_KEY", "test-api-session")
...
class TestPgrepKillExpansion:
    ...
    def test_kill_pidof_expansion_detected(self):
        ...
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof mikasa_cli.main)")
        assert dangerous is True
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("mikasa_GATEWAY_SESSION", "mikasa_CRON_SESSION",
                      "mikasa_YOLO_MODE",
                      "mikasa_SESSION_KEY", "mikasa_INTERACTIVE")
        }
        os.environ.pop("mikasa_YOLO_MODE", None)
        os.environ.pop("mikasa_INTERACTIVE", None)
        ...
        os.environ.pop("mikasa_CRON_SESSION", None)
        os.environ["mikasa_GATEWAY_SESSION"] = "1"
        os.environ["mikasa_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("mikasa_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"mikasa_INTERACTIVE": "1"}, clear=False):
