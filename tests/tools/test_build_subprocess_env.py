...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    assert env["mikasa_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_mikasa_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see MIKASA_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('mikasa_HOME'), "
    ...
    assert result["home"] == str(MIKASA_HOME)
