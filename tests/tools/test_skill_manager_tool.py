...
class TestSecurityScanGate:
    ...
    def test_guard_flag_handles_config_error(self):
        ...
        with patch("mikasa_cli.config.load_config", side_effect=RuntimeError("boom")):
    ...
    def test_guard_flag_quoted_false_stays_disabled(self):
        ...
        for quoted in ("false", "False", "0", "no", "off"):
            with patch("mikasa_cli.config.load_config",
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points MIKASA_HOME at ``tmp_path/.hermes`` so skill_usage's archive path
    (``get_mikasa_home()``) resolves into the same tree the skill manager
    ...
    skills_root = MIKASA_HOME / "skills"
