...
class TestHermesToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        ...
        self.assertIn("os.path.join(tempfile.gettempdir(), \"mikasa_rpc\")", src)
        self.assertNotIn('os.environ.get("mikasa_RPC_DIR", "/tmp/hermes_rpc")', src)
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "mikasa_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "mikasa_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/hermes_exec_", mkdir_cmd)
        self.assertIn("mikasa_RPC_DIR=/data/data/com.termux/files/usr/tmp/hermes_exec_", run_cmd)
    ...
    def test_timezone_shell_quoted_in_remote_execution(self):
        """mikasa_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"mikasa_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import mikasa_constants; print(mikasa_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("mikasa_constants.py", result["output"])

    def test_single_tool_call(self):
        """Script calls terminal and prints the result."""
        code = """
from mikasa_tools import terminal
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from mikasa_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from mikasa_tools import json_parse
...
"""
    ...
    def test_retry_helper_all_fail(self):
        ...
        code = """
from mikasa_tools import retry
def always_fail():
...
class TestStubSchemaDrift(unittest.TestCase):
    ...
    def test_generated_module_accepts_all_params(self):
        ...
        compile(src, "mikasa_tools.py", "exec")
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_hermes_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("mikasa_RPC_SOCKET", child_env)


    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["mikasa_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("mikasa_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    def test_generated_module_sends_token(self):
        ...
        src = generate_hermes_tools_module(["terminal"], transport="uds")
        self.assertIn("mikasa_RPC_TOKEN", src)
