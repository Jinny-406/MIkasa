...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("mikasa_PROFILE", "parent-worker")
    monkeypatch.setenv("mikasa_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("mikasa_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
    ...
    monkeypatch.setenv("mikasa_KANBAN_RUN_ID", str(run_id))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setenv("mikasa_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("mikasa_KANBAN_RUN_ID", "123")
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("mikasa_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    monkeypatch.setenv("mikasa_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("mikasa_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("mikasa_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("mikasa_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("mikasa_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("mikasa_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("mikasa_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("mikasa_DELEGATED_CHILD_CONTEXT") is None
    assert env["mikasa_HOME"] == str(home)
    assert env["mikasa_DELEGATED_CHILD_CONTEXT"] == "1"
    assert "mikasa_KANBAN_TASK" not in env
    assert "mikasa_KANBAN_RUN_ID" not in env
    assert "mikasa_KANBAN_DB" not in env
    assert "mikasa_KANBAN_WORKSPACE" not in env
    assert "mikasa_KANBAN_CLAIM_LOCK" not in env
