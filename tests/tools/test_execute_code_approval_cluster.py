...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "mikasa_HOME": "/h", "mikasa_PROFILE": "p",
        "mikasa_CONFIG": "/c.yaml", "mikasa_ENV": "/e",
        "mikasa_DELEGATED_CHILD_CONTEXT": "1",
        # other HERMES_* → dropped (broad prefix removed)
        "mikasa_BASE_URL": "https://x", "mikasa_INTERACTIVE": "1",
        "mikasa_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "mikasa_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "mikasa_BASE_URL", "mikasa_INTERACTIVE", "mikasa_KANBAN_DB",
