...
@pytest.fixture
def MIKASA_HOME(monkeypatch):
    d = tempfile.mkdtemp(prefix="mikasa_wa_test_")
    home = os.path.join(d, ".hermes")
    os.makedirs(home)
    monkeypatch.setenv("mikasa_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import mikasa_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(MIKASA_HOME):
...
def test_invalid_subsystem_is_off(MIKASA_HOME):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(MIKASA_HOME):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(MIKASA_HOME, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(MIKASA_HOME, monkeypatch):
    ...
    monkeypatch.setattr("mikasa_cli.config.load_config", _boom)
...
def test_handle_approve_all(MIKASA_HOME):
...
def test_handle_approval_on(MIKASA_HOME):
    from mikasa_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(MIKASA_HOME):
    from mikasa_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(MIKASA_HOME, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(MIKASA_HOME, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(MIKASA_HOME):
