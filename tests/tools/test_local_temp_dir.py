...
def test_default_is_hermes_cache_not_tmp(tmp_path, monkeypatch):
    ...
    MIKASA_HOME (cache/terminal), NOT tmpfs /tmp."""
    import mikasa_constants  # noqa: F401 — resolves MIKASA_HOME per call
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "mikasa_bg_dead1.log").exists()
