...
class TestDumpSubagentTimeoutDiagnostic:
    ...
    def test_writes_log_with_expected_sections(self, MIKASA_HOME):
        ...
        p = Path(path)
        assert p.is_file()
        # File lives under MIKASA_HOME/logs/
        assert p.parent == MIKASA_HOME / "logs"
    ...
    def test_returns_none_on_unwritable_logs_dir(self, tmp_path, monkeypatch):
        # Point MIKASA_HOME at an unwritable path so logs/ can't be created
        # (simulates permission-denied). Helper must not raise.
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        bogus = tmp_path / "does-not-exist" / ".hermes"
        monkeypatch.setenv("mikasa_HOME", str(bogus))
        child = _StubChild()
...
class TestRunSingleChildTimeoutDump:
    ...
    def test_zero_api_calls_writes_dump_and_surfaces_path(self, MIKASA_HOME, monkeypatch):
        ...
        assert dump_path.parent == MIKASA_HOME / "logs"
    ...
    def test_non_timeout_error_has_null_timeout_metadata(self, MIKASA_HOME, monkeypatch):
