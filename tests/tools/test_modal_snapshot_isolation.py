...
def _restore_tool_modules():
    original_mikasa_home = os.environ.get("mikasa_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_mikasa_home is None:
            os.environ.pop("mikasa_HOME", None)
        else:
            os.environ["mikasa_HOME"] = original_mikasa_home
        _reset_modules(("tools", "mikasa_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    hermes_cli = types.ModuleType("mikasa_cli")
    hermes_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["mikasa_cli"] = hermes_cli
    MIKASA_HOME = tmp_path / "hermes-home"
    os.environ["mikasa_HOME"] = str(MIKASA_HOME)
    sys.modules["mikasa_cli.config"] = types.SimpleNamespace(
        get_mikasa_home=lambda: MIKASA_HOME,
    )
    ...
    return {
        "snapshot_store": MIKASA_HOME / "modal_snapshots.json",
