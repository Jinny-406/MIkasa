...
class TestDeregisterAuthorization:
    ...
    def test_plugin_cannot_deregister_unowned_tool_without_opt_in(self):
        reg = self._reg()
        reg.register_plugin_override_policy("mikasa_plugins.evil", False)
        with patch.object(ToolRegistry, "_caller_module", return_value="mikasa_plugins.evil"):
            import pytest
    ...
    def test_plugin_root_module_can_deregister_submodule_handler(self):
        ...
        """
        reg = ToolRegistry()
        reg.register_plugin_override_policy("mikasa_plugins.pkg", False)
        handler = eval("lambda *a, **k: 'sub'", {"__name__": "mikasa_plugins.pkg.handlers"})
        reg.register(
        ...
        with patch.object(ToolRegistry, "_caller_module", return_value="mikasa_plugins.pkg"):
    ...
    def test_opted_in_plugin_submodule_can_deregister(self):
        ...
        ``_plugin_override_policy.get("mikasa_plugins.allowed.cleanup")`` →
        ...
        reg.register_plugin_override_policy("mikasa_plugins.allowed", True)
        with patch.object(ToolRegistry, "_caller_module", return_value="mikasa_plugins.allowed.cleanup"):
            reg.deregister("protected")
    ...
    def test_full_bypass_blocked(self):
        ...
        with pytest.raises(PermissionError):
            evil_handler = eval("lambda *a, **k: 'hijacked'", {"__name__": "mikasa_plugins.evil"})
