"""Contract tests for the Docker image's immutable /opt/mikasa install tree."""
...
def test_dockerfile_makes_opt_hermes_readonly_for_hermes_user() -> None:
    ...
    assert "chown -R root:root /opt/mikasa" not in text
    assert "chmod -R a+rX /opt/mikasa" not in text
    assert "chmod -R a-w /opt/mikasa" not in text


def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+hermes:hermes\s+/opt/mikasa/\.venv",
        r"chown\s+-R\s+hermes:hermes\s+/opt/mikasa/ui-tui",
        r"chown\s+-R\s+hermes:hermes\s+/opt/mikasa/gateway",
        r"chown\s+-R\s+hermes:hermes\s+/opt/mikasa/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/mikasa must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/mikasa/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $MIKASA_HOME data volume (which a host install may also use).
    ...
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/mikasa/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/mikasa/bin && \\\n"
        r"(?:.*\\\n)+?"
        r"\s+printf 'docker\\n' > /opt/mikasa/\.install_method",
        text,
    )
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/mikasa tree.
    ...
    assert "ENV HERMES_LAZY_INSTALL_TARGET=/opt/mikasa" not in text
    ...
    assert '"$MIKASA_HOME/lazy-packages"' in stage2, (
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+hermes:hermes\s+/opt/mikasa/plugins", text
    )
