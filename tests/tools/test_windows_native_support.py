...
class TestConfigureWindowsStdio:
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("mikasa_cli.stdio", None)
        # Fresh import now; tests import from mikasa_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import mikasa_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("mikasa_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from mikasa_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("mikasa_cli.kanban_db_dispatch", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "mikasa_cli" / "web_server_chat.py").read_text(encoding="utf-8")
    ...
    def test_pty_handler_checks_availability_flag(self):
        ...
        root = Path(__file__).resolve().parents[2]
        source = (root / "mikasa_cli" / "web_routers" / "chat_ws.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    ...
    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "mikasa_cli/main.py", "gateway/run.py"],
    )
...
class TestSubprocessCompatHelpers:
    """mikasa_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from mikasa_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from mikasa_cli._subprocess_compat import resolve_node_command
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "mikasa_cli" / "kanban_db_dispatch.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    @pytest.mark.parametrize(
        "relpath",
        [
            "mikasa_cli/tools_config.py",
            "mikasa_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_mikasa_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_mikasa_home" in source
        assert 'get_mikasa_home() / "cache" / "terminal"' in source
...
class TestGitBashPathNormalization:
    ...
    def test_posix_noop(self):
        ...
        from mikasa_cli.worktree_ops import _normalize_git_bash_path
        if sys.platform != "win32":
    ...
    def test_windows_translation(self):
        ...
        """
        from mikasa_cli import worktree_ops as cli_mod
...
class TestWorktreeSymlinkFallback:
    ...
    def test_source_has_symlink_fallback(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "mikasa_cli" / "worktree_ops.py").read_text(encoding="utf-8")
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_hermes_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "mikasa_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "mikasa_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "mikasa_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from mikasa_cli._subprocess_compat import" in block
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (MIKASA_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "mikasa_HOME) from the restart spec."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import mikasa_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "mikasa_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        ``get_mikasa_home`` because the real one's ``Path.resolve()`` consults
        ...
        with mock.patch.object(
            ...
            "mikasa_cli.config.get_mikasa_home", return_value="C:/hermes"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("mikasa_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("mikasa_TEST_SECRET", "")
