...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects MIKASA_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("mikasa_INTERACTIVE", "mikasa_GATEWAY_SESSION", "mikasa_EXEC_ASK", "mikasa_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithAllowSafeCommand:
    ...
    def test_both_allow(self, mock_tirith):
        os.environ["mikasa_INTERACTIVE"] = "1"
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["mikasa_INTERACTIVE"] = "1"
        session_key = os.getenv("mikasa_SESSION_KEY", "default")
