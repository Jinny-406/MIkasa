...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        MIKASA_HOME.mkdir()
        (MIKASA_HOME / "token.json").write_text("{}")

        with patch.dict(os.environ, {"mikasa_HOME": str(MIKASA_HOME)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(MIKASA_HOME / "token.json")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        (MIKASA_HOME / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        MIKASA_HOME = tmp_path / ".hermes"
        (MIKASA_HOME / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape MIKASA_HOME."""
        monkeypatch.setenv("mikasa_HOME", str(tmp_path / ".hermes"))
        (tmp_path / ".hermes").mkdir()

        # Create a sensitive file one level above MIKASA_HOME
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside MIKASA_HOME
    ...
    def test_nested_subdir_inside_mikasa_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of MIKASA_HOME must be allowed."""
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        subdir = MIKASA_HOME / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside MIKASA_HOME pointing outside must be rejected."""
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))

        # Create a sensitive file outside MIKASA_HOME
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside MIKASA_HOME pointing outside
        symlink = MIKASA_HOME / "evil_link.json"
        try:
        ...
        # The resolved path escapes MIKASA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, MIKASA_HOME: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape MIKASA_HOME."""
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        ...
        sensitive.write_text("{}")
        self._write_config(MIKASA_HOME, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(MIKASA_HOME, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside MIKASA_HOME via config must still mount."""
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))

        (MIKASA_HOME / "oauth.json").write_text("{}")
        self._write_config(MIKASA_HOME, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        (MIKASA_HOME / "cache" / "documents").mkdir(parents=True)
        (MIKASA_HOME / "cache" / "audio").mkdir(parents=True)
        (MIKASA_HOME / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_mikasa_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        MIKASA_HOME = tmp_path / ".hermes"
        MIKASA_HOME.mkdir()
        # Use legacy dir name with content — get_mikasa_dir prefers
        # populated old over new.
        legacy_doc = MIKASA_HOME / "document_cache"
        legacy_img = MIKASA_HOME / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(MIKASA_HOME / "document_cache") in host_paths
        assert str(MIKASA_HOME / "image_cache") in host_paths
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``MIKASA_HOME/images``, not
        ...
        """
        MIKASA_HOME = tmp_path / ".hermes"
        (MIKASA_HOME / "images").mkdir(parents=True)
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.hermes/images" in by_container
        assert by_container["/root/.hermes/images"] == str(MIKASA_HOME / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        (MIKASA_HOME / "images").mkdir(parents=True)
        upload = MIKASA_HOME / "images" / "upload_20260722_181019_1.png"
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        MIKASA_HOME = tmp_path / ".hermes"
        img_dir = MIKASA_HOME / "cache" / "images"
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(MIKASA_HOME / "cache" / "images" / "x.png"))
        assert mapped == "/root/.hermes/cache/images/x.png"
        assert (MIKASA_HOME / "cache" / "images").is_dir()
...
class TestToAgentVisiblePathPerBackend:
    ...
    def _staged(self, tmp_path, monkeypatch):
        MIKASA_HOME = tmp_path / ".hermes"
        (MIKASA_HOME / "attachments").mkdir(parents=True)
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        return str(MIKASA_HOME / "attachments" / "drop.zip")
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        MIKASA_HOME = tmp_path / ".hermes"
        doc_dir = MIKASA_HOME / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — MIKASA_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping MIKASA_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"mikasa_HOME": str(home)}):
