...
def test_output_path_rejects_hermes_oauth_store(tmp_path, monkeypatch):
    ...
    MIKASA_HOME.mkdir()
    monkeypatch.setattr(file_safety, "_mikasa_home_path", lambda: MIKASA_HOME)
    monkeypatch.setattr(file_safety, "_hermes_root_path", lambda: MIKASA_HOME)

    target = MIKASA_HOME / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    MIKASA_HOME = tmp_path / "hermes-home"
    token_dir = MIKASA_HOME / "mcp-tokens"
