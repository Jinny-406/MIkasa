...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("mikasa_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    ...
    def test_default_directory_uses_mikasa_home(self):
        """When no directory override, spill under MIKASA_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="hermes-home-")
        try:
            with patch.dict(os.environ, {"mikasa_HOME": test_home}):
                # Also patch get_mikasa_home to the env var to mirror production.
            ...
            # ~/.hermes/hook_outputs depending on get_mikasa_home behaviour.
            candidates = [
