...
the whole pytest session's logging to `<MIKASA_HOME>/logs/agent.log`.

If MIKASA_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets MIKASA_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(mikasa_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_mikasa_home_is_sandboxed_before_imports(self):
        # Deliberately NOT os.environ: by test time the per-test `_isolate_env`
        # fixture has sandboxed MIKASA_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import mikasa_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set MIKASA_HOME before test modules import"
        assert Path(home).resolve() != _real_mikasa_home().resolve(), (
            f"mikasa_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("mikasa_cli.main")

        real_logs = str(_real_mikasa_home() / "logs")
