...
The fix is ``ENV HERMES_TUI_DIR=/opt/mikasa/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    ...
    inner = (
        "source /opt/mikasa/.venv/bin/activate && "
        "cd /opt/mikasa && "
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    """mikasa_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
    ...
    assert r.stdout.strip() == "/opt/mikasa/ui-tui", (
        f"mikasa_TUI_DIR={r.stdout.strip()!r} (expected /opt/mikasa/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from mikasa_cli.main_tui_launch import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/mikasa/ui-tui')\n"
