...
Build the real image and verify at runtime:
  ...
  1. /opt/mikasa is not writable by the hermes user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and HERMES_DISABLE_LAZY_INSTALLS are set
  3. /opt/mikasa/.install_method contains "docker" (code-scoped stamp)
  4. $MIKASA_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $MIKASA_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The hermes user must not be able to modify /opt/mikasa.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/mikasa as the hermes user
        "touch /opt/mikasa/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hermes user can write to /opt/mikasa (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/mikasa/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hermes user can write to /opt/mikasa/.venv: {r.stdout}"
    )
...
def test_hermes_disable_lazy_installs_and_dont_write_bytecode(
    ...
    assert "ENV_OK" in r.stdout, (
        ...
        f"mikasa_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $MIKASA_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $MIKASA_HOME was not healed on boot: "
        f"{r.stdout}"
    )
