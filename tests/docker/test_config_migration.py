...
Build the real image and verify: a config.yaml present in $MIKASA_HOME
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $MIKASA_HOME must be migrated on boot by
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $MIKASA_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/mikasa/scripts/docker_config_migrate.py && "
