...
def test_dashboard_service_resets_home(
    ...
    the process env.
    """
    start_container(built_image, container_name, "mikasa_DASHBOARD=1", "mikasa_DASHBOARD_HOST=127.0.0.1")
    ...
    r = docker_exec_sh(
        ...
        '    /opt/mikasa/docker/s6-rc.d/dashboard/run && '
        '  echo "HOME=/opt/data"; '
        'fi',
        timeout=15,
    )
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    """
    start_container(built_image, container_name, "mikasa_DASHBOARD=1", "mikasa_DASHBOARD_HOST=0.0.0.0")
