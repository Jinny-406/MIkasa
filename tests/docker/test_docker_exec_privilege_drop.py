...
The shim (docker/hermes-exec-shim.sh, installed at /opt/mikasa/bin/hermes)
...
3. Files written under $MIKASA_HOME from a ``docker exec`` session land
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``$MIKASA_HOME/logs/container-boot.log``: it is written by
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `hermes config set` which writes config.yaml under MIKASA_HOME.
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/mikasa/bin earliest on PATH; main-wrapper.sh
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    # provoke a write into MIKASA_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # MIKASA_HOME that docker exec just wrote.
    r = subprocess.run(
