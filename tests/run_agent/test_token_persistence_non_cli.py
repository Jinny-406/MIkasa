...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    hermes_state = ModuleType("mikasa_state")
    hermes_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "mikasa_state", hermes_state)
    hermes_state_registry = ModuleType("mikasa_state_registry")
    hermes_state_registry.acquire = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "mikasa_state_registry", hermes_state_registry)
