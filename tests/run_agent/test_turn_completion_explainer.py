...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("mikasa_cli.config.load_config", return_value=config or {}),
...
def test_classify_persistence_error_categories():
    ...
    from mikasa_state import classify_persistence_error
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from mikasa_state import SessionCompressionInProgressError
    from mikasa_state_errors import CompressionSessionBusyError
    from mikasa_state import classify_persistence_error
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from mikasa_state import classify_persistence_error
    from mikasa_state_errors import SessionTurnLeaseLostError
...
def test_persistence_error_causes_tuple_matches_classifier():
    ...
    from mikasa_state import classify_persistence_error
    from mikasa_state_errors import PERSISTENCE_ERROR_CAUSES
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("mikasa_TURN_COMPLETION_EXPLAINER", None)
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"mikasa_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("mikasa_TURN_COMPLETION_EXPLAINER", None)
        with patch("mikasa_cli.config.load_config", counting_load):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from mikasa_state import StateDbCorruptError, classify_persistence_error
