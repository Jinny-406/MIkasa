...
1. ``load_mikasa_dotenv()`` auto-loads ``~/.hermes/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_mikasa_dotenv(MIKASA_HOME=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "mikasa_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
