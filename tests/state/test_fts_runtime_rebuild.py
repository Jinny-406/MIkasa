...
import mikasa_state
import mikasa_state_holders
import mikasa_state_schema
from mikasa_state import SessionDB
from mikasa_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from mikasa_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m mikasa_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                ...
                "be-child",
                "ssh",
                "--cmd=python -m mikasa_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/mikasa-agent/.venv/bin/hermes gateway"),
            ("python3", "/opt/mikasa-agent/tools/check_state.py"),
            ("hermes-monitor", "gateway"),
            ("hermesctl", "serve"),
            ("python3", "worker.py", "mikasa_cli.main"),
            ("python3", "-m", "other.module", "mikasa_cli.main"),
            ("python3", "-c", "mikasa_cli.main"),
            ("python3", "-Icprint('mikasa_cli.main')", "mikasa_cli/main.py"),
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("/usr/bin/python3", "-m", "mikasa_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "mikasa_cli.main", "gateway"),
            ("/usr/bin/python3", "-mmikasa_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "mikasa_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "mikasa_cli.main"),
            (
                "/opt/mikasa-agent/.venv/bin/python",
                "/opt/mikasa-agent/hermes_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "mikasa_cli/main.py", "gateway"),
            ("python3", "/opt/mikasa-agent/run_agent.py", "--query", "hello"),
        ),
    )
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        cmdline_path.write_bytes(
            b"python3\x00-m\x00mikasa_cli.main\x00chat\x00"
        )
        ...
        assert "mikasa_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from mikasa_state import StateDbCorruptError
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="mikasa_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
