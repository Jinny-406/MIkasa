"""Tests for mikasa_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports mikasa_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported mikasa_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("mikasa_bootstrap", None)
    import mikasa_bootstrap  # noqa: WPS433
    return mikasa_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import mikasa_bootstrap as its
    ...
    # import mikasa_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "mikasa_cli/main.py",   # hermes CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import mikasa_bootstrap' and that
        ...
        ``mikasa_bootstrap`` as a top-level module) leaves hermes
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when mikasa_bootstrap hasn't been
        ...
        assert first_import_name == "mikasa_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'mikasa_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import mikasa_bootstrap' line to be the first import."
        )
...
class TestHardenImportPath:
    ...
    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("mikasa_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["mikasa_PYTHON_SRC_ROOT"] = env
            elif "mikasa_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["mikasa_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/mikasa")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("mikasa_PYTHON_SRC_ROOT", None)
            else:
                os.environ["mikasa_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/mikasa", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/mikasa"])
        assert result[0] == "/opt/mikasa"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Hermes root.
        assert result.index("/opt/mikasa") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["mikasa_PYTHON_SRC_ROOT"] = "/env/mikasa"
            hb.harden_import_path()
            assert sys.path[0] == "/env/mikasa"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("mikasa_PYTHON_SRC_ROOT", None)
