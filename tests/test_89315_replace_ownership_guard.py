...
cannot prove belongs to this MIKASA_HOME.

Design contract (v3, after andrexibiza's second review): ownership is decided
by the persisted identity record ALONE — exact ``_same_mikasa_home`` equality
bound to the live target by exact PID + start-time. A readable live argv
carries no MIKASA_HOME, so it can never prove home ownership; it only feeds a
...
def profile_env(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME mirroring tests/hermes_cli/test_profiles.py."""
...
def _record(pid=424242, start=111222333, home=None, argv=None):
    return {
        "pid": pid,
        "kind": "hermes-gateway",
        "argv": argv or ["python", "-m", "mikasa_cli.main", "gateway", "run"],
        "start_time": start,
        "mikasa_home": home,
    }
...
class TestRecordAuthority:
    def test_valid_bound_same_home_record_allows(self, profile_env):
        ...
        with (
            ...
            patch(
                "gateway.status._get_process_mikasa_home",
    ...
    def test_legacy_record_without_home_refuses(self, profile_env):
        """A pre-MIKASA_HOME-stamping record cannot prove ownership."""
        from gateway.run import _replace_target_belongs_to_other_profile

        legacy = _record(home=None)
        legacy.pop("mikasa_home")
...
class TestArgvConsistencyCheck:
    ...
    def test_prefix_collision_is_not_a_conflict(self, profile_env):
        ...
        assert (
            conflict("python -m mikasa_cli.main --profile timothy gateway run", tim_home)
            is True
        )
        # Our own exact name stays consistent.
        assert (
            conflict("python -m mikasa_cli.main --profile tim gateway run", tim_home)
            is False
        )
        assert (
            conflict("python -m mikasa_cli.main -p tim gateway run", tim_home)
            is False
        )

    def test_explicit_home_flag_exact_compare(self, profile_env):
        """mikasa_HOME= on the argv compares path-exactly, not by prefix."""
        from gateway.run import (
            _looks_like_profile_conflict_from_cmdline as conflict,
        )
        ...
        assert (
            conflict(
                "python -m mikasa_cli.main MIKASA_HOME=/home/x/.hermes/profiles/timothy gateway run",
                tim_home,
            )
            is True
        )
        assert (
            conflict(
                "python -m mikasa_cli.main --hermes-home /home/x/.hermes/profiles/tim/ gateway run",
                tim_home,
            )
    ...
    def test_consistency_contradiction_refuses_even_with_agreeing_record(
        ...
        with (
            ...
            patch(
                ...
                return_value="python -m mikasa_cli.main --profile other-profile gateway run",
            ),
            patch(
                "gateway.status._get_process_mikasa_home",
                return_value=profile_env / ".hermes",
