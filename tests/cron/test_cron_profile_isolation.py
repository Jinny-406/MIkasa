...
profile's MIKASA_HOME and EXECUTE under it.
...
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active MIKASA_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever MIKASA_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    MIKASA_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import mikasa_constants

    monkeypatch.setattr(
        mikasa_constants, "_get_platform_default_mikasa_home", lambda: root
    )
    monkeypatch.setenv("mikasa_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile MIKASA_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "mikasa_home"
    ...
    assert mikasa_constants.get_mikasa_home().resolve() == profile_home.resolve()
    assert mikasa_constants.get_default_mikasa_root().resolve() == root.resolve()

    # cron/jobs.py computes HERMES_DIR from get_mikasa_home() at import, so a
