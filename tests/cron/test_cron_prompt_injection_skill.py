...
def cron_env(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `MIKASA_HOME` alone doesn't reach it. We also patch the
    ...
    skills_dir.mkdir()
    (MIKASA_HOME / "cron").mkdir()
    (MIKASA_HOME / "cron" / "output").mkdir()
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
    monkeypatch.setenv("mikasa_BUNDLES_DIR", str(MIKASA_HOME / "skill-bundles"))
    ...
    monkeypatch.setattr(_skills_tool, "mikasa_HOME", MIKASA_HOME)
    ...
    return MIKASA_HOME, _scheduler


def _plant_skill(MIKASA_HOME: Path, name: str, body: str) -> None:
    """Drop a SKILL.md into ~/.hermes/skills/<name>/ bypassing skills_guard."""
    skill_dir = MIKASA_HOME / "skills" / name
...
def _plant_bundle(MIKASA_HOME: Path, name: str, skills: list[str], instruction: str = "") -> None:
    """Drop a bundle YAML into ~/.hermes/skill-bundles/ and refresh cache."""
    bundles_dir = MIKASA_HOME / "skill-bundles"
...
class TestBuildJobPromptScansSkillContent:
    ...
    def test_builtin_style_github_api_example_is_allowed(self, cron_env):
        MIKASA_HOME, scheduler = cron_env
        _plant_skill(
            MIKASA_HOME,
            "github-auth",
    ...
    def test_bundle_name_shadows_skill_name_for_cron_jobs(self, cron_env):
        MIKASA_HOME, scheduler = cron_env
        _plant_skill(MIKASA_HOME, "article-pipeline", "Standalone skill should not win.")
        _plant_skill(MIKASA_HOME, "bundle-member", "Bundle member should win.")
        _plant_bundle(MIKASA_HOME, "article-pipeline", ["bundle-member"])
