...
These exercise the real store against a temp MIKASA_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated MIKASA_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("mikasa_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_mikasa_home() reads the env live.
    yield tmp_path
