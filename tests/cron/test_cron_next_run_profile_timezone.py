...
profiles' cron stores via ``set_mikasa_home_override()`` + ``use_cron_store()``.
...
def test_foreign_process_tick_persists_owning_profile_offset(
    ...
    monkeypatch.setenv("mikasa_HOME", str(backend_home))
    ...
    token = set_mikasa_home_override(str(profile_a))
    try:
