...
resolve_runtime_provider against a temp MIKASA_HOME) and the create_job
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("mikasa_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
...
def _run_with_current_provider_and_model(
    ...
    *,
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._mikasa_home", tmp_path), \
         patch("cron.scheduler._get_mikasa_home", return_value=tmp_path), \
         patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
         patch("mikasa_cli.env_loader.load_mikasa_dotenv"), \
         patch("mikasa_cli.env_loader.reset_secret_source_cache"), \
         patch("mikasa_state_registry.acquire", return_value=fake_db), \
