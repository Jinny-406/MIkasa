...
def cron_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(J, "mikasa_DIR", MIKASA_HOME)
    monkeypatch.setattr(J, "CRON_DIR", MIKASA_HOME / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", MIKASA_HOME / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", MIKASA_HOME / "cron" / "output")
