...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "mikasa_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setenv("mikasa_TEST_RUNTIME_KEY", "unused-placeholder")

    def _fake_runtime(**_kwargs):
        return {
            "api_key": os.environ.get("mikasa_TEST_RUNTIME_KEY", ""),
            "base_url": None,
    ...
    monkeypatch.setattr(
        "mikasa_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
    )
