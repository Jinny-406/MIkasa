...
def _install_fake_slack_sender(monkeypatch, result_factory):
    ...
    if entry is None:
        ...
        import mikasa_cli.plugins as hp_boot
    ...
    import mikasa_cli.plugins as hp
...
def slack_platform_config(monkeypatch, tmp_path):
    ...
    # Config caches are process-global; clear them so the temp MIKASA_HOME wins.
    try:
...
class TestMediaPolicyEnvBridge:
    ...
    def test_bridge_helper_exists_and_applies_config(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("mikasa_HOME", str(home))
        for var in (
            "mikasa_MEDIA_DELIVERY_STRICT",
            "mikasa_MEDIA_ALLOW_DIRS",
            "mikasa_MEDIA_TRUST_RECENT_FILES",
        ):
        ...
        assert os.environ.get("mikasa_MEDIA_DELIVERY_STRICT") == "1"
        assert str(allow_dir) in os.environ.get("mikasa_MEDIA_ALLOW_DIRS", "")
        assert os.environ.get("mikasa_MEDIA_TRUST_RECENT_FILES") == "0"
    ...
    def test_standalone_filter_honors_bridged_allowlist(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("mikasa_HOME", str(home))
        for var in ("mikasa_MEDIA_DELIVERY_STRICT", "mikasa_MEDIA_ALLOW_DIRS"):
            monkeypatch.delenv(var, raising=False)
