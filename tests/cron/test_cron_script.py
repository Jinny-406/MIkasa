...
- Path resolution (absolute, relative to MIKASA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp MIKASA_HOME."""
    MIKASA_HOME = tmp_path / ".hermes"
    MIKASA_HOME.mkdir()
    (MIKASA_HOME / "cron").mkdir()
    (MIKASA_HOME / "cron" / "output").mkdir()
    (MIKASA_HOME / "scripts").mkdir()
    monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "mikasa_DIR", MIKASA_HOME)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", MIKASA_HOME / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", MIKASA_HOME / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", MIKASA_HOME / "cron" / "output")
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("mikasa_SESSION_PLATFORM") is None
        assert os.environ.get("mikasa_SESSION_CHAT_ID") is None
        assert os.environ.get("mikasa_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_CRON_SCRIPT_TIMEOUT", "2")
