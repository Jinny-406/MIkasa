...
def hermes_env(tmp_path, monkeypatch):
    """Isolate MIKASA_HOME for each test so jobs/scripts don't leak."""
    ...
    # Reload modules that cache get_mikasa_home() at import time.
    import importlib
...
def test_run_job_no_agent_reloads_dotenv_before_script(hermes_env, monkeypatch):
    ...
    def fake_load(*, MIKASA_HOME=None, project_env=None):
        loaded_homes.append(MIKASA_HOME)
        return []

    monkeypatch.setattr(env_loader, "load_mikasa_dotenv", fake_load)
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_mikasa_dotenv was not called on the no_agent path"
