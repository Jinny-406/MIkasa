...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("mikasa_cli.tools_config._get_platform_tools",
    ...
    def test_resolver_keeps_memory_from_platform_fallback(self):
        job = {"enabled_toolsets": None}
        with patch(
            "mikasa_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._mikasa_home", tmp_path), \
             patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
             patch("mikasa_cli.env_loader.load_mikasa_dotenv"), \
             patch("mikasa_cli.env_loader.reset_secret_source_cache"), \
             patch("mikasa_state_registry.acquire", return_value=fake_db), \
             patch(
    ...
    def _run_job_patches(self, tmp_path, extra=()):
        ...
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        base = [
            patch("cron.scheduler._mikasa_home", tmp_path),
            patch("cron.scheduler_delivery._resolve_origin", return_value=None),
            patch("mikasa_cli.env_loader.load_mikasa_dotenv"),
            patch("mikasa_cli.env_loader.reset_secret_source_cache"),
            patch("mikasa_state_registry.acquire", return_value=fake_db),
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("mikasa_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("mikasa_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("mikasa_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("mikasa_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("mikasa_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("mikasa_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("mikasa_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("mikasa_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_heartbeats_oneshot_claim_in_both_wait_modes(
        ...
        monkeypatch.setenv("mikasa_CRON_TIMEOUT", timeout_value)
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this MIKASA_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_mikasa_dotenv`` (which then re-applies external secret sources).
        """
        ...
        with patch("cron.scheduler._mikasa_home", tmp_path), \
             patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
             patch("mikasa_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("mikasa_cli.env_loader.load_mikasa_dotenv", _record_load), \
             patch("mikasa_state_registry.acquire", return_value=fake_db), \
             patch(
                 "mikasa_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("mikasa_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("mikasa_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("mikasa_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        ...
        with patch("cron.scheduler._mikasa_home", tmp_path), \
             ...
             patch("mikasa_cli.env_loader.load_mikasa_dotenv"), \
             patch("mikasa_cli.env_loader.reset_secret_source_cache"), \
             patch("mikasa_cli.runtime_provider.resolve_runtime_provider",
...
class TestRunJobModelResolution:
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("mikasa_MODEL", "env-model")
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """mikasa_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("mikasa_CRON_MAX_PARALLEL", "1")
        call_times = []
