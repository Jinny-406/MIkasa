...
class TestSessionDbInitTimeout:
    def test_sessiondb_init_preserves_multiplex_profile_context(
        ...
        def make_session_db(*args, **kwargs):
            observed_homes.append(get_mikasa_home())
            return fake_db

        job = {"id": "profile-sessiondb", "name": "test", "prompt": "hello"}
        profile_token = set_mikasa_home_override(profile_home)
        try:
            with patch("cron.scheduler._mikasa_home", None), \
                 patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
                 patch("mikasa_cli.env_loader.load_mikasa_dotenv"), \
                 patch("mikasa_cli.env_loader.reset_secret_source_cache"), \
                 patch("mikasa_state_registry.acquire", side_effect=make_session_db), \
                 patch(
            ...
            reset_mikasa_home_override(profile_token)
    ...
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        with patch("cron.scheduler._mikasa_home", tmp_path), \
             ...
             patch("mikasa_cli.env_loader.load_mikasa_dotenv"), \
             patch("mikasa_cli.env_loader.reset_secret_source_cache"), \
             patch("mikasa_state_registry.acquire"), \
             patch(
                 "mikasa_cli.runtime_provider.resolve_runtime_provider",
                 return_value=_RUNTIME,
             ), \
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        ...
        monkeypatch.setenv("mikasa_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "mikasa_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._mikasa_home", tmp_path), \
                 ...
                 patch("mikasa_cli.env_loader.load_mikasa_dotenv"), \
                 patch("mikasa_cli.env_loader.reset_secret_source_cache"), \
                 patch("mikasa_state_registry.acquire", side_effect=_hanging_then_capture), \
                 patch(
                     "mikasa_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._mikasa_home", tmp_path), \
             ...
             patch("mikasa_cli.env_loader.load_mikasa_dotenv"), \
             patch("mikasa_cli.env_loader.reset_secret_source_cache"), \
             patch("mikasa_state_registry.acquire") as mock_db_cls, \
             patch(
