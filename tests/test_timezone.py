...
class TestHermesTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["mikasa_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["mikasa_TIMEZONE"] = "UTC"
        result = hermes_time.now()
...
class TestGetTimezone:
    ...
    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("mikasa_TIMEZONE", raising=False)

        monkeypatch.setenv("mikasa_HOME", str(first_home))
        assert str(hermes_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch MIKASA_HOME in one process.
        monkeypatch.setenv("mikasa_HOME", str(second_home))
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_mikasa_home_override(str(homes[key]))
                try:
                    tz = hermes_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        ...
        probe = (
            ...
            'print("mikasa_TIMEZONE=" + os.environ.get("mikasa_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "mikasa_TIMEZONE=NOT_SET" in result["output"], (
            "mikasa_TIMEZONE should not leak into child env (only TZ)"
        )
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        os.environ["mikasa_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hermes_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["mikasa_TIMEZONE"] = "US/Eastern"
        _reset_hermes_time_cache()
