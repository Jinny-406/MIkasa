"""Tests for the remembered results in `MIKASA_HOME_key`.

`Path.resolve()` is a filesystem call. `MIKASA_HOME_key` sits under
...
class TestSameAnswerAsBefore:
    ...
    def test_matches_the_uncached_calculation(self, tmp_path, case):
        ...
        assert hc.MIKASA_HOME_key(target) == _uncached(target)

    def test_matches_for_the_default_home(self):
        assert hc.MIKASA_HOME_key() == _uncached()
    ...
    def test_accepts_a_path_object(self, tmp_path):
        (tmp_path / "real").mkdir()
        assert hc.MIKASA_HOME_key(tmp_path / "real") == _uncached(tmp_path / "real")

    def test_second_call_returns_the_same_string(self, tmp_path):
        (tmp_path / "real").mkdir()
        first = hc.MIKASA_HOME_key(str(tmp_path / "real"))
        second = hc.MIKASA_HOME_key(str(tmp_path / "real"))
...
class TestWhatGetsRemembered:
    def test_an_existing_path_is_remembered(self, tmp_path):
        (tmp_path / "real").mkdir()
        hc.MIKASA_HOME_key(str(tmp_path / "real"))
    ...
    def test_a_missing_path_is_not_remembered(self, tmp_path):
        ...
        hc.MIKASA_HOME_key(str(tmp_path / "not_there"))
        assert hc._HOME_KEY_CACHE == {}

    def test_a_path_created_later_picks_up_the_real_answer(self, tmp_path):
        later = tmp_path / "later"
        before = hc.MIKASA_HOME_key(str(later))
        later.mkdir()
        after = hc.MIKASA_HOME_key(str(later))
    ...
    def test_different_paths_get_their_own_entries(self, tmp_path):
        for name in ("a", "b", "c"):
            (tmp_path / name).mkdir()
            hc.MIKASA_HOME_key(str(tmp_path / name))
...
class TestHomeChanges:
    def test_pointing_mikasa_home_somewhere_else_gives_a_new_key(
        ...
        monkeypatch.setenv("mikasa_HOME", str(first))
        key_one = hc.MIKASA_HOME_key()
        monkeypatch.setenv("mikasa_HOME", str(second))
        key_two = hc.MIKASA_HOME_key()
...
class TestSymlinks:
    def test_a_link_resolves_to_its_target(self, tmp_path):
        ...
        assert hc.MIKASA_HOME_key(str(link)) == _uncached(str(link))
        assert hc.MIKASA_HOME_key(str(link)) == hc.MIKASA_HOME_key(str(target))
