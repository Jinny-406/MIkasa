...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        assert "mikasa_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "mikasa_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        # Isolated MIKASA_HOME so we don't read the developer's config.yaml.
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "mikasa_LANGFUSE_PUBLIC_KEY", "mikasa_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("mikasa_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_accepts_documented_prefix(self, monkeypatch):
        ...
        assert plugin._validate_langfuse_key(
            "mikasa_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz"
        ) is None
        assert plugin._validate_langfuse_key(
            "mikasa_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz"
        ) is None
    ...
    def test_placeholder_public_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("mikasa_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("mikasa_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "mikasa_LANGFUSE_PUBLIC_KEY" in text
    ...
    def test_placeholder_secret_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("mikasa_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz")
        monkeypatch.setenv("mikasa_LANGFUSE_SECRET_KEY", "test-key")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "mikasa_LANGFUSE_SECRET_KEY" in text
    ...
    def test_both_placeholders_one_warning_with_both_keys(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("mikasa_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("mikasa_LANGFUSE_SECRET_KEY", "placeholder")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
...
class TestCaptureModes:
    ...
    def test_default_mode_is_sanitized(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.delenv("mikasa_LANGFUSE_CAPTURE", raising=False)
        assert mod._capture_mode() == "sanitized"

    def test_invalid_mode_falls_back_and_warns_once(self, monkeypatch, caplog):
        mod = self._fresh_plugin()
        monkeypatch.setenv("mikasa_LANGFUSE_CAPTURE", "everything")
        with caplog.at_level(logging.WARNING):
            assert mod._capture_mode() == "sanitized"
            assert mod._capture_mode() == "sanitized"
        warnings = [r for r in caplog.records if "mikasa_LANGFUSE_CAPTURE" in r.getMessage()]
        assert len(warnings) == 1

    def test_metadata_mode_omits_content(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("mikasa_LANGFUSE_CAPTURE", "metadata")
    ...
    def test_sanitized_mode_redacts_secrets(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("mikasa_LANGFUSE_CAPTURE", "sanitized")
        samples = {
    ...
    def test_full_mode_keeps_secret_shaped_text(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("mikasa_LANGFUSE_CAPTURE", "full")
...
class TestAtexitFinalization(TestTurnTraceIsolation):
    ...
    def test_atexit_hook_is_registered_on_client_init(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_LANGFUSE_PUBLIC_KEY", "pk-lf-0123456789abcdef")
        monkeypatch.setenv("mikasa_LANGFUSE_SECRET_KEY", "sk-lf-0123456789abcdef")
