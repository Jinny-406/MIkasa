...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("mikasa_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("mikasa_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "mikasa_DASHBOARD_OAUTH_CLIENT_ID" in nous_plugin.LAST_SKIP_REASON
    ...
    def test_registers_with_default_portal_url_when_only_client_id_set(
        ...
        monkeypatch.setenv("mikasa_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("mikasa_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        nous_plugin.register(ctx)
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("mikasa_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.nousresearch.com"
...
class TestConfigYamlSource:
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``mikasa_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "mikasa_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        nous_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
...
class TestStartLogin:
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "mikasa_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["mikasa_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["mikasa_session_pkce"] != b.cookie_payload[
            "mikasa_session_pkce"
        ]
