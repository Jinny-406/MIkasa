...
@pytest.fixture
def tmp_mikasa_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
def test_store_and_load_photon_token(tmp_mikasa_home: Path) -> None:
    photon_auth.store_photon_token("abc123def456")
    assert photon_auth.load_photon_token() == "abc123def456"

    auth_json = json.loads((tmp_mikasa_home / "auth.json").read_text())
...
def test_save_auth_never_world_readable(tmp_mikasa_home: Path) -> None:
    """auth.json must be created 0o600 — no window at process umask."""
    photon_auth.store_photon_token("secret-token")
    mode = (tmp_mikasa_home / "auth.json").stat().st_mode & 0o777
    assert mode == 0o600


def test_store_project_credentials_round_trip(
    tmp_mikasa_home: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def _hold_auth_lock_then_release(hold_event: threading.Event, release_event: threading.Event) -> None:
    from mikasa_cli.auth import _auth_store_lock
