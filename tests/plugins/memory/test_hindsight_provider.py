...
from mikasa_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_mikasa_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", MIKASA_HOME=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", MIKASA_HOME=str(tmp_path), platform="cli")
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        MIKASA_HOME = tmp_path / "hermes-home"
        user_home = tmp_path / "user-home"
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_mikasa_home", lambda: MIKASA_HOME)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("mikasa_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("mikasa_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(MIKASA_HOME), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (MIKASA_HOME / ".env").exists()
        assert not (MIKASA_HOME / "hindsight" / "config.json").exists()
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mikasa_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("mikasa_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(MIKASA_HOME), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (MIKASA_HOME / ".env").read_text()
...
class TestSyncTurn:
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_mikasa_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", MIKASA_HOME=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", MIKASA_HOME=str(tmp_path), platform="cli")
...
class TestRetainIndicator:
    ...
    def test_status_callback_wired_from_initialize(self, tmp_path, monkeypatch):
        ...
        p.initialize(session_id="s", MIKASA_HOME=str(tmp_path), status_callback=cb)
...
class TestAvailability:
    def test_available_with_api_key(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_mikasa_home",
...
class TestPostSetupEnvEncoding:
    def _run_cloud_post_setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mikasa_cli.memory_setup._curses_select",
                            lambda *a, **kw: 0)  # cloud mode
        monkeypatch.setattr("mikasa_cli.config.save_config", lambda c: None)
...
class TestClientAutoUpgradeRoutesThroughLazyDeps:
    ...
    def _init_with_outdated_client(self, tmp_path, monkeypatch, outcome):
        ...
        provider = HindsightMemoryProvider()
        provider.initialize(session_id="s", MIKASA_HOME=str(tmp_path), platform="cli")
        return calls
...
class TestMultiplexBackgroundScope:
    ...
    def scoped_embedded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_mikasa_home", lambda: home)
        home_tok = set_mikasa_home_override(str(home))
        scope_tok = set_secret_scope(build_profile_secret_scope(home))
        yield created, home
        set_multiplex_active(False)
        reset_secret_scope(scope_tok)
        reset_mikasa_home_override(home_tok)
    ...
    def test_daemon_start_thread_resolves_profile_secret(self, scoped_embedded):
        created, home = scoped_embedded
        p = HindsightMemoryProvider()
        p.initialize(session_id="s1", MIKASA_HOME=str(home), platform="cli")
