...
the runtime plugin — uses the underscore form ("mikasa_work"). The lookup
...
a real honcho.json (temp MIKASA_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        ...
        "hosts": {
            ...
            "mikasa_work": {"peerName": "alice", "aiPeer": "hermes"},
            "mikasa_my_profile": {"peerName": "bob", "aiPeer": "hermes"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "mikasa_cli.profiles.list_profiles",
        ...
        assert host == "mikasa_work"  # not "hermes.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'mikasa_my_profile';
