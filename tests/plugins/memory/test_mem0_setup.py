...
def _inject_fake_hermes_cli(monkeypatch):
    ...
    fake_config_mod = types.ModuleType("mikasa_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("mikasa_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_hermes_cli = types.ModuleType("mikasa_cli")
    fake_mikasa_cli.config = fake_config_mod
    fake_hermes_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "mikasa_cli", fake_hermes_cli)
    monkeypatch.setitem(sys.modules, "mikasa_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "mikasa_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.mem0._setup.get_mikasa_home", lambda: tmp_path)
