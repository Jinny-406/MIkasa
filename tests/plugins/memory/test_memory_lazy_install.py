...
class TestSealedVenvDurableTarget:
    ...
    def test_ensure_installs_into_durable_target_on_sealed_venv(
        ...
        monkeypatch.setenv("mikasa_LAZY_INSTALL_TARGET", str(tmp_path / "lazy"))
        ...
        def fake_install(specs, **kw):
            ...
            captured["target_env"] = os.environ.get("mikasa_LAZY_INSTALL_TARGET")
    ...
    def test_sealed_venv_without_target_blocks(self, feature, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_DISABLE_LAZY_INSTALLS", "1")
        monkeypatch.delenv("mikasa_LAZY_INSTALL_TARGET", raising=False)
        monkeypatch.setattr(
            "mikasa_cli.config.load_config",
            lambda: {"security": {"allow_lazy_installs": True}},
        )
