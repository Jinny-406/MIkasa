...
def test_setup_linux_loads_null_sink_and_virtual_source():
    ...
    assert info["device_name"] == "mikasa_meet_src"
    assert info["write_target"] == "mikasa_meet_sink"
    assert info["sample_rate"] == 48000
    assert info["channels"] == 2
    assert info["module_ids"] == [42, 43]

    # Properties.
    assert br.device_name == "mikasa_meet_src"
    assert br.write_target == "mikasa_meet_sink"
