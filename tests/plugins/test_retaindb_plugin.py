...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure MIKASA_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", MIKASA_HOME=str(tmp_path / ".hermes"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_HOME", str(MIKASA_HOME))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", MIKASA_HOME=str(MIKASA_HOME))
        return p
