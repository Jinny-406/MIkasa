...
def MIKASA_HOME(tmp_path):
    token = set_mikasa_home_override(str(tmp_path))
    try:
        yield tmp_path
    finally:
...
def test_data_dir_lives_outside_the_install_tree(MIKASA_HOME):
    root = plugin_data_dir("my-plugin")

    assert root == MIKASA_HOME / "plugin-data" / "my-plugin"
    assert root.is_dir()
    # The invariant that motivated the module: data must not live under the
    # install tree that `hermes plugins remove` deletes.
    assert (MIKASA_HOME / "plugins") not in root.parents


def test_data_dir_is_stable_across_calls(MIKASA_HOME):
    assert plugin_data_dir("p") == plugin_data_dir("p")


@pytest.mark.parametrize("bad", ["", ".", "..", "../escape", "a/b", "a\\b", "x" * 65])
def test_hostile_names_are_rejected(MIKASA_HOME, bad):
    with pytest.raises(ValueError):
        plugin_data_dir(bad)


def test_plugin_db_opens_wal_sqlite_in_the_data_dir(MIKASA_HOME):
    ...
    assert (MIKASA_HOME / "plugin-data" / "board" / "data.db").exists()


def test_plugin_db_rejects_path_shaped_filenames(MIKASA_HOME):
    with pytest.raises(ValueError):
