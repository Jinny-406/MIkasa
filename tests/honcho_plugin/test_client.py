...
from mikasa_cli.profiles import _get_default_mikasa_home
...
class TestResolveConfigPath:
    def test_prefers_mikasa_home_when_exists(self, tmp_path):
        MIKASA_HOME = tmp_path / "hermes"
        MIKASA_HOME.mkdir()
        local_cfg = MIKASA_HOME / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: MIKASA_HOME points at ~/.hermes/profiles/<name>, so
        # _get_default_mikasa_home() must resolve back to ~/.hermes — that's
...
class TestResolveActiveHost:
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("mikasa_HONCHO_HOST", None)
            # Temporarily remove hermes_cli.profiles to simulate import failure
            saved = sys.modules.get("mikasa_cli.profiles")
            sys.modules["mikasa_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "hermes"
            finally:
                if saved is not None:
                    sys.modules["mikasa_cli.profiles"] = saved
                else:
                    sys.modules.pop("mikasa_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="mikasa_coder")
        assert config.host == "mikasa_coder"
        assert config.workspace_id == "hermes"  # shared workspace
        assert config.ai_peer == "mikasa_coder"
...
class TestGetHonchoClient:
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_mikasa_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("mikasa_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("mikasa_cli.profiles.get_active_profile_name", return_value="default"), \
