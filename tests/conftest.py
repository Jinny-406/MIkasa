...
2. **Isolated MIKASA_HOME.** MIKASA_HOME points to a per-test tempdir so
   code reading ``~/.hermes/*`` via ``get_mikasa_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_mikasa_home()`` is a bug to fix at the callsite.)
...
# ── Sandbox MIKASA_HOME before ANY test module is imported ──────────────────
# `hermes_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_mikasa_home()` and attaches rotating file handlers to the ROOT logger.
...
#
# The `_isolate_env` fixture below also sandboxes MIKASA_HOME, but fixtures run
...
# the REAL Hermes root — capture it BEFORE the sandbox rewires MIKASA_HOME,
...
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("mikasa_KANBAN_HOME", "").strip()
_PRE_SANDBOX_mikasa_home = os.environ.get("mikasa_HOME", "")


def _mikasa_home_points_at_production(value: str) -> bool:
    """True when a pre-set MIKASA_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    MIKASA_HOME=~/.hermes``) hand pytest the PRODUCTION home. Historically
...
if _mikasa_home_points_at_production(os.environ.get("mikasa_HOME", "")):
    _SESSION_mikasa_home = tempfile.mkdtemp(prefix="hermes-test-home-")
    os.environ["mikasa_HOME"] = _SESSION_mikasa_home
    atexit.register(shutil.rmtree, _SESSION_mikasa_home, True)
...
# the child at the same moment the child lost the MIKASA_HOME redirect.
...
os.environ["mikasa_TEST_ISOLATION"] = os.environ.get("mikasa_HOME", "") or "1"

#: MIKASA_HOME as it stood when conftest was imported - i.e. before any test
...
MIKASA_HOME_AT_CONFTEST_IMPORT = os.environ.get("mikasa_HOME", "")
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # mikasa_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "mikasa_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "mikasa_INTERACTIVE",
    "mikasa_QUIET",
    "mikasa_TOOL_PROGRESS",
    "mikasa_TOOL_PROGRESS_MODE",
    "mikasa_MAX_ITERATIONS",
    "mikasa_SESSION_PLATFORM",
    "mikasa_SESSION_CHAT_ID",
    "mikasa_SESSION_CHAT_NAME",
    "mikasa_SESSION_CHAT_TYPE",
    "mikasa_SESSION_THREAD_ID",
    "mikasa_SESSION_SOURCE",
    "mikasa_SESSION_KEY",
    "mikasa_GATEWAY_SESSION",
    "mikasa_CRON_SESSION",
    "_HERMES_GATEWAY",
    "mikasa_PLATFORM",
    "mikasa_MODEL",
    "mikasa_INFERENCE_MODEL",
    "mikasa_INFERENCE_PROVIDER",
    "mikasa_TUI_PROVIDER",
    "mikasa_MANAGED",
    "mikasa_MANAGED_DIR",
    "mikasa_DEV",
    "mikasa_CONTAINER",
    "mikasa_EPHEMERAL_SYSTEM_PROMPT",
    "mikasa_TIMEZONE",
    "mikasa_REDACT_SECRETS",
    "mikasa_BACKGROUND_NOTIFICATIONS",
    "mikasa_EXEC_ASK",
    "mikasa_HOME_MODE",
    "mikasa_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.hermes/kanban.db instead of the per-test MIKASA_HOME.
    "mikasa_KANBAN_DB",
    "mikasa_KANBAN_BOARD",
    "mikasa_KANBAN_HOME",
    "mikasa_KANBAN_WORKSPACES_ROOT",
    "mikasa_KANBAN_LOGS_ROOT",
    "mikasa_KANBAN_TASK",
    "mikasa_KANBAN_WORKSPACE",
    "mikasa_KANBAN_RUN_ID",
    "mikasa_KANBAN_CLAIM_LOCK",
    "mikasa_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "mikasa_DELEGATED_CHILD_CONTEXT",
    "mikasa_TENANT",
    ...
    "mikasa_DASHBOARD_OAUTH_CLIENT_ID",
    "mikasa_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MIKASA_HOME to per-test tempdirs so code that
    ...
    monkeypatch.setenv("mikasa_HONCHO_HOST", "hermes")

    # 3. Redirect MIKASA_HOME to a per-test tempdir. Code that reads
    #    ``~/.hermes/*`` via ``get_mikasa_home()`` now gets the tempdir.
    #
    ...
    #    ``Path.home() / ".hermes"`` instead of ``get_mikasa_home()``
    #    is a bug to fix at the callsite.
    fake_mikasa_home = tmp_path / "mikasa_test"
    fake_mikasa_home.mkdir()
    (fake_mikasa_home / "sessions").mkdir()
    (fake_mikasa_home / "cron").mkdir()
    (fake_mikasa_home / "memories").mkdir()
    (fake_mikasa_home / "skills").mkdir()
    monkeypatch.setenv("mikasa_HOME", str(fake_mikasa_home))
    ...
    monkeypatch.setenv("mikasa_TEST_ISOLATION", str(fake_mikasa_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("mikasa_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. hermes_state computes ``DEFAULT_DB_PATH = get_mikasa_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from mikasa_state import ...``) that
    ...
    hermes_state_mod = sys.modules.get("mikasa_state")
    if hermes_state_mod is not None and hasattr(hermes_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            hermes_state_mod, "DEFAULT_DB_PATH", fake_mikasa_home / "state.db"
        )
    ...
    monkeypatch.setenv("mikasa_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.hermes/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import mikasa_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path MIKASA_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_mikasa_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from mikasa_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# allow-list because test-level fixtures legitimately move MIKASA_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session MIKASA_HOME sandbox rewired the env), so the
    ...
    if _PRE_SANDBOX_mikasa_home and not _mikasa_home_points_at_production(
        _PRE_SANDBOX_mikasa_home
    ):
        # MIKASA_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_mikasa_root().resolve()
    # No pre-existing MIKASA_HOME: the real root is the platform default,
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    move MIKASA_HOME to sibling tempdirs are unaffected.
    ...
    """
    _kdb = sys.modules.get("mikasa_cli.kanban_db")
    _kdbc = sys.modules.get("mikasa_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import mikasa_cli.kanban_db`
...
#     installs where MIKASA_HOME is not ~/.hermes) into the guard's
...
# so subprocess children that import mikasa_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("mikasa_state")
    ...
    if _PRE_SANDBOX_mikasa_home and not _mikasa_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_mikasa_home).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # ``get_mikasa_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from mikasa_constants import get_mikasa_home_override, set_mikasa_home_override

        if get_mikasa_home_override() is not None:
            set_mikasa_home_override(None)
    except Exception:
        pass
...
def _wal_is_usable() -> bool:
    ...
    ``DEFAULT_DB_PATH`` from ``get_mikasa_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_mikasa_home`` fixture
    redirects ``MIKASA_HOME`` — permanently caches the DEVELOPER'S REAL
...
#      process environment: ``os.environ["mikasa_VOICE_TTS"] = "1"``. The
...
def _live_system_guard(request, monkeypatch):
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m mikasa_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # equivalent `python -m mikasa_cli.main update`).  These commands
        ...
        if "update" in low and (
            ...
            or
            # python -m mikasa_cli.main update --gateway
            ("mikasa_cli" in low and "update" in low.split())
            or
...
def _audio_playback_guard(request, monkeypatch):
    ...
    try:
        import mikasa_cli.voice as _voice
    except Exception:
