...
def MIKASA_HOME(tmp_path, monkeypatch):
    monkeypatch.setenv("mikasa_HOME", str(tmp_path / ".hermes-home"))
...
def test_record_verify_run_marks_workspace_passed(MIKASA_HOME):
    project = _workspace(MIKASA_HOME)
...
def test_record_verify_run_records_failure(MIKASA_HOME):
    project = _workspace(MIKASA_HOME)
...
def test_cli_passing_run_writes_ledger_evidence(MIKASA_HOME, capsys):
    project = _workspace(MIKASA_HOME, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(MIKASA_HOME, capsys):
    project = _workspace(MIKASA_HOME, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(MIKASA_HOME, capsys):
...
def test_cli_run_uses_hermes_session_id_env(MIKASA_HOME, capsys, monkeypatch):
    monkeypatch.setenv("mikasa_SESSION_ID", "sess-42")
    project = _workspace(MIKASA_HOME, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_passing_verify_run_satisfies_stop_guard(MIKASA_HOME, capsys):
    project = _workspace(MIKASA_HOME, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_nudge_mentions_hermes_verify_when_recipe_has_start(MIKASA_HOME):
    project = _workspace(MIKASA_HOME, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
...
def test_nudge_mentions_hermes_verify_when_manifest_exists(MIKASA_HOME):
...
def test_nudge_keeps_plain_wording_without_recipe_start(MIKASA_HOME):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(MIKASA_HOME, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
...
def test_nudge_recipe_detection_failure_is_silent(MIKASA_HOME, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(MIKASA_HOME, capsys):
    project = _workspace(MIKASA_HOME)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(MIKASA_HOME, capsys):
...
def test_merge_skips_commands_recipe_already_has(MIKASA_HOME, capsys):
    project = _workspace(MIKASA_HOME, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
