...
from mikasa_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "mikasa_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import mikasa_cli.plugins
    import mikasa_cli.web_server
    ...
    # 404 path.
    monkeypatch.setenv("mikasa_SERVE_HEADLESS", "0")
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "mikasa_cli.main", "-p", "default"]
    assert exec_call["env"]["mikasa_HOME"] == str(tmp_path)
...
def test_dashboard_lifecycle_flags_skip_limit_adjustment(monkeypatch, lifecycle_flag):
    ...
    from mikasa_cli import main as cli_main
    import mikasa_cli.main_dashboard as hermes_cli_main_dashboard
