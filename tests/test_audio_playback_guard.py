...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("mikasa_VOICE_TTS") == "1"
    ...
    # calls it (a late `from mikasa_cli.voice import speak_text`), with the
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    """
    assert "mikasa_VOICE_TTS" not in os.environ
    assert "mikasa_VOICE" not in os.environ
