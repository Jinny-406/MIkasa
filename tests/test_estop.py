...
def MIKASA_HOME(tmp_path, monkeypatch):
    """Point MIKASA_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(MIKASA_HOME):
    assert estop.is_engaged() is False
    estop.engage()
    assert (MIKASA_HOME / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(MIKASA_HOME):
    estop.engage()
    assert estop.disengage() is True
    assert not (MIKASA_HOME / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(MIKASA_HOME):
    ...
    raw = json.loads((MIKASA_HOME / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(MIKASA_HOME):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(MIKASA_HOME):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (MIKASA_HOME / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(MIKASA_HOME):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(MIKASA_HOME):
...
def test_paused_reply_without_reason(MIKASA_HOME):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(MIKASA_HOME, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(MIKASA_HOME, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(MIKASA_HOME, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(MIKASA_HOME):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(MIKASA_HOME):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(MIKASA_HOME):
...
def test_cli_pause_engages_with_reason(MIKASA_HOME, capsys):
    from mikasa_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(MIKASA_HOME, capsys):
    from mikasa_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(MIKASA_HOME, capsys):
    from mikasa_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(MIKASA_HOME, capsys):
    from mikasa_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from mikasa_cli.main import _BUILTIN_SUBCOMMANDS
...
def test_status_line_when_paused(MIKASA_HOME):
    from mikasa_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(MIKASA_HOME, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when MIKASA_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(MIKASA_HOME):
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(MIKASA_HOME):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from mikasa_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    """fleet-analyst-class: MIKASA_HOME is a profile dir; pause lives at root.

    A process launched with MIKASA_HOME=~/.hermes/profiles/fleet-analyst must
