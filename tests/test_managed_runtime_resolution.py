...
Hermes installs runtimes for itself — ``uv`` at ``$MIKASA_HOME/bin/uv``, Node at
``$MIKASA_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
...
# Runtimes Hermes provisions into MIKASA_HOME and must therefore resolve
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("mikasa_cli/update_cmd_deps.py", "uv"): (
    ...
    ("mikasa_cli/update_cmd_deps.py", "npm"): (
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        ...
        + "\n\n$MIKASA_HOME/bin (uv) and $MIKASA_HOME/node are not on an "
        ...
        "  node/npm -> mikasa_constants.find_node_executable()\n"
        "  PATH env -> mikasa_constants.iter_hermes_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(mikasa_constants, helper))


def test_managed_uv_helpers_exist():
    from mikasa_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
