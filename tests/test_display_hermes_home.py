"""display_mikasa_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom MIKASA_HOME under
...
class TestDisplayHermesHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        with patch.object(hc, "get_mikasa_home", return_value=nested):
            out = hc.display_mikasa_home()
    ...
    def test_default_home_unchanged(self):
        ...
        with patch.object(hc, "get_mikasa_home",
    ...
    def test_outside_home_falls_back_to_absolute(self):
        ...
        outside = Path("/opt/mikasa-custom") if os.name != "nt" else Path("C:/opt/mikasa-custom")
        with patch.object(hc, "get_mikasa_home", return_value=outside):
            out = hc.display_mikasa_home()
