...
($INSTALL_DIR), not into the shared $MIKASA_HOME.

Background (shared-$MIKASA_HOME bug)
------------------------------------
$MIKASA_HOME is a data directory users frequently bind-mount into a Docker
...
$MIKASA_HOME the two installs would fight over one slot — and the container,
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$MIKASA_HOME/\.install_method"', text), (
        "install.sh must not stamp $MIKASA_HOME/.install_method — that data "
