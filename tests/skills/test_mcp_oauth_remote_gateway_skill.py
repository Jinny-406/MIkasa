...
SESSION_REVOKED / REFRESH_DEAD), the MIKASA_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, MIKASA_HOME=str(tokens_dir.parent))), \
         patch.object(mod.urllib.request, "urlopen", side_effect=fake_urlopen), \
         patch.object(sys, "argv", ["diagnose-oauth-mcp.py", *argv]):
        # Force the env-var fallback path (ignore any importable mikasa_constants).
        with patch.object(mod, "_mikasa_home", lambda: str(tokens_dir.parent)):
            buf = io.StringIO()
