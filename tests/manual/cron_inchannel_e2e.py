...
Uses a throwaway MIKASA_HOME so it never touches ~/.hermes. No real names.
"""
...
def _fresh_home():
    """Point MIKASA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_mikasa_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["mikasa_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER MIKASA_HOME is set.
...
# a different get_mikasa_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"mikasa_HOME (throwaway): {HOME}")
