...
from mikasa_state_common import FTS_CJK_STALE_KEY, FTS_STALE_KEY, _FTS_CJK_TRIGGERS, _FTS_TRIGGERS

# caplog tests pin the "mikasa_state" logger name.
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("mikasa_FTS5_CJK_SO")
    return Path(env).expanduser() if env else get_mikasa_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("mikasa_CJK_FTS", "1").strip().lower() not in ("0", "false", "off", "no")
