"""state.db repair, backup and writability preflight (split from mikasa_state).
...
from mikasa_startup_watchdog import report_startup_progress
...
# Log-record parity with the origin module (caplog tests pin "mikasa_state").
logger = logging.getLogger("mikasa_state")
...
def _claim_repair_attempt(db_path: Path) -> bool:
    ...
    from mikasa_state import _repair_attempt_lock, _repair_attempted_paths
...
def _try_lock_nonblocking(handle) -> None:
    ...
    from mikasa_state import _IS_WINDOWS
    if _IS_WINDOWS:
...
def _cross_process_repair_lock(db_path: Path):
    ...
    #36644).
    """
    from mikasa_state import _IS_WINDOWS, _REPAIR_LOCK_TIMEOUT_SECONDS
...
def _repair_snapshot_timeout_seconds(source_path: Path) -> float:
    ...
    from mikasa_state import _REPAIR_LOCK_TIMEOUT_SECONDS
    source_bytes = 0
...
def preflight_db_writability(db_path: Path, *, db_label: str = "state.db") -> None:
    ...
    with contextlib.suppress(Exception):  # pragma: no cover - defensive
        home = Path(get_mikasa_home()).resolve()
...
def _reapply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from mikasa_state_wal import _apply_macos_checkpoint_barrier, _enforce_macos_synchronous_full
    try:
...
def apply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from mikasa_state_wal import _apply_synchronous_pragma
    ok = _reapply_durability_barriers(conn)
    with contextlib.suppress(Exception):
        from mikasa_cli.config import cfg_get, load_config_readonly  # local: avoids an import cycle
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    See #50502.
    """
    from mikasa_state import SessionDB, load_fts5_cjk_extension
...
def _probe_journal_mode_for_repair(db_path: Path) -> Optional[str]:
    ...
    from mikasa_state_wal import _on_disk_journal_mode
    try:
...
def _strategy_rebuild_fts(conn: sqlite3.Connection) -> None:
    ...
    from mikasa_state import load_fts5_cjk_extension
