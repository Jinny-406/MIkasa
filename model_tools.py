...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # Carry profile + approval/sudo context so get_mikasa_home() resolves correctly.
...
def _tool_defs_cache_key(
    ...
    return (
        ...
        bool(os.environ.get("mikasa_KANBAN_TASK")), bool(skip_tool_search_assembly),
...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        if (os.environ.get("mikasa_KANBAN_TASK") and not _is_delegated_child_context()
