...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from mikasa_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    if hermes_bin:
        ...
        try:
            ...
            found = _ilu.find_spec("mikasa_cli") is not None
        except Exception:
            found = False
        if not found:
            return "bot-chat delivery failed: hermes CLI not resolvable"
        argv = [sys.executable, "-m", "mikasa_cli.main"]
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's MIKASA_HOME must not shadow it.
        env.pop("mikasa_HOME", None)
