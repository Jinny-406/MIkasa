...
from mikasa_time import now as _hermes_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    from mikasa_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    try:
        from mikasa_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    from mikasa_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_min_allowance_minutes() -> float:
    ...
    with contextlib.suppress(Exception):
        ...
        if _cfg_val is not None:
            ...
            if val > 0:
                return val
    raw = os.getenv("mikasa_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    try:
        path = _get_mikasa_home() / "cron" / "inflight_forced_releases.jsonl"
...
def mark_running_jobs_interrupted(
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_mikasa_home())
                for job_id in (
...
def _cron_inactivity_seconds() -> float:
    ...
    raw = os.getenv("mikasa_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
...
# Per-fire usage audit log; resolves via _get_mikasa_home() so profile-scoped paths work.
def _usage_audit_path() -> Path:
    return _get_mikasa_home() / "cron" / "usage_audit.jsonl"
...
_mikasa_home: Path | None = None


def _get_mikasa_home() -> Path:
    ...
    resolving the active MIKASA_HOME at call time means a profile's jobs are stored AND executed under that
    profile's home (its .env, config.yaml, scripts, skills).
    """
    return _mikasa_home or get_mikasa_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    MIKASA_HOME = _get_mikasa_home()
    lock_dir = MIKASA_HOME / "cron"
...
def _run_no_agent_job(
    ...
    try:
        ...
        load_mikasa_dotenv(MIKASA_HOME=_get_mikasa_home())
    except Exception:
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    model = job.get("model") or os.getenv("mikasa_MODEL") or ""
    _cron_default_provider = ""
    _cfg: dict = {}
    _model_cfg: Any = {}
    try:
        from mikasa_cli.config import read_user_config_raw
        _cfg_path = str(_get_mikasa_home() / "config.yaml")
    ...
    if not (isinstance(model, str) and model.strip()):
        raise RuntimeError(
            ...
            f"mikasa_MODEL={os.getenv('mikasa_MODEL', '')!r}, "
...
def _load_prefill_messages(cfg: dict, job_id: str) -> Optional[list]:
    ...
    prefill_file = (
        os.getenv("mikasa_PREFILL_MESSAGES_FILE", "")
    ...
    if not pfpath.is_absolute():
        pfpath = _get_mikasa_home() / pfpath
...
def _open_cron_session_db(job: dict):
    ...
    # handle (#96290). Bounded with its own timeout (separate from mikasa_CRON_TIMEOUT, which only watches
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if final_response.strip() and turn_exit_reason:
        ...
        _explainer_variants = []
        try:
            from mikasa_state_errors import PERSISTENCE_ERROR_CAUSES as _causes
        except Exception:
...
_CRON_DELIVERY_VARS = (
    "mikasa_CRON_AUTO_DELIVER_PLATFORM",
    "mikasa_CRON_AUTO_DELIVER_CHAT_ID",
    "mikasa_CRON_AUTO_DELIVER_THREAD_ID")
...
class _CronRunScope:
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._cron_session_var = _VAR_MAP["mikasa_CRON_SESSION"]
...
def _reload_dotenv_and_publish_delivery_target(job: dict) -> None:
    ...
    if delivery_target:
        _VAR_MAP["mikasa_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
        _VAR_MAP["mikasa_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
        _VAR_MAP["mikasa_CRON_AUTO_DELIVER_THREAD_ID"].set(
...
def run_one_job(
    ...
    profile_home = _get_mikasa_home().resolve()
    with _running_lock:
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(build_profile_secret_scope(_get_mikasa_home()))
        ...
        _terminal_scope_token = install_profile_terminal_scope(_get_mikasa_home())
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    execution_id = str(job["execution_id"])
    job_id = str(job["id"])
    handoff_dir = _get_mikasa_home() / "cron" / "external-workers"
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as payload_file:
            json.dump(
                {
                    "job": job,
                    "profile_home": str(_get_mikasa_home().resolve()),
    ...
    worker_env = build_subprocess_env(
        ...
        extra={"mikasa_HOME": str(_get_mikasa_home().resolve())},
    )
    try:
        process = subprocess.Popen(
            scoped_command,
...
def _run_external_worker_payload(payload_path: Path, ack_path: Path) -> bool:
    ...
    home_token = set_mikasa_home_override(profile_home)
...
def _resolve_max_parallel_workers() -> Optional[int]:
    ...
    try:
        _env_par = os.getenv("mikasa_CRON_MAX_PARALLEL", "").strip()
        if _env_par:
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            from mikasa_logging import setup_logging

            setup_logging(MIKASA_HOME=_get_mikasa_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
