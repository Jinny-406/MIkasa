...
# Cron is per-profile by design: anchor at get_mikasa_home() (active profile home), NOT
# get_default_mikasa_root() — the shared root would funnel every profile's jobs into one jobs.json
# and run them under the ticker's MIKASA_HOME, leaking config/credentials/skills across profiles.
# Each profile owns its own cron store under its own MIKASA_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same MIKASA_HOME — so a job authored in profile `coder` lives in
...
# `_get_mikasa_home()` / `_get_lock_paths()` resolution in cron/scheduler.py. See #4707.
HERMES_DIR = get_mikasa_home().resolve()
...
def _current_cron_store() -> _CronStorePaths:
    ...
    get_mikasa_home(), so re-pointing MIKASA_HOME after import uses ITS OWN store rather than the
...
def _is_named_profile_path(path: Path) -> bool:
    """True if *path* is under ``<MIKASA_HOME>/profiles/<name>/`` (default/custom homes are not).
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_mikasa_home() / "config.yaml"
...
def _machine_id() -> str:
    ...
    explicit = os.getenv("mikasa_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if kind == "once":
        ...
        # same MIKASA_HOME must not re-dispatch it while in flight, and advancing next_run_at by a
