...
def _current_profile_name() -> Optional[str]:
    ...
    ``hermes_cli.profiles.get_active_profile_name`` (from ``MIKASA_HOME``); ``None`` if neither."""
    for env_name in ("mikasa_PROFILE_NAME", "mikasa_PROFILE"):
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<MIKASA_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_mikasa_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_mikasa_home() falls back to Path.home(), which raises when neither MIKASA_HOME nor
