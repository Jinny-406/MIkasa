...
# Per-profile by design (anchored on get_mikasa_home(), see cron/jobs.py). Optional test override;
# production resolves the path at CALL time so multiplexed profile ticks (set_mikasa_home_override)
...
# get_mikasa_home() (profile home), not the shared default root. Same pattern as cron/executions.py.
...
def _current_suggestions_file() -> Path:
    return SUGGESTIONS_FILE or (get_mikasa_home().resolve() / "cron" / "suggestions.json")
