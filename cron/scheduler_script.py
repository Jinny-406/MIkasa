...
def _get_script_timeout() -> int:
    ...
    resolved = _timeout_from_env_or_config(
        "mikasa_CRON_SCRIPT_TIMEOUT", "script_timeout_seconds", _positive_int,
        "cron script timeout",
    )
...
def _get_media_send_timeout() -> int:
    ...
    resolved = _timeout_from_env_or_config(
        "mikasa_CRON_MEDIA_SEND_TIMEOUT", "media_send_timeout_seconds", _positive_int,
...
def _get_session_db_timeout() -> float:
    ...
    resolved = _timeout_from_env_or_config(
        "mikasa_CRON_SESSION_DB_TIMEOUT", "session_db_timeout_seconds", float,
...
def _resolve_script_path(script_path: str) -> tuple[Optional[Path], Optional[str]]:
    ...
    inside MIKASA_HOME/scripts/ (relative, absolute and ``~`` paths are all validated — path
    traversal / absolute-path injection); contract of lifecycle_guard._expand_candidate_path."""
    scripts_dir = _sched._get_mikasa_home() / "scripts"
    ...
    # Traversal / absolute-path / symlink escape guard — MUST stay inside MIKASA_HOME/scripts/.
    try:
...
def _run_job_script(
    ...
    Args: script_path: Path to the script. Relative paths are resolved against MIKASA_HOME/scripts/.
