...
# Ensure sibling modules (_mikasa_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _mikasa_home import display_mikasa_home, get_mikasa_home

MIKASA_HOME = get_mikasa_home()
TOKEN_PATH = MIKASA_HOME / "google_token.json"
CLIENT_SECRET_PATH = MIKASA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = MIKASA_HOME / "google_oauth_pending.json"
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_mikasa_home()}/google_token.json")
