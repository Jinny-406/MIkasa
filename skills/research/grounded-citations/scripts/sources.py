...
Ledger path resolution (first wins):
  ...
  $MIKASA_HOME/cache/citations/ledger.json
"""
...
from _mikasa_home import get_mikasa_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("mikasa_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_mikasa_home() / "cache" / "citations" / "ledger.json"
