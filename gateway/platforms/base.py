...
def build_auto_tts_output_path(platform) -> str:
    ...
    audio_path = os.path.join(
        tempfile.gettempdir(), "mikasa_voice", f"tts_reply_{uuid.uuid4().hex[:12]}.{ext}")
...
def _config_section(name: str) -> dict:
    ...
    try:
        from mikasa_cli.config import load_config_readonly as _load_config
...
from mikasa_constants import get_default_mikasa_root, get_mikasa_dir, get_mikasa_home
...
IMAGE_CACHE_DIR = get_mikasa_dir("cache/images", "image_cache")
...
def _cache_dir_accessors(kind: str, constant_name: str, new_subpath: str, old_name: str):
    ...
    get_mikasa_dir (active profile) unless a test monkeypatched the module constant away from
    its import-time default, and creates the directory; ``cleanup(max_age_hours=24)`` deletes
    older files and returns the count."""
    def get_dir() -> Path:
        d = get_mikasa_dir(new_subpath, old_name)
...
AUDIO_CACHE_DIR = get_mikasa_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_mikasa_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_mikasa_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_mikasa_dir("cache/screenshots", "browser_screenshots")
...
_HERMES_ROOT = get_default_mikasa_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "mikasa_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "mikasa_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "mikasa_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "mikasa_MEDIA_DELIVERY_STRICT"
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    *(_mikasa_home / "cache" / d for d in _MEDIA_DELIVERY_CACHE_SUBDIRS))
...
# Credential stores at the MIKASA_HOME root, denied per-file so skills/, logs/ and agent-written
...
def _profile_cache_roots() -> List[Path]:
    ...
    roots cover only the active MIKASA_HOME). Enumerated at check time so profiles created after
    startup count and are allowlisted BEFORE the ``/root`` denylist (MIKASA_HOME symlinked).

    ``MIKASA_HOME=/opt/data``) while the model emits a profile-scoped path silently fails delivery.
    ...
    when MIKASA_HOME is symlinked under a denied prefix and $HOME is not that prefix). See issue #31733.
    """
    try:
...
def _kanban_root() -> Path:
    ...
    return Path(os.environ.get("mikasa_KANBAN_HOME", "").strip() or _HERMES_ROOT).expanduser()
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    override = os.environ.get("mikasa_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    return [*map(Path, _MEDIA_DELIVERY_DENIED_PREFIXES),
            ...
            *(r / rel for r in (_mikasa_home, _HERMES_ROOT) for rel in _ROOT_CREDENTIAL_PATHS),
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("mikasa_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower() or "interrupt")
        self._busy_text_debounce_seconds: float = _float_env("mikasa_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35)
        self._busy_text_hard_cap_seconds: float = _float_env("mikasa_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0)
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        """Acquire a scoped lock for this adapter; True on success. A live cross-MIKASA_HOME
    ...
    def _wire_plugin_handlers(self, native: Any = None) -> None:
        ...
        try:
            from mikasa_cli.plugins import get_plugin_manager
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        from mikasa_cli.commands import (is_interrupt_then_dispatch, should_bypass_active_session)
    ...
    def _get_human_delay() -> float:
        """Random human-like pacing delay (s) from mikasa_HUMAN_DELAY_MODE: "off" (default) |
        "natural" 800-2500ms | "custom" via HERMES_HUMAN_DELAY_MIN_MS /
        HERMES_HUMAN_DELAY_MAX_MS."""
        mode = os.getenv("mikasa_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        lo, hi = 800, 2500
        if mode != "natural":  # custom mode tolerates malformed env vars
            lo = _or_default(lambda: int(os.getenv("mikasa_HUMAN_DELAY_MIN_MS", str(lo))), lo)
            hi = _or_default(lambda: int(os.getenv("mikasa_HUMAN_DELAY_MAX_MS", str(hi))), hi)
    ...
    def build_source(
        ...
        profile is stamped on ``source.profile`` for per-profile MIKASA_HOME isolation."""
