...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Mikasa Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/mikasa) is mirrored to MIKASA_HOME so npm install works."""
    import shutil
    ...
    MIKASA_HOME_bridge = get_mikasa_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if MIKASA_HOME_bridge.exists():
        return MIKASA_HOME_bridge
    try:
        MIKASA_HOME_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, MIKASA_HOME_bridge, dirs_exist_ok=False)
        return MIKASA_HOME_bridge
    except Exception:
        return install_bridge
