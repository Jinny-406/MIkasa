...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    try:
        from mikasa_cli.profiles import profile_matches_home
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from mikasa_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("mikasa_MAX_TOKENS")
    if env_max_tokens:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            ...
            with suppress(Exception):
                ...
                db_path = str(get_mikasa_home() / "response_store.db")
        ...
        from mikasa_state_wal import apply_wal_with_fallback
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        from mikasa_cli.model_switch import resolve_effective_model
        profile_name = ""
        with suppress(Exception):
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from mikasa_cli.auth import has_usable_secret
        ...
        if not profile:
            with suppress(Exception):
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_mikasa_home())
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from mikasa_cli.profiles import get_profile_dir
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_mikasa_home()``
        per profile). Sync, for ``_create_agent``; handlers use ``_ensure_session_db_async``."""
        if self._session_db is not None:
            return self._session_db
        try:
            from mikasa_constants import get_mikasa_home
            return self._open_and_cache_session_db(get_mikasa_home())
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "mikasa_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "mikasa_browser" if text == "browser" else text
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "mikasa-agent", "version": _hermes_version()})
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from mikasa_cli.inventory import build_model_options_payload, load_picker_context
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<MIKASA_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            try:
                from mikasa_state import get_mikasa_home
                root = Path(get_mikasa_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the MIKASA_HOME
