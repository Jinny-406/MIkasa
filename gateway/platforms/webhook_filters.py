...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    ...
    if raw == "~/.hermes" or raw.startswith("~/.hermes/"):
        return MIKASA_HOME / raw[len("~/.hermes/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else MIKASA_HOME / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under MIKASA_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from mikasa_constants import get_mikasa_home
    scripts_root = (get_mikasa_home() / "scripts").resolve()
