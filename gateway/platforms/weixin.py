...
def _account_dir(MIKASA_HOME: str) -> Path:
    path = Path(MIKASA_HOME) / "weixin" / "accounts"
...
def save_weixin_account(MIKASA_HOME: str, *, account_id: str, token: str, base_url: str, user_id: str = "") -> None:
    path = _account_dir(MIKASA_HOME) / f"{account_id}.json"
...
def load_weixin_account(MIKASA_HOME: str, account_id: str) -> Optional[Dict[str, Any]]:
    return _read_json(_account_dir(MIKASA_HOME) / f"{account_id}.json")


class ContextTokenStore:
    """Disk-backed ``context_token`` cache keyed by account + peer."""

    def __init__(self, MIKASA_HOME: str):
        self._root = _account_dir(MIKASA_HOME)
...
def _load_sync_buf(MIKASA_HOME: str, account_id: str) -> str:
    data = _read_json(_account_dir(MIKASA_HOME) / f"{account_id}.sync.json")
    return data.get("get_updates_buf", "") if isinstance(data, dict) else ""


def _save_sync_buf(MIKASA_HOME: str, account_id: str, sync_buf: str) -> None:
    atomic_json_write(_account_dir(MIKASA_HOME) / f"{account_id}.sync.json", {"get_updates_buf": sync_buf})
...
async def qr_login(MIKASA_HOME: str, *, bot_type: str = "3", timeout_seconds: int = 480) -> Optional[Dict[str, str]]:
    if not AIOHTTP_AVAILABLE:
        ...
        while time.monotonic() < deadline:
            ...
            if status == "wait":
                ...
                if not creds["account_id"] or not creds["token"]:
                    ...
                    return None
                save_weixin_account(MIKASA_HOME, **creds)
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._mikasa_home = MIKASA_HOME = str(get_mikasa_home())
        self._token_store = ContextTokenStore(MIKASA_HOME)
        ...
        persisted = load_weixin_account(MIKASA_HOME, self._account_id) if self._account_id and not self._token else None
        if persisted:
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._mikasa_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if response.get("get_updates_buf"):
                    ...
                    _save_sync_buf(self._mikasa_home, self._account_id, sync_buf)
...
async def send_weixin_direct(
    *, extra: Dict[str, Any], token: Optional[str], chat_id: str, message: str, media_files: Optional[List[Tuple[str, bool]]] = None,
    ...
    token_store = ContextTokenStore(str(get_mikasa_home()))
