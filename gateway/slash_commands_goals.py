...
class GatewayGoalCommandsMixin:
    ...
    async def _handle_heartbeat_command(self, event: MessageEvent) -> str:
        ...
        from mikasa_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        args = (event.get_command_args() or "").strip()
        lower = args.lower()
    ...
    async def _handle_loop_command(self, event: MessageEvent) -> str:
        ...
        try:
            from mikasa_cli.loops import LoopManager, dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
