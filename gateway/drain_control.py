...
(or removes) ``{MIKASA_HOME}/.drain_request.json`` and a gateway watcher reacts;
an ACTIVE marker means ``gateway_state -> "draining"``.  Two lenient staleness
signals (either suffices): epoch mismatch (MIKASA_HOME is a durable volume on
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting MIKASA_HOME."""
    return Path(home if home is not None else get_mikasa_home()) / _DRAIN_REQUEST_FILENAME
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    survived a container/VM restart (MIKASA_HOME is a durable Fly volume on Hermes Cloud) and the lifecycle
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    machine restart on the durable MIKASA_HOME volume, or an expired same-epoch orphan (#85433), is ignored
