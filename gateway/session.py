...
def build_session_context_prompt(context: SessionContext, *, redact_pii: bool = False) -> str:
    ...
    lines.append(f"- `\"local\"` → Save to local files only ({display_mikasa_home()}/cron/output/)")
    for platform, home in context.home_channels.items():
...
class SessionStore(
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig, has_active_processes_fn=None):
        ...
        # redirected ``get_mikasa_home()`` for the turn (its docstring lists "sessions" among what it
        ...
        self._profile_home_cache: Dict[str, Optional[Path]] = {}  # profile -> MIKASA_HOME (hits)
        ...
        try:
            ...
            self._routing_home: Optional[Path] = Path(get_mikasa_home())
        except Exception:
