...
class GatewayInboundMixin:
    ...
    def _hm_estop_turn_allowed(self, event: "MessageEvent", source: SessionSource) -> bool:
        ...
        with suppress(Exception):
            ...
            if _estop_cmd:
                from mikasa_cli.commands import resolve_command as _resolve_estop_cmd
    ...
    def _hm_write_update_response(response_text: str) -> Optional[str]:
        ...
        response_path = _mikasa_home / ".update_response"
        try:
            tmp = response_path.with_suffix(".tmp")
            tmp.write_text(response_text, encoding="utf-8")
            tmp.replace(response_path)
            (_mikasa_home / ".update_prompt.json").unlink(missing_ok=True)
        except OSError as e:
            return str(e)
        return None
    ...
    def _hm_update_prompt_reply(self, event: "MessageEvent", _quick_key: str) -> Optional[str]:
        ...
        if cmd in {"approve", "yes"}:
            ...
            if cmd:
                with suppress(Exception):
                    from mikasa_cli.commands import resolve_command as _resolve_update_cmd
    ...
    def _hm_evict_idle_stale_agent(self, _quick_key: str) -> None:
        ...
        _raw_stale_timeout = _float_env("mikasa_AGENT_TIMEOUT", 1800)
    ...
    async def _hm_busy_slash_or_photo(
        ...
        from mikasa_cli.commands import resolve_command as _resolve_cmd_inner
    ...
    def _hm_busy_telegram_grace_queue(
        ...
        _grace = float(os.getenv("mikasa_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0"))
    ...
    async def _hm_resolve_command(
        ...
        from mikasa_cli.commands import is_gateway_known_command, resolve_command as _resolve_cmd
    ...
    async def _hm_dispatch_quick_and_plugin_commands(
        ...
        if command:
            try:
                from mikasa_cli.plugins import get_plugin_command_handler
    ...
    def _hm_unknown_slash_reply(command: str, source: SessionSource) -> Optional[str]:
        ...
        from mikasa_cli.commands import GATEWAY_KNOWN_COMMANDS
    ...
    async def _dispatch_plugin_message_injection(
        ...
        await adapter.handle_message(MessageEvent(
            ...
            metadata={
                "mikasa_plugin_id": plugin_id, "mikasa_plugin_injection": True,
