"""Gateway runtime status helpers: PID/lock/marker files under ``{MIKASA_HOME}`` (one set per
...
from mikasa_constants import _get_platform_default_mikasa_home, get_mikasa_home
...
def record_start_and_check_storm(
    ...
    try:
        path = get_mikasa_home() / "gateway-starts.log"
...
def _get_process_mikasa_home() -> Path:
    """Launch-home MIKASA_HOME for identity files (PID, lock, status, markers):
    ``get_mikasa_home()`` honors the per-session ``_mikasa_home_OVERRIDE`` and would misroute
    them."""
    val = os.environ.get("mikasa_HOME", "").strip()
    return Path(val) if val else _get_platform_default_mikasa_home()


def _canonical_mikasa_home(path: Path | str) -> Path:
    """Stable absolute MIKASA_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_mikasa_home(left: Path | str, right: Path | str) -> bool:
    """Compare MIKASA_HOME paths with the host platform's case semantics."""
    left_c = os.path.normcase(str(_canonical_mikasa_home(left)))
    return left_c == os.path.normcase(str(_canonical_mikasa_home(right)))


def recorded_gateway_home_conflicts(
    record: Optional[dict[str, Any]], *, expected_home: Optional[Path | str] = None
) -> bool:
    """True when a persisted gateway record names a DIFFERENT MIKASA_HOME (cross-profile kill guard:
    profile B's stop must never SIGTERM profile A). ``expected_home`` overrides the comparison base.
    Legacy records without ``MIKASA_HOME`` prove nothing -> False; a comparison failure fails
    closed -> True."""
    recorded_home = record.get("mikasa_home") if isinstance(record, dict) else None
    if not isinstance(recorded_home, str) or not recorded_home.strip():
        return False
    try:
        base = expected_home if expected_home is not None else _get_process_mikasa_home()
        return not _same_mikasa_home(recorded_home, base)
    except Exception:
        return True


# Mirrors hermes_cli.profiles._PROFILE_ID_RE -- duplicated so gateway identity code
# stays import-light (mikasa_constants + stdlib only).
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    try:
        canonical = _canonical_mikasa_home(home)
    except Exception:
        return None
    if canonical.parent.name == "profiles" and _PROFILE_LABEL_RE.match(canonical.name):
        return canonical.name
    import mikasa_constants
    default_homes = (mikasa_constants.get_default_mikasa_root, _get_platform_default_mikasa_home)
    for default_home in default_homes:
        with contextlib.suppress(Exception):
            if _same_mikasa_home(canonical, default_home()):
                return "default"
    return None
...
def _get_runtime_status_path() -> Path:
    return _get_process_mikasa_home() / _RUNTIME_STATUS_FILE


def _get_lock_dir() -> Path:
    """Machine-local dir for token-scoped gateway locks; ``HERMES_GATEWAY_LOCK_DIR`` overrides."""
    override = os.getenv("mikasa_GATEWAY_LOCK_DIR")
    if override:
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    if any(b in ("hermes-gateway", "hermes-gateway.exe") for b in basenames):
        return "run"
    joined = " ".join(tokens)
    if "mikasa_cli.main" not in joined and "mikasa_cli/main.py" not in joined and not any(
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    ``MIKASA_HOME=`` on argv; the default gateway runs bare. Separators normalized."""
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return any(needle in command_lc for needle in (
            f"--profile {profile_lc}", f"-p {profile_lc}", f"mikasa_home={home_lc}"
        ))
    # Default profile: accept unless argv names another profile or a conflicting explicit
    # MIKASA_HOME= (its absence is not disqualifying -- MIKASA_HOME usually arrives via the env).
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    return not ("mikasa_home=" in command_lc and f"mikasa_home={home_lc}" not in command_lc)
...
def _build_pid_record() -> dict:
    return {
        ...
        "mikasa_home": str(_canonical_mikasa_home(_get_process_mikasa_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    """
    try:
        from mikasa_cli.build_info import get_code_identity
...
def _pid_record_belongs_to_current_profile(record: Optional[dict[str, Any]]) -> bool:
    """True when the record's ``MIKASA_HOME`` matches the current process (legacy records: True);
    another MIKASA_HOME's record must be ignored or the default gateway assumes its identity."""
    if not isinstance(record, dict):
        return False
    record_home = record.get("mikasa_home")
    return not record_home or _same_mikasa_home(record_home, _get_process_mikasa_home())
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # Active-profile context: the record's MIKASA_HOME must match this process so a stale record
...
def acquire_scoped_lock(
    ...
    # does not say WHICH profile). Omitted when not inferable; readers fall back to MIKASA_HOME.
    profile = _profile_label_for_home(_get_process_mikasa_home())
    if profile:
...
def _get_takeover_marker_path(MIKASA_HOME: Optional[Path] = None) -> Path:
    """Takeover marker path; ``MIKASA_HOME`` is given only for a verified cross-home handoff."""
    home = _canonical_mikasa_home(MIKASA_HOME or _get_process_mikasa_home())
    return home / _TAKEOVER_MARKER_FILENAME


def _get_planned_stop_marker_path() -> Path:
    return _get_process_mikasa_home() / _PLANNED_STOP_MARKER_FILENAME
...
def _consume_pid_marker_for_self(path: Path, *, ttl_s: int) -> bool:
    ...
    # cross-MIKASA_HOME --replace while ignoring a marker accidentally written into another
    ...
    target_home = record.get("target_mikasa_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_mikasa_home(target_home, our_home):
            return False
    else:
        replacer_home = record.get("replacer_mikasa_home")
        if replacer_home is not None and not _same_mikasa_home(replacer_home, our_home):
            return False
...
def write_takeover_marker(
    ...
    try:
        marker_home = _canonical_mikasa_home(target_home or _get_process_mikasa_home())
        ...
        return _write_marker(_get_takeover_marker_path(marker_home), {
            ...
            "target_mikasa_home": str(marker_home), "replacer_pid": os.getpid(),
            "replacer_mikasa_home": str(_canonical_mikasa_home(_get_process_mikasa_home())),
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    raw_home = record.get("mikasa_home")
    if (
        ...
        return None
    target_home = _canonical_mikasa_home(raw_home)
    ...
    pid_record_home = pid_record.get("mikasa_home")
    if (
        ...
        or not _same_mikasa_home(pid_record_home, target_home)
    ):
        return None
