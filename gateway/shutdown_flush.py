...
All hooks write atomic JSON payloads under ``<MIKASA_HOME>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active MIKASA_HOME."""
    from mikasa_constants import get_mikasa_home
    flush_dir = get_mikasa_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<MIKASA_HOME>/pending_messages/``), so a runtime cap rotation no longer silently
