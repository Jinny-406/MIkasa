...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``hermes update --gateway`` and its watcher exchange under MIKASA_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_mikasa_home / ".update_pending.json",
            claimed=_mikasa_home / ".update_pending.claimed.json", output=_mikasa_home / ".update_output.txt",
            exit_code=_mikasa_home / ".update_exit_code",
            prompt=_mikasa_home / ".update_prompt.json", response=_mikasa_home / ".update_response",
        )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _mikasa_home, _non_conversational_metadata
        notify_path = _mikasa_home / ".restart_notify.json"
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if not error:
            return
        from mikasa_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        if classify_persistence_error(error) == "corrupt":
            # Copy-pasteable, so name the real store (profiles / MIKASA_HOME do not live under ~/.hermes).
