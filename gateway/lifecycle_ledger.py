...
``<MIKASA_HOME>/state/gateway.lifecycle.json`` closes the gap:
...
def _process_mikasa_home() -> Path:
    """mikasa_HOME for process-level identity files (ignore task overrides)."""
...
def _home_path(home: Optional[Path], *relative: str) -> Path:
    return (_process_mikasa_home() if home is None else home).joinpath(*relative)


def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<MIKASA_HOME>/state/gateway.lifecycle.json``."""
