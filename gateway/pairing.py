...
# path would freeze the MIKASA_HOME/profile context at gateway boot, ignoring later
...
# computing the path eagerly freezes it to whatever MIKASA_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# mikasa_constants.set_mikasa_home_override) is established afterward. A freshly-started, short-lived
...
def _default_pairing_dir() -> Path:
    return PAIRING_DIR if PAIRING_DIR is not None else get_mikasa_dir("platforms/pairing", "pairing")
...
def _write_allowlist_env(env_var: str, ids: list) -> None:
    ...
    with contextlib.suppress(Exception):
        from mikasa_cli.config import save_env_value, remove_env_value
...
class PairingStore:
    ...
    profile's MIKASA_HOME exactly as ``hermes -p <name> pairing ...`` does, so multiplex
    ...
    def __init__(self, profile: Optional[str] = None):
        profile_home = None
        if profile:
            root = get_default_mikasa_root()
            profile_home = root if profile == "default" else root / "profiles" / profile
        self._dir = get_mikasa_dir("platforms/pairing", "pairing", home=profile_home) if profile else _default_pairing_dir()
