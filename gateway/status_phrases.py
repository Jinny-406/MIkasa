...
``gateway/assets/status_phrases.yaml``; users add profile-relative catalogs under ``MIKASA_HOME``
via ``status_phrases.yaml`` / ``status_phrases/*.yaml`` or ``display.status_phrases: {path:
<MIKASA_HOME-relative>, mode: append|replace}``. Absolute paths and ``..`` escapes are ignored on
...
def resolve_status_phrase_catalog(user_config: Mapping[str, Any] | None,
    ...
    _merge_phrase_paths(catalog, list(_CONVENTIONAL_RELATIVE_PATHS), base_dir=MIKASA_HOME)
    ...
    for section in sections:
        for key in _CONFIG_KEYS:
            _merge_phrase_config(catalog, section.get(key), base_dir=MIKASA_HOME)
    return catalog
