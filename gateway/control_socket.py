...
boundary. POSIX: ``$MIKASA_HOME/gateway.sock`` (or a temp-dir socket + ``gateway.sock.path`` pointer
...
def windows_pipe_name(home: Path) -> str:
    """Per-MIKASA_HOME named pipe path (Windows transport)."""
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (env.get("XPC_SERVICE_NAME", "").startswith("ai.hermes")
        ...
        return "launchd"
    if env.get("mikasa_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        **{k: record.get(k) for k in ("kind", "pid", "start_time", "mikasa_home")},
        "profile": _profile_label_for_home(record.get("mikasa_home") or ""),
