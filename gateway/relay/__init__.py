...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from mikasa_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Mikasa Agent:**" and shadow the connector's
        # linked-owner fallback, which actually disambiguates. Only a customized
        # name is forwarded.
        if value == "Mikasa Agent":
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    if not token_url:
        from mikasa_cli.auth import resolve_nous_access_token
