...
class GatewayAdapterLifecycleMixin:
    ...
    def _adapter_disconnect_timeout_secs(self) -> float:
        ...
        override = self._env_timeout_override("mikasa_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT")
    ...
    def _platform_connect_timeout_secs(self, platform=None, *, initial: bool = False) -> float:
        ...
        override = self._env_timeout_override("mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT")
    ...
    async def _handoff_watcher(self, interval: float = 2.0, drain_timeout: float = 30.0) -> None:
        ...
        async def _tick(profile_name: Optional[str] = None) -> None:
            ...
            for row in pending:
                ...
                # ``set_mikasa_home_override`` and ``set_secret_scope`` are ContextVar-based — ensure_future
    ...
    async def _start_secondary_profile_adapters(self) -> int:
        ...
        Each profile connects under its own MIKASA_HOME + secret scope; credential/listener collisions
    ...
    def _make_default_profile_message_handler(self):
        ...
        from gateway.run import _async_profile_runtime_scope, get_mikasa_home
        default_home = Path(get_mikasa_home())
    ...
    async def _handle_gateway_platform_event(self, event: dict, source) -> None:
        ...
        with _log_suppressed(logging.DEBUG, "gateway_platform_event hook dispatch failed", exc_info=True):
            from mikasa_cli.lifecycle import has_hook, invoke_hook
    ...
    def _make_default_profile_platform_event_handler(self):
        ...
        from gateway.run import _profile_runtime_scope, get_mikasa_home
        default_home = Path(get_mikasa_home())
    ...
    def _make_adapter_auth_check(
        ...
        """
        from gateway.run import get_mikasa_home
        transport_home = Path(get_mikasa_home()) if self._multiplex_on() and profile_name is None else None
