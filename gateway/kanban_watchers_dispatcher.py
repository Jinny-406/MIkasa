...
def _kbc():
    from mikasa_cli import kanban_db_connect
    return kanban_db_connect


def _kbd():
    from mikasa_cli import kanban_db_dispatch
...
class _KanbanDispatcher:
    ...
    def auto_decompose_tick(self, auto_decompose_per_tick: int) -> int:
        ...
        """
        try:
            from mikasa_cli import kanban_decompose as _decomp
        ...
        for slug in self._board_slugs():
            ...
            prev_env = os.environ.get("mikasa_KANBAN_BOARD")
            try:
                os.environ["mikasa_KANBAN_BOARD"] = slug
                try:
                ...
                if prev_env is None:
                    ...
                    os.environ["mikasa_KANBAN_BOARD"] = prev_env
        return successes
