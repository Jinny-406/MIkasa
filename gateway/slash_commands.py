...
from mikasa_cli.config import atomic_config_write, cfg_get
    ...
    if sys.platform == "win32":
        ...
        subprocess.Popen(
            [sys.executable, "-c", _WINDOWS_UPDATE_HELPER, str(output_path), str(exit_code_path),
             sys.executable, "-m", "mikasa_cli.main", "update", "--gateway"],
...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _mikasa_home
        ...
        async def _write_marker(name: str, build, label: str) -> None:
            try:
                await asyncio.to_thread(atomic_json_write, _mikasa_home / name, build(), indent=None)
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Mikasa Agent version."""
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_mikasa_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        from mikasa_cli.debug import (_GATEWAY_PRIVACY_NOTICE, _best_effort_sweep_expired_pastes,
        ...
        # ``get_mikasa_home()`` and uploads them to a public paste. Losing the contextvar override
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        import json
        from gateway.run import _mikasa_home, _resolve_hermes_bin
        ...
        if src.platform not in self._UPDATE_ALLOWED_PLATFORMS:
            try:
                ...
                return t("gateway.update.platform_not_messaging")
        if is_managed():
            return f"✗ {format_managed_message('update Mikasa Agent')}"
        if not (Path(__file__).parent.parent.resolve() / '.git').exists():
            return t("gateway.update.not_git_repo")
        hermes_cmd = _resolve_hermes_bin()
        if not hermes_cmd:
            return t("gateway.update.hermes_cmd_not_found")
        pending_path = _mikasa_home / ".update_pending.json"
        output_path = _mikasa_home / ".update_output.txt"
        exit_code_path = _mikasa_home / ".update_exit_code"
        pending = {
