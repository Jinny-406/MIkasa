...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    from mikasa_cli.install_identity import _fsync_directory
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local MIKASA_HOME override; the process environment
        # retains the installation root and is the authority here.
        root = os.environ.get("mikasa_HOME") or get_mikasa_home()
...
def catalog_mapping(
    *, installation_id: str, protocol_versions: Iterable[int] = (PROTOCOL_VERSION,),
    ...
    persistent_process = bool(persistent_process and os.getenv("mikasa_DESKTOP") != "1")
    profile = str(target_profile or "").strip() or (os.getenv("mikasa_PROFILE") or "default").strip() or "default"
...
def _configured_room_link_url() -> str | None:
    ...
    if (override := os.getenv("mikasa_ROOM_LINK_URL")) is not None:
        return override
