...
``<MIKASA_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _process_mikasa_home() -> Path:
    """mikasa_HOME for process-level identity files (ignore profile overrides)."""
    val = os.environ.get("mikasa_HOME", "").strip()
    return Path(val) if val else get_mikasa_home()


def _home(home: Optional[Path]) -> Path:
    return home if home is not None else _process_mikasa_home()
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<MIKASA_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
