...
class TurnRunner:
    ...
    def _progress_onboarding_hint(self, kwargs: dict) -> None:
        ...
        from gateway.run import _mikasa_home, _load_gateway_config
        ctx = self._ctx
        try:
            if (kwargs.get("duration") or 0) >= ctx._LONG_TOOL_THRESHOLD_S and ctx.progress_mode == "all":
                ...
                if gate_on and not is_seen(cfg, TOOL_PROGRESS_FLAG):
                    ...
                    mark_seen(_mikasa_home / "config.yaml", TOOL_PROGRESS_FLAG)
        except Exception as err:
    ...
    def run_sync(self):
        ...
        — never os.environ["mikasa_SESSION_KEY"], which would misroute approvals across sessions.
        """
        ...
        # tool worker threads. We deliberately do NOT write os.environ["mikasa_SESSION_KEY"] here:
