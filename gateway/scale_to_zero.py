...
SCALE_TO_ZERO_ENV = "mikasa_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(MIKASA_HOME: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under MIKASA_HOME."""
    if MIKASA_HOME is None:
        from mikasa_constants import get_mikasa_home
        MIKASA_HOME = get_mikasa_home()
    return Path(MIKASA_HOME) / DASHBOARD_CLIENT_HEARTBEAT_REL
