...
from mikasa_cli.config import _is_ssh_remote_tilde_cwd, cfg_get
...
async def run_codex_hygiene_compaction(
    ...
    * Evicting the cached live agent afterwards destroys the only real context: the next turn spawns an
    ...
    # copy_context carries profile secret scope / MIKASA_HOME override (executors don't propagate ContextVars).
...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from mikasa_cli.commands_platforms import _sanitize_telegram_name
...
def _startup_restore_drain_timeout_secs() -> float:
    ...
    """
    return _float_env("mikasa_STARTUP_RESTORE_DRAIN_TIMEOUT", _STARTUP_RESTORE_DRAIN_TIMEOUT_SECS_DEFAULT)
...
def _startup_warmup_timeout_secs() -> float:
    ...
    return _float_env("mikasa_STARTUP_WARMUP_TIMEOUT", _STARTUP_WARMUP_TIMEOUT_SECS_DEFAULT)
...
def _build_gateway_agent_history(
    ...
    from mikasa_time import get_timezone as _get_msg_tz
...
def _restart_notification_pending() -> bool:
    ...
    return (_mikasa_home / ".restart_notify.json").exists()


def _planned_restart_notification_path() -> Path:
    return _mikasa_home / ".restart_pending.json"
...
from mikasa_constants import get_mikasa_home, get_mikasa_home_override
...
_env_path = _mikasa_home / '.env'
load_mikasa_dotenv(MIKASA_HOME=_mikasa_home, project_env=Path(__file__).resolve().parents[1] / '.env')
...
def _reload_runtime_env_preserving_config_authority() -> None:
    ...
    if not is_multiplex_active():
        load_mikasa_dotenv(
            MIKASA_HOME=_mikasa_home, project_env=Path(__file__).resolve().parents[1] / '.env')
    _bridge_max_turns_from_config(_mikasa_home)
...
def _current_max_iterations() -> int:
    ...
    return _resolve_turn_limit(os.getenv("mikasa_MAX_ITERATIONS"))
...
def _load_profile_secret_scope(profile_home: "Path") -> dict:
    ...
    from mikasa_constants import set_mikasa_home_override, reset_mikasa_home_override
    # Caller already hydrated external sources off-loop (#99519).
    from agent.secret_scope import build_profile_secret_scope
    from mikasa_cli.env_loader import hydrate_profile_secret_sources

    home_token = set_mikasa_home_override(str(profile_home))
    try:
        hydrate_profile_secret_sources(Path(profile_home))
        return build_profile_secret_scope(Path(profile_home))
    finally:
        reset_mikasa_home_override(home_token)
...
def _profile_runtime_scope(
    ...
    ``set_mikasa_home_override`` is a contextvar (reaches the agent worker via ``copy_context()``);
...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    ``mcp_servers`` from ``get_mikasa_home()``'s config, so an unscoped call only connects the launch
...
from mikasa_cli.config_defaults import DEFAULT_CONFIG as _DEFAULT_CONFIG
os.environ["mikasa_TURN_LEASE_TIMEOUT"] = str(_DEFAULT_CONFIG["agent"]["gateway_turn_lease_timeout"])
...
_AGENT_ENV_BRIDGE = {
    "gateway_timeout": "mikasa_AGENT_TIMEOUT",
    "gateway_turn_lease_timeout": "mikasa_TURN_LEASE_TIMEOUT",
    "gateway_timeout_warning": "mikasa_AGENT_TIMEOUT_WARNING",
    "gateway_notify_interval": "mikasa_AGENT_NOTIFY_INTERVAL",
    "session_stall_timeout": "mikasa_SESSION_STALL_TIMEOUT",
    # Internal bridge only — config.yaml (agent.reconnect_attention_after) is the documented setting.
    "reconnect_attention_after": "mikasa_RECONNECT_ATTENTION_AFTER_SECONDS",
    "restart_drain_timeout": "mikasa_RESTART_DRAIN_TIMEOUT",
    "cron_drain_timeout": "mikasa_CRON_DRAIN_TIMEOUT",
    "gateway_auto_continue_freshness": "mikasa_AUTO_CONTINUE_FRESHNESS",
    "gateway_startup_restore_drain_timeout": "mikasa_STARTUP_RESTORE_DRAIN_TIMEOUT",
    "gateway_startup_warmup_timeout": "mikasa_STARTUP_WARMUP_TIMEOUT"}
# config-authoritative knobs for the session-search index (env stays the cross-process carrier).
_SESSIONS_ENV_BRIDGE = {"cjk_fts": "mikasa_CJK_FTS", "search_slow_ms": "mikasa_SEARCH_SLOW_MS"}
_DISPLAY_ENV_BRIDGE = {
    "busy_input_mode": "mikasa_GATEWAY_BUSY_INPUT_MODE",
    "busy_text_mode": "mikasa_GATEWAY_BUSY_TEXT_MODE",
    "busy_ack_enabled": "mikasa_GATEWAY_BUSY_ACK_ENABLED"}
...
def _bridge_max_turns_to_env(agent_cfg: Any) -> None:
    ...
    if raw is not None:
        os.environ["mikasa_MAX_ITERATIONS"] = str(raw)
    elif "mikasa_MAX_ITERATIONS" in os.environ:
        del os.environ["mikasa_MAX_ITERATIONS"]
...
def _bridge_auxiliary_config_to_env(_auxiliary_cfg: dict) -> None:
    ...
    try:
        from mikasa_cli.plugins import get_plugin_auxiliary_tasks
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    if (isinstance(_display_cfg, dict) and "busy_steer_ack_enabled" in _display_cfg
            and "mikasa_GATEWAY_BUSY_STEER_ACK_ENABLED" not in os.environ):
        os.environ["mikasa_GATEWAY_BUSY_STEER_ACK_ENABLED"] = str(_display_cfg["busy_steer_ack_enabled"])
    _tz_cfg = _cfg.get("timezone", "")
    if _tz_cfg and isinstance(_tz_cfg, str):
        os.environ["mikasa_TIMEZONE"] = _tz_cfg.strip()
    _security_cfg = _cfg.get("security", {})
    if isinstance(_security_cfg, dict) and _security_cfg.get("redact_secrets") is not None:
        os.environ["mikasa_REDACT_SECRETS"] = str(_security_cfg["redact_secrets"]).lower()
    ...
    if isinstance(_gateway_cfg, dict):
        ...
        if _trust_recent_seconds is not None:
            os.environ["mikasa_MEDIA_TRUST_RECENT_SECONDS"] = str(_trust_recent_seconds)
        ...
        if ("platform_connect_timeout" in _gateway_cfg
                and not os.environ.get("mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()):
            os.environ["mikasa_GATEWAY_PLATFORM_CONNECT_TIMEOUT"] = str(_gateway_cfg["platform_connect_timeout"])
...
def _load_bridge_config(config_path: Path) -> dict:
    ...
    from mikasa_cli.config import _expand_env_vars, read_user_config_raw
...
_config_path = _mikasa_home / 'config.yaml'
_cfg: dict = {}
...
try:
    from mikasa_constants import apply_ipv4_preference
...
try:
    from mikasa_cli.config import print_config_warnings
    print_config_warnings()
except Exception as _bootstrap_exc:
    print(f"  Warning: config validation failed: {_bootstrap_exc}", file=sys.stderr)

try:
    from mikasa_cli.config import warn_deprecated_cwd_env_vars
    warn_deprecated_cwd_env_vars()
except Exception as _bootstrap_exc:
    print(f"  Warning: deprecation check failed: {_bootstrap_exc}", file=sys.stderr)

os.environ["mikasa_QUIET"] = "1"  # gateway runs quiet: no debug output, cwd used directly
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from mikasa_cli.runtime_provider import (
        resolve_runtime_provider, format_runtime_provider_error, _get_model_config)
    from mikasa_cli.auth import AuthError, is_rate_limited_auth_error
...
def _resolve_gateway_model_context(model: Optional[str] = None) -> _GatewayModelContext:
    ...
    def _read_config() -> None:
        ...
        try:
            from mikasa_cli.config import get_compatible_custom_providers
    ...
    def _pin_still_applies() -> bool:
        ...
        from mikasa_cli.route_identity import should_clear_context_pin
    ...
    def _custom_ctx() -> Optional[int]:
        from mikasa_cli.config import get_custom_provider_context_length
...
def _resolve_runtime_agent_kwargs_for_provider(provider: str) -> dict:
    ...
    from mikasa_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error
    try:
...
def _deep_merge_request_overrides(base: Optional[dict], override: Optional[dict]) -> dict:
    ...
    from mikasa_cli.config import _deep_merge
...
def _try_resolve_fallback_provider() -> dict | None:
    ...
    from mikasa_cli.runtime_provider import resolve_runtime_provider
    try:
        ...
        for entry in fb_list:
            try:
                from mikasa_cli.fallback_config import resolve_entry_api_key
                runtime = resolve_runtime_provider(
...
def _check_unavailable_skill(command_name: str) -> str | None:
    ...
    try:
        ...
        from mikasa_constants import get_optional_skills_dir
...
def _gateway_config_home() -> Path:
    ...
    override = get_mikasa_home_override()
    return Path(override) if override else _mikasa_home
...
def _load_gateway_config(config_path: "Path | None" = None) -> dict:
    ...
    Defaults to the active gateway home (``_mikasa_home`` monkeypatches apply); multiplexers pass a path.
    """
    ...
    try:
        from mikasa_cli.config import get_config_path, read_raw_config
...
def _resolve_hermes_bin() -> Optional[list[str]]:
    """Hermes update command argv: ``hermes`` on PATH, else ``python -m mikasa_cli.main``, else None."""
    import shutil
    ...
    try:
        ...
        if importlib.util.find_spec("mikasa_cli") is not None:
            return [sys.executable, "-m", "mikasa_cli.main"]
    except Exception:
        pass
    return None
...
_RECONNECT_ATTENTION_AFTER_SECONDS = _float_env("mikasa_RECONNECT_ATTENTION_AFTER_SECONDS", 7200)
...
class GatewayRunner(
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            # WARNING (not DEBUG) so it lands in errors.log; else an NFS MIKASA_HOME silently loses /resume etc.
    ...
    def _open_session_db_for_active_scope(self, raise_on_error: bool = False) -> Any:
        ...
        ``SessionDB()`` reads the context-local MIKASA_HOME; one handle cached per path. Construction
        ...
        resolves ``_default_db_path()`` at call time through the context-local MIKASA_HOME override
        ...
        #88235 broadcast.
        """
        from mikasa_state import AsyncSessionDB, _default_db_path
        from mikasa_state_registry import acquire
        from gateway.session_db_recovery import RecoverableHandleCache
    ...
    def close_all_session_db_handles(self) -> None:
        ...
        def _close(db) -> None:
            ...
            # instead (#90837).
            from mikasa_state_registry import release_or_close
            try:
    ...
    _VOICE_MODE_PATH = _mikasa_home / "gateway_voice_mode.json"
    ...
    def _thread_metadata_for_source(
        ...
        if profile and metadata is not None:
            ...
            metadata["mikasa_profile"] = profile
        return metadata
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's MIKASA_HOME serves this source: ``source.profile``, then
        ``_profile_name_for_source`` (sources bypassing ``build_source``), then the active profile."""
        from gateway.profile_routing import ProfileRouteRejected
        from mikasa_cli.profiles import get_active_profile_name, get_profile_dir, profile_exists
        ...
        try:
            ...
            logger.warning(
                ...
                "falling back to global MIKASA_HOME: %s",
...
def _housekeeping_paste_sweep() -> None:
    from mikasa_cli.debug import _sweep_expired_pastes
...
def _housekeeping_auto_archive() -> None:
    ...
    from mikasa_cli.config import load_config as _load_full_config
    from mikasa_state_registry import acquire, release_or_close
    _sess_cfg = (_load_full_config().get("sessions") or {})
    if _sess_cfg.get("auto_archive", False):
        _adb = acquire()
        try:
...
def _housekeeping_deferred_fts_retry() -> None:
    ...
    from mikasa_state_registry import borrow_live_shared_session_dbs
...
def _housekeeping_memory_trim() -> None:
    ...
    from mikasa_cli.mem_trim import trim_memory
...
def _drain_restart_safe_cron_deliveries(adapters, loop, runner=None) -> None:
    ...
    for profile_name, profile_home in _handoff_watch_scopes(runner):
        ...
        if profile_adapters is None:
            continue
        with _profile_runtime_scope(profile_home or get_mikasa_home()):
...
def _replace_target_belongs_to_other_profile(existing_pid: int) -> bool:
    ...
    by exact PID + start-time; live argv can never PROVE ownership (no MIKASA_HOME), it is only a
    ...
    try:
        from gateway.status import (
            _get_pid_path, _get_process_mikasa_home, _get_process_start_time, _pid_from_record,
            _read_pid_record, _record_looks_like_gateway, _read_process_cmdline, _same_mikasa_home)
        our_home = _get_process_mikasa_home()
        ...
        if _get_process_start_time(existing_pid) != recorded_start:
            return refuse("pid record start-time does not match the live process %s (stale/PID-reuse record).",
                          existing_pid)
        recorded_home = record.get("mikasa_home")
        if not isinstance(recorded_home, str) or not recorded_home.strip():
            return refuse("pid record predates MIKASA_HOME stampings; ownership of PID %s unprovable.",
                          existing_pid)
        if not _same_mikasa_home(recorded_home, our_home):
            return refuse("pid record belongs to a different MIKASA_HOME (%s, ours %s). Remove the stale PID "
                          "record or stop the owning profile explicitly.", recorded_home, our_home,
                          level=logging.ERROR)
        # Argv never proves ownership; an explicit contradicting --profile / MIKASA_HOME= still refuses.
        ...
        if live_cmdline and _looks_like_profile_conflict_from_cmdline(live_cmdline, our_home):
            return refuse("target PID %s command line explicitly advertises a different profile than "
                          "mikasa_HOME %s.", existing_pid, our_home, level=logging.ERROR)
        return False
    except Exception:
...
def _looks_like_profile_conflict_from_cmdline(command: str, our_home) -> bool:
    ...
    def _env_home_value() -> Optional[str]:
        """mikasa_HOME=<path> env-style assignment on the argv, token-exact."""
        prefix = "mikasa_HOME="
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    """Handle a live gateway PID under this MIKASA_HOME: replace it (``--replace``) or refuse.
    ...
    if not replace:
        MIKASA_HOME = str(get_mikasa_home())
        logger.error(
            "Another gateway instance is already running (PID %d, MIKASA_HOME=%s). "
            "Use 'hermes gateway restart' to replace it, or 'hermes gateway stop' first.",
            existing_pid, MIKASA_HOME)
        print(
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        from gateway.status import _get_process_mikasa_home
        logger.error(
            "Refusing --replace: PID %d cannot be proven to belong "
            "to this profile's gateway (MIKASA_HOME %s). Remove the "
            "stale PID record or stop the owning profile explicitly.",
            existing_pid, _get_process_mikasa_home())
        return False
    ...
    with suppress(Exception):
        (get_mikasa_home() / "gateway.pid").unlink(missing_ok=True)
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    from mikasa_logging import setup_logging, _safe_stderr
    setup_logging(MIKASA_HOME=_mikasa_home, mode="gateway")

    def _security_audit() -> None:
        # Warn-on-load, never blocks: surfaces root / weak-SSH / unauthenticated-listener exposure.
        from mikasa_cli.security_audit_startup import log_startup_security_warnings

        def _raw_cfg():
            from mikasa_cli.config import read_raw_config
            return read_raw_config()

        log_startup_security_warnings(MIKASA_HOME=_mikasa_home, config=_best_effort(_raw_cfg))
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _mikasa_home / "logs" / "gateway-shutdown-diag.log", _shutdown_ctx["signal"], timeout_seconds=5.0)
...
async def _start_gateway_start_control_socket(runner):
    ...
    try:
        ...
        # becomes the authoritative gateway for its MIKASA_HOME, so from here on "does a socket answer?" is
        ...
        def _pause_for_update_handler() -> dict:
            try:
                from mikasa_cli.gateway import _get_restart_drain_timeout
...
async def _start_gateway_shutdown_tail(
    ...
    def _stop_keepalive() -> None:
        from mikasa_cli.nous_auth_keepalive import stop_nous_auth_keepalive
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duplicate-instance guard scoped to MIKASA_HOME; distinct-home multi-profile setups coexist.
...
def _guard_corrupt_user_config() -> None:
    ...
    from mikasa_cli.config import InvalidUserConfigError, require_parseable_user_config
...
def _exit_after_graceful_shutdown(exit_code: int) -> None:
    ...
    def _drain_logs() -> None:
        ...
        from mikasa_logging import drain_log_queue
