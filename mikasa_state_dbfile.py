"""state.db file-level health helpers, split out of ``mikasa_state.py``.
...
def _read_sqlite_application_id(db_path: Path) -> "Optional[int]":
    ...
    from mikasa_state_errors import _STATE_DB_APPLICATION_ID_OFFSET
...
def refuse_deleted_wal_generation(db_path) -> None:
    ...
    *before* ``sqlite3.connect`` so a second opener cannot mint a replacement WAL inode."""
    from mikasa_state import DeletedWalGenerationError
    from mikasa_state_errors import _DELETED_WAL_GENERATION_MSG
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    try:
        from mikasa_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug("mikasa_cli.sqlite_safe_read unavailable; opening %s untracked "
...
def collect_state_db_stats(db_path: Path) -> Dict[str, Any]:
    ...
    from mikasa_state import _connect_tracked_db
