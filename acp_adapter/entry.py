...
def _load_env() -> None:
    """Load .env from mikasa_HOME (default ``~/.hermes``)."""
    from mikasa_cli.env_loader import load_mikasa_dotenv

    MIKASA_HOME = get_mikasa_home()
    loaded = load_mikasa_dotenv(MIKASA_HOME=MIKASA_HOME)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", MIKASA_HOME / ".env")
...
def _run_setup() -> None:
    from mikasa_cli.main import main as hermes_main
...
def main(argv: list[str] | None = None) -> None:
    ...
    if os.environ.get("mikasa_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
