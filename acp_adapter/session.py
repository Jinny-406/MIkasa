...
from mikasa_constants import get_mikasa_home, translate_cwd_for_wsl_backend, windows_path_to_wsl
...
class SessionManager:
    ...
    def _get_db(self):
        ...
        minimal test env). ``MIKASA_HOME`` is resolved here, not via the import-time
        ``DEFAULT_DB_PATH``, so test fixtures that change the env var later are honoured."""
        if self._db_instance is None:
            try:
                from mikasa_state import SessionDB
                self._db_instance = SessionDB(db_path=get_mikasa_home() / "state.db")
            except Exception:
