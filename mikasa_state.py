#!/usr/bin/env python3
"""SQLite state store for Mikasa Agent: session metadata, message history, model
...
from mikasa_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
from mikasa_state_errors import (
...
from mikasa_state_guard import (
...
from mikasa_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from mikasa_state_sessions import SessionSessionsMixin
from mikasa_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from mikasa_state_portability import SessionPortabilityMixin
from mikasa_state_telegram import SessionTelegramTopicsMixin
from mikasa_state_schema import SessionSchemaMixin
import mikasa_state_holders as _state_holders
from mikasa_state_dbfile import (
...
from mikasa_state_messages import SessionMessagesMixin
from mikasa_state_wal import _WAL_INCOMPAT_MARKERS, apply_database_pragmas, apply_wal_with_fallback
from mikasa_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from mikasa_state_titles import SessionTitlesMixin
from mikasa_state_usage import SessionUsageMixin
from mikasa_state_maintenance import SessionMaintenanceMixin
from mikasa_state_gateway import SessionGatewayMixin
from mikasa_state_compression import SessionCompressionMixin
from mikasa_state_search import SessionSearchMixin
...
def _configured_transcript_limit(key: str, fallback: int = _MAX_SAFE_MESSAGES) -> int:
    ...
    try:
        from mikasa_cli.config import load_config_readonly
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_mikasa_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_mikasa_home()`` is resolved fresh (a runtime MIKASA_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_mikasa_home() / "state.db"
...
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-MIKASA_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``MIKASA_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                "Tests must run against a temporary MIKASA_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "mikasa_HOME. If this test genuinely needs the live database, mark it with "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to MIKASA_HOME/sessions/<id>.jsonl (state.db was replaced under a
    live process). Returns the path, or None if nothing to write."""
    sid = str(session_id or "").strip()
    if not sid or not messages:
        return None
    sessions_dir = get_mikasa_home() / "sessions"
...
class SessionDB(
    ...
    def close(self):
        ...
        if self._shared_registry_owned:
            from mikasa_state_registry import release
            release(self)
            return
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('mikasa_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('mikasa_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('mikasa_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('mikasa_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('mikasa_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('mikasa_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('mikasa_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('mikasa_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('mikasa_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('mikasa_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('mikasa_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('mikasa_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('mikasa_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('mikasa_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('mikasa_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('mikasa_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('mikasa_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('mikasa_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('mikasa_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('mikasa_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('mikasa_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('mikasa_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('mikasa_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('mikasa_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('mikasa_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('mikasa_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('mikasa_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('mikasa_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('mikasa_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('mikasa_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('mikasa_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('mikasa_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('mikasa_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('mikasa_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('mikasa_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('mikasa_state_registry', 'release_or_close'),
    'report_startup_progress': ('mikasa_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('mikasa_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('mikasa_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('mikasa_state_wal', 'sqlite_source_id'),
    'workspace_key': ('mikasa_state_sessions', 'workspace_key'),
}
