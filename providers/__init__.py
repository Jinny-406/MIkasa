...
2. User plugins: ``$MIKASA_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$MIKASA_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$MIKASA_HOME/plugins/model-providers/`` if it exists."""
    try:
        from mikasa_constants import get_mikasa_home

        d = get_mikasa_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$MIKASA_HOME/plugins/`` if it exists.
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple MIKASA_HOME profiles don't alias each other.
...
def _discover_entry_point_providers() -> None:
    ...
    A distribution ships::
        ...
        [project.entry-points."mikasa_agent.plugins"]
    ...
    * **Provider targets only.** The ``hermes_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$MIKASA_HOME``) keep their
    ...
    try:
        from mikasa_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$MIKASA_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``hermes plugins install`` at
          ``$MIKASA_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    #    filesystem steps means a bundled or ``$MIKASA_HOME`` profile of the
    ...
    # 2. User plugins — under $MIKASA_HOME/plugins/model-providers/<name>/.
    ...
    #     Those clone into $MIKASA_HOME/plugins/<name>/ — flat, NOT under
